...
export function LoginScreen({ onLogin }) {
  ...
  async function doLogin() {
    ...
    try {
      ...
      if (requiresVerification) {
        ...
        setError(
          L(
            "Per completare l’accesso devi verificare la tua email. Ti abbiamo inviato un nuovo link di conferma."
          )
        );
        setLoading(false);
        return;
  ...
  async function doRegister() {
    ...
    if (!cleanFirstName) {
      setError(L("Inserisci il tuo nome."));
      return;
    }
    if (!cleanLastName) {
      setError(L("Inserisci il tuo cognome."));
      return;
    }
    if (!cleanEmail.includes("@")) {
      setError(L("Email non valida."));
      return;
    }
    var usernameValidation = cleanUsername
      ? validateUsername(cleanUsername)
      : "USERNAME_REQUIRED";
    if (usernameValidation) {
      setError(usernameErrorText(usernameValidation));
      return;
    }
    if (password.length < 6) {
      setError(L("Password: minimo 6 caratteri."));
      return;
    }
    ...
    try {
      sessionStorage.setItem(
        "fainance_registration_pending_email_v1",
        cleanEmail
      );
    } catch (_pendingRegistrationError) {}
    setLoading(true);
    try {
      var authUser: any = await registerEmailAccount(cleanEmail, password);
      try {
        await fainancePromiseTimeout(
          saveUsernameForUser(authUser.uid, cleanUsername, "", cleanEmail),
          7000,
          "Timeout salvataggio username."
        );
        var consentRecordedAt = new Date().toISOString();
        await fainancePromiseTimeout(
          mergeUserProfile(authUser.uid, {
      ...
      try {
        sessionStorage.setItem(
          "fainance_registration_notice_v1",
          JSON.stringify({ email: cleanEmail, createdAt: Date.now() })
        );
      } catch (_registrationNoticeError) {}
      await signOutAccount();
      try {
        sessionStorage.removeItem("fainance_registration_pending_email_v1");
      } catch (_pendingRegistrationError) {}
      setMode("login");
      setPassword("");
      setConfirmPwd("");
      ...
      if (err && err.message && String(err.message).indexOf("USERNAME_") === 0)
        setError(usernameErrorText(String(err.message)));
      else
        setError(
          err.code === "auth/email-already-in-use"
            ? L("Email già registrata.")
            : L("Errore: ") + (err && err.message ? err.message : String(err))
        );
    } finally {
      setLoading(false);
    ...
    >
      {!edit ? (
        <>
          ...
          <button
            onClick={function () {
              if (window.confirm(PL("Uscire dall'account?"))) onLogout();
            }}
            style={{
              width: "100%",
