...
function App({ currentUser, onLogout, fbUser, onProfileUpdate }) {
  ...
  var [shareProjects, setShareProjects] = useStorage(
          ...
          async function (snap: any) {
            ...
            if (snap.exists()) {
              ...
              var cloudShareProjects = Array.isArray(d.shareProjects)
                ? d.shareProjects
                : [];
              var mergedShareProjects = mergeShareProjectArrays(
                cloudShareProjects,
  ...
  function mergeShareProjectsFromCloud(cloudProjects) {
    ...
    setShareProjects(function (list) {
      return mergeShareProjectArrays(list, cloudProjects);
    });
  }
    ...
    function () {
      ...
      function applyProjects() {
        ...
        setShareProjects(function (current) {
          return mergeShareProjectArrays(current, remoteArr);
        });
      }
        ...
        >
          {!appUpdatePopup.force && (
            <button
              ...
              aria-label={translatePopupText("Chiudi")}
              style={{
                position: "absolute",
                top: 10,
                right: 10,
      ...
      >
        ...
        >
          <button
            ...
            aria-label={SL("Chiudi configurazione")}
            style={{
              position: "absolute",
              right: 13,
              top: 13,
      ...
      >
        ...
        >
          <button
            ...
            aria-label={GL("Salta guida")}
            style={{
              position: "absolute",
              right: 14,
              top: 14,
              border: "1px solid " + borderC,
              background: dark ? "#252535" : "#F8FAFF",
              color: subC,
              borderRadius: 12,
              width: 36,
              height: 36,
    ...
    return (
      ...
      >
        ...
        >
          <button
            onClick={function () {
              dismissPremiumTrialPrompt("closed");
            }}
            aria-label={translateUiRuntimeText("Chiudi")}
            style={{
              position: "absolute",
              top: 12,
              right: 12,
              ...
              height: 36,
              borderRadius: 12,
              border: "1px solid rgba(243,111,111,.35)",
              background: "#F36F6F",
              color: "#fff",
              fontSize: 24,
              fontWeight: 950,
              cursor: "pointer",
              lineHeight: 1,
              boxShadow: "0 6px 16px rgba(243,111,111,.22)",
            }}
          >
            ×
          </button>
  ...
  function widgetLockedMessage(widgetType) {
    return (
      ...
      ". Vai in Info per cambiare piano."
    );
  }
  ...
  function settingLockedMessage(requiredPlan) {
    return (
      "🔒 Impostazione non inclusa nel piano " +
      planLabel(currentPlan, lang) +
      ". Disponibile dal piano " +
      settingPlanName(requiredPlan) +
      ". Vai in Info per cambiare piano."
    );
  }
  function blockSetting(requiredPlan) {
    ...
    setToast({
      text: translateUiRuntimeText(settingLockedMessage(requiredPlan)),
      type: "error",
      color: "#E24B4A",
      icon: "🚫",
    });
  ...
  var settingsSections = [
    ...
    },
    {
      id: "security",
      icon: "🔐",
      label: translateUiRuntimeText("Sicurezza"),
      desc: translateUiRuntimeText("Blocco biometrico dell’app"),
    },
    {
      id: "general",
      icon: "🌐",
    ...
    },
    {
      id: "info_support",
      icon: "🆘",
      label: translateUiRuntimeText("Info, Piani & Supporto"),
      desc: translateUiRuntimeText("Piani, supporto, dati e informazioni app"),
    },
  ];
  ...
  function deleteShareProject(pid) {
    setShareProjects(function (list) {
      return (list || []).filter(function (p) {
        return String(p.id) !== String(pid);
      });
    });
    deleteDoc(doc(fbDb, "shareProjects", String(pid))).catch(function () {});
    if (String(shareSelectedProjectId) === String(pid))
      setShareSelectedProjectId(null);
    setToast("Progetto Share eliminato");
  }
          ...
          {method === "pin" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              <input
                defaultValue=""
                onChange={function (e) {
                  unlockPinRef.current = String(e.currentTarget.value || "")
                    .replace(/\D/g, "")
                    .slice(0, 4);
                }}
