...
export function ImportData(){
  ...
  function resetAll(){setStep(0);setRows([]);setCols([]);setPreview([]);setUnknowns({expenseCategory:[],paymentMethod:[],incomeType:[],patrimonioEntry:[]});setReplacementMap({});setUnknownDecision("create");setMap({date:"",amount:"",desc:"",cat:"",method:"",itype:"",rate:"",entryId:"",monthKey:"",patNote:""});setMsg("");}
  ...
  function handleFile(e){var file=e.target.files[0];if(!file)return;e.target.value="";var isExcel=/\.(xlsx|xls)$/i.test(file.name);if(isExcel){var reader=new FileReader();reader.onload=function(ev){parseXLSXBuffer(ev.target.result).then(function(data){if(!data||data.length<2){setMsg(L("Nessun dato nel file Excel."));return;}setCols(data[0].map(function(h){return String(h||"").trim();}));setRows(data.slice(1).filter(function(r){return r.some(function(c){return c!==undefined&&c!==null&&String(c).trim()!=="";});}));setStep(1);setMsg("");});};reader.readAsArrayBuffer(file);}else{var reader2=new FileReader();reader2.onload=function(ev){var text=ev.target.result;var parsed=parseCSVText(text);if(!parsed||parsed.length<2){setMsg(L("File CSV vuoto o non valido."));return;}setCols(parsed[0]);setRows(parsed.slice(1).filter(function(r){return r.some(function(c){return c;});}));setStep(1);setMsg("");};reader2.readAsText(file,"UTF-8");}}
  function buildPreview(){
    ...
    var p=rows.map(function(r){var rawA=r[ai]||"";var amt=parseMoney(rawA);var rateVal=0;if(ri>=0){var rv=parseInt(r[ri]);if(!isNaN(rv)&&rv>1&&rv<=60)rateVal=rv;}
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:14}}>{importSuccess&&<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:500,display:"flex",alignItems:"center",justifyContent:"center",padding:16}} onClick={function(){setImportSuccess(null);}}><div style={{background:dark?"#1e1e30":"#fff",borderRadius:20,padding:32,textAlign:"center",maxWidth:320,width:"100%",boxShadow:"0 8px 40px rgba(0,0,0,0.2)"}}><div style={{fontSize:56,marginBottom:12}}>✅</div><div style={{fontSize:20,fontWeight:700,color:tc,marginBottom:8}}>{L("Importazione completata!")}</div><div style={{fontSize:15,color:sc,marginBottom:6}}><span style={{fontSize:28,fontWeight:700,color:tabColor,display:"block",marginBottom:4}}>{importSuccess.count}</span>{L(importSuccess.type)} {L("importate con successo")}</div><button onClick={function(){setImportSuccess(null);}} style={{marginTop:16,background:"linear-gradient(135deg,#7F77DD,#378ADD)",color:"#fff",border:"none",borderRadius:12,padding:"12px 32px",fontSize:14,fontWeight:600,cursor:"pointer"}}>OK</button></div></div>}
    ...
    {step===0&&<div style={{background:dark?"#252535":"#f9f9f9",borderRadius:12,border:"2px dashed "+(dark?"#555":"#ddd"),padding:"36px 24px",textAlign:"center",cursor:"pointer"}} onClick={function(){fileRef.current.click();}} onDragOver={function(e){e.preventDefault();}} onDrop={function(e){e.preventDefault();var f=e.dataTransfer.files[0];if(f){var fake={target:{files:[f],value:""}};handleFile(fake);}}}><div style={{fontSize:36,marginBottom:8}}>📂</div><div style={{fontSize:14,fontWeight:500,color:tc,marginBottom:4}}>{L("Trascina qui un file Excel o CSV")}</div><div style={{fontSize:12,color:sc,marginBottom:12}}>{L("oppure clicca per sfogliare")}</div><div style={{display:"flex",justifyContent:"center",gap:8}}><span style={{background:dark?"#333":"#eee",borderRadius:8,padding:"3px 10px",fontSize:12,color:sc}}>.xlsx</span><span style={{background:dark?"#333":"#eee",borderRadius:8,padding:"3px 10px",fontSize:12,color:sc}}>.xls</span><span style={{background:dark?"#333":"#eee",borderRadius:8,padding:"3px 10px",fontSize:12,color:sc}}>.csv</span></div></div>}<input ref={fileRef} type="file" accept=".csv,.xlsx,.xls,.txt" style={{display:"none"}} onChange={handleFile}/>
