...
export function ExpenseForm({onSave,type}){
  ...
  var eI={amount:"",itype:((sIncomeTypes&&sIncomeTypes[0]&&sIncomeTypes[0].id)||"salario"),desc:"",date:todayStr(),rateizzato:false,rate:12,rateDirection:"forward"};
...
export function BulkEntry({onSave,type,maxRows,limitMessage}){
  ...
  var sMethods=useMemo(function(){return sortedMethods(methods,methodOrder,methodSortMode);},[methods,methodOrder,methodSortMode]);
  var sIncomeTypes=useMemo(function(){return orderedIncomeTypesForContext(ctx);},[ctx.incomeTypes,ctx.incomeTypeOrder,ctx.customIncomeTypes,ctx.incomeTypeOverrides]);
  function blank(){var dc=(sCats.find(function(c){return c.id===4;})||sCats[0]||{id:1});var dm=(sMethods.find(function(m){return m.id===2;})||sMethods[0]||{id:1});return{_id:Date.now()+Math.random(),amount:"",catId:dc.id,methodId:dm.id,itype:((sIncomeTypes&&sIncomeTypes[0]&&sIncomeTypes[0].id)||"salario"),desc:"",date:todayStr(),rateizzato:false,rate:12,rateDirection:"forward"};}
