...
function effectiveExpenseCats(list:any){
  return (Array.isArray(list)&&list.length)?list:DEFAULT_CATS;
}
function effectiveMethods(list:any){
  return (Array.isArray(list)&&list.length)?list:DEFAULT_METHODS;
}
...
export function BarChart({data,width,height}){var {dark}=useApp();if(!data||!data.length)return null;var max=Math.max.apply(null,data.map(function(d){return Math.max(d.exp||0,d.inc||0);}));if(!max)max=1;var pl=4,pr=4,pt=8,pb=22,cw=width-pl-pr,ch=height-pt-pb,bw=cw/data.length,barW=Math.max(3,Math.floor(bw*0.32)),gap=2,tc=dark?"#888":"#bbb";return <svg data-no-translate="true" className="notranslate" width={width} height={height}>{[0,0.5,1].map(function(p){var y=pt+ch*(1-p);return <line key={p} x1={pl} y1={y} x2={width-pr} y2={y} stroke={dark?"#2a2a3e":"#f0f0f0"} strokeWidth={1}/>;})}{data.map(function(d,i){var cx2=pl+i*bw+bw/2,hE=d.exp>0?Math.max(2,Math.round((d.exp/max)*ch)):0,hI=d.inc>0?Math.max(2,Math.round((d.inc/max)*ch)):0;return <g key={i}>{hE>0&&<rect x={cx2-barW-gap/2} y={pt+ch-hE} width={barW} height={hE} fill="#E24B4A" rx={2} opacity={0.82}/>}{hI>0&&<rect x={cx2+gap/2} y={pt+ch-hI} width={barW} height={hI} fill="#1D9E75" rx={2} opacity={0.82}/>}<text className="notranslate" data-no-translate="true" x={cx2} y={height-5} textAnchor="middle" fontSize={8} fill={tc}>{d.label}</text></g>;})}</svg>;}
export function LineChart({data,width,height,color}){var {dark}=useApp();if(!data||data.length<2)return null;color=color||"#7F77DD";var max=Math.max.apply(null,data.map(function(d){return d.value||0;}));var min=Math.min.apply(null,data.map(function(d){return d.value||0;}));var range=max-min;if(!range){range=max||1;}var pl=4,pr=4,pt=8,pb=22,cw=width-pl-pr,ch=height-pt-pb;var pts=data.map(function(d,i){return{x:pl+i*(cw/(data.length-1)),y:pt+ch-((d.value-min)/range)*ch,v:d.value,label:d.label};});var line=pts.map(function(p,i){return(i===0?"M":"L")+p.x.toFixed(1)+" "+p.y.toFixed(1);}).join(" ");var area=line+" L"+pts[pts.length-1].x+" "+(pt+ch)+" L"+pts[0].x+" "+(pt+ch)+" Z";var gid="lg"+color.replace("#","");var tc=dark?"#888":"#bbb";return <svg data-no-translate="true" className="notranslate" width={width} height={height}><defs><linearGradient id={gid} x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={color} stopOpacity="0.25"/><stop offset="100%" stopColor={color} stopOpacity="0"/></linearGradient></defs>{[0,0.5,1].map(function(p){var y=pt+ch*(1-p);return <line key={p} x1={pl} y1={y} x2={width-pr} y2={y} stroke={dark?"#2a2a3e":"#f0f0f0"} strokeWidth={1}/>;})}<path d={area} fill={"url(#"+gid+")"}/><path d={line} fill="none" stroke={color} strokeWidth={2} strokeLinejoin="round" strokeLinecap="round"/>{pts.map(function(p,i){return <text key={i} className="notranslate" data-no-translate="true" x={p.x} y={height-5} textAnchor="middle" fontSize={8} fill={tc}>{p.label}</text>;})}</svg>;}
...
function translateFainanceTextWithLocal(v,lang){
  var k=String(v||"");
  var E={
    "Aggiungi Area":{it:"Aggiungi Area",en:"Add Area",es:"Añadir área",fr:"Ajouter une zone",de:"Bereich hinzufügen",pt:"Adicionar área",pl:"Dodaj obszar",nl:"Gebied toevoegen",ro:"Adaugă zonă",el:"Προσθήκη περιοχής"},
    "Aggiungi Categoria":{it:"Aggiungi Categoria",en:"Add Category",es:"Añadir categoría",fr:"Ajouter une catégorie",de:"Kategorie hinzufügen",pt:"Adicionar categoria",pl:"Dodaj kategorię",nl:"Categorie toevoegen",ro:"Adaugă categorie",el:"Προσθήκη κατηγορίας"},
    "Aggiungi Metodo":{it:"Aggiungi Metodo",en:"Add Method",es:"Añadir método",fr:"Ajouter un mode",de:"Methode hinzufügen",pt:"Adicionar método",pl:"Dodaj metodę",nl:"Methode toevoegen",ro:"Adaugă metodă",el:"Προσθήκη μεθόδου"},
    "Categoria aggiunta correttamente.":{it:"Categoria aggiunta correttamente.",en:"Category added successfully.",es:"Categoría añadida correctamente.",fr:"Catégorie ajoutée correctement.",de:"Kategorie erfolgreich hinzugefügt.",pt:"Categoria adicionada corretamente.",pl:"Kategoria została poprawnie dodana.",nl:"Categorie succesvol toegevoegd.",ro:"Categoria a fost adăugată corect.",el:"Η κατηγορία προστέθηκε επιτυχώς."},
    "Metodo aggiunto correttamente.":{it:"Metodo aggiunto correttamente.",en:"Method added successfully.",es:"Método añadido correctamente.",fr:"Mode ajouté correctement.",de:"Methode erfolgreich hinzugefügt.",pt:"Método adicionado corretamente.",pl:"Metoda została poprawnie dodana.",nl:"Methode succesvol toegevoegd.",ro:"Metoda a fost adăugată corect.",el:"Η μέθοδος προστέθηκε επιτυχώς."},
    "Voce aggiunta correttamente.":{it:"Voce aggiunta correttamente.",en:"Item added successfully.",es:"Elemento añadido correctamente.",fr:"Élément ajouté correctement.",de:"Eintrag erfolgreich hinzugefügt.",pt:"Item adicionado corretamente.",pl:"Pozycja została poprawnie dodana.",nl:"Item succesvol toegevoegd.",ro:"Elementul a fost adăugat corect.",el:"Το στοιχείο προστέθηκε επιτυχώς."},
    "Cancella":{it:"Cancella",en:"Clear",es:"Borrar",fr:"Effacer",de:"Leeren",pt:"Limpar",pl:"Wyczyść",nl:"Wissen",ro:"Șterge",el:"Καθαρισμός"},
    ...
    "Area eliminata correttamente.":{it:"Area eliminata correttamente.",en:"Area deleted successfully.",es:"Área eliminada correctamente.",fr:"Zone supprimée correctement.",de:"Bereich erfolgreich gelöscht.",pt:"Área eliminada corretamente.",pl:"Obszar został poprawnie usunięty.",nl:"Gebied succesvol verwijderd.",ro:"Zona a fost ștearsă corect.",el:"Η περιοχή διαγράφηκε επιτυχώς."},
    "Categoria eliminata correttamente.":{it:"Categoria eliminata correttamente.",en:"Category deleted successfully.",es:"Categoría eliminada correctamente.",fr:"Catégorie supprimée correctement.",de:"Kategorie erfolgreich gelöscht.",pt:"Categoria eliminada corretamente.",pl:"Kategoria została poprawnie usunięta.",nl:"Categorie succesvol verwijderd.",ro:"Categoria a fost ștearsă corect.",el:"Η κατηγορία διαγράφηκε επιτυχώς."},
    "Metodo eliminato correttamente.":{it:"Metodo eliminato correttamente.",en:"Method deleted successfully.",es:"Método eliminado correctamente.",fr:"Mode supprimé correctement.",de:"Methode erfolgreich gelöscht.",pt:"Método eliminado corretamente.",pl:"Metoda została poprawnie usunięta.",nl:"Methode succesvol verwijderd.",ro:"Metoda a fost ștearsă corect.",el:"Η μέθοδος διαγράφηκε επιτυχώς."},
    "Icona":{it:"Icona",en:"Icon",es:"Icono",fr:"Icône",de:"Symbol",pt:"Ícone",pl:"Ikona",nl:"Pictogram",ro:"Pictogramă",el:"Εικονίδιο"}
  };
  if(E[k])return E[k][lang]||E[k].it||k;
  return translateFainanceText(k,lang);
}
export function EmojiPicker({value,onChange}){var ctx=useApp();var dark=ctx.dark;var [open,setOpen]=useState(false);var bg=dark?"#252535":"#fff";var border=dark?"#444":"#ddd";var color=dark?"#eee":"#333";return <div style={{position:"relative",display:"inline-block"}}><button type="button" onClick={function(e){e.preventDefault();e.stopPropagation();setOpen(function(o){return !o;});}} style={{minWidth:44,height:42,fontSize:22,background:dark?"#2a2a3e":"#f5f5f5",border:"1px solid "+border,borderRadius:10,padding:"4px 10px",cursor:"pointer",color:color,lineHeight:1}}>{value||"📦"}</button>{open&&<div style={{position:"absolute",zIndex:260,background:bg,border:"1px solid "+border,borderRadius:14,padding:8,display:"grid",gridTemplateColumns:"repeat(8, 1fr)",gap:4,width:312,maxHeight:320,overflowY:"auto",boxShadow:"0 8px 28px rgba(0,0,0,0.22)",top:48,left:0}}>{EMOJI_LIST.map(function(e){return <button type="button" title={String(e)} key={e} onClick={function(ev){ev.preventDefault();ev.stopPropagation();onChange(e);setOpen(false);}} style={{width:32,height:32,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,background:value===e?(dark?"#343050":"#EEEDFE"):"transparent",border:"1px solid "+(value===e?"#7F77DD":"transparent"),color:color,cursor:"pointer",padding:0,borderRadius:8,fontWeight:900,overflow:"hidden"}}>{e}</button>;})}</div>}</div>;}
...
export function Toast({msg,onDone,color}){
  ...
  var tr=ctx&&ctx.translateUiRuntimeText?ctx.translateUiRuntimeText:function(v){try{return translateFainanceTextWithLocal(v,(ctx&&ctx.lang)||"it");}catch(e){return v;}};
...
export function AlertPopup({newAlerts,onClose}){
  var ctx=useApp();
  var {fmt}=ctx;
  function L(s){return translateFainanceTextWithLocal(s,(ctx&&ctx.lang)||"it");}
...
export function ExpenseForm({onSave,type}){
  ...
  var defaultCat=(sCats.find(function(c){return String(c.id)==="4";})||sCats[0]||{id:1});
  var defaultMethod=(sMethods.find(function(m){return String(m.id)==="2";})||sMethods[0]||{id:1});
  ...
  function save(){var amount=parseMoney(f.amount);if(!amount||amount<=0)return;var item={id:Date.now(),amount:amount,desc:f.desc,date:f.date,rateizzato:f.rateizzato,rate:f.rate,rateDirection:f.rateDirection||"forward"};if(type==="expense"){item.catId=f.catId;item.methodId=f.methodId;}else{item.type=f.itype;}onSave(item);setF(type==="expense"?eE:eI);}
  return <div style={{background:panelBg,border:"1px solid "+borderC,borderRadius:16,padding:isMobile?8:18,boxShadow:dark?"none":"0 8px 28px rgba(83,74,183,0.08)",display:"flex",flexDirection:"column",gap:isMobile?6:12}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:isMobile?"42% 1fr":"minmax(180px,.8fr) 1.2fr",gap:isMobile?8:12,alignItems:"stretch"}}>
      ...
      {type==="expense"?<div style={{display:"grid",gridTemplateColumns:"1fr",gap:isMobile?7:10}}>
        <div style={fieldCard}><label style={label}>{t.category}</label>{catSortMode==="custom"?<select value={f.catId} onChange={function(e){setF(function(p){return{...p,catId:e.target.value};});}} style={inp}>{sCats.map(function(c){return <option key={c.id} value={c.id}>{c.icon} {c.name}</option>;})}</select>:<select value={f.catId} onChange={function(e){setF(function(p){return{...p,catId:e.target.value};});}} style={inp}>{grps.map(function(g){var gc=safeCatsForGroup(sCats,grps,g);return gc.length?<optgroup key={g.id} label={g.name}>{gc.map(function(c){return <option key={c.id} value={c.id}>{c.icon} {c.name}</option>;})}</optgroup>:null;})}</select>}</div>
      ...
      </div>:<div style={{display:"flex",flexDirection:"column",justifyContent:"center"}}><div style={fieldCard}><label style={label}>{t.incomeType}</label><select value={f.itype} onChange={function(e){setF(function(p){return{...p,itype:e.target.value};});}} style={inp}>{((ctx.incomeTypes&&ctx.incomeTypes.length)?ctx.incomeTypes:getAllIncomeTypes()).map(function(it){return <option key={it.id} value={it.id}>{it.icon} {it.name}</option>;})}</select></div></div>}
    </div>
    ...
    <button onClick={save} style={{background:mainButtonC,color:"#fff",border:"none",borderRadius:btnRadius,padding:isMobile?11:15,fontSize:15,fontWeight:900,cursor:"pointer",boxShadow:dark?"none":"0 6px 18px "+mainButtonC+"44"}}>{type==="expense"?TL("Salva Uscita"):TL("Salva Entrata")}</button>
  </div>;
}
...
export function ReceiptScanPanel({onSave,shareMode}){
  ...
  return <div style={{background:dark?"linear-gradient(160deg,#252535 0%,#1e1e30 100%)":"linear-gradient(160deg,#ffffff 0%,#f7fbff 100%)",border:"1px solid "+borderC,borderRadius:16,padding:isMobile?12:18,display:"flex",flexDirection:"column",gap:14,boxShadow:dark?"none":"0 10px 30px rgba(55,138,221,0.12)"}}>
    ...
    <button onClick={save} style={{background:confirmButtonColor||"#378ADD",color:"#fff",border:"none",borderRadius:btnRadius,padding:isMobile?12:15,fontSize:15,fontWeight:900,cursor:"pointer",boxShadow:dark?"none":"0 6px 18px "+(confirmButtonColor||"#378ADD")+"44"}}>{L("Salva uscita da scontrino")}</button>
  </div>;
}
export function BulkEntry({onSave,type,maxRows,limitMessage}){
  ...
  var L=ctx.translateUiRuntimeText||function(s){return translateFainanceTextWithLocal(s,(ctx&&ctx.lang)||"it");};
  var sCats=useMemo(function(){return sortedCats(cats,catOrder,catSortMode,expenseGroups);},[cats,catOrder,catSortMode,expenseGroups]);
  var sMethods=useMemo(function(){return sortedMethods(methods,methodOrder,methodSortMode);},[methods,methodOrder,methodSortMode]);
    ...
    return <div>
      {rows.map(function(r,idx){return <div key={r._id} style={{background:dark?"#1e1e30":"#f9f9f9",borderRadius:12,border:"1px solid "+(dark?"#444":"#eee"),padding:12,marginBottom:10,position:"relative"}}>
        ...
        {type==="expense"?<div style={{display:"grid",gridTemplateColumns:"42% 1fr",gap:8,marginBottom:6,alignItems:"end"}}>
          ...
          <div><label style={{fontSize:11,color:tc,display:"block",marginBottom:3,fontWeight:700}}>{L('Importo')}</label><input type="number" value={r.amount} onChange={function(e){upd(r._id,"amount",e.target.value);}} style={{...inp,height:120,fontSize:38,fontWeight:950,borderRadius:14}} placeholder="0,00"/></div>
          <div><label style={{fontSize:11,color:tc,display:"block",marginBottom:3,fontWeight:700}}>{L('Tipo')}</label><select value={r.itype} onChange={function(e){upd(r._id,"itype",e.target.value);}} style={{...inp,height:42}}>{((ctx.incomeTypes&&ctx.incomeTypes.length)?ctx.incomeTypes:getAllIncomeTypes()).map(function(it){return <option key={it.id} value={it.id}>{it.icon} {it.name}</option>;})}</select></div>
        </div>}
...
export function SettingsList({items,setItems,label,showGroup,showIcon,groupList,isMethod,allowArchive}){
  var ctx=useApp();function L(v){return translateFainanceTextWithLocal(v,(ctx&&ctx.lang)||"it");}
  var t=ctx.t,dark=ctx.dark,isMobile=ctx.isMobile;
  var expenses=ctx.expenses||[],incomes=ctx.incomes||[],recurring=ctx.recurring||[],budgetPlan=ctx.budgetPlan||{};
  var gl=groupList||[];
  var [newName,setNewName]=useState("");var [newColor,setNewColor]=useState(COLORS[0]);var [newGroup,setNewGroup]=useState(gl[0]?gl[0].id:"");var [newIcon,setNewIcon]=useState("📦");
  ...
  var inp={width:"100%",borderRadius:8,border:"1px solid "+(dark?"#444":"#ddd"),padding:"7px 10px",fontSize:14,background:dark?"#2a2a3e":"#fff",color:dark?"#eee":"#333",boxSizing:"border-box"};
  var tc=dark?"#eee":"#333";var sc=dark?"#aaa":"#888";var borderC=dark?"#444":"#eee";var cardBg=dark?"#252535":"#fff";var baseBlocked=ctx.currentPlan==="free";var setToast=ctx.setToast;
  var entity=isMethod?"Metodo":(showGroup?"Categoria":"Voce");
  var addLabel=isMethod?"Aggiungi Metodo":(showGroup?"Aggiungi Categoria":"Aggiungi");
  function lockedWarn(){if(setToast)setToast({text:"Funzione disponibile dal piano Base.",type:"warning",color:"#EF9F27",icon:"⚠️"});}
  function resetNew(){setNewName("");setNewColor(COLORS[0]);setNewGroup(gl[0]?gl[0].id:"");setNewIcon("📦");}
  function normalizeItemGroup(x){return String((x&&x.group!=null?x.group:(x&&x.area!=null?x.area:""))||"");}
  function add(){if(baseBlocked){lockedWarn();return;}var nm=newName.trim();if(!nm)return;setItems([...(items||[]),{id:Date.now(),name:nm,color:newColor,icon:showIcon?newIcon:undefined,...(showGroup?{group:newGroup}:{})}]);resetNew();if(setToast){var ok=isMethod?"Metodo aggiunto correttamente.":(showGroup?"Categoria aggiunta correttamente.":"Voce aggiunta correttamente.");setToast({text:L(ok),type:"success",icon:"✅"});}}
  ...
  function del(id){if(baseBlocked){lockedWarn();return;}if(itemIsUsed(id)){usedDeleteWarn();return;}var confirmKey=isMethod?"Sei sicuro di voler eliminare il Metodo?":"Sei sicuro di voler eliminare la Categoria?";if(typeof window!=="undefined"&&!window.confirm(L(confirmKey)))return;setItems((items||[]).filter(function(i){return String(i.id)!==String(id);}));if(setToast)setToast({text:L(isMethod?"Metodo eliminato correttamente.":"Categoria eliminata correttamente."),type:"success",icon:"🗑️"});}
  function startEdit(item){if(baseBlocked){lockedWarn();return;}setEditId(item.id);setEditName(item.name);setEditColor(item.color||COLORS[0]);setEditGroup(normalizeItemGroup(item)||(gl[0]?gl[0].id:""));setEditIcon(item.icon||"📦");}
  function saveEdit(){if(baseBlocked){lockedWarn();return;}if(!editName.trim())return;setItems((items||[]).map(function(i){return String(i.id)===String(editId)?{...i,name:editName.trim(),color:editColor||COLORS[0],icon:showIcon?editIcon:i.icon,...(showGroup?{group:editGroup}:{})}:i;}));setEditId(null);}
  function toggleArchive(id){if(baseBlocked){lockedWarn();return;}setItems((items||[]).map(function(i){return String(i.id)===String(id)?{...i,archived:!i.archived}:i;}));}
  var sourceItems=(items||[]).filter(function(i){return !i.archived;});
  var grouped=showGroup&&gl.length?(function(){var known={};gl.forEach(function(g){known[String(g.id)]=true;});var base=gl.map(function(g){return{...g,items:sourceItems.filter(function(c){return normalizeItemGroup(c)===String(g.id);})};});var orphanItems=sourceItems.filter(function(c){return !known[normalizeItemGroup(c)];});if(orphanItems.length)base.push({id:"_orphan",name:L("Senza area"),icon:"📂",color:COLORS[0],items:orphanItems});var total=base.reduce(function(a,g){return a+(g.items?g.items.length:0);},0);if(!total&&sourceItems.length)return[{id:"_all",name:L("Senza area"),icon:"📂",items:sourceItems}];return base;})():[{id:"_",name:"",items:sourceItems}];
  function row(item){return <div key={item.id} style={{background:cardBg,borderRadius:10,border:"1px solid "+borderC,padding:"10px 12px",marginBottom:6,opacity:item.archived?0.55:1}}>{editId===item.id?<div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>{showIcon&&<EmojiPicker value={editIcon} onChange={setEditIcon}/>}<input type="color" value={editColor} onChange={function(e){setEditColor(e.target.value);}} style={{width:32,height:32,padding:0,border:"none",borderRadius:6,cursor:"pointer",flexShrink:0}}/><input value={editName} onChange={function(e){setEditName(e.target.value);}} style={{...inp,flex:1,minWidth:120}} onKeyDown={function(e){if(e.key==="Enter")saveEdit();}}/>{showGroup&&gl.length>0&&<select value={editGroup} onChange={function(e){setEditGroup(e.target.value);}} style={{...inp,flex:1,minWidth:120}}>{gl.map(function(g2){return <option key={g2.id} value={g2.id}>{(g2.icon||"")+" "+L(g2.name)}</option>;})}</select>}<Btn onClick={saveEdit} style={{padding:"8px 12px",fontSize:13}}>{L(t.save||"Salva")}</Btn><Btn onClick={function(){setEditId(null);}} bg={dark?"#333":"#f0f0f0"} color={tc} style={{padding:"8px 12px",fontSize:13}}>{L("Annulla")}</Btn></div>:<div style={{display:"flex",alignItems:"center",gap:10,minWidth:0}}>{showIcon&&<span style={{fontSize:18,flexShrink:0}}>{item.icon||"📦"}</span>}<div style={{width:12,height:12,borderRadius:"50%",background:item.color||COLORS[0],flexShrink:0}}/><span style={{flex:1,fontSize:14,color:tc,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{L(item.name)}</span>{item.archived&&<span style={{fontSize:10,background:"#f0f0f0",color:"#888",borderRadius:10,padding:"1px 6px"}}>{L("Archiviato")}</span>}<button onClick={function(){startEdit(item);}} style={{background:"#EEF4FF",border:"1px solid #BFD7FF",borderRadius:8,cursor:"pointer",color:"#378ADD",fontSize:14,padding:"5px 8px",fontWeight:700}}>✏️</button>{allowArchive!==false&&<button onClick={function(){toggleArchive(item.id);}} title={item.archived?L("Ripristina"):L("Archivia")} style={{background:"none",border:"none",cursor:"pointer",fontSize:15,color:"#bbb"}}>{item.archived?"📂":"🗂"}</button>}<button onClick={function(){del(item.id);}} style={{background:"#FFF0F0",border:"1px solid #FFD0D0",borderRadius:8,cursor:"pointer",color:"#E24B4A",fontSize:14,padding:"5px 8px",fontWeight:700}}>🗑️</button></div>}</div>;}
  return <div>
    {grouped.map(function(g){return <div key={g.id}>{showGroup&&g.name&&g.items.length>0&&<div style={{fontSize:11,fontWeight:700,color:"#aaa",textTransform:"uppercase",letterSpacing:1,margin:"12px 0 6px",display:"flex",alignItems:"center",gap:6}}>{g.icon&&<span style={{fontSize:13}}>{g.icon}</span>}<span>{L(g.name)}</span></div>}{(g.items||[]).map(row)}</div>;})}
    <div onClick={function(){if(baseBlocked)lockedWarn();}} style={{background:baseBlocked?(dark?"#342b16":"#FFF8E1"):(dark?"linear-gradient(135deg,#252535,#1e1e30)":"linear-gradient(135deg,#ffffff,#f7fbff)"),borderRadius:18,border:"1.5px solid "+(baseBlocked?(dark?"#6a5520":"#FFD54F"):(dark?"#3d3d55":"#dfe8f6")),padding:14,marginTop:14,boxShadow:dark?"none":"0 8px 24px rgba(55,138,221,0.08)"}}>
      <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:12}}><div style={{width:44,height:44,borderRadius:14,background:newColor+"22",border:"1px solid "+newColor+"66",display:"flex",alignItems:"center",justifyContent:"center",fontSize:24,flexShrink:0}}>{showIcon?newIcon:"+"}</div><div style={{flex:1,minWidth:0}}><div style={{fontSize:14,fontWeight:900,color:tc}}>{L("Nuova voce")}</div><div style={{fontSize:12,color:sc,lineHeight:1.35}}>{L(label)}</div></div></div>
      <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":(showGroup&&gl.length>0?"auto auto minmax(0,1.1fr) minmax(0,1fr) auto auto":"auto auto minmax(0,1fr) auto auto"),gap:10,alignItems:"center"}}>{showIcon&&<div style={{display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:11,color:sc,minWidth:52}}>{L("Icona")}</span><EmojiPicker value={newIcon} onChange={setNewIcon}/></div>}<div style={{display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:11,color:sc,minWidth:52}}>{L("Colore")}</span><input type="color" value={newColor} disabled={baseBlocked} onChange={function(e){if(baseBlocked){lockedWarn();return;}setNewColor(e.target.value);}} style={{width:42,height:42,padding:0,border:"1px solid "+borderC,borderRadius:12,cursor:"pointer",flexShrink:0,background:"transparent"}}/></div><input placeholder={L("Nome voce")} value={newName} disabled={baseBlocked} onChange={function(e){if(baseBlocked){lockedWarn();return;}setNewName(e.target.value);}} onKeyDown={function(e){if(e.key==="Enter")add();}} style={{...inp,minWidth:0,height:44,borderRadius:12}}/>{showGroup&&gl.length>0&&<select value={newGroup} disabled={baseBlocked} onChange={function(e){if(baseBlocked){lockedWarn();return;}setNewGroup(e.target.value);}} style={{...inp,minWidth:0,height:44,borderRadius:12}}>{gl.map(function(g2){return <option key={g2.id} value={g2.id}>{(g2.icon||"")+" "+L(g2.name)}</option>;})}</select>}<button onClick={add} style={{background:baseBlocked?"#EF9F27":"linear-gradient(135deg,#7F77DD,#378ADD)",color:"#fff",border:"none",borderRadius:14,padding:"0 18px",height:44,cursor:"pointer",fontSize:14,fontWeight:900,whiteSpace:"nowrap",width:isMobile?"100%":"auto"}}>{L(addLabel)}</button><button onClick={resetNew} style={{background:dark?"#333":"#f0f0f0",color:tc,border:"1px solid "+borderC,borderRadius:14,padding:"0 16px",height:44,cursor:"pointer",fontSize:14,fontWeight:800,whiteSpace:"nowrap",width:isMobile?"100%":"auto"}}>{L("Cancella")}</button></div>
    </div>
  </div>;
}
export function AreasEditor({onlyPatrimonio}){
  ...
  var tc=dark?"#eee":"#333";var sc=dark?"#aaa":"#888";var borderC=dark?"#444":"#eee";var cardBg=dark?"#252535":"#fff";
  var inp={width:"100%",borderRadius:8,border:"1px solid "+borderC,padding:"7px 10px",fontSize:14,background:dark?"#2a2a3e":"#fff",color:tc,boxSizing:"border-box"};
  ...
  var baseBlocked=ctx.currentPlan==="free";var setToast=ctx.setToast;
  var [newName,setNewName]=useState("");var [newColor,setNewColor]=useState(COLORS[0]);var [newIcon,setNewIcon]=useState("📋");
  function lockedWarn(){if(setToast)setToast({text:"Funzione disponibile dal piano Base.",type:"warning",color:"#EF9F27",icon:"⚠️"});}
  function resetForm(){setNewName("");setNewColor(COLORS[0]);setNewIcon("📋");}
  function add(){if(baseBlocked){lockedWarn();return;}var nm=newName.trim();if(!nm)return;setCurrent([...(current||[]),{id:"area_"+Date.now(),name:nm,color:newColor,icon:newIcon}]);resetForm();if(setToast)setToast({text:L("Area aggiunta correttamente."),type:"success",icon:"✅"});}
  function del(id){if(baseBlocked){lockedWarn();return;}if(typeof window!=="undefined"&&!window.confirm(L("Sei sicuro di voler eliminare l'Area?")))return;setCurrent((current||[]).filter(function(g){return String(g.id)!==String(id);}));if(setToast)setToast({text:L("Area eliminata correttamente."),type:"success",icon:"🗑️"});}
  function rename(id,v){if(baseBlocked){lockedWarn();return;}setCurrent((current||[]).map(function(g){return String(g.id)===String(id)?{...g,name:v}:g;}));}
  function recolor(id,v){if(baseBlocked){lockedWarn();return;}setCurrent((current||[]).map(function(g){return String(g.id)===String(id)?{...g,color:v}:g;}));}
  function reicon(id,v){if(baseBlocked){lockedWarn();return;}setCurrent((current||[]).map(function(g){return String(g.id)===String(id)?{...g,icon:v}:g;}));}
  return <div style={{display:"flex",flexDirection:"column",gap:14}}>
    <div style={{display:"flex",gap:0,background:dark?"#252535":"#f5f5f5",borderRadius:10,padding:3,marginBottom:4,flexWrap:"wrap"}}>{(onlyPatrimonio?[{id:"patrimonio",label:"💎 Patrimonio"}]:[{id:"expense",label:"💸 Uscite"},{id:"income",label:"💰 Entrate"},{id:"method",label:"💳 Metodi di pagamento"}]).map(function(tb){return <button key={tb.id} onClick={function(){setAreaTab(tb.id);}} style={{flex:1,padding:"8px",border:"none",borderRadius:8,background:areaTab===tb.id?(dark?"#444":"#fff"):"transparent",color:areaTab===tb.id?tc:sc,fontSize:12,cursor:"pointer",fontWeight:areaTab===tb.id?600:400,minWidth:80}}>{L(tb.label)}</button>;})}</div>
    {(current||[]).map(function(g){return <div key={g.id} style={{background:cardBg,borderRadius:10,border:"1px solid "+borderC,padding:"10px 12px",display:"flex",alignItems:"center",gap:8}}><EmojiPicker value={g.icon||"📋"} onChange={function(v){reicon(g.id,v);}}/><input type="color" value={g.color||COLORS[0]} onChange={function(e){recolor(g.id,e.target.value);}} style={{width:34,height:34,padding:0,border:"none",borderRadius:8,cursor:"pointer",flexShrink:0,background:"transparent"}}/><input value={g.name} onChange={function(e){rename(g.id,e.target.value);}} style={{...inp,flex:1}}/><button onClick={function(){del(g.id);}} style={{background:"#FFF0F0",border:"1px solid #FFD0D0",borderRadius:8,cursor:"pointer",color:"#E24B4A",fontSize:14,padding:"7px 10px",fontWeight:800}}>🗑️</button></div>;})}
    <div onClick={function(){if(baseBlocked)lockedWarn();}} style={{background:baseBlocked?(dark?"#342b16":"#FFF8E1"):cardBg,borderRadius:12,border:"1.5px dashed "+(baseBlocked?(dark?"#6a5520":"#FFD54F"):(dark?"#555":"#ddd")),padding:"12px"}}><div style={{display:"grid",gridTemplateColumns:"auto auto minmax(0,1fr)",gap:8,alignItems:"center"}}><EmojiPicker value={newIcon} onChange={setNewIcon}/><input type="color" value={newColor} disabled={baseBlocked} onChange={function(e){if(baseBlocked){lockedWarn();return;}setNewColor(e.target.value);}} style={{width:42,height:42,padding:0,border:"none",borderRadius:8,cursor:"pointer",background:"transparent"}}/><input placeholder={L("Nome area")} value={newName} disabled={baseBlocked} onChange={function(e){if(baseBlocked){lockedWarn();return;}setNewName(e.target.value);}} onKeyDown={function(e){if(e.key==="Enter")add();}} style={{...inp,flex:1,height:42}}/></div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginTop:10}}><button onClick={add} style={{background:baseBlocked?"#EF9F27":"linear-gradient(135deg,#7F77DD,#378ADD)",color:"#fff",border:"none",borderRadius:10,padding:"11px 12px",cursor:"pointer",fontSize:14,fontWeight:900}}>{L("Aggiungi Area")}</button><button onClick={resetForm} style={{background:dark?"#333":"#f0f0f0",color:tc,border:"1px solid "+borderC,borderRadius:10,padding:"11px 12px",cursor:"pointer",fontSize:14,fontWeight:800}}>{L("Cancella")}</button></div></div>
  </div>;
}
...
export function PatrimonioSettingsPanel({forcedSection,allowEditing=true,onLocked}={}){
  var ctx=useApp();
  function L(v){return translateFainanceTextWithLocal(v,(ctx&&ctx.lang)||"it");}
...
export function ImportData(){
  ...
  function optionList(kind){if(kind==="expenseCategory")return cats;if(kind==="paymentMethod")return methods;if(kind==="incomeType")return incomeTypes;return pEntries;}
  ...
  function handleFile(e){
    ...
    if(isExcel){
      ...
      reader.onerror=function(){setImportLoading(null);setMsg(L("Errore durante la lettura del file."));};
      reader.onload=function(ev){
      ...
      reader2.onerror=function(){setImportLoading(null);setMsg(L("Errore durante la lettura del file."));};
      reader2.onload=function(ev){
        setImportProgress(file.name,"Analisi file",90);
        var text=ev.target.result;var parsed=parseImportedDelimitedText(text);
        if(!parsed||parsed.length<2){setImportLoading(null);setMsg(L("File CSV vuoto o non valido."));return;}
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:14}}>{importSuccess&&<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:500,display:"flex",alignItems:"center",justifyContent:"center",padding:16}} onClick={function(){setImportSuccess(null);}}><div style={{background:dark?"#1e1e30":"#fff",borderRadius:20,padding:32,textAlign:"center",maxWidth:320,width:"100%",boxShadow:"0 8px 40px rgba(0,0,0,0.2)"}}><div style={{fontSize:56,marginBottom:12}}>✅</div><div style={{fontSize:20,fontWeight:700,color:tc,marginBottom:8}}>{L("Importazione completata!")}</div><div style={{fontSize:15,color:sc,marginBottom:6}}><span style={{fontSize:28,fontWeight:700,color:tabColor,display:"block",marginBottom:4}}>{importSuccess.count}</span>{L(importSuccess.type)} {L("importate con successo")}</div><button onClick={function(){setImportSuccess(null);}} style={{marginTop:16,background:"linear-gradient(135deg,#7F77DD,#378ADD)",color:"#fff",border:"none",borderRadius:12,padding:"12px 32px",fontSize:14,fontWeight:600,cursor:"pointer"}}>OK</button></div></div>}
    ...
    </div>}<input ref={fileRef} type="file" accept=".xlsx,.xls,.csv,text/csv,application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" style={{display:"none"}} onChange={handleFile}/>
