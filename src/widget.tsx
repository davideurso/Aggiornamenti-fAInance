...
export function EmojiPicker({value,onChange}){var [open,setOpen]=useState(false);function iconStyle(e){var txt=String(e||"");return {fontSize:txt.length>2?11:20,fontWeight:txt.length>2?900:500,background:"none",border:"none",cursor:"pointer",padding:txt.length>2?"2px 3px":"2px 4px",borderRadius:4,minWidth:28,height:28,lineHeight:"24px",display:"inline-flex",alignItems:"center",justifyContent:"center"};}return <div style={{position:"relative"}}><button onClick={function(){setOpen(function(o){return !o;});}} style={{fontSize:String(value||"").length>2?13:22,fontWeight:String(value||"").length>2?900:500,background:"#f5f5f5",border:"1px solid #ddd",borderRadius:8,padding:"4px 10px",cursor:"pointer",minWidth:44,height:36}}>{value||"📦"}</button>{open&&<div style={{position:"absolute",zIndex:200,background:"#fff",border:"1px solid #ddd",borderRadius:10,padding:8,display:"flex",flexWrap:"wrap",gap:4,width:292,maxHeight:320,overflowY:"auto",boxShadow:"0 4px 16px rgba(0,0,0,0.15)",top:40,left:0}}>{EMOJI_LIST.map(function(e){return <button key={e} title={String(e)} onClick={function(){onChange(e);setOpen(false);}} style={iconStyle(e)}>{e}</button>;})}</div>}</div>;}
...
export function RecurringManager(){
  ...
  function recIcon(r){return r.icon||autoIcon(r.name)||(r.rtype==="income"?"💰":"💸");}
