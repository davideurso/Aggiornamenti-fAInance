...
export function LineChart({data,width,height,color}){var ctx=useApp();var dark=ctx.dark,fmt=ctx.fmt,isMobile=!!ctx.isMobile;if(!data||!data.length)return null;color=color||"#7F77DD";var max=Math.max.apply(null,data.map(function(d){return d.value||0;}));var min=Math.min.apply(null,data.map(function(d){return d.value||0;}));var range=max-min;if(!range){range=max||1;}var w=width||320;var showAxis=w>=220;var pl=showAxis?52:8,pr=5,pt=8,pb=isMobile?31:27,cw=w-pl-pr,ch=height-pt-pb;function axisValue(v){try{if(fmt)return fmt(v);}catch(e){}var n=Math.round(Number(v)||0);return "€ "+String(n);}var ticks=[max,(max+min)/2,min];var pts=data.map(function(d,i){var x=data.length===1?pl+(cw/2):pl+i*(cw/(data.length-1));var y=data.length===1?pt+(ch/2):pt+ch-((d.value-min)/range)*ch;return{x:x,y:y,v:d.value,label:d.label};});var line=pts.map(function(p,i){return(i===0?"M":"L")+p.x.toFixed(1)+" "+p.y.toFixed(1);}).join(" ");var area=data.length===1?"":line+" L"+pts[pts.length-1].x+" "+(pt+ch)+" L"+pts[0].x+" "+(pt+ch)+" Z";var gid="lg"+color.replace("#","");var tc=dark?"#888":"#999";var maxLabels=w<255?3:(w<390?4:7);var labelStep=Math.max(1,Math.ceil(Math.max(1,pts.length-1)/Math.max(1,maxLabels-1)));function showLabel(i){return i===0||i===pts.length-1||i%labelStep===0;}return <svg data-no-translate="true" className="notranslate" width="100%" height={height} viewBox={"0 0 "+w+" "+height} preserveAspectRatio="none" style={{display:"block",overflow:"visible"}}><defs><linearGradient id={gid} x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={color} stopOpacity="0.25"/><stop offset="100%" stopColor={color} stopOpacity="0"/></linearGradient></defs>{[0,0.5,1].map(function(p){var y=pt+ch*(1-p);return <g key={p}>{showAxis&&<text className="notranslate" data-no-translate="true" x={2} y={Math.max(9,y+3)} textAnchor="start" fontSize={8} fill={tc}>{axisValue(ticks[p===0?0:(p===0.5?1:2)])}</text>}<line x1={pl} y1={y} x2={w-pr} y2={y} stroke={dark?"#2a2a3e":"#f0f0f0"} strokeWidth={1}/></g>;})}{area&&<path d={area} fill={"url(#"+gid+")"}/>}<path d={line} fill="none" stroke={color} strokeWidth={2} strokeLinejoin="round" strokeLinecap="round"/>{data.length===1&&<circle cx={pts[0].x} cy={pts[0].y} r={4} fill={color}/>} {pts.map(function(p,i){if(!showLabel(i))return null;var anchor=i===0?"start":(i===pts.length-1?"end":"middle");return <text key={i} className="notranslate" data-no-translate="true" x={p.x} y={height-6} textAnchor={anchor} fontSize={7.5} fill={tc}>{p.label}</text>;})}</svg>;}
...
export function Toggle({label,checked,onChange,color}){var ctx=useApp();var dark=ctx.dark;var c=color||ctx.secondaryButtonColor||"#7FC8F8";return <div style={{display:"flex",alignItems:"center",gap:10,userSelect:"none"}}><button type="button" onClick={onChange} aria-pressed={!!checked} style={{width:44,height:24,borderRadius:12,background:checked?c:"#ccc",position:"relative",transition:"background 0.2s",flexShrink:0,border:"none",padding:0,cursor:"pointer",WebkitTapHighlightColor:"transparent",touchAction:"manipulation"}}><div style={{position:"absolute",top:3,left:checked?23:3,width:18,height:18,borderRadius:"50%",background:"#fff",transition:"left 0.2s"}}/></button>{!!label&&<span onClick={onChange} style={{fontSize:13,color:checked?c:(dark?"#aaa":"#888"),fontWeight:checked?500:400,cursor:"pointer"}}>{label}</span>}</div>;}
...
export function SettingsList({items,setItems,label,showGroup,showIcon,groupList,isMethod,allowArchive,usageScope}){
  ...
  function itemIsUsed(id){
    var sid=String(id);
    function activeRow(x){return x&&!x.deleted&&String(x.status||"")!=="deleted";}
    function exactRef(row,fields){return (fields||[]).some(function(field){var v=row&&row[field];return v!==undefined&&v!==null&&String(v)!==""&&String(v)===sid;});}
    var scope=usageScope||(isMethod?"method":"generic");
    if(scope==="method")return expenses.filter(activeRow).some(function(e){return exactRef(e,["methodId","method"]);})||incomes.filter(activeRow).some(function(e){return exactRef(e,["methodId","method"]);})||recurring.filter(activeRow).some(function(e){return exactRef(e,["methodId","method"]);});
    if(scope==="expenseCategory")return expenses.filter(activeRow).some(function(e){return exactRef(e,["catId","categoryId"]);})||recurring.filter(activeRow).some(function(e){return String(e.rtype||"expense")!=="income"&&exactRef(e,["catId","categoryId"]);});
    if(scope==="incomeCategory")return incomes.filter(activeRow).some(function(e){return exactRef(e,["type","incomeType","typeId","catId","categoryId"]);})||recurring.filter(activeRow).some(function(e){return String(e.rtype||"")==="income"&&exactRef(e,["incomeType","type","typeId","catId","categoryId"]);});
    return false;
  }
...
export function SortOrderPanel(){
  var ctx=useApp();
  var cats=effectiveExpenseCats(ctx.cats),setCats=ctx.setCats,methods=effectiveMethods(ctx.methods),setMethods=ctx.setMethods,catOrder=ctx.catOrder,setCatOrder=ctx.setCatOrder,methodOrder=ctx.methodOrder,setMethodOrder=ctx.setMethodOrder,catSortMode=ctx.catSortMode,setCatSortMode=ctx.setCatSortMode,methodSortMode=ctx.methodSortMode,setMethodSortMode=ctx.setMethodSortMode,dark=ctx.dark,expenseGroups=effectiveExpenseGroups(ctx.expenseGroups);
  var tc=dark?"#eee":"#333";var sc=dark?"#aaa":"#888";var borderC=dark?"#444":"#eee";var cardBg=dark?"#252535":"#fff";
