...
export function SettingsList({items,setItems,label,showGroup,showIcon,groupList,isMethod,allowArchive,usageScope}){var ctx=useApp();function L(v){return translateFainanceText(v,(ctx&&ctx.lang)||"it");}var t=ctx.t,dark=ctx.dark,isMobile=ctx.isMobile;var expenses=ctx.expenses||[],incomes=ctx.incomes||[],recurring=ctx.recurring||[];var gl=groupList||[];if(usageScope==="expenseCategory")gl=effectiveExpenseGroups(gl);var [newName,setNewName]=useState("");var [newColor,setNewColor]=useState(COLORS[0]);var [newGroup,setNewGroup]=useState(gl[0]?gl[0].id:"");var [newIcon,setNewIcon]=useState("📦");var [editId,setEditId]=useState(null);var [editName,setEditName]=useState("");var [editColor,setEditColor]=useState("");var [editGroup,setEditGroup]=useState(gl[0]?gl[0].id:"");var [editIcon,setEditIcon]=useState("📦");var [sortMode,setSortMode]=useState("group");var dragIdx=useRef(null);var dragOverIdx=useRef(null);var inp={width:"100%",borderRadius:8,border:"1px solid "+(dark?"#444":"#ddd"),padding:"7px 10px",fontSize:14,background:dark?"#2a2a3e":"#fff",color:dark?"#eee":"#333"};var tc=dark?"#eee":"#333";var sc=dark?"#aaa":"#888";var borderC=dark?"#444":"#eee";var baseBlocked=ctx.currentPlan==="free";var setToast=ctx.setToast;function lockedWarn(){if(setToast)setToast({text:"Funzione disponibile dal piano Base.",type:"warning",color:"#EF9F27",icon:"⚠️"});}
  ...
  function fallbackItemId(id){var sid=String(id);var list=(items||[]).filter(function(x){return String(x.id)!==sid&&!x.archived;});var other=list.find(function(x){return String(x.name||"").toLowerCase()==="altro"||String(x.name||"").toLowerCase()==="other";});return other?other.id:(list[0]?list[0].id:"");}
  function cleanupReferencesBeforeDelete(id){var sid=String(id);function same(v){return String(v==null?"":v)===sid;}var fb=fallbackItemId(id);if(usageScope==="expenseCategory"){if(ctx.setExpenses)ctx.setExpenses(function(list){return (list||[]).map(function(e){return same(e.catId)||same(e.categoryId)?{...e,catId:fb||"",categoryId:fb||""}:e;});});if(ctx.setRecurring)ctx.setRecurring(function(list){return (list||[]).map(function(e){return String(e.rtype||"expense")!=="income"&&(same(e.catId)||same(e.categoryId))?{...e,catId:fb||"",categoryId:fb||""}:e;});});if(ctx.setBudgetPlan)ctx.setBudgetPlan(function(prev){var bp=prev||{};return {...bp,items:(bp.items||[]).filter(function(b){return !same(b.catId)&&!same(b.categoryId);})};});if(ctx.setCatOrder)ctx.setCatOrder(function(prev){return (prev||[]).filter(function(x){return String(x)!==sid;});});if(ctx.defaultExpenseCat&&String(ctx.defaultExpenseCat)===sid&&ctx.setDefaultExpenseCat)ctx.setDefaultExpenseCat(fb||"");}
    else if(usageScope==="incomeCategory"){if(ctx.setIncomes)ctx.setIncomes(function(list){return (list||[]).map(function(e){return same(e.type)||same(e.incomeType)||same(e.typeId)||same(e.catId)||same(e.categoryId)?{...e,type:"",incomeType:"",typeId:"",catId:"",categoryId:""}:e;});});if(ctx.setRecurring)ctx.setRecurring(function(list){return (list||[]).map(function(e){return String(e.rtype||"")==="income"&&(same(e.incomeType)||same(e.type)||same(e.typeId))?{...e,incomeType:"",type:"",typeId:""}:e;});});}
    else if(isMethod||usageScope==="method"){if(ctx.setExpenses)ctx.setExpenses(function(list){return (list||[]).map(function(e){return same(e.methodId)||same(e.method)?{...e,methodId:fb||"",method:fb||""}:e;});});if(ctx.setIncomes)ctx.setIncomes(function(list){return (list||[]).map(function(e){return same(e.methodId)||same(e.method)?{...e,methodId:fb||"",method:fb||""}:e;});});if(ctx.setRecurring)ctx.setRecurring(function(list){return (list||[]).map(function(e){return same(e.methodId)||same(e.method)?{...e,methodId:fb||"",method:fb||""}:e;});});}}
  function del(id){if(baseBlocked){lockedWarn();return;}var used=itemIsUsed(id);var msg=used?L("Questa voce ha riferimenti nascosti o ricorrenti. Vuoi eliminarla comunque? I riferimenti verranno spostati o puliti in automatico."):L("Confermi la cancellazione?");if(typeof window!=="undefined"&&!window.confirm(msg))return;if(used)cleanupReferencesBeforeDelete(id);setItems(items.filter(function(i){return String(i.id)!==String(id);}));if(setToast)setToast({text:L("Cancellazione completata"),type:"success",icon:"🗑️"});}
