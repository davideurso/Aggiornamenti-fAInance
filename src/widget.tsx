...
import { useApp, useStorage, COLORS, ICON_DATABASE, ICON_THEMES, DEFAULT_CATS, DEFAULT_METHODS,
...
export function Toggle({label,checked,onChange,color}){var ctx=useApp();var dark=ctx.dark;var c=color||ctx.secondaryButtonColor||"#5FAFE5";return <button type="button" aria-pressed={!!checked} onClick={onChange} style={{display:"flex",alignItems:"center",justifyContent:label?"space-between":"center",gap:10,minWidth:56,minHeight:40,padding:"6px",margin:-6,border:"none",background:"transparent",cursor:"pointer",userSelect:"none",touchAction:"manipulation",WebkitTapHighlightColor:"transparent"}}><div style={{width:46,height:26,borderRadius:13,background:checked?c:"#ccc",position:"relative",transition:"background 0.2s",flexShrink:0}}><div style={{position:"absolute",top:3,left:checked?23:3,width:20,height:20,borderRadius:"50%",background:"#fff",transition:"left 0.2s"}}/></div>{label&&<span style={{fontSize:13,color:checked?c:(dark?"#aaa":"#888"),fontWeight:checked?500:400}}>{label}</span>}</button>;}
...
export function PopupCloseButton({onClick,dark,label}){
  ...
  return <button type="button" onClick={onClick} aria-label={label||"Chiudi"} style={{width:42,height:42,borderRadius:14,border:"1px solid #FCA5A5",background:dark?"#3A2228":"#FFF5F5",color:"#F87171",fontSize:22,fontWeight:950,lineHeight:1,cursor:"pointer",boxShadow:"0 4px 14px #F8717133",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>×</button>;
}

export function FainancePickerModal({open,title,onClose,children,zIndex}){
  var ctx=useApp();var dark=ctx.dark;
  var textC=dark?"#eee":"#333";var borderC=dark?"#444":"#E4E6EC";var cardBg=dark?"#242436":"#fff";
  if(!open)return null;
  return <div role="dialog" aria-modal="true" onClick={function(e){if(e.target===e.currentTarget)onClose();}} style={{position:"fixed",inset:0,zIndex:zIndex||10150,background:"rgba(20,20,30,0.66)",backdropFilter:"blur(5px)",display:"flex",alignItems:"center",justifyContent:"center",padding:16,boxSizing:"border-box"}}>
    <div onClick={function(e){e.stopPropagation();}} style={{width:"min(620px,100%)",maxHeight:"90vh",display:"flex",flexDirection:"column",overflow:"hidden",background:cardBg,border:"1px solid "+borderC,borderRadius:24,boxShadow:"0 24px 80px rgba(20,20,30,0.34)"}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:12,padding:"16px 18px",background:dark?"#2B2B40":"#F7F8FC",borderBottom:"1px solid "+borderC}}>
        <div style={{fontSize:18,fontWeight:950,color:textC,lineHeight:1.15}}>{title}</div>
        <PopupCloseButton onClick={onClose} dark={dark} label={(ctx.translateUiRuntimeText||function(x){return x;})("Chiudi")}/>
      </div>
      <div style={{padding:18,overflowY:"auto",WebkitOverflowScrolling:"touch"}}>{children}</div>
    </div>
  </div>;
}

function IconPickerModal({open,value,onChange,onClose,accent}){
  var ctx=useApp();var dark=ctx.dark;var secondary=ctx.secondaryButtonColor||"#5FAFE5";
  var lang=(ctx&&ctx.lang)||"it";var L2=ctx.translateUiRuntimeText||function(x){return translateFainanceText?translateFainanceText(x,lang):x;};
  var [search,setSearch]=useState("");var [theme,setTheme]=useState("all");
  useEffect(function(){if(open){setSearch("");setTheme("all");}},[open]);
  var selected=normalizeIconGlyph(value)||"📦";
  var rows=useMemo(function(){
    var base=(ICON_DATABASE||[]).slice();
    if(selected&&!base.some(function(item){return normalizeIconGlyph(item.icon)===selected;}))base.unshift({id:"selected_custom",icon:selected,theme:"eventi",label:"",keywords:selected});
    var q=String(search||"").trim().toLowerCase();
    return base.filter(function(item){
      if(theme!=="all"&&item.theme!==theme)return false;
      if(!q)return true;
      var themeInfo=(ICON_THEMES||[]).find(function(t){return t.id===item.theme;});
      var hay=[item.icon,item.label,item.keywords,themeInfo?L2(themeInfo.label):""].join(" ").toLowerCase();
      return hay.indexOf(q)>=0;
    });
  },[search,theme,selected,lang]);
  var border=dark?"#49495D":"#E2E4EA";var panel=dark?"#202033":"#F7F8FC";var text=dark?"#F5F5FA":"#2F3142";var sub=dark?"#AAAABD":"#707382";
  return <FainancePickerModal open={open} title={L2("Scegli un'icona")} onClose={onClose} zIndex={10200}>
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      <div style={{position:"relative"}}>
        <span style={{position:"absolute",left:12,top:"50%",transform:"translateY(-50%)",fontSize:17,color:sub,pointerEvents:"none"}}>🔎</span>
        <input autoFocus value={search} onChange={function(e){setSearch(e.target.value);}} placeholder={L2("Cerca un'icona")} style={{width:"100%",boxSizing:"border-box",border:"1px solid "+border,borderRadius:12,padding:"11px 12px 11px 40px",fontSize:14,background:dark?"#1E1E30":"#fff",color:text,outline:"none"}}/>
      </div>
      <div style={{display:"flex",gap:7,overflowX:"auto",paddingBottom:3,WebkitOverflowScrolling:"touch"}}>
        {(ICON_THEMES||[]).map(function(item){var active=theme===item.id;return <button type="button" key={item.id} onClick={function(){setTheme(item.id);}} style={{flexShrink:0,border:"1px solid "+(active?secondary:border),background:active?secondary:(dark?"#29293D":"#fff"),color:active?"#fff":text,borderRadius:999,padding:"8px 12px",fontSize:12,fontWeight:active?900:750,cursor:"pointer",whiteSpace:"nowrap"}}>{L2(item.label)}</button>;})}
      </div>
      <div style={{background:panel,border:"1px solid "+border,borderRadius:16,padding:10}}>
        {rows.length>0?<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(48px,1fr))",gap:8,maxHeight:"46vh",overflowY:"auto",padding:2}}>
          {rows.map(function(item){var icon=normalizeIconGlyph(item.icon);var active=icon===selected;return <button type="button" key={item.id||icon} title={item.label||icon} onClick={function(){onChange(icon);onClose();}} style={{height:48,minWidth:48,borderRadius:12,border:active?"2px solid "+(accent||secondary):"1px solid "+border,background:active?(accent||secondary)+"22":(dark?"#29293D":"#fff"),fontSize:25,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:active?"0 4px 12px "+(accent||secondary)+"30":"none"}}>{icon}</button>;})}
        </div>:<div style={{textAlign:"center",padding:"30px 12px",fontSize:13,color:sub}}>{L2("Nessuna icona trovata")}</div>}
      </div>
    </div>
  </FainancePickerModal>;
}
...
function normalizeHtmlColor(value){
  ...
  if(/^#[0-9a-fA-F]{3}$/.test(raw))raw="#"+raw.slice(1).split("").map(function(c){return c+c;}).join("");
  if(!/^#[0-9a-fA-F]{6}$/.test(raw))return "";
  return raw.toUpperCase();
}

function clampColorValue(value,min,max){return Math.min(max,Math.max(min,Number(value)||0));}
function hexToRgbColor(value){var hex=normalizeHtmlColor(value)||"#7F77DD";return {r:parseInt(hex.slice(1,3),16),g:parseInt(hex.slice(3,5),16),b:parseInt(hex.slice(5,7),16)};}
function rgbToHexColor(r,g,b){return "#"+[r,g,b].map(function(v){return Math.round(clampColorValue(v,0,255)).toString(16).padStart(2,"0");}).join("").toUpperCase();}
function rgbToHsvColor(r,g,b){
  r=clampColorValue(r,0,255)/255;g=clampColorValue(g,0,255)/255;b=clampColorValue(b,0,255)/255;
...
function hexToHsvColor(value){var rgb=hexToRgbColor(value);return rgbToHsvColor(rgb.r,rgb.g,rgb.b);}
function hsvToHexColor(h,s,v){
  h=((Number(h)||0)%360+360)%360;s=clampColorValue(s,0,100)/100;v=clampColorValue(v,0,100)/100;
...
function ColorPickerModal({open,value,onChange,onClose}){
  var ctx=useApp();var dark=ctx.dark;var primary=ctx.confirmButtonColor||"#378ADD";
  var lang=(ctx&&ctx.lang)||"it";var L2=ctx.translateUiRuntimeText||function(x){return translateFainanceText?translateFainanceText(x,lang):x;};
  var initial=normalizeHtmlColor(value)||"#7F77DD";var initialHsv=hexToHsvColor(initial);
  var [draft,setDraft]=useState(initial);var [html,setHtml]=useState(initial);var [hue,setHue]=useState(initialHsv.h);var [saturation,setSaturation]=useState(initialHsv.s);var [brightness,setBrightness]=useState(initialHsv.v);var [mode,setMode]=useState("preset");
  var svRef=useRef<any>(null);var hueRef=useRef<any>(null);var dragRef=useRef("");
  useEffect(function(){if(open){var next=normalizeHtmlColor(value)||"#7F77DD";var hsv=hexToHsvColor(next);setDraft(next);setHtml(next);setHue(hsv.h);setSaturation(hsv.s);setBrightness(hsv.v);setMode((COLORS||[]).slice(0,10).some(function(c){return normalizeHtmlColor(c)===next;})?"preset":"custom");dragRef.current="";}},[open,value]);
  var valid=normalizeHtmlColor(html);
  var border=dark?"#49495D":"#E2E4EA";var panel=dark?"#202033":"#F7F8FC";var text=dark?"#F5F5FA":"#2F3142";var sub=dark?"#AAAABD":"#707382";
  function chooseDefault(c){onChange(c);onClose();}
  function applyCustom(){if(!valid)return;onChange(valid);onClose();}
  function setFromHsv(nextHue,nextSaturation,nextBrightness){
    var h=clampColorValue(nextHue,0,360),s=clampColorValue(nextSaturation,0,100),v=clampColorValue(nextBrightness,0,100);var color=hsvToHexColor(h,s,v);
    setHue(h);setSaturation(s);setBrightness(v);setDraft(color);setHtml(color);
  }
  function updateSaturationBrightness(e){
    var el=svRef.current;if(!el)return;var rect=el.getBoundingClientRect();var x=clampColorValue(e.clientX-rect.left,0,rect.width);var y=clampColorValue(e.clientY-rect.top,0,rect.height);
    setFromHsv(hue,(x/Math.max(1,rect.width))*100,100-(y/Math.max(1,rect.height))*100);
  }
  function updateHue(e){
    var el=hueRef.current;if(!el)return;var rect=el.getBoundingClientRect();var x=clampColorValue(e.clientX-rect.left,0,rect.width);
    setFromHsv((x/Math.max(1,rect.width))*360,saturation,brightness);
  }
  function startDrag(kind,e){dragRef.current=kind;try{if(e.currentTarget&&e.currentTarget.setPointerCapture)e.currentTarget.setPointerCapture(e.pointerId);}catch(err){}if(kind==="sv")updateSaturationBrightness(e);else updateHue(e);}
  function continueDrag(kind,e){if(dragRef.current!==kind)return;if(kind==="sv")updateSaturationBrightness(e);else updateHue(e);}
  function stopDrag(){dragRef.current="";}
  function changeHtml(raw){setHtml(raw);var color=normalizeHtmlColor(raw);if(color){var hsv=hexToHsvColor(color);setDraft(color);setHue(hsv.h);setSaturation(hsv.s);setBrightness(hsv.v);}}
  return <FainancePickerModal open={open} title={L2("Scegli un colore")} onClose={onClose} zIndex={10200}>
    <div style={{display:"flex",flexDirection:"column",gap:16}}>
      <div style={{display:"grid",gridTemplateColumns:"repeat(2,minmax(0,1fr))",border:"1px solid "+border,borderRadius:12,overflow:"hidden",background:dark?"#29293D":"#F4F5F8"}}>
        <button type="button" onClick={function(){setMode("preset");}} style={{border:"none",borderBottom:mode==="preset"?"3px solid "+primary:"3px solid transparent",background:mode==="preset"?(dark?"#35354B":"#fff"):"transparent",color:mode==="preset"?primary:text,padding:"12px 8px",fontSize:14,fontWeight:mode==="preset"?900:750,cursor:"pointer"}}>{L2("Predefiniti")}</button>
        <button type="button" onClick={function(){setMode("custom");}} style={{border:"none",borderBottom:mode==="custom"?"3px solid "+primary:"3px solid transparent",background:mode==="custom"?(dark?"#35354B":"#fff"):"transparent",color:mode==="custom"?primary:text,padding:"12px 8px",fontSize:14,fontWeight:mode==="custom"?900:750,cursor:"pointer"}}>{L2("Personalizzato")}</button>
      </div>
      {mode==="preset"&&<div style={{background:panel,border:"1px solid "+border,borderRadius:16,padding:14}}>
        <div style={{fontSize:12,fontWeight:900,color:text,marginBottom:11}}>{L2("Colori predefiniti")}</div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(5,minmax(42px,1fr))",gap:10}}>
          {COLORS.slice(0,10).map(function(c){var active=normalizeHtmlColor(value)===normalizeHtmlColor(c);return <button type="button" key={c} aria-label={c} onClick={function(){chooseDefault(c);}} style={{height:48,borderRadius:13,border:active?"3px solid "+(dark?"#fff":"#2F3142"):"3px solid transparent",background:c,cursor:"pointer",boxShadow:active?"0 0 0 3px "+c+"35":"none",display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:18,fontWeight:950,textShadow:"0 1px 4px rgba(0,0,0,.28)"}}>{active?"✓":""}</button>;})}
        </div>
      </div>}
      {mode==="custom"&&<div style={{background:panel,border:"1px solid "+border,borderRadius:16,padding:14}}>
        <div style={{fontSize:12,fontWeight:900,color:text,marginBottom:11}}>{L2("Colore personalizzato")}</div>
        <div ref={svRef} role="slider" aria-label={L2("Colore personalizzato")} tabIndex={0} onPointerDown={function(e){startDrag("sv",e);}} onPointerMove={function(e){continueDrag("sv",e);}} onPointerUp={stopDrag} onPointerCancel={stopDrag} style={{position:"relative",height:190,borderRadius:15,overflow:"hidden",cursor:"crosshair",touchAction:"none",background:"linear-gradient(to top,#000,transparent),linear-gradient(to right,#fff,hsl("+hue+",100%,50%))",border:"1px solid "+border,boxShadow:dark?"inset 0 0 0 1px rgba(255,255,255,.04)":"inset 0 0 0 1px rgba(0,0,0,.03)"}}>
          <span style={{position:"absolute",left:saturation+"%",top:(100-brightness)+"%",width:22,height:22,borderRadius:"50%",border:"3px solid #fff",boxShadow:"0 1px 8px rgba(0,0,0,.75)",transform:"translate(-50%,-50%)",background:draft,pointerEvents:"none",boxSizing:"border-box"}}/>
        </div>
        <div ref={hueRef} role="slider" aria-label={L2("Colore")} tabIndex={0} onPointerDown={function(e){startDrag("hue",e);}} onPointerMove={function(e){continueDrag("hue",e);}} onPointerUp={stopDrag} onPointerCancel={stopDrag} style={{position:"relative",height:30,borderRadius:999,marginTop:13,cursor:"pointer",touchAction:"none",background:"linear-gradient(to right,#F00 0%,#FF0 16.66%,#0F0 33.33%,#0FF 50%,#00F 66.66%,#F0F 83.33%,#F00 100%)",border:"1px solid "+border}}>
          <span style={{position:"absolute",left:(hue/360*100)+"%",top:"50%",width:24,height:36,borderRadius:10,border:"3px solid #fff",boxShadow:"0 2px 8px rgba(0,0,0,.5)",transform:"translate(-50%,-50%)",background:"hsl("+hue+",100%,50%)",pointerEvents:"none",boxSizing:"border-box"}}/>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"64px minmax(0,1fr)",gap:12,alignItems:"end",marginTop:14}}>
          <div style={{width:64,height:46,borderRadius:12,background:draft,border:"2px solid "+(dark?"rgba(255,255,255,.3)":"rgba(47,49,66,.16)"),boxShadow:"0 4px 14px "+draft+"35",boxSizing:"border-box"}}/>
          <div><label style={{fontSize:11,fontWeight:800,color:sub,display:"block",marginBottom:6}}>{L2("Codice HTML")}</label><input value={html} onChange={function(e){changeHtml(e.target.value);}} placeholder="#7F77DD" style={{width:"100%",height:46,boxSizing:"border-box",border:"1px solid "+(html&&!valid?"#E24B4A":border),borderRadius:12,padding:"0 12px",fontSize:15,fontWeight:850,letterSpacing:.4,background:dark?"#29293D":"#fff",color:text,outline:"none"}}/></div>
        </div>
        {html&&!valid&&<div style={{fontSize:11,color:"#E24B4A",fontWeight:800,marginTop:7}}>{L2("Inserisci un codice HTML valido")}</div>}
      </div>}
      {mode==="custom"&&<div style={{display:"grid",gridTemplateColumns:"repeat(2,minmax(0,1fr))",gap:10}}>
        <button type="button" disabled={!valid} onClick={applyCustom} style={{border:"none",borderRadius:ctx.btnRadius||12,padding:"12px 14px",background:valid?primary:"#A8A8A8",color:"#fff",fontSize:14,fontWeight:900,cursor:valid?"pointer":"not-allowed"}}>{L2("Applica colore")}</button>
        <button type="button" onClick={onClose} style={{border:"1px solid "+border,borderRadius:ctx.btnRadius||12,padding:"12px 14px",background:dark?"#333348":"#EFF0F4",color:text,fontSize:14,fontWeight:850,cursor:"pointer"}}>{L2("Annulla")}</button>
      </div>}
    </div>
  </FainancePickerModal>;
}
...
export function AppIconSelector({value,onChange,accent,dark:darkProp,compact}){
  var ctx=useApp();var dark=darkProp==null?ctx.dark:darkProp;var [open,setOpen]=useState(false);var selected=normalizeIconGlyph(value)||"🎯";var border=dark?"#49495D":"#E2E4EA";var panel=dark?"#29293D":"#fff";var chevron=dark?"#AAAABD":"#7B7E8D";var small=!!compact;
  return <><button type="button" onClick={function(){setOpen(true);}} style={{width:small?46:"100%",minWidth:small?46:0,height:small?38:54,display:"flex",alignItems:"center",justifyContent:small?"center":"space-between",gap:10,background:panel,border:"1px solid "+border,borderRadius:small?10:12,padding:small?4:"7px 10px",cursor:"pointer",boxSizing:"border-box",flexShrink:0}}>
    <span style={{width:small?30:40,height:small?30:40,borderRadius:small?9:11,display:"flex",alignItems:"center",justifyContent:"center",fontSize:small?20:24,background:(accent||"#7F77DD")+"20",border:"1px solid "+(accent||"#7F77DD")+"55"}}>{selected}</span>
    {!small&&<span style={{fontSize:22,color:chevron,fontWeight:850}}>›</span>}
  </button><IconPickerModal open={open} value={selected} onChange={onChange} onClose={function(){setOpen(false);}} accent={accent}/></>;
}

export function AppColorSelector({value,onChange,dark:darkProp,disabled,compact}){
  var ctx=useApp();var dark=darkProp==null?ctx.dark:darkProp;var [open,setOpen]=useState(false);var selected=normalizeHtmlColor(value)||"#7F77DD";var border=dark?"#49495D":"#E2E4EA";var panel=dark?"#29293D":"#fff";var chevron=dark?"#AAAABD":"#7B7E8D";
  var small=!!compact;
  return <><button type="button" disabled={!!disabled} onClick={function(){if(!disabled)setOpen(true);}} aria-label={L("Colore")} style={{width:small?42:"100%",minWidth:small?42:0,height:small?38:54,display:"flex",alignItems:"center",justifyContent:small?"center":"flex-start",gap:10,background:panel,border:"1px solid "+border,borderRadius:small?10:12,padding:small?4:"7px 10px",cursor:disabled?"not-allowed":"pointer",opacity:disabled?0.55:1,boxSizing:"border-box",flexShrink:0}}>
    <span style={{width:small?28:36,height:small?28:36,borderRadius:small?8:11,background:selected,border:"2px solid "+(dark?"rgba(255,255,255,.34)":"rgba(47,49,66,.18)"),boxShadow:"0 3px 10px "+selected+"35",flexShrink:0}}/>
    {!small&&<><span style={{flex:1}}/><span style={{fontSize:22,color:chevron,fontWeight:850}}>›</span></>}
  </button><ColorPickerModal open={open} value={selected} onChange={onChange} onClose={function(){setOpen(false);}}/></>;
}
    ...
    <div style={{background:"#fff",borderRadius:20,width:"100%",maxWidth:420,overflow:"hidden",boxShadow:"0 8px 40px rgba(0,0,0,0.2)"}}>
      ...
      <div style={{padding:"16px 20px",maxHeight:340,overflowY:"auto"}}>
        {newAlerts.map(function(al){return <div key={al.id} style={{marginBottom:14,padding:"12px 14px",background:"#fff5f5",borderRadius:12,border:"1px solid #fcc"}}>
          <div style={{fontSize:14,fontWeight:600,color:"#C03030",marginBottom:4,display:"flex",alignItems:"center",gap:7}}><span style={{fontSize:18}}>{normalizeIconGlyph(al.icon)||"🔔"}</span><span>{al.name}</span></div>
...
export function GoalsPanel(){
  var ctx=useApp();var t=ctx.t,goals=ctx.goals,setGoals=ctx.setGoals,sym=ctx.sym,dark=ctx.dark,btnRadius=ctx.btnRadius,secondaryButtonColor=ctx.secondaryButtonColor||"#5FAFE5";
  ...
  function addGoal(){if(!goalFormValid)return;if(editingGoalId){setGoals(function(p){return p.map(function(g){return g.id===editingGoalId?{...g,...form,id:g.id,target:parseFloat(form.target),saved:parseFloat(form.saved)||0,icon:normalizeIconGlyph(form.icon)||"🎯"}:g;});});setEditingGoalId(null);setForm(blank);setShowAdd(false);if(setToast)setToast(L("Obiettivo modificato correttamente"));return;}if(!goalsAllowed()){showGoalLimit();return;}setGoals(function(p){return [...p,{...form,id:Date.now(),target:parseFloat(form.target),saved:parseFloat(form.saved)||0,icon:normalizeIconGlyph(form.icon)||"🎯"}];});setForm(blank);setShowAdd(false);if(setToast)setToast(L("Obiettivo creato correttamente"));}
  function startEditGoal(g){setEditingGoalId(g.id);setForm({...blank,...g,target:String(g.target||0),saved:String(g.saved||0),icon:normalizeIconGlyph(g.icon)||"🎯"});setShowAdd(true);}
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:16}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:10}}><div><div style={{fontSize:16,fontWeight:900,color:tc}}>🎯 {L("Obiettivi di risparmio")}</div>{planRemaining&&<div style={{fontSize:12,color:goalsAllowed()?sc:(dark?"#ffd58a":"#856404"),marginTop:2}}>{L("Disponibili")}: {planRemaining("goals",(goals||[]).length)===Infinity?L("illimitati"):planRemaining("goals",(goals||[]).length)}</div>}</div><Btn onClick={function(){if(!goalsAllowed()){showGoalLimit();return;}setEditingGoalId(null);setForm(blank);setShowAdd(true);}} bg={goalsAllowed()?null:"#999"} style={{padding:"10px 18px",fontWeight:900}}>{L("Crea Obiettivo")}</Btn></div>
    ...
    {showAdd&&<div role="dialog" aria-modal="true" onClick={function(e){if(e.target===e.currentTarget)cancelGoalForm();}} style={{position:"fixed",inset:0,zIndex:10000,background:"rgba(20,20,30,0.62)",backdropFilter:"blur(5px)",display:"flex",alignItems:"center",justifyContent:"center",padding:16,boxSizing:"border-box"}}>
      <div onClick={function(e){e.stopPropagation();}} style={{background:dark?"#242436":"#fff",borderRadius:24,border:"1px solid "+borderC,width:"min(620px,100%)",maxHeight:"92vh",overflowY:"auto",boxSizing:"border-box",boxShadow:"0 24px 80px rgba(20,20,30,0.34)"}}>
        <div style={{display:"flex",alignItems:"center",gap:13,padding:"18px 20px",background:dark?"#2B2B40":"#F7F8FC",borderBottom:"1px solid "+borderC,borderRadius:"24px 24px 0 0"}}>
          <div style={{flex:1,minWidth:0}}><div style={{fontSize:19,fontWeight:950,color:tc,lineHeight:1.15}}>{L(editingGoalId?"Modifica Obiettivo":"Crea Obiettivo")}</div></div>
          <PopupCloseButton onClick={cancelGoalForm} dark={dark} label={L("Chiudi")}/>
        </div>
        <div style={{padding:20,display:"flex",flexDirection:"column",gap:14}}>
          <div style={{display:"grid",gridTemplateColumns:"repeat(2,minmax(0,1fr))",gap:12,alignItems:"stretch"}}>
            <div style={{minWidth:0}}><label style={{fontSize:11,fontWeight:850,color:sc,display:"block",marginBottom:7,textTransform:"uppercase",letterSpacing:.4}}>{L("Icona")}</label><AppIconSelector value={form.icon||"🎯"} onChange={function(icon){setForm(function(p){return{...p,icon:icon};});}} accent={form.color||"#7F77DD"} dark={dark}/></div>
            <div style={{minWidth:0}}><label style={{fontSize:11,fontWeight:850,color:sc,display:"block",marginBottom:7,textTransform:"uppercase",letterSpacing:.4}}>{L("Colore")}</label><AppColorSelector value={form.color||"#7F77DD"} onChange={function(color){setForm(function(p){return{...p,color:color};});}} dark={dark}/></div>
          </div>
          <div style={{background:dark?"#202033":"#F8F8FC",border:"1px solid "+borderC,borderRadius:16,padding:14}}>
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(190px,1fr))",gap:12}}>
              <div><label style={{fontSize:11,fontWeight:800,color:sc,display:"block",marginBottom:5}}>{L("Nome")}</label><input value={form.name} onChange={function(e){setForm(function(p){return{...p,name:e.target.value};});}} style={{...inp,borderRadius:12,padding:"11px 12px"}} placeholder={L("es. Vacanza")}/></div>
              <div><label style={{fontSize:11,fontWeight:800,color:sc,display:"block",marginBottom:5}}>{L("Periodo")}</label><div style={{display:"flex",background:dark?"#2B2B40":"#ECEEF4",borderRadius:12,padding:4,gap:4}}><button type="button" onClick={function(){setForm(function(p){return{...p,period:"monthly"};});}} style={{flex:1,padding:"9px 8px",border:"1px solid "+(form.period==="monthly"?secondaryButtonColor:"transparent"),borderRadius:9,background:form.period==="monthly"?secondaryButtonColor:"transparent",color:form.period==="monthly"?"#fff":sc,fontWeight:form.period==="monthly"?850:650,cursor:"pointer",boxShadow:form.period==="monthly"&&!dark?"0 3px 10px "+secondaryButtonColor+"40":"none"}}>{L("Mensile")}</button><button type="button" onClick={function(){setForm(function(p){return{...p,period:"annual"};});}} style={{flex:1,padding:"9px 8px",border:"1px solid "+(form.period==="annual"?secondaryButtonColor:"transparent"),borderRadius:9,background:form.period==="annual"?secondaryButtonColor:"transparent",color:form.period==="annual"?"#fff":sc,fontWeight:form.period==="annual"?850:650,cursor:"pointer",boxShadow:form.period==="annual"&&!dark?"0 3px 10px "+secondaryButtonColor+"40":"none"}}>{L("Annuale")}</button></div></div>
              <div><label style={{fontSize:11,fontWeight:800,color:sc,display:"block",marginBottom:5}}>Target ({sym})</label><input type="number" value={form.target} onChange={function(e){setForm(function(p){return{...p,target:e.target.value};});}} style={{...inp,borderRadius:12,padding:"11px 12px"}}/></div>
              <div><label style={{fontSize:11,fontWeight:800,color:sc,display:"block",marginBottom:5}}>{L("Già risparmiato")}</label><input type="number" value={form.saved} onChange={function(e){setForm(function(p){return{...p,saved:e.target.value};});}} style={{...inp,borderRadius:12,padding:"11px 12px"}}/></div>
              <div style={{gridColumn:"1/-1"}}><label style={{fontSize:11,fontWeight:800,color:sc,display:"block",marginBottom:5}}>{L("Scadenza")}</label><input type="date" value={form.deadline} onChange={function(e){setForm(function(p){return{...p,deadline:e.target.value};});}} style={{...inp,borderRadius:12,padding:"11px 12px"}}/></div>
            </div>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(2,minmax(0,1fr))",gap:10}}><Btn onClick={addGoal} disabled={!goalFormValid} bg={goalFormValid?null:"#A8A8A8"} style={{padding:"12px 16px",fontWeight:900}}>{editingGoalId?L("Modifica Obiettivo"):L("Crea Obiettivo")}</Btn><Btn onClick={cancelGoalForm} bg={dark?"#333348":"#EFF0F4"} color={dark?"#eee":"#5E6070"} style={{padding:"12px 16px",fontWeight:850}}>{t.cancel}</Btn></div>
        </div>
      </div>
    </div>}
    {goals.length===0&&!showAdd&&<div style={{textAlign:"center",color:"#ccc",padding:"40px 0",fontSize:14}}>{L("Nessun obiettivo. Creane uno!")}</div>}
    <div style={{display:"flex",flexDirection:"column",gap:14}}>{goals.map(function(g,idx){var saved=Number(g.saved||0),target=Number(g.target||0),pct=target>0?Math.min(100,(saved/target)*100):0,done=pct>=100,daysLeft=null;if(g.deadline)daysLeft=Math.ceil((new Date(g.deadline)-new Date())/86400000);return <div key={g.id} style={{background:cardBg,borderRadius:16,border:"1px solid "+borderC,padding:16,position:"relative",overflow:"hidden",boxShadow:dark?"none":"0 4px 18px rgba(0,0,0,0.04)"}}><div style={{position:"absolute",top:0,left:0,width:pct+"%",height:4,background:done?"#1D9E75":g.color}}/><div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:12,marginBottom:10}}><div style={{display:"flex",alignItems:"center",gap:12,minWidth:0}}><div style={{width:48,height:48,borderRadius:14,background:g.color+"22",display:"flex",alignItems:"center",justifyContent:"center",fontSize:24,flexShrink:0}}>{normalizeIconGlyph(g.icon)||"🎯"}</div><div style={{minWidth:0}}><div style={{fontSize:18,fontWeight:900,color:tc,lineHeight:1.12}}>{g.name}</div><div style={{fontSize:12,color:sc,marginTop:3}}>{L(g.period==="annual"?"Annuale":"Mensile")}{g.deadline&&" · "+(daysLeft>0?daysLeft+L("g"):L("Scaduto"))}</div></div></div><div style={{display:"flex",gap:5,alignItems:"center",flexShrink:0}}><button disabled={idx===0} onClick={function(){moveGoal(g.id,-1);}} style={{width:30,height:30,borderRadius:10,border:"1px solid "+borderC,background:dark?"#1e1e30":"#f8f8f8",color:idx===0?"#bbb":tc,cursor:idx===0?"default":"pointer"}}>↑</button><button disabled={idx===goals.length-1} onClick={function(){moveGoal(g.id,1);}} style={{width:30,height:30,borderRadius:10,border:"1px solid "+borderC,background:dark?"#1e1e30":"#f8f8f8",color:idx===goals.length-1?"#bbb":tc,cursor:idx===goals.length-1?"default":"pointer"}}>↓</button><button title={L("Modifica Obiettivo")} onClick={function(){startEditGoal(g);}} style={{background:"#EEF4FF",border:"1px solid #BFD7FF",cursor:"pointer",color:"#378ADD",fontSize:14,padding:"5px 8px",borderRadius:8,fontWeight:700}}>✏️</button><button title={L("Elimina")} onClick={function(){if(!window.confirm(L("Eliminare questo obiettivo?")))return;setGoals(function(p){return p.filter(function(x){return x.id!==g.id;});});if(setToast)setToast(L("Obiettivo eliminato correttamente"));}} style={{background:"#FFF0F0",border:"1px solid #FFD0D0",cursor:"pointer",color:"#E24B4A",fontSize:14,padding:"5px 8px",borderRadius:8,fontWeight:700}}>🗑️</button></div></div><div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",gap:10,marginBottom:6}}><span style={{fontSize:18,fontWeight:900,color:g.color}}>{fmtAmt(saved,sym)}</span><span style={{fontSize:14,color:sc}}>{L("di")} {fmtAmt(target,sym)}</span></div><div style={{background:dark?"#333":"#f0f0f0",borderRadius:8,height:9,overflow:"hidden"}}><div style={{background:done?"#1D9E75":g.color,height:9,borderRadius:8,width:pct+"%"}}/></div><div style={{display:"flex",justifyContent:"space-between",marginTop:5,marginBottom:12}}><span style={{fontSize:12,color:done?"#1D9E75":sc,fontWeight:700}}>{done?L("Completato!"):Math.round(pct)+"%"}</span>{!done&&<span style={{fontSize:12,color:sc}}>{L("mancano")} {fmtAmt(Math.max(0,target-saved),sym)}</span>}</div>{!done&&<div style={{background:dark?"#1e1e30":"#F8FAFC",border:"1px solid "+(dark?"#444":"#E5E7EB"),borderRadius:14,padding:12}}><div style={{fontSize:12,color:sc,fontWeight:800,marginBottom:8}}>{L("Aggiungi importo risparmiato")}</div><div style={{display:"grid",gridTemplateColumns:"minmax(0,1fr) auto",gap:10,alignItems:"center"}}><label style={{display:"flex",alignItems:"center",gap:8,background:dark?"#252535":"#fff",border:"1px solid "+(dark?"#555":"#DADDE5"),borderRadius:12,padding:"0 12px",minWidth:0}}><span style={{fontSize:18,color:g.color,fontWeight:900}}>{sym}</span><input type="number" placeholder="0,00" value={addAmt[g.id]||""} onChange={function(e){setAddAmt(function(p){return{...p,[g.id]:e.target.value};});}} onKeyDown={function(e){if(e.key==="Enter"&&Number(addAmt[g.id])>0)addSaving(g.id);}} style={{width:"100%",border:"none",outline:"none",padding:"13px 0",fontSize:16,background:"transparent",color:tc,minWidth:0}}/></label><button onClick={function(){addSaving(g.id);}} disabled={!(Number(addAmt[g.id])>0)} style={{height:48,minWidth:116,border:"none",borderRadius:12,background:Number(addAmt[g.id])>0?g.color:"#A8A8A8",color:"#fff",fontSize:15,fontWeight:900,cursor:Number(addAmt[g.id])>0?"pointer":"not-allowed",boxShadow:Number(addAmt[g.id])>0?(dark?"none":"0 5px 14px "+g.color+"44"):"none"}}>+ {L("Aggiungi")}</button></div></div>}</div>;})}</div>
  </div>;
}
export function AlertsPanel(){
  var ctx=useApp();var t=ctx.t,alerts=ctx.alerts,setAlerts=ctx.setAlerts,cats=effectiveExpenseCats(ctx.cats),expenses=ctx.expenses,sym=ctx.sym,dark=ctx.dark,curMonthKey=ctx.curMonthKey,budgetPlan=ctx.budgetPlan,expenseGroups=effectiveExpenseGroups(ctx.expenseGroups),setToast=ctx.setToast,secondaryButtonColor=ctx.secondaryButtonColor||"#5FAFE5";
  function L(s){return translateFainanceText(s,(ctx&&ctx.lang)||"it");}
  ...
  var blank={name:"",icon:"🔔",color:"#E24B4A",type:"cat",catId:cats[0]?cats[0].id:1,groupId:groups[0]?groups[0].id:"casa",budget:"",triggerMode:"immediate",triggerPct:0,customText:"",period:"monthly"};
  ...
  function normalizeForm(){return{...form,icon:normalizeIconGlyph(form.icon)||"🔔",catId:Number(form.catId),budget:parseFloat(form.budget),triggerPct:parseFloat(form.triggerPct)||0};}
  ...
  function startEdit(al){setForm({name:al.name||"",icon:normalizeIconGlyph(al.icon)||"🔔",color:al.color||"#E24B4A",type:al.type||"cat",catId:al.catId||blank.catId,groupId:al.groupId||blank.groupId,budget:String(al.budget||""),triggerMode:al.triggerMode||"immediate",triggerPct:al.triggerPct||0,customText:al.customText||"",period:al.period||"monthly"});setEditingId(al.id);setShowAdd(true);}
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:16}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:10}}><div style={{display:"flex",alignItems:"center",gap:8}}><div style={{fontSize:14,fontWeight:700,color:tc}}>🔔 {L("Alert di spesa")}</div>{triggered>0&&<span style={{background:"#E24B4A",color:"#fff",borderRadius:20,padding:"2px 8px",fontSize:12,fontWeight:600}}>{triggered} {L("attivi")}</span>}</div><Btn onClick={function(){if(alertLimitReached)return;setEditingId(null);setForm(blank);setShowAdd(true);}} bg={alertLimitReached?"#999":null} disabled={alertLimitReached}>{L("Crea Alert")}</Btn></div>
    {alertLimitReached&&<LimitReachedBox/>}
    {showAdd&&!alertLimitReached&&<div role="dialog" aria-modal="true" onClick={function(e){if(e.target===e.currentTarget)cancelForm();}} style={{position:"fixed",inset:0,zIndex:10000,background:"rgba(20,20,30,0.62)",backdropFilter:"blur(5px)",display:"flex",alignItems:"center",justifyContent:"center",padding:16,boxSizing:"border-box"}}>
      <div onClick={function(e){e.stopPropagation();}} style={{background:dark?"#242436":"#fff",borderRadius:24,border:"1px solid "+borderC,width:"min(620px,100%)",maxHeight:"92vh",overflowY:"auto",boxSizing:"border-box",boxShadow:"0 24px 80px rgba(20,20,30,0.34)"}}>
        <div style={{display:"flex",alignItems:"center",gap:13,padding:"18px 20px",background:dark?"#2B2B40":"#F7F8FC",borderBottom:"1px solid "+borderC,borderRadius:"24px 24px 0 0"}}>
          <div style={{flex:1,minWidth:0}}><div style={{fontSize:19,fontWeight:950,color:tc,lineHeight:1.15}}>{L(editingId?"Modifica Alert":"Crea Alert")}</div></div>
          <PopupCloseButton onClick={cancelForm} dark={dark} label={L("Chiudi")}/>
        </div>
        <div style={{padding:20,display:"flex",flexDirection:"column",gap:14}}>
          <div style={{display:"grid",gridTemplateColumns:"repeat(2,minmax(0,1fr))",gap:12,alignItems:"stretch"}}><div style={{minWidth:0}}><label style={{fontSize:11,fontWeight:850,color:sc,display:"block",marginBottom:7,textTransform:"uppercase",letterSpacing:.4}}>{L("Icona")}</label><AppIconSelector value={form.icon||"🔔"} onChange={function(icon){setForm(function(p){return{...p,icon:icon};});}} accent={form.color||"#E24B4A"} dark={dark}/></div><div style={{minWidth:0}}><label style={{fontSize:11,fontWeight:850,color:sc,display:"block",marginBottom:7,textTransform:"uppercase",letterSpacing:.4}}>{L("Colore")}</label><AppColorSelector value={form.color||"#E24B4A"} onChange={function(color){setForm(function(p){return{...p,color:color};});}} dark={dark}/></div></div>
          <div style={{background:dark?"#202033":"#F8F8FC",border:"1px solid "+borderC,borderRadius:16,padding:14,display:"flex",flexDirection:"column",gap:12}}>
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(190px,1fr))",gap:12}}><div><label style={{fontSize:11,fontWeight:800,color:sc,display:"block",marginBottom:5}}>{L("Nome alert")}</label><input value={form.name} onChange={function(e){setForm(function(p){return{...p,name:e.target.value};});}} style={{...inp,borderRadius:12,padding:"11px 12px"}} placeholder={L("es. Budget ristorante")}/></div><div><label style={{fontSize:11,fontWeight:800,color:sc,display:"block",marginBottom:5}}>{L("Budget")} ({sym})</label><input type="number" value={form.budget} onChange={function(e){setForm(function(p){return{...p,budget:e.target.value};});}} style={{...inp,borderRadius:12,padding:"11px 12px"}} placeholder="200"/></div></div>
            <div><label style={{fontSize:11,fontWeight:800,color:sc,display:"block",marginBottom:5}}>{L("Periodo")}</label><div style={{display:"flex",background:dark?"#2B2B40":"#ECEEF4",borderRadius:12,padding:4,gap:4}}><button type="button" onClick={function(){setForm(function(p){return{...p,period:"monthly"};});}} style={{flex:1,padding:"9px 8px",border:"1px solid "+(form.period==="monthly"?secondaryButtonColor:"transparent"),borderRadius:9,background:form.period==="monthly"?secondaryButtonColor:"transparent",color:form.period==="monthly"?"#fff":sc,fontWeight:form.period==="monthly"?850:650,cursor:"pointer",boxShadow:form.period==="monthly"&&!dark?"0 3px 10px "+secondaryButtonColor+"40":"none"}}>{L("Mensile")}</button><button type="button" onClick={function(){setForm(function(p){return{...p,period:"annual"};});}} style={{flex:1,padding:"9px 8px",border:"1px solid "+(form.period==="annual"?secondaryButtonColor:"transparent"),borderRadius:9,background:form.period==="annual"?secondaryButtonColor:"transparent",color:form.period==="annual"?"#fff":sc,fontWeight:form.period==="annual"?850:650,cursor:"pointer",boxShadow:form.period==="annual"&&!dark?"0 3px 10px "+secondaryButtonColor+"40":"none"}}>{L("Annuale")}</button></div></div>
            <div><label style={{fontSize:11,fontWeight:800,color:sc,display:"block",marginBottom:5}}>{L("Testo personalizzato")}</label><textarea value={form.customText} onChange={function(e){setForm(function(p){return{...p,customText:e.target.value};});}} style={{...inp,borderRadius:12,padding:"11px 12px",height:72,resize:"vertical"}} placeholder={L("es. Troppo al ristorante!")}/></div>
          </div>
          <div style={{background:dark?"#202033":"#F8F8FC",border:"1px solid "+borderC,borderRadius:16,padding:14,display:"flex",flexDirection:"column",gap:12}}>
            <div><label style={{fontSize:11,fontWeight:800,color:sc,display:"block",marginBottom:5}}>{L("Tipo soglia")}</label><div style={{display:"flex",background:dark?"#2B2B40":"#ECEEF4",borderRadius:12,padding:4,gap:4}}><button type="button" onClick={function(){handleTypeChange("cat");}} style={{flex:1,padding:"9px 8px",border:"1px solid "+(form.type==="cat"?secondaryButtonColor:"transparent"),borderRadius:9,background:form.type==="cat"?secondaryButtonColor:"transparent",color:form.type==="cat"?"#fff":sc,fontWeight:form.type==="cat"?850:650,cursor:"pointer",boxShadow:form.type==="cat"&&!dark?"0 3px 10px "+secondaryButtonColor+"40":"none"}}>{L("Singola categoria")}</button><button type="button" onClick={function(){handleTypeChange("group");}} style={{flex:1,padding:"9px 8px",border:"1px solid "+(form.type==="group"?secondaryButtonColor:"transparent"),borderRadius:9,background:form.type==="group"?secondaryButtonColor:"transparent",color:form.type==="group"?"#fff":sc,fontWeight:form.type==="group"?850:650,cursor:"pointer",boxShadow:form.type==="group"&&!dark?"0 3px 10px "+secondaryButtonColor+"40":"none"}}>{L("Area")}</button></div></div>
            {form.type==="cat"?<div><label style={{fontSize:11,fontWeight:800,color:sc,display:"block",marginBottom:5}}>{L("Categoria")}</label><select value={form.catId} onChange={function(e){handleCatChange(e.target.value);}} style={{...inp,borderRadius:12,padding:"11px 12px"}}>{groups.map(function(g){return <optgroup key={g.id} label={g.name}>{safeCatsForGroup(cats,groups,g).map(function(c){return <option key={c.id} value={c.id}>{c.icon} {c.name}</option>;})}</optgroup>;})}</select></div>:<div><label style={{fontSize:11,fontWeight:800,color:sc,display:"block",marginBottom:5}}>{L("Area")}</label><select value={form.groupId} onChange={function(e){handleGroupChange(e.target.value);}} style={{...inp,borderRadius:12,padding:"11px 12px"}}>{groups.map(function(g){return <option key={g.id} value={g.id}>{g.name}</option>;})}</select></div>}
          </div>
          <div style={{background:dark?"#202033":"#F8F8FC",border:"1px solid "+borderC,borderRadius:16,padding:14,display:"flex",flexDirection:"column",gap:12}}>
            <div><label style={{fontSize:11,fontWeight:800,color:sc,display:"block",marginBottom:5}}>{L("Modalità attivazione")}</label><div style={{display:"flex",background:dark?"#2B2B40":"#ECEEF4",borderRadius:12,padding:4,gap:4}}><button type="button" onClick={function(){setForm(function(p){return{...p,triggerMode:"immediate"};});}} style={{flex:1,padding:"9px 8px",border:"1px solid "+(form.triggerMode==="immediate"?secondaryButtonColor:"transparent"),borderRadius:9,background:form.triggerMode==="immediate"?secondaryButtonColor:"transparent",color:form.triggerMode==="immediate"?"#fff":sc,fontWeight:form.triggerMode==="immediate"?850:650,cursor:"pointer",boxShadow:form.triggerMode==="immediate"&&!dark?"0 3px 10px "+secondaryButtonColor+"40":"none"}}>{L("Superamento immediato")}</button><button type="button" onClick={function(){setForm(function(p){return{...p,triggerMode:"pct"};});}} style={{flex:1,padding:"9px 8px",border:"1px solid "+(form.triggerMode==="pct"?secondaryButtonColor:"transparent"),borderRadius:9,background:form.triggerMode==="pct"?secondaryButtonColor:"transparent",color:form.triggerMode==="pct"?"#fff":sc,fontWeight:form.triggerMode==="pct"?850:650,cursor:"pointer",boxShadow:form.triggerMode==="pct"&&!dark?"0 3px 10px "+secondaryButtonColor+"40":"none"}}>{L("Dopo % superamento")}</button></div></div>
            {form.triggerMode==="pct"&&<div><label style={{fontSize:11,fontWeight:800,color:sc,display:"block",marginBottom:5}}>% (es. 25 attiva al 125%)</label><input type="number" min={0} max={200} value={form.triggerPct} onChange={function(e){setForm(function(p){return{...p,triggerPct:e.target.value};});}} style={{...inp,borderRadius:12,padding:"11px 12px"}} placeholder="25"/></div>}
          </div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(2,minmax(0,1fr))",gap:10}}><Btn onClick={saveAlert} disabled={!alertFormValid} bg={alertFormValid?null:"#A8A8A8"} style={{padding:"12px 16px",fontWeight:900}}>{L(editingId?"Modifica Alert":"Crea Alert")}</Btn><Btn onClick={cancelForm} bg={dark?"#333348":"#EFF0F4"} color={dark?"#eee":"#5E6070"} style={{padding:"12px 16px",fontWeight:850}}>{t.cancel}</Btn></div>
        </div>
      </div>
    </div>}
    {alerts.length===0&&!showAdd&&<div style={{textAlign:"center",color:"#ccc",padding:"40px 0",fontSize:14}}>{L("Nessun alert configurato.")}</div>}
    {alerts.map(function(al){var spent=getSpent(al),p=getPct(al),trig=isTriggered(al),barColor=p>=100?"#E24B4A":p>=75?"#EF9F27":"#1D9E75";var cat2=al.type==="cat"?cats.find(function(c){return c.id===al.catId;}):null;var grp2=al.type==="group"?groups.find(function(g){return g.id===al.groupId;}):null;var label=al.type==="cat"?(cat2?cat2.icon+" "+cat2.name:"?"):("📂 "+(grp2?grp2.name:al.groupId));var tLabel=al.triggerMode==="immediate"?L("al 100%"):L("al ")+(100+(al.triggerPct||0))+"%";var alertIcon=normalizeIconGlyph(al.icon)||"🔔";var alertColor=al.color||"#E24B4A";return <div key={al.id} style={{background:trig?(dark?"#3a1a1a":"#fff5f5"):(dark?"#252535":"#fff"),borderRadius:16,border:"1px solid "+(trig?"#F3A9A9":borderC),padding:"15px 16px",boxShadow:dark?"none":"0 4px 18px rgba(0,0,0,.04)",position:"relative",overflow:"hidden"}}><div style={{position:"absolute",top:0,left:0,right:0,height:4,background:alertColor}}/>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10,gap:12}}>
        <div style={{display:"flex",alignItems:"flex-start",gap:11,minWidth:0}}><div style={{width:44,height:44,borderRadius:14,background:alertColor+"18",border:"1px solid "+alertColor+"55",display:"flex",alignItems:"center",justifyContent:"center",fontSize:23,flexShrink:0}}>{alertIcon}</div><div style={{minWidth:0}}><div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>{trig&&<span>⚠️</span>}<div style={{fontSize:15,fontWeight:850,color:trig?"#E24B4A":tc}}>{al.name}</div><span style={{fontSize:10,background:al.period==="annual"?"#EEF":"#EFE",color:al.period==="annual"?"#534AB7":"#1D9E75",borderRadius:10,padding:"2px 7px",fontWeight:750}}>{L(al.period==="annual"?"Annuale":"Mensile")}</span></div><div style={{fontSize:11,color:sc,marginTop:3}}>{label} · {L("attiva")} {tLabel}</div>{al.customText&&<div style={{fontSize:12,color:trig?"#E24B4A":sc,marginTop:5,fontStyle:"italic"}}>"{al.customText}"</div>}</div></div>
        <div style={{display:"flex",gap:6,flexShrink:0}}><button title={L("Modifica Alert")} onClick={function(){startEdit(al);}} style={{background:"#EEF4FF",border:"1px solid #BFD7FF",borderRadius:9,cursor:"pointer",color:"#378ADD",fontSize:14,padding:"6px 9px",fontWeight:700}}>✏️</button><button title={L("Elimina")} onClick={function(){if(!window.confirm(L("Eliminare questo alert?")))return;setAlerts(function(p){return p.filter(function(x){return x.id!==al.id;});});if(setToast)setToast(L("Alert eliminato correttamente"));}} style={{background:"#FFF0F0",border:"1px solid #FFD0D0",borderRadius:9,cursor:"pointer",color:"#E24B4A",fontSize:14,padding:"6px 9px",fontWeight:700}}>🗑️</button></div>
      </div>
      <div style={{display:"flex",justifyContent:"space-between",marginBottom:5}}><span style={{fontSize:14,fontWeight:800,color:barColor}}>{fmtAmt(spent,sym)}</span><span style={{fontSize:12,color:sc}}>{L("budget")} {fmtAmt(al.budget,sym)}</span></div><div style={{background:dark?"#333":"#f0f0f0",borderRadius:8,height:7,overflow:"hidden"}}><div style={{background:barColor,height:7,borderRadius:8,width:Math.min(100,p)+"%"}}/></div><div style={{fontSize:11,color:barColor,marginTop:4,textAlign:"right",fontWeight:750}}>{Math.round(p)}%</div>
    </div>;})}
  </div>;
}
...
export function BudgetPlanPanel(){
  ...
  var setTab=ctx.setTab,setMobileMenu=ctx.setMobileMenu,userKey=ctx.userKey,btnRadius=ctx.btnRadius||14,confirmButtonColor=ctx.confirmButtonColor||"#7F77DD";
  var L=ctx.translateUiRuntimeText||function(s){return s;};
  var groups=effectiveExpenseGroups(expenseGroups);
  var tc=dark?"#eee":"#333";var sc=dark?"#aaa":"#888";var borderC=dark?"#444":"#eee";var cardBg=dark?"#252535":"#fff";
  ...
  function openBudgetStatistics(){
    try{if(userKey)localStorage.setItem(userKey("stats_tab_v1"),JSON.stringify("budget"));}catch(e){}
    try{if(typeof window!=="undefined")window.dispatchEvent(new CustomEvent("fainance-open-stats-tab",{detail:"budget"}));}catch(e){}
    if(setTab)setTab("stats");if(setMobileMenu)setMobileMenu(false);
  }
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:16}}>
    ...
    <div style={{background:"linear-gradient(135deg,#EF9F2722,#1D9E7522)",borderRadius:14,border:"1px solid "+(dark?"#444":"#ddd"),padding:isMobile?16:18,display:"flex",alignItems:isMobile?"stretch":"center",justifyContent:"space-between",gap:14,flexDirection:isMobile?"column":"row"}}>
      <div style={{minWidth:0}}>
        <div style={{fontSize:20,fontWeight:850,color:tc,display:"flex",alignItems:"center",gap:8}}>💰 {L("Budget")}</div>
        <div style={{fontSize:12,color:sc,marginTop:5,lineHeight:1.45}}>{L("Configura il budget mensile e consulta l'andamento nelle statistiche.")}</div>
      </div>
      <button type="button" onClick={openBudgetStatistics} style={{background:confirmButtonColor,color:"#fff",border:"none",borderRadius:btnRadius,padding:"10px 14px",fontSize:13,fontWeight:800,cursor:"pointer",whiteSpace:"nowrap",alignSelf:isMobile?"stretch":"center"}}>📊 {L("Statistiche Budget")}</button>
    </div>
...
export function SettingsList({items,setItems,label,showGroup,showIcon,groupList,isMethod,allowArchive,usageScope}){
  ...
  var inp={width:"100%",borderRadius:10,border:"1px solid "+(dark?"#444":"#ddd"),padding:"10px 11px",fontSize:14,background:dark?"#2a2a3e":"#fff",color:dark?"#eee":"#333",boxSizing:"border-box"};var tc=dark?"#eee":"#333";var sc=dark?"#aaa":"#888";var borderC=dark?"#444":"#eee";var baseBlocked=ctx.currentPlan==="free";var setToast=ctx.setToast;var groupIds=(gl||[]).map(function(g){return String(g.id);});
  ...
  function closeEdit(){setEditId(null);setEditName("");setEditColor("");setEditGroup(gl[0]?gl[0].id:"");setEditIcon("📦");}
  ...
  function saveEdit(){if(baseBlocked){lockedWarn();return;}if(!String(editName||"").trim())return;setItems((items||[]).map(function(i){return String(i.id)===String(editId)?{...i,name:editName.trim(),color:editColor,icon:editIcon,custom:true,updatedAt:new Date().toISOString(),...(showGroup?{group:editGroup}:{})}:i;}));closeEdit();}
  ...
  function editDialogLabel(){if(/categoria/i.test(createLabel))return "Modifica categoria";if(/metodo/i.test(createLabel))return "Modifica metodo di pagamento";if(/area/i.test(createLabel))return "Modifica area";if(/voce/i.test(createLabel))return "Modifica voce";return "Modifica";}
  function nameFieldLabel(){if(/categoria/i.test(createLabel))return "Nome categoria";if(/metodo/i.test(createLabel))return "Nome metodo";if(/area/i.test(createLabel))return "Nome area";return "Nome voce";}
  function ModalShell({title,onClose,icon,color,setIcon,setColor,name,setName,group,setGroup,onSave}){var valid=!baseBlocked&&!!String(name||"").trim();return <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.58)",zIndex:980,display:"flex",alignItems:"center",justifyContent:"center",padding:"7vh 16px 3vh",boxSizing:"border-box",overflowY:"auto"}} onMouseDown={function(e){if(e.target===e.currentTarget)onClose();}}><div style={{position:"relative",width:"100%",maxWidth:520,background:dark?"#252535":"#fff",borderRadius:22,border:"1px solid "+borderC,boxShadow:"0 18px 65px rgba(0,0,0,0.38)",padding:"20px 18px 18px"}}><button type="button" onClick={onClose} aria-label={L("Chiudi")} style={{position:"absolute",right:14,top:14,width:34,height:34,borderRadius:"50%",border:"none",background:"#E24B4A",color:"#fff",fontSize:22,fontWeight:900,lineHeight:"32px",cursor:"pointer",boxShadow:"0 5px 14px rgba(226,75,74,.28)"}}>×</button><div style={{fontSize:18,fontWeight:950,color:tc,marginBottom:16,paddingRight:44}}>{L(title)}</div><div style={{display:"flex",flexDirection:"column",gap:13}}>{showIcon?<div style={{display:"grid",gridTemplateColumns:"minmax(0,1fr) minmax(0,1fr)",gap:12,alignItems:"start"}}><div><div style={{fontSize:12,fontWeight:850,color:sc,marginBottom:6}}>{L("Icona")}</div><EmojiPicker value={icon} onChange={setIcon}/></div><div><div style={{fontSize:12,fontWeight:850,color:sc,marginBottom:6}}>{L("Colore")}</div><AppColorSelector value={color} disabled={baseBlocked} onChange={function(v){if(baseBlocked){lockedWarn();return;}setColor(v);}} compact={true}/></div></div>:<div><div style={{fontSize:12,fontWeight:850,color:sc,marginBottom:6}}>{L("Colore")}</div><AppColorSelector value={color} disabled={baseBlocked} onChange={function(v){if(baseBlocked){lockedWarn();return;}setColor(v);}} compact={true}/></div>}<div><div style={{fontSize:12,fontWeight:850,color:sc,marginBottom:6}}>{L(nameFieldLabel())}</div><input autoFocus value={name} disabled={baseBlocked} onChange={function(e){setName(e.target.value);}} onKeyDown={function(e){if(e.key==="Enter"&&valid)onSave();}} placeholder={L(nameFieldLabel())} style={inp}/></div>{showGroup&&gl.length>0&&<div><div style={{fontSize:12,fontWeight:850,color:sc,marginBottom:6}}>{L("Area")}</div><select value={group} disabled={baseBlocked} onChange={function(e){setGroup(e.target.value);}} style={inp}>{gl.map(function(g2){return <option key={g2.id} value={g2.id}>{L(g2.name)}</option>;})}</select></div>}</div><div style={{display:"flex",gap:9,marginTop:18}}><Btn onClick={onSave} disabled={!valid} bg={valid?confirmButtonColor:"#A8A8A8"} style={{flex:1,padding:12,fontWeight:950}}>{L("Salva")}</Btn><Btn onClick={onClose} bg={dark?"#333":"#f0f0f0"} color={tc} style={{padding:"12px 16px",fontWeight:900}}>{L("Annulla")}</Btn></div></div></div>;}
  return <div>{grouped.map(function(g){if(!g.items.length)return null;return <div key={g.id}>{showGroup&&g.name&&<div style={{fontSize:11,fontWeight:600,color:"#aaa",textTransform:"uppercase",letterSpacing:1,margin:"12px 0 6px"}}>{L(g.name)}</div>}{g.items.map(function(item){return <div key={item.id} style={{background:dark?"#252535":"#fff",borderRadius:10,border:"1px solid "+(dark?"#444":"#eee"),padding:"10px 12px",marginBottom:8,opacity:item.archived?0.6:1}}><div style={{display:"flex",alignItems:"center",gap:10}}>{showIcon&&<span style={{fontSize:18}}>{item.icon||"📦"}</span>}<div style={{width:12,height:12,borderRadius:"50%",background:item.color,flexShrink:0}}/><span style={{flex:1,fontSize:14,color:tc,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{L(item.name)}</span>{item.archived&&<span style={{fontSize:10,background:dark?"#333":"#f0f0f0",color:sc,borderRadius:10,padding:"1px 6px"}}>{L("Archiviato")}</span>}<button onClick={function(){startEdit(item);}} style={{background:"#EEF4FF",border:"1px solid #BFD7FF",borderRadius:8,cursor:"pointer",color:"#378ADD",fontSize:14,padding:"5px 8px",fontWeight:700}}>✏️</button><button onClick={function(){toggleArchive(item.id);}} title={item.archived?L("Ripristina"):L("Archivia")} style={{background:"none",border:"none",cursor:"pointer",fontSize:15,color:"#bbb"}}>{item.archived?"📂":"🗂"}</button><button onClick={function(){del(item.id);}} style={{background:"#FFF0F0",border:"1px solid #FFD0D0",borderRadius:8,cursor:"pointer",color:"#E24B4A",fontSize:14,padding:"5px 8px",fontWeight:700}}>🗑️</button></div></div>;})}</div>;})}<button type="button" onClick={function(){if(baseBlocked){lockedWarn();return;}setShowCreate(true);}} style={{width:"100%",marginTop:8,background:baseBlocked?"#EF9F27":confirmButtonColor,color:"#fff",border:"none",borderRadius:btnRadius,padding:"13px 16px",cursor:"pointer",fontSize:15,fontWeight:900,boxShadow:dark?"none":"0 6px 18px rgba(55,138,221,0.25)"}}>＋ {L(createLabel)}</button>{showCreate&&<ModalShell title={createLabel} onClose={resetCreate} icon={newIcon} color={newColor} setIcon={setNewIcon} setColor={setNewColor} name={newName} setName={setNewName} group={newGroup} setGroup={setNewGroup} onSave={add}/>} {editId!==null&&<ModalShell title={editDialogLabel()} onClose={closeEdit} icon={editIcon} color={editColor} setIcon={setEditIcon} setColor={setEditColor} name={editName} setName={setEditName} group={editGroup} setGroup={setEditGroup} onSave={saveEdit}/>}</div>;
}
export function AreasEditor({onlyPatrimonio}){
  var ctx=useApp();function L(v){return translateFainanceText(v,(ctx&&ctx.lang)||"it");}
  var expenseGroups=ctx.expenseGroups,setExpenseGroups=ctx.setExpenseGroups,incomeGroups=ctx.incomeGroups,setIncomeGroups=ctx.setIncomeGroups,methodGroups=ctx.methodGroups,setMethodGroups=ctx.setMethodGroups,patrimonioAreas=ctx.patrimonioAreas,setPatrimonioAreas=ctx.setPatrimonioAreas,dark=ctx.dark;
  var [areaTab,setAreaTab]=useState(onlyPatrimonio?"patrimonio":"expense");
  var tc=dark?"#eee":"#333";var sc=dark?"#aaa":"#888";var borderC=dark?"#444":"#eee";var cardBg=dark?"#252535":"#fff";
  ...
  var isPatrimonio=areaTab==="patrimonio";var baseBlocked=ctx.currentPlan==="free";var setToast=ctx.setToast;var confirmButtonColor=ctx.confirmButtonColor||"#7F77DD";var btnRadius=ctx.btnRadius||14;
  var [modalMode,setModalMode]=useState(null);var [editId,setEditId]=useState(null);var [draftName,setDraftName]=useState("");var [draftColor,setDraftColor]=useState(COLORS[0]);var [draftIcon,setDraftIcon]=useState("📋");
  function lockedWarn(){if(setToast)setToast({text:L("Funzione disponibile dal piano Base."),type:"warning",color:"#EF9F27",icon:"⚠️"});}
  function closeModal(){setModalMode(null);setEditId(null);setDraftName("");setDraftColor(COLORS[0]);setDraftIcon("📋");}
  function openCreate(){if(baseBlocked){lockedWarn();return;}setEditId(null);setDraftName("");setDraftColor(COLORS[0]);setDraftIcon("📋");setModalMode("create");}
  function openEdit(g){if(baseBlocked){lockedWarn();return;}setEditId(g.id);setDraftName(g.name||"");setDraftColor(g.color||COLORS[0]);setDraftIcon(g.icon||"📋");setModalMode("edit");}
  function saveModal(){if(baseBlocked){lockedWarn();return;}var name=String(draftName||"").trim();if(!name)return;if(modalMode==="edit"){setCurrent((current||[]).map(function(g){return String(g.id)===String(editId)?{...g,name:name,color:draftColor,...(isPatrimonio?{icon:draftIcon}:{})}:g;}));}else{var nitem={id:"area_"+Date.now(),name:name,color:draftColor,...(isPatrimonio?{icon:draftIcon}:{})};setCurrent([...(current||[]),nitem]);}closeModal();}
  function del(id){if(baseBlocked){lockedWarn();return;}setCurrent((current||[]).filter(function(g){return String(g.id)!==String(id);}));}
  function tabLabel(){return areaTab==="expense"?"Uscite":areaTab==="income"?"Entrate":areaTab==="method"?"Metodi di pagamento":"Patrimonio";}
  function AreaModal(){if(!modalMode)return null;var valid=!baseBlocked&&!!String(draftName||"").trim();return <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.58)",zIndex:980,display:"flex",alignItems:"center",justifyContent:"center",padding:"7vh 16px 3vh",boxSizing:"border-box",overflowY:"auto"}} onMouseDown={function(e){if(e.target===e.currentTarget)closeModal();}}><div style={{position:"relative",width:"100%",maxWidth:500,background:cardBg,borderRadius:22,border:"1px solid "+borderC,boxShadow:"0 18px 65px rgba(0,0,0,0.38)",padding:"20px 18px 18px"}}><button type="button" onClick={closeModal} aria-label={L("Chiudi")} style={{position:"absolute",right:14,top:14,width:34,height:34,borderRadius:"50%",border:"none",background:"#E24B4A",color:"#fff",fontSize:22,fontWeight:900,lineHeight:"32px",cursor:"pointer",boxShadow:"0 5px 14px rgba(226,75,74,.28)"}}>×</button><div style={{fontSize:18,fontWeight:950,color:tc,marginBottom:4,paddingRight:44}}>{L(modalMode==="edit"?"Modifica area":"Nuova area")}</div><div style={{fontSize:12,color:sc,marginBottom:16}}>{L(tabLabel())}</div><div style={{display:"flex",flexDirection:"column",gap:13}}>{isPatrimonio?<div style={{display:"grid",gridTemplateColumns:"minmax(0,1fr) minmax(0,1fr)",gap:12,alignItems:"start"}}><div><div style={{fontSize:12,fontWeight:850,color:sc,marginBottom:6}}>{L("Icona")}</div><EmojiPicker value={draftIcon} onChange={setDraftIcon}/></div><div><div style={{fontSize:12,fontWeight:850,color:sc,marginBottom:6}}>{L("Colore")}</div><AppColorSelector value={draftColor} disabled={baseBlocked} onChange={setDraftColor} compact={true}/></div></div>:<div><div style={{fontSize:12,fontWeight:850,color:sc,marginBottom:6}}>{L("Colore")}</div><AppColorSelector value={draftColor} disabled={baseBlocked} onChange={setDraftColor} compact={true}/></div>}<div><div style={{fontSize:12,fontWeight:850,color:sc,marginBottom:6}}>{L("Nome area")}</div><input autoFocus disabled={baseBlocked} placeholder={L("Nome area")} value={draftName} onChange={function(e){setDraftName(e.target.value);}} onKeyDown={function(e){if(e.key==="Enter"&&valid)saveModal();}} style={{width:"100%",borderRadius:10,border:"1px solid "+borderC,padding:"10px 11px",fontSize:14,background:dark?"#2a2a3e":"#fff",color:tc,boxSizing:"border-box"}}/></div></div><div style={{display:"flex",gap:9,marginTop:18}}><Btn onClick={saveModal} disabled={!valid} bg={valid?confirmButtonColor:"#A8A8A8"} style={{flex:1,padding:12,fontWeight:950}}>{L("Salva")}</Btn><Btn onClick={closeModal} bg={dark?"#333":"#f0f0f0"} color={tc} style={{padding:"12px 16px",fontWeight:900}}>{L("Annulla")}</Btn></div></div></div>;}
  return <div style={{display:"flex",flexDirection:"column",gap:14}}>
    <div style={{display:"flex",gap:0,background:dark?"#252535":"#f5f5f5",borderRadius:10,padding:3,marginBottom:4,flexWrap:"wrap"}}>
      {(onlyPatrimonio?[{id:"patrimonio",label:"💎 Patrimonio"}]:[{id:"expense",label:"💸 Uscite"},{id:"income",label:"💰 Entrate"},{id:"method",label:"💳 Metodi di pagamento"}]).map(function(tb){return <button key={tb.id} onClick={function(){closeModal();setAreaTab(tb.id);}} style={{flex:1,padding:"8px",border:"none",borderRadius:8,background:areaTab===tb.id?(dark?"#444":"#fff"):"transparent",color:areaTab===tb.id?tc:sc,fontSize:12,cursor:"pointer",fontWeight:areaTab===tb.id?600:400,minWidth:80}}>{L(tb.label)}</button>;})}
    </div>
    {(current||[]).map(function(g){return <div key={g.id} style={{background:cardBg,borderRadius:10,border:"1px solid "+borderC,padding:"10px 12px",display:"flex",alignItems:"center",gap:10}}>{isPatrimonio&&<span style={{fontSize:18}}>{g.icon||"📋"}</span>}<span style={{width:12,height:12,borderRadius:"50%",background:g.color||COLORS[0],flexShrink:0}}/><span style={{flex:1,fontSize:14,color:tc,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{L(g.name)}</span><button type="button" onClick={function(){openEdit(g);}} style={{background:"#EEF4FF",border:"1px solid #BFD7FF",borderRadius:8,cursor:"pointer",color:"#378ADD",fontSize:14,padding:"5px 8px",fontWeight:700}}>✏️</button><button type="button" onClick={function(){del(g.id);}} style={{background:"#FFF0F0",border:"1px solid #FFD0D0",borderRadius:8,cursor:"pointer",color:"#E24B4A",fontSize:14,padding:"5px 8px",fontWeight:700}}>🗑️</button></div>;})}
    <button type="button" onClick={openCreate} style={{width:"100%",background:baseBlocked?"#EF9F27":confirmButtonColor,color:"#fff",border:"none",borderRadius:btnRadius,padding:"13px 16px",fontSize:15,fontWeight:900,cursor:"pointer"}}>＋ {L("Nuova area")}</button>
    <AreaModal/>
  </div>;
}
