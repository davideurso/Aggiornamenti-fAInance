...
export function ExpenseForm({onSave,type}){
  ...
  function readFormDraft(tp){var blank=emptyFormForType(tp);try{var raw=localStorage.getItem(formDraftKey(tp));if(!raw)return blank;var saved=JSON.parse(raw);if(!saved||typeof saved!=="object")return blank;return {...blank,...saved};}catch(e){return blank;}}
  function isMeaningfulFormDraft(value,tp){var blank=emptyFormForType(tp);if(!value)return false;if(String(value.amount||"").trim())return true;if(String(value.desc||"").trim())return true;if(String(value.date||"")!==String(blank.date||""))return true;if(!!value.rateizzato)return true;if(String(value.rate||"")!==String(blank.rate||""))return true;if(String(value.rateDirection||"")!==String(blank.rateDirection||""))return true;if(tp==="expense"){if(String(value.catId||"")!==String(blank.catId||""))return true;if(String(value.methodId||"")!==String(blank.methodId||""))return true;}else if(String(value.itype||"")!==String(blank.itype||""))return true;return false;}
  var [f,setF]=useState(function(){return readFormDraft(type);});
  useEffect(function(){setF(readFormDraft(type));},[type,defaultCat.id,defaultMethod.id]);
  ...
  var amountText=String(f.amount||"").trim()?String(f.amount):"0,00";
  ...
  return <div style={{background:dark?"#181827":"#fff",border:"1px solid "+borderC,borderRadius:18,padding:isMobile?10:18,boxShadow:dark?"none":"0 8px 28px rgba(83,74,183,0.08)",display:"flex",flexDirection:"column",gap:isMobile?10:14}}>
    ...
    <div style={{background:amountGradient,borderRadius:18,padding:isMobile?"16px 14px":"22px 26px",display:"grid",gridTemplateColumns:isMobile?"44px minmax(0,1fr) 58px":"58px minmax(0,1fr) 86px",gap:isMobile?6:14,alignItems:"center",color:"#fff",boxShadow:dark?"none":"0 10px 24px rgba(83,74,183,.18)"}}>
      ...
      <div style={{minWidth:0,textAlign:"center"}}>
        ...
        <div style={{position:"relative"}}>
          {!String(f.amount||"").trim()&&<div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",fontSize:isMobile?36:52,fontWeight:950,color:"#fff",lineHeight:1,pointerEvents:"none",textShadow:"0 2px 10px rgba(0,0,0,.12)"}}>0,00</div>}
          <input type="text" inputMode="decimal" placeholder="" value={f.amount} onChange={function(e){setF(function(p){return{...p,amount:e.target.value};});}} style={{width:"100%",border:"none",background:"transparent",padding:0,textAlign:"center",fontSize:isMobile?36:52,fontWeight:950,color:"#fff",WebkitTextFillColor:"#fff",outline:"none",lineHeight:1,textShadow:"0 2px 10px rgba(0,0,0,.12)"}}/>
        </div>
        {!String(f.amount||"").trim()&&<div style={{fontSize:isMobile?13:16,color:whiteSoft,fontWeight:600,marginTop:4}}>{TL("Tocca per modificare")}</div>}
      </div>
    ...
    {type==="expense"?<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:isMobile?8:12}}>
      <div style={fieldCard}><label style={{...label,color:dark?sc:"#343647"}}>{t.category}</label>{catSortMode==="custom"?<select value={f.catId} onChange={function(e){setF(function(p){return{...p,catId:e.target.value};});}} style={{...inp,padding:isMobile?"10px 10px 10px 2px":"11px 12px 11px 4px",paddingLeft:isMobile?2:4,fontSize:isMobile?13:16,fontWeight:750,border:"none",background:"transparent",height:isMobile?48:52}}>{sCats.map(function(c){return <option key={c.id} value={c.id}>{c.icon} {c.name}</option>;})}</select>:<select value={f.catId} onChange={function(e){setF(function(p){return{...p,catId:e.target.value};});}} style={{...inp,padding:isMobile?"10px 10px 10px 2px":"11px 12px 11px 4px",paddingLeft:isMobile?2:4,fontSize:isMobile?13:16,fontWeight:750,border:"none",background:"transparent",height:isMobile?48:52}}>{grps.map(function(g){var gc=safeCatsForGroup(sCats,grps,g);return gc.length?<optgroup key={g.id} label={g.name}>{gc.map(function(c){return <option key={c.id} value={c.id}>{c.icon} {c.name}</option>;})}</optgroup>:null;})}</select>}</div>
    ...
    <button onClick={save} style={{background:mainButtonC,color:"#fff",border:"none",borderRadius:btnRadius,padding:isMobile?15:17,fontSize:isMobile?16:18,fontWeight:950,cursor:"pointer",boxShadow:dark?"none":"0 8px 20px "+mainButtonC+"44",letterSpacing:.2}}>{type==="expense"?TL("Salva Uscita"):TL("Salva Entrata")}</button>
  </div>;
}
...
export function ReceiptScanPanel({onSave,shareMode}){
  ...
  return <div style={{background:dark?"linear-gradient(160deg,#252535 0%,#1e1e30 100%)":"linear-gradient(160deg,#ffffff 0%,#f7fbff 100%)",border:"1px solid "+borderC,borderRadius:16,padding:isMobile?12:18,display:"flex",flexDirection:"column",gap:14,boxShadow:dark?"none":"0 10px 30px rgba(55,138,221,0.12)"}}>
    ...
    <button onClick={save} style={{background:confirmButtonColor||"#378ADD",color:"#fff",border:"none",borderRadius:btnRadius,padding:isMobile?12:15,fontSize:15,fontWeight:900,cursor:"pointer",boxShadow:dark?"none":"0 6px 18px "+(confirmButtonColor||"#378ADD")+"44"}}>{L(shareMode?"Usa dati scontrino":"Salva uscita da scontrino")}</button>
  </div>;
}
      ...
      {rows.map(function(r,idx){return <div key={r._id} style={{background:dark?"#181827":"#fff",borderRadius:18,border:"1px solid "+(dark?"#3d3d50":"#E6E2F7"),padding:10,marginBottom:12,boxShadow:dark?"none":"0 8px 24px rgba(83,74,183,0.08)"}}>
        ...
        <div style={{background:amountGradient,borderRadius:18,padding:"14px 14px",display:"grid",gridTemplateColumns:"44px minmax(0,1fr) 58px",gap:6,alignItems:"center",color:"#fff",boxShadow:dark?"none":"0 10px 24px rgba(83,74,183,.12)",marginBottom:10}}>
          ...
          <div style={{minWidth:0,textAlign:"center"}}>
            ...
            <div style={{position:"relative"}}>{!String(r.amount||"").trim()&&<div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",fontSize:34,fontWeight:950,color:"#fff",lineHeight:1,pointerEvents:"none"}}>0,00</div>}<input type="text" inputMode="decimal" placeholder="" value={r.amount} onChange={function(e){upd(r._id,"amount",e.target.value);}} style={{width:"100%",border:"none",background:"transparent",padding:0,textAlign:"center",fontSize:34,fontWeight:950,color:"#fff",WebkitTextFillColor:"#fff",outline:"none",lineHeight:1}}/></div>
            {!String(r.amount||"").trim()&&<div style={{fontSize:13,color:"rgba(255,255,255,.70)",fontWeight:600,marginTop:4}}>{L("Tocca per modificare")}</div>}
          </div>
        ...
        </div>:<div style={{...fieldCard,marginBottom:10}}><label style={{fontSize:11,fontWeight:800,color:tc,display:"block",marginBottom:6}}>{L('Tipo')}</label><select value={r.itype} onChange={function(e){upd(r._id,"itype",e.target.value);}} style={{...inp,padding:"10px 10px 10px 2px",paddingLeft:2,fontSize:13,fontWeight:750,border:"none",background:"transparent",height:48}}>{((ctx.incomeTypes&&ctx.incomeTypes.length)?ctx.incomeTypes:getAllIncomeTypes()).map(function(it){return <option key={it.id} value={it.id}>{it.icon} {it.name}</option>;})}</select></div>}
...
export function RecurringManager(){
  ...
  function makeBlank(){return{name:"",icon:"🔁",amount:"",catId:sCats[0]?sCats[0].id:1,methodId:sMethods[0]?sMethods[0].id:1,rtype:"expense",incomeType:(incomeTypes[0]&&incomeTypes[0].id)||"salario",dayOfMonth:1,annualMonth:now.getMonth()+1,rateizzato:false,rate:12,frequency:"monthly"};}
...
export function SettingsList({items,setItems,label,showGroup,showIcon,groupList,isMethod,allowArchive,usageScope}){
  ...
  function itemIsUsed(id){var sid=String(id);function same(v){return String(v==null?"":v)===sid;}var scope=usageScope||(isMethod?"method":"generic");if(scope==="method")return expenses.some(function(e){return same(e.methodId)||same(e.method);})||incomes.some(function(e){return same(e.methodId)||same(e.method);})||recurring.some(function(e){return same(e.methodId)||same(e.method);});if(scope==="expenseCategory")return expenses.some(function(e){return same(e.catId)||same(e.categoryId);})||recurring.some(function(e){return String(e.rtype||"expense")!=="income"&&(same(e.catId)||same(e.categoryId));});if(scope==="incomeCategory")return incomes.some(function(e){return same(e.type)||same(e.incomeType)||same(e.typeId)||same(e.catId)||same(e.categoryId);})||recurring.some(function(e){return String(e.rtype||"")==="income"&&(same(e.incomeType)||same(e.type)||same(e.typeId)||same(e.catId)||same(e.categoryId));});return false;}
  function del(id){if(baseBlocked){lockedWarn();return;}if(itemIsUsed(id)){setItems((items||[]).map(function(i){return String(i.id)===String(id)?{...i,archived:true,custom:true,updatedAt:new Date().toISOString()}:i;}));if(setToast)setToast({text:L("Questa voce è già usata nello storico o nelle ricorrenze: per proteggere i dati è stata archiviata, non eliminata."),type:"warning",color:"#EF9F27",icon:"🛡️"});return;}if(typeof window!=="undefined"&&!window.confirm(L("Confermi la cancellazione?")))return;setItems((items||[]).filter(function(i){return String(i.id)!==String(id);}));if(setToast)setToast({text:L("Cancellazione completata"),type:"success",icon:"🗑️"});}
  ...
  function saveEdit(){if(baseBlocked){lockedWarn();return;}setItems((items||[]).map(function(i){return i.id===editId?{...i,name:editName,color:editColor,icon:editIcon,custom:true,updatedAt:new Date().toISOString(),...(showGroup?{group:editGroup}:{})}:i;}));setEditId(null);}
  function toggleArchive(id){if(baseBlocked){lockedWarn();return;}setItems((items||[]).map(function(i){return i.id===id?{...i,archived:!i.archived,custom:true,updatedAt:new Date().toISOString()}:i;}));}
  var grouped=showGroup&&gl.length?gl.map(function(g){return{...g,items:(items||[]).filter(function(c){var cg=String(c.group||"");return !c.archived&&(cg===String(g.id)||(usageScope==="expenseCategory"&&String(g.id)==="altro"&&(!cg||groupIds.indexOf(cg)<0)));})};}).concat([{id:"__archived",name:"Archiviati",items:(items||[]).filter(function(c){return !!c.archived;})}]):[{id:"_",name:"",items:(items||[])}];
