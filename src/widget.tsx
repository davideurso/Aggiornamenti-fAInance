...
export function Toast({msg,onDone,color}){
  ...
  return <div style={{position:"fixed",top:46,left:"50%",transform:"translateX(-50%)",background:bg,color:"#fff",borderRadius:22,padding:"16px 22px",fontSize:14,fontWeight:800,zIndex:600,boxShadow:"0 8px 40px rgba(0,0,0,0.28)",textAlign:"center",minWidth:240,maxWidth:"88vw",lineHeight:1.35}}>
...
export function ReceiptScanPanel({onSave}){
  ...
  async function openReceiptCameraFromWidget(){
    ...
    try{
      var previewOk=await openReceiptCameraPreview();
      if(previewOk)return;
    ...
    return <div>
      {rows.map(function(r,idx){return <div key={r._id} style={{background:dark?"#1e1e30":"#f9f9f9",borderRadius:12,border:"1px solid "+(dark?"#444":"#eee"),padding:14,marginBottom:12,position:"relative"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
          <div><label style={{fontSize:11,color:tc,display:"block",marginBottom:3}}>{L('Importo')}</label><input type="number" value={r.amount} onChange={function(e){upd(r._id,"amount",e.target.value);}} style={inp} placeholder="0,00"/></div>
          <div><label style={{fontSize:11,color:tc,display:"block",marginBottom:3}}>{L("Data")}</label><DatePickerField value={r.date} onChange={function(v){upd(r._id,"date",v);}}/></div>
        </div>
        {type==="expense"?<>
          <div style={{marginBottom:8}}><label style={{fontSize:11,color:tc,display:"block",marginBottom:3}}>{L('Categoria')}</label><select value={r.catId} onChange={function(e){upd(r._id,"catId",e.target.value);}} style={inp}>{sCats.map(function(c){return <option key={c.id} value={c.id}>{c.icon} {c.name}</option>;})}</select></div>
          <div style={{marginBottom:8}}><label style={{fontSize:11,color:tc,display:"block",marginBottom:3}}>{L('Metodo')}</label><select value={r.methodId} onChange={function(e){upd(r._id,"methodId",e.target.value);}} style={inp}>{sMethods.map(function(m){return <option key={m.id} value={m.id}>{m.icon} {m.name}</option>;})}</select></div>
        </>:<div style={{marginBottom:8}}><label style={{fontSize:11,color:tc,display:"block",marginBottom:3}}>{L('Tipo')}</label><select value={r.itype} onChange={function(e){upd(r._id,"itype",e.target.value);}} style={inp}>{(ctx.incomeTypes||getAllIncomeTypes()).map(function(it){return <option key={it.id} value={it.id}>{it.icon} {it.name}</option>;})}</select></div>}
...
export function RecurringManager(){
  ...
  var sCats=useMemo(function(){return sortedCats(cats,catOrder,catSortMode,expenseGroups);},[cats,catOrder,catSortMode,expenseGroups]);
  var sMethods=useMemo(function(){return sortedMethods(methods,methodOrder,methodSortMode);},[methods,methodOrder,methodSortMode]);
  function makeBlank(){return{name:"",amount:"",catId:sCats[0]?sCats[0].id:1,methodId:sMethods[0]?sMethods[0].id:1,rtype:"expense",incomeType:(incomeTypes[0]&&incomeTypes[0].id)||"salario",dayOfMonth:1,rateizzato:false,rate:12};}
  ...
  function normalizePayload(){return{...form,id:editingId||Date.now(),name:String(form.name||"").trim(),amount:parseFloat(form.amount),catId:Number(form.catId),methodId:Number(form.methodId),dayOfMonth:Number(form.dayOfMonth),rate:Number(form.rate||12),rateizzato:!!form.rateizzato};}
...
export function SettingsList({items,setItems,label,showGroup,showIcon,groupList,isMethod,allowArchive}){var ctx=useApp();function L(v){return translateFainanceText(v,(ctx&&ctx.lang)||"it");}var t=ctx.t,dark=ctx.dark,isMobile=ctx.isMobile;var gl=groupList||[];var [newName,setNewName]=useState("");var [newColor,setNewColor]=useState(COLORS[0]);var [newGroup,setNewGroup]=useState(gl[0]?gl[0].id:"");var [newIcon,setNewIcon]=useState("📦");var [editId,setEditId]=useState(null);var [editName,setEditName]=useState("");var [editColor,setEditColor]=useState("");var [editGroup,setEditGroup]=useState(gl[0]?gl[0].id:"");var [editIcon,setEditIcon]=useState("📦");var [sortMode,setSortMode]=useState("group");var dragIdx=useRef(null);var dragOverIdx=useRef(null);var inp={width:"100%",borderRadius:8,border:"1px solid "+(dark?"#444":"#ddd"),padding:"7px 10px",fontSize:14,background:dark?"#2a2a3e":"#fff",color:dark?"#eee":"#333"};var tc=dark?"#eee":"#333";var sc=dark?"#aaa":"#888";var borderC=dark?"#444":"#eee";var baseBlocked=ctx.currentPlan==="free";var setToast=ctx.setToast;function lockedWarn(){if(setToast)setToast({text:"Funzione disponibile dal piano Base.",type:"warning",color:"#EF9F27",icon:"⚠️"});}
  ...
  function del(id){if(baseBlocked){lockedWarn();return;}setItems(items.filter(function(i){return i.id!==id;}));}
...
export function DeleteDataPanel(){
  ...
  var sectionOptions=[
    ...
    {id:"categories",icon:"🏷",label:L("Categorie uscite"),count:cats.length,clear:function(){setCats(DEFAULT_CATS);setExpenseGroups(DEFAULT_EXPENSE_GROUPS);}},
    {id:"methods",icon:"💳",label:L("Metodi pagamento"),count:methods.length,clear:function(){setMethods(DEFAULT_METHODS);setMethodGroups(DEFAULT_METHOD_GROUPS);}},
    {id:"patrimonioConfig",icon:"⚙️",label:L("Config. patrimonio"),count:(patrimonioAreas||[]).length+(patrimonioEntries||[]).length,clear:function(){setPatrimonioAreas(DEFAULT_PATRIMONIO_AREAS);setPatrimonioEntries(DEFAULT_PATRIMONIO_ENTRIES);}},
  ];
