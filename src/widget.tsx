...
export function GoalsPanel(){
  ...
  var [showAdd,setShowAdd]=useState(false);var [editingGoalId,setEditingGoalId]=useState(null);var [addAmt,setAddAmt]=useState({});var [highlightedGoalId,setHighlightedGoalId]=useState("");
  ...
  var tc=dark?"#eee":"#333";var sc=dark?"#aaa":"#888";var borderC=dark?"#444":"#eee";var cardBg=dark?"#252535":"#fff";
  function focusGoalFromWidget(goalId){
    var id=String(goalId||"");if(!id)return false;
    setHighlightedGoalId(id);
    setTimeout(function(){try{var nodes=document.querySelectorAll("[data-fainance-goal-id]");for(var i=0;i<nodes.length;i++){if(String(nodes[i].getAttribute("data-fainance-goal-id")||"")===id){nodes[i].scrollIntoView({behavior:"smooth",block:"center"});break;}}}catch(e){}},180);
    return true;
  }
  function consumeGoalWidgetAction(detail){
    var payload=detail&&detail.goalId?{goalId:detail.goalId,ts:Date.now()}:null;
    try{if(!payload){var raw=localStorage.getItem("fainance_goal_widget_action_v1")||"";if(raw)payload=JSON.parse(raw);}}catch(e){}
    if(!payload||!payload.goalId)return false;
    if(payload.ts&&Date.now()-Number(payload.ts)>60000){try{localStorage.removeItem("fainance_goal_widget_action_v1");}catch(e){}return false;}
    try{localStorage.removeItem("fainance_goal_widget_action_v1");}catch(e){}
    return focusGoalFromWidget(payload.goalId);
  }
  useEffect(function(){
    function handler(e){consumeGoalWidgetAction(e&&e.detail?e.detail:null);}
    try{window.addEventListener("fainance-open-goal",handler);}catch(e){}
    var timers=[80,260,650,1200].map(function(ms){return setTimeout(function(){consumeGoalWidgetAction(null);},ms);});
    return function(){try{window.removeEventListener("fainance-open-goal",handler);}catch(e){}timers.forEach(function(t){clearTimeout(t);});};
  },[(goals||[]).map(function(g){return String(g.id);}).join("|")]);
