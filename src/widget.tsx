...
import { useApp, useStorage, COLORS, EMOJI_LIST, DEFAULT_CATS, DEFAULT_METHODS,
  ...
  getAllIncomeTypes, sortedCats, sortedMethods, fmtDate, fmtAmt,
...
export function RatePicker({value,onChange}){var {t,btnRadius}=useApp();return <div style={{display:"flex",gap:6,flexWrap:"wrap",marginTop:8}}>{[2,3,4,6,12,24].map(function(n){return <button key={n} onClick={function(){onChange(n);}} style={{padding:"5px 12px",borderRadius:btnRadius,border:"1px solid "+(value===n?"#7F77DD":"#ddd"),background:value===n?"#EEEDFE":"transparent",color:value===n?"#534AB7":"#666",fontSize:13,cursor:"pointer"}}>{n} {t.months}</button>;})}</div>;}
...
export function Toast({msg,onDone,color}){useEffect(function(){var timer=setTimeout(onDone,2800);return function(){clearTimeout(timer);};},[]);return <div style={{position:"fixed",top:46,left:"50%",transform:"translateX(-50%)",background:color||"#1D9E75",color:"#fff",borderRadius:20,padding:"20px 34px",fontSize:15,fontWeight:600,zIndex:600,boxShadow:"0 8px 40px rgba(0,0,0,0.28)",textAlign:"center",minWidth:220,maxWidth:"88vw"}}><div style={{fontSize:36,marginBottom:20}}>✅</div><div>{msg}</div></div>;}
  ...
  return <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.55)",zIndex:500,display:"flex",alignItems:"center",justifyContent:"center",padding:16}} onClick={function(e){if(e.target===e.currentTarget)onClose();}}>
    <div style={{background:"#fff",borderRadius:20,width:"100%",maxWidth:420,overflow:"hidden",boxShadow:"0 8px 40px rgba(0,0,0,0.2)"}}>
      <div style={{background:"#E24B4A",padding:"18px 20px",display:"flex",alignItems:"center",gap:10}}>
        ...
        <div style={{flex:1}}>
          <div style={{fontSize:16,fontWeight:700,color:"#fff"}}>Alert superato!</div>
          <div style={{fontSize:12,color:"rgba(255,255,255,0.85)"}}>{newAlerts.length===1?"1 nuova soglia superata":newAlerts.length+" nuove soglie superate"}</div>
        </div>
        <button onClick={onClose} style={{background:"rgba(255,255,255,0.2)",border:"none",borderRadius:8,color:"#fff",fontSize:18,cursor:"pointer",padding:"4px 10px"}}>x</button>
      </div>
      <div style={{padding:"16px 20px",maxHeight:340,overflowY:"auto"}}>
        {newAlerts.map(function(al){return <div key={al.id} style={{marginBottom:14,padding:"12px 14px",background:"#fff5f5",borderRadius:12,border:"1px solid #fcc"}}>
          ...
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
            ...
            <span style={{fontSize:12,color:"#aaa"}}>budget {al.budgetFmt}</span>
          </div>
      ...
      <div style={{padding:"12px 20px",borderTop:"1px solid #eee"}}>
        <button onClick={onClose} style={{width:"100%",background:"#E24B4A",color:"#fff",border:"none",borderRadius:10,padding:"11px",fontWeight:600,fontSize:14,cursor:"pointer"}}>Ho capito</button>
      </div>
    </div>
  </div>;
}
...
export function ExpenseForm({onSave,type}){
  ...
  var eE={amount:"",catId:defaultCat.id,methodId:defaultMethod.id,desc:"",date:todayStr(),rateizzato:false,rate:12};
  var eI={amount:"",itype:"salario",desc:"",date:todayStr(),rateizzato:false,rate:12};
  ...
  function save(){if(!f.amount||isNaN(f.amount)||Number(f.amount)<=0)return;var item={id:Date.now(),amount:parseFloat(f.amount),desc:f.desc,date:f.date,rateizzato:f.rateizzato,rate:f.rate};if(type==="expense"){item.catId=Number(f.catId);item.methodId=Number(f.methodId);}else{item.type=f.itype;}onSave(item);setF(type==="expense"?eE:eI);}
  return <div style={{background:panelBg,border:"1px solid "+borderC,borderRadius:16,padding:isMobile?8:18,boxShadow:dark?"none":"0 8px 28px rgba(83,74,183,0.08)",display:"flex",flexDirection:"column",gap:isMobile?6:12}}>
    <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:0}}>
      ...
      <div style={{flex:1}}>
        <div style={{fontSize:isMobile?13:16,fontWeight:800,color:tc}}>{type==="expense"?"Nuova uscita":"Nuova entrata"}</div>
        {!isMobile&&<div style={{fontSize:12,color:sc}}>Importo, categoria, data e conferma</div>}
      </div>
    </div>
    <div style={{...fieldCard,borderColor:btnC+"55",background:dark?"#1e1e30":"#fff"}}>
      <label style={label}>{t.amount} ({sym})</label>
      <input type="number" placeholder="0,00" value={f.amount} onChange={function(e){setF(function(p){return{...p,amount:e.target.value};});}} style={{...inp,border:"none",background:"transparent",padding:"0",fontSize:isMobile?22:34,fontWeight:800,color:btnC}}/>
    </div>
    {type==="expense"?<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:isMobile?6:12}}>
      ...
      <div style={fieldCard}><label style={label}>Metodo di pagamento</label><select value={f.methodId} onChange={function(e){setF(function(p){return{...p,methodId:e.target.value};});}} style={inp}>{sMethods.map(function(m){return <option key={m.id} value={m.id}>{m.icon} {m.name}</option>;})}</select></div>
    </div>:<div style={fieldCard}><label style={label}>{t.incomeType}</label><select value={f.itype} onChange={function(e){setF(function(p){return{...p,itype:e.target.value};});}} style={inp}>{(ctx.incomeTypes||getAllIncomeTypes()).map(function(it){return <option key={it.id} value={it.id}>{it.icon} {it.name}</option>;})}</select></div>}
    <div style={fieldCard}><label style={label}>{t.description}</label><input type="text" placeholder="Descrizione" value={f.desc} onChange={function(e){setF(function(p){return{...p,desc:e.target.value};});}} onKeyDown={function(e){if(e.key==="Enter")save();}} style={inp}/></div>
...
export function ReceiptScanPanel({onSave}){
  ...
  var [f,setF]=useState({amount:"",catId:defaultCat.id,methodId:defaultMethod.id,desc:"Scontrino",date:todayStr()});
    ...
    try{
      ...
      if(isNative){
        ...
        if(img){
          ...
          setReceiptText("");
          setMsg("Foto scattata. Lettura automatica in corso...");
          runOCRWithImage(img,nm);
          return;
        }
      }
    }catch(e){
      try{console.warn("Apertura camera nativa non riuscita",e);}catch(_e){}
    }finally{
      setTimeout(function(){nativeCameraOpeningRef.current=false;},1200);
    }
    setTimeout(function(){try{if(fileRef.current)fileRef.current.click();}catch(e){}},80);
    setTimeout(function(){try{if(fileRef.current)fileRef.current.click();}catch(e){}},420);
  }
  useEffect(function(){
  ...
  function parseAmount(text){
    ...
    function toNum(v){var raw=String(v||"").replace(/\s/g,"");if(!raw)return 0;var hasComma=raw.indexOf(",")>=0,hasDot=raw.indexOf(".")>=0;if(hasComma&&hasDot){raw=raw.replace(/\./g,"").replace(",", ".");}else if(hasComma){raw=raw.replace(",", ".");}var n=parseFloat(raw);return isNaN(n)?0:n;}
    function amountMatches(l){var out=[];var textLine=String(l||"");var re=/(?:€|eur|euro)?\s*(\d{1,5}(?:[\.\s]\d{3})*(?:[,\.]\d{1,2}))\s*(?:€|eur|euro)?/ig;var m;while((m=re.exec(textLine))!==null){var raw=m[1]||m[0];var n=toNum(raw);if(n>0&&n<100000)out.push({raw:raw,amount:n});}return out;}
    var candidates=[];
    lines.forEach(function(l,idx){
      var low=normalizeReceiptText(l);
      var around=normalizeReceiptText(((lines[idx-1]||"")+" "+l+" "+(lines[idx+1]||"")));
      var ms=amountMatches(l);
      if(!ms.length)return;
      ms.forEach(function(m){
        var score=0;
        var local=low;
        if(/\b(tot|tot\.|totale|total)\b/.test(local))score+=220;
        if(/tot\s*\.?\s*(euro|eur)|totale\s*(euro|eur)|total\s*(euro|eur)/.test(local))score+=260;
        if(/totale\s+complessivo|totale\s+documento|totale\s+doc|totale\s+scontrino|totale\s+pagare|totale\s+da\s+pagare|da\s+pagare|importo\s+totale|amount\s+due|grand\s+total/.test(local))score+=280;
        if(/\b(tot|tot\.|totale|total)\b/.test(around))score+=80;
        if(/importo\s+pagato|pagato|pagamento|transazione|saldo/.test(local))score+=45;
        if(/contanti|cash|carta|bancomat|pos|visa|mastercard|pagobancomat|elettronico|elettronica/.test(local))score+=20;
        if(/subtotale|sub\s*totale|parziale|imponibile/.test(local))score-=80;
        if(/iva|imposta|aliquota|tax|resto|cambio|sconto|abbuono|arrotond|residuo/.test(local))score-=160;
        if(/prezzo|prez\.|pz|qta|quantita|quantita|reparto|iva\s*%|descrizione|articolo|item/.test(local))score-=75;
        if(m.amount<1)score-=30;
        if(m.amount>=1&&m.amount<3&&!/\b(tot|tot\.|totale|total)\b/.test(local))score-=35;
        score+=idx*0.25;
        candidates.push({amount:m.amount,score:score,line:l,index:idx});
      });
    });
    candidates=candidates.filter(function(c){return c.amount>0&&c.amount<100000;});
    if(!candidates.length)return "";
    var totalCandidates=candidates.filter(function(c){return /\b(tot|tot\.|totale|total)\b|tot\s*\.?\s*(euro|eur)|totale\s*(euro|eur)|da\s+pagare|importo\s+totale/i.test(normalizeReceiptText(c.line));});
  ...
  function inferReceiptDescription(text,merchant,suggested){var all=normalizeReceiptText((text||"")+" "+(merchant||"")+" "+(suggested||""));if(/benu/.test(all)&&/farmacia/.test(all))return "Benu Farmacia";if(/farmacia|parafarmacia/.test(all))return "Farmacia";if(/esselunga/.test(all))return "Esselunga";if(/carrefour/.test(all))return "Carrefour";if(/conad/.test(all))return "Conad";if(/coop/.test(all))return "Coop";if(/lidl/.test(all))return "Lidl";if(/eurospin/.test(all))return "Eurospin";if(/aldi/.test(all))return "Aldi";if(/ikea/.test(all))return "IKEA";if(/leroy/.test(all))return "Leroy Merlin";if(/q8/.test(all))return "Q8";if(/eni/.test(all))return "ENI";if(!isBadReceiptDescription(merchant))return String(merchant||"").trim().slice(0,60);return "Scontrino";}
  ...
  async function runOCRWithImage(img,name){
    if(!img){setMsg("Prima scatta o carica la foto dello scontrino.");return;}
    setOcrLoading(true);setMsg("Sto leggendo lo scontrino...");setOcrConfidence(null);
    try{
      var token="";try{if(fbAuth.currentUser)token=await fbAuth.currentUser.getIdToken();}catch(e){}
      var res=await fetch(RECEIPT_OCR_ENDPOINT,{method:"POST",headers:{"Content-Type":"application/json",...(token?{"Authorization":"Bearer "+token}:{})},body:JSON.stringify({imageBase64:img,imageData:img,imageName:name||imageName||"receipt.jpg",categories:sCats.map(function(c){return{name:c.name,id:c.id};}),methods:sMethods.map(function(m){return{name:m.name,id:m.id};}),locale:"it-IT"})});
      var data=await res.json().catch(function(){return{};});
      if(!res.ok||data.ok===false)throw new Error(data.error||("Errore OCR "+res.status));
      applyParsed(data,data.text||"");
      setMsg(data.amount?"Scontrino letto automaticamente. Controlla l'anteprima e salva.":"Testo letto, ma il totale non è sicuro: controlla e completa l'importo.");
    }catch(err){setMsg("Lettura automatica non riuscita: "+(err&&err.message?err.message:"errore sconosciuto")+". Puoi comunque compilare manualmente l'anteprima.");}
    setOcrLoading(false);
  }
  function runOCR(){runOCRWithImage(imageUrl,imageName||"receipt.jpg");}
  function onFile(e){var file=e.target.files&&e.target.files[0];if(!file)return;var nm=file.name||"scontrino.jpg";setImageName(nm);setOcrConfidence(null);setReceiptText("");setMsg("Foto caricata. Preparo l'immagine e avvio la lettura...");prepareReceiptImage(file).then(function(img){if(!img){setMsg("Non riesco a leggere questa immagine. Prova con un'altra foto.");return;}setImageUrl(img);setMsg("Foto caricata. Lettura automatica in corso...");runOCRWithImage(img,nm);});}
  function save(){if(!f.amount||isNaN(f.amount)||Number(f.amount)<=0){setMsg("Inserisci un importo valido prima di salvare.");return;}onSave({id:Date.now(),amount:parseFloat(f.amount),catId:Number(f.catId),methodId:Number(f.methodId),desc:f.desc||"Scontrino",date:f.date,rateizzato:false,rate:12,receipt:true,receiptImageName:imageName||"",receiptOcrText:receiptText||"",receiptOcrConfidence:ocrConfidence});setImageUrl("");setImageName("");setReceiptText("");setOcrConfidence(null);setMsg("Scontrino salvato come uscita.");setF({amount:"",catId:defaultCat.id,methodId:defaultMethod.id,desc:"Scontrino",date:todayStr()});if(fileRef.current)fileRef.current.value="";if(uploadRef.current)uploadRef.current.value="";}
  return <div style={{background:dark?"linear-gradient(160deg,#252535 0%,#1e1e30 100%)":"linear-gradient(160deg,#ffffff 0%,#f7fbff 100%)",border:"1px solid "+borderC,borderRadius:16,padding:isMobile?10:18,display:"flex",flexDirection:"column",gap:12,boxShadow:dark?"none":"0 8px 28px rgba(55,138,221,0.08)"}}>
    <div style={{display:"flex",alignItems:"center",gap:12}}><div style={{width:44,height:44,borderRadius:14,background:expenseColor+"22",display:"flex",alignItems:"center",justifyContent:"center",fontSize:24}}>📷</div><div style={{flex:1}}><div style={{fontSize:17,fontWeight:900,color:tc}}>Scansiona scontrino</div><div style={{fontSize:12,color:sc}}>Scatta una foto: la lettura parte automaticamente. Controlla e salva la spesa.</div></div></div>
    ...
    <div style={{border:"2px dashed "+(dark?"#555":"#cfd8ea"),borderRadius:16,padding:isMobile?18:24,textAlign:"center",background:dark?"#1e1e30":"#f8fbff"}}>{imageUrl?<img src={imageUrl} alt="Scontrino" style={{maxWidth:"100%",maxHeight:220,borderRadius:12,objectFit:"contain",display:"block",margin:"0 auto 10px"}}/>:<div style={{fontSize:38,marginBottom:8}}>🧾</div>}<div style={{fontSize:14,fontWeight:800,color:tc}}>{imageUrl?"Cambia foto":"Aggiungi scontrino"}</div><div style={{fontSize:12,color:sc,marginTop:4}}>{ocrLoading?"Lettura automatica in corso...":"Puoi scattare una foto o caricare un'immagine dalla galleria."}</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginTop:12}}><button type="button" onClick={function(e){e.stopPropagation();if(fileRef.current)fileRef.current.click();}} style={{background:expenseColor,color:"#fff",border:"none",borderRadius:btnRadius,padding:"9px 10px",fontSize:12,fontWeight:900,cursor:"pointer"}}>📷 Fotografa</button><button type="button" onClick={function(e){e.stopPropagation();if(uploadRef.current)uploadRef.current.click();}} style={{background:dark?"#333":"#fff",color:tc,border:"1px solid "+borderC,borderRadius:btnRadius,padding:"9px 10px",fontSize:12,fontWeight:900,cursor:"pointer"}}>🖼️ Carica foto</button></div></div>
    {imageUrl&&<button onClick={runOCR} disabled={ocrLoading} style={{background:ocrLoading?"#aaa":expenseColor,color:"#fff",border:"none",borderRadius:btnRadius,padding:"10px 14px",fontSize:13,fontWeight:900,cursor:ocrLoading?"not-allowed":"pointer"}}>{ocrLoading?"Lettura in corso...":"Rileggi automaticamente"}</button>}
    {ocrConfidence!=null&&<div style={{fontSize:11,color:sc,marginTop:-4}}>Confidenza OCR: {Math.round(Number(ocrConfidence)*100)}%</div>}
    {msg&&<div style={{background:dark?"#1e1e30":"#fff8e1",border:"1px solid "+(dark?"#555":"#ffe08a"),borderRadius:12,padding:"10px 12px",fontSize:12,color:dark?"#ddd":"#745400",lineHeight:1.45}}>{msg}</div>}
    <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 1fr",gap:10}}><div style={card}><label style={label}>Importo ({sym})</label><input type="number" value={f.amount} onChange={function(e){setF(function(p){return{...p,amount:e.target.value};});}} placeholder="0,00" style={{...inp,fontSize:22,fontWeight:900,color:expenseColor}}/></div><div style={card}><label style={label}>Data</label><DatePickerField value={f.date} onChange={function(v){setF(function(p){return{...p,date:v};});}}/></div><div style={card}><label style={label}>Categoria</label><select value={f.catId} onChange={function(e){setF(function(p){return{...p,catId:e.target.value};});}} style={inp}>{sCats.map(function(c){return <option key={c.id} value={c.id}>{c.icon} {c.name}</option>;})}</select></div><div style={card}><label style={label}>Metodo</label><select value={f.methodId} onChange={function(e){setF(function(p){return{...p,methodId:e.target.value};});}} style={inp}>{sMethods.map(function(m){return <option key={m.id} value={m.id}>{m.icon} {m.name}</option>;})}</select></div><div style={{...card,gridColumn:isMobile?"auto":"1/-1"}}><label style={label}>Descrizione</label><input value={f.desc} onChange={function(e){setF(function(p){return{...p,desc:e.target.value};});}} placeholder="Negozio o descrizione" style={inp}/></div></div>
    <button onClick={save} style={{background:"linear-gradient(135deg,"+expenseColor+",#7F77DD)",color:"#fff",border:"none",borderRadius:btnRadius,padding:isMobile?12:15,fontSize:15,fontWeight:900,cursor:"pointer",boxShadow:dark?"none":"0 6px 18px "+expenseColor+"44"}}>Salva uscita da scontrino</button>
  </div>;
}
export function BulkEntry({onSave,type}){
  var lang=(ctx&&ctx.lang)||"it";
  var t=TRANSLATIONS[lang]||TRANSLATIONS.it;
  ...
  var sCats=useMemo(function(){return sortedCats(cats,catOrder,catSortMode,expenseGroups);},[cats,catOrder,catSortMode,expenseGroups]);
  var sMethods=useMemo(function(){return sortedMethods(methods,methodOrder,methodSortMode);},[methods,methodOrder,methodSortMode]);
  function blank(){var dc=(sCats.find(function(c){return c.id===4;})||sCats[0]||{id:1});var dm=(sMethods.find(function(m){return m.id===2;})||sMethods[0]||{id:1});return{_id:Date.now()+Math.random(),amount:"",catId:dc.id,methodId:dm.id,itype:"salario",desc:"",date:todayStr(),rateizzato:false,rate:12};}
  var [rows,setRows]=useState([blank()]);
  var inp={width:"100%",borderRadius:8,border:"1px solid "+(dark?"#444":"#ddd"),padding:"8px 10px",fontSize:14,background:dark?"#2a2a3e":"#fff",color:dark?"#eee":"#333"};
  ...
  function saveAll(){var valid=rows.filter(function(r){return r.amount&&!isNaN(r.amount)&&Number(r.amount)>0;});if(!valid.length)return;onSave(valid.map(function(r){return{id:Date.now()+Math.random(),amount:parseFloat(r.amount),catId:Number(r.catId),methodId:Number(r.methodId),type:r.itype,desc:r.desc,date:r.date,rateizzato:r.rateizzato,rate:r.rate};}));setRows([blank()]);}
  var tc=dark?"#aaa":"#666";

  // MOBILE: card layout per ogni riga
  if(isMobile){
    return <div>
      {rows.map(function(r,idx){return <div key={r._id} style={{background:dark?"#1e1e30":"#f9f9f9",borderRadius:12,border:"1px solid "+(dark?"#444":"#eee"),padding:14,marginBottom:12,position:"relative"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
          <span style={{fontSize:13,fontWeight:600,color:tc}}>Voce {idx+1}</span>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
          <div><label style={{fontSize:11,color:tc,display:"block",marginBottom:3}}>Importo</label><input type="number" value={r.amount} onChange={function(e){upd(r._id,"amount",e.target.value);}} style={inp} placeholder="0,00"/></div>
          <div><label style={{fontSize:11,color:tc,display:"block",marginBottom:3}}>Data</label><DatePickerField value={r.date} onChange={function(v){upd(r._id,"date",v);}}/></div>
        </div>
        {type==="expense"?<>
          <div style={{marginBottom:8}}><label style={{fontSize:11,color:tc,display:"block",marginBottom:3}}>Categoria</label><select value={r.catId} onChange={function(e){upd(r._id,"catId",e.target.value);}} style={inp}>{sCats.map(function(c){return <option key={c.id} value={c.id}>{c.icon} {c.name}</option>;})}</select></div>
          <div style={{marginBottom:8}}><label style={{fontSize:11,color:tc,display:"block",marginBottom:3}}>Metodo</label><select value={r.methodId} onChange={function(e){upd(r._id,"methodId",e.target.value);}} style={inp}>{sMethods.map(function(m){return <option key={m.id} value={m.id}>{m.icon} {m.name}</option>;})}</select></div>
        </>:<div style={{marginBottom:8}}><label style={{fontSize:11,color:tc,display:"block",marginBottom:3}}>Tipo</label><select value={r.itype} onChange={function(e){upd(r._id,"itype",e.target.value);}} style={inp}>{(ctx.incomeTypes||getAllIncomeTypes()).map(function(it){return <option key={it.id} value={it.id}>{it.icon} {it.name}</option>;})}</select></div>}
        <div style={{marginBottom:8}}><label style={{fontSize:11,color:tc,display:"block",marginBottom:3}}>Descrizione</label><input type="text" value={r.desc} onChange={function(e){upd(r._id,"desc",e.target.value);}} style={inp} placeholder="Inserisci una descrizione..."/></div>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          <input type="checkbox" checked={r.rateizzato} onChange={function(){upd(r._id,"rateizzato",!r.rateizzato);}} id={"rate"+r._id}/>
          <label htmlFor={"rate"+r._id} style={{fontSize:13,color:tc}}>Rateizza</label>
          {r.rateizzato&&<select value={r.rate} onChange={function(e){upd(r._id,"rate",Number(e.target.value));}} style={{...sinp,width:80}}>{[2,3,4,6,12,24].map(function(n){return <option key={n} value={n}>{n} mesi</option>;})}</select>}
        </div>
      </div>;})}
      <div style={{display:"flex",gap:10,marginTop:4}}>
        <Btn onClick={function(){setRows(function(p){return [...p,blank()];});}} bg={dark?"#333":"#f0f0f0"} color={dark?"#eee":"#444"}>{t.addRow}</Btn>
        <Btn onClick={saveAll} bg={type==="expense"?"#E24B4A":"#1D9E75"}>{t.saveAll} ({rows.length})</Btn>
      </div>
    </div>;
  }

  // DESKTOP: tabella orizzontale
  return <div><div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}><thead><tr style={{background:dark?"#252535":"#f5f5f5"}}><th style={{padding:"8px 6px",textAlign:"left",fontWeight:500,color:tc,minWidth:80}}>{t.amount}</th>{type==="expense"?<><th style={{padding:"8px 6px",textAlign:"left",fontWeight:500,color:tc,minWidth:120}}>{t.category}</th><th style={{padding:"8px 6px",textAlign:"left",fontWeight:500,color:tc,minWidth:100}}>{t.method}</th></>:<th style={{padding:"8px 6px",textAlign:"left",fontWeight:500,color:tc}}>{t.incomeType}</th>}<th style={{padding:"8px 6px",textAlign:"left",fontWeight:500,color:tc,minWidth:120}}>{t.description}</th><th style={{padding:"8px 6px",textAlign:"left",fontWeight:500,color:tc,minWidth:126}}>{t.date}</th><th style={{padding:"8px 6px",color:tc}}>÷</th><th></th></tr></thead><tbody>{rows.map(function(r){return <tr key={r._id} style={{borderBottom:"1px solid "+(dark?"#333":"#f0f0f0")}}><td style={{padding:5}}><input type="number" value={r.amount} onChange={function(e){upd(r._id,"amount",e.target.value);}} style={{...sinp,width:78}} placeholder="0,00"/></td>{type==="expense"?<><td style={{padding:5}}><select value={r.catId} onChange={function(e){upd(r._id,"catId",e.target.value);}} style={sinp}>{sCats.map(function(c){return <option key={c.id} value={c.id}>{c.icon} {c.name}</option>;})}</select></td><td style={{padding:5}}><select value={r.methodId} onChange={function(e){upd(r._id,"methodId",e.target.value);}} style={sinp}>{sMethods.map(function(m){return <option key={m.id} value={m.id}>{m.icon} {m.name}</option>;})}</select></td></>:<td style={{padding:5}}><select value={r.itype} onChange={function(e){upd(r._id,"itype",e.target.value);}} style={sinp}>{(ctx.incomeTypes||getAllIncomeTypes()).map(function(it){return <option key={it.id} value={it.id}>{it.icon} {it.name}</option>;})}</select></td>}<td style={{padding:5}}><input type="text" value={r.desc} onChange={function(e){upd(r._id,"desc",e.target.value);}} style={{...sinp,minWidth:110}} placeholder="Inserisci una descrizione..."/></td><td style={{padding:5}}><input type="date" value={r.date} onChange={function(e){upd(r._id,"date",e.target.value);}} style={{...sinp,minWidth:126}}/></td><td style={{padding:5,textAlign:"center"}}><input type="checkbox" checked={r.rateizzato} onChange={function(){upd(r._id,"rateizzato",!r.rateizzato);}}/>{r.rateizzato&&<select value={r.rate} onChange={function(e){upd(r._id,"rate",Number(e.target.value));}} style={{...sinp,marginTop:3,width:62}}>{[2,3,4,6,12,24].map(function(n){return <option key={n} value={n}>{n}m</option>;})}</select>}</td><td style={{padding:5}}><button onClick={function(){setRows(function(p){return p.filter(function(x){return x._id!==r._id;});});}} style={{background:"none",border:"none",cursor:"pointer",color:"#aaa",fontSize:15}}>x</button></td></tr>;})} </tbody></table></div><div style={{display:"flex",gap:10,marginTop:12}}><Btn onClick={function(){setRows(function(p){return [...p,blank()];});}} bg={dark?"#333":"#f0f0f0"} color={dark?"#eee":"#444"}>{t.addRow}</Btn><Btn onClick={saveAll} bg={type==="expense"?"#E24B4A":"#1D9E75"}>{t.saveAll} ({rows.length})</Btn></div></div>;
}
export function RecurringManager(){
  var ctx=useApp();var t=ctx.t,cats=ctx.cats,methods=ctx.methods,recurring=ctx.recurring,setRecurring=ctx.setRecurring,sym=ctx.sym,dark=ctx.dark,curMonthKey=ctx.curMonthKey,confirmRecurring=ctx.confirmRecurring,catOrder=ctx.catOrder,methodOrder=ctx.methodOrder,catSortMode=ctx.catSortMode,methodSortMode=ctx.methodSortMode,expenseGroups=ctx.expenseGroups;
  var [showAdd,setShowAdd]=useState(false);
  var sCats=useMemo(function(){return sortedCats(cats,catOrder,catSortMode,expenseGroups);},[cats,catOrder,catSortMode,expenseGroups]);
  var sMethods=useMemo(function(){return sortedMethods(methods,methodOrder,methodSortMode);},[methods,methodOrder,methodSortMode]);
  var blank={name:"",amount:"",catId:sCats[0]?sCats[0].id:1,methodId:sMethods[0]?sMethods[0].id:1,rtype:"expense",incomeType:"salario",dayOfMonth:1,rateizzato:false,rate:12};
  ...
  function add(){if(!form.name||!form.amount)return;setRecurring(function(p){return [...p,{...form,id:Date.now(),amount:parseFloat(form.amount)}];});setForm(blank);setShowAdd(false);}
  return <div style={{display:"flex",flexDirection:"column",gap:16}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><div style={{fontSize:14,fontWeight:500,color:tc}}>Transazioni ricorrenti</div><Btn onClick={function(){setShowAdd(function(s){return !s;});}}>{t.addRecurring}</Btn></div>
    {pending.length>0?<div style={{background:"#FFF8E1",borderRadius:12,border:"1px solid #FFD54F",padding:14}}><div style={{fontSize:13,fontWeight:500,color:"#856404",marginBottom:10}}>{t.pendingConfirm} {MONTHS_FULL[now.getMonth()]}:</div>{pending.map(function(r){return <div key={r.id} style={{display:"flex",alignItems:"center",gap:10,marginBottom:8,flexWrap:"wrap"}}><div style={{flex:1}}><div style={{fontSize:13,fontWeight:500}}>{r.name}</div><div style={{fontSize:11,color:"#888"}}>{fmtAmt(r.amount,sym)}</div></div><Btn onClick={function(){confirmRecurring(r,curMonthKey);}} bg="#1D9E75" style={{padding:"6px 12px",fontSize:12}}>{t.confirm}</Btn><Btn onClick={function(){setRecurring(function(p){return p.map(function(x){return x.id===r.id?{...x,skipped:[...(x.skipped||[]),curMonthKey]}:x;});});}} bg="#f0f0f0" color="#666" style={{padding:"6px 12px",fontSize:12}}>{t.skip}</Btn></div>;})}</div>:<div style={{background:"#e8f8f0",borderRadius:12,padding:"12px 16px",fontSize:13,color:"#1D9E75"}}>{t.confirmed}</div>}
    {showAdd&&<div style={{background:dark?"#252535":"#f9f9f9",borderRadius:12,border:"1px solid "+(dark?"#444":"#eee"),padding:16}}><div style={{display:"flex",flexDirection:"column",gap:10}}><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}><div><label style={{fontSize:11,color:sc,display:"block",marginBottom:3}}>{t.name}</label><input value={form.name} onChange={function(e){setForm(function(p){return{...p,name:e.target.value};});}} style={inp}/></div><div><label style={{fontSize:11,color:sc,display:"block",marginBottom:3}}>{t.amount}</label><input type="number" value={form.amount} onChange={function(e){setForm(function(p){return{...p,amount:e.target.value};});}} style={inp}/></div></div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}><div><label style={{fontSize:11,color:sc,display:"block",marginBottom:3}}>Tipo</label><select value={form.rtype} onChange={function(e){setForm(function(p){return{...p,rtype:e.target.value};});}} style={inp}><option value="expense">{t.expenses}</option><option value="income">{t.incomes}</option></select></div><div><label style={{fontSize:11,color:sc,display:"block",marginBottom:3}}>{t.dayOfMonth}</label><input type="number" min={0} max={31} value={form.dayOfMonth} onChange={function(e){setForm(function(p){return{...p,dayOfMonth:Number(e.target.value)};});}} style={inp}/></div></div>{form.rtype==="expense"?<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}><div><label style={{fontSize:11,color:sc,display:"block",marginBottom:3}}>{t.category}</label><select value={form.catId} onChange={function(e){setForm(function(p){return{...p,catId:e.target.value};});}} style={inp}>{sCats.map(function(c){return <option key={c.id} value={c.id}>{c.icon} {c.name}</option>;})}</select></div><div><label style={{fontSize:11,color:sc,display:"block",marginBottom:3}}>{t.method}</label><select value={form.methodId} onChange={function(e){setForm(function(p){return{...p,methodId:e.target.value};});}} style={inp}>{sMethods.map(function(m){return <option key={m.id} value={m.id}>{m.icon} {m.name}</option>;})}</select></div></div>:<div><label style={{fontSize:11,color:sc,display:"block",marginBottom:3}}>{t.incomeType}</label><select value={form.incomeType} onChange={function(e){setForm(function(p){return{...p,incomeType:e.target.value};});}} style={inp}>{(ctx.incomeTypes||getAllIncomeTypes()).map(function(it){return <option key={it.id} value={it.id}>{it.icon} {it.name}</option>;})}</select></div>}<Toggle label={t.instalment} checked={form.rateizzato} onChange={function(){setForm(function(p){return{...p,rateizzato:!p.rateizzato};});}}/>{form.rateizzato&&<RatePicker value={form.rate} onChange={function(n){setForm(function(p){return{...p,rate:n};});}}/>}<div style={{display:"flex",gap:8}}><Btn onClick={add}>{t.save}</Btn><Btn onClick={function(){setShowAdd(false);}} bg={dark?"#333":"#f0f0f0"} color={dark?"#eee":"#666"}>{t.cancel}</Btn></div></div></div>}
    <div style={{display:"flex",flexDirection:"column",gap:8}}>{recurring.length===0&&<div style={{fontSize:13,color:"#ccc",textAlign:"center",padding:"24px 0"}}>{t.noData}</div>}{recurring.map(function(r){return <div key={r.id} style={{background:dark?"#252535":"#fff",borderRadius:10,border:"1px solid "+(dark?"#444":"#eee"),padding:"12px 14px",display:"flex",alignItems:"center",gap:12}}><div style={{width:10,height:10,borderRadius:"50%",background:r.rtype==="expense"?"#E24B4A":"#1D9E75",flexShrink:0}}/><div style={{flex:1}}><div style={{fontSize:14,fontWeight:500,color:tc}}>{r.name}</div><div style={{fontSize:11,color:sc}}>{fmtAmt(r.amount,sym)}</div></div><button onClick={function(){setRecurring(function(p){return p.filter(function(x){return x.id!==r.id;});});}} style={{background:"none",border:"none",cursor:"pointer",color:"#ccc",fontSize:16}}>x</button></div>;})} </div>
  </div>;
}
export function GoalsPanel(){
  var ctx=useApp();var t=ctx.t,goals=ctx.goals,setGoals=ctx.setGoals,sym=ctx.sym,dark=ctx.dark,btnRadius=ctx.btnRadius;
  var [showAdd,setShowAdd]=useState(false);var [addAmt,setAddAmt]=useState({});
  ...
  function addGoal(){if(!form.name||!form.target)return;setGoals(function(p){return [...p,{...form,id:Date.now(),target:parseFloat(form.target),saved:parseFloat(form.saved)||0}];});setForm(blank);setShowAdd(false);}
  function addSaving(gid){var v=parseFloat(addAmt[gid]);if(!v||v<=0)return;setGoals(function(p){return p.map(function(g){return g.id===gid?{...g,saved:Number(g.saved||0)+v}:g;});});setAddAmt(function(p){return{...p,[gid]:""};});}
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:16}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:10}}><div style={{fontSize:16,fontWeight:900,color:tc}}>🎯 Obiettivi di risparmio</div><Btn onClick={function(){setShowAdd(function(s){return !s;});}} bg="#7F77DD" style={{padding:"10px 18px",fontWeight:900}}>+ Nuovo</Btn></div>
    {showAdd&&<div style={{background:dark?"#252535":"#f9f9f9",borderRadius:14,border:"1px solid "+borderC,padding:18}}><div style={{display:"flex",gap:10,alignItems:"flex-start",marginBottom:10}}><div><label style={{fontSize:11,color:sc,display:"block",marginBottom:4}}>Icona</label><div style={{display:"flex",flexWrap:"wrap",gap:3,maxWidth:200}}>{GOAL_ICONS.map(function(ic){return <button key={ic} onClick={function(){setForm(function(p){return{...p,icon:ic};});}} style={{fontSize:17,background:form.icon===ic?"#EEEDFE":"none",border:form.icon===ic?"1px solid #7F77DD":"1px solid transparent",borderRadius:6,padding:"2px 4px",cursor:"pointer"}}>{ic}</button>;})}</div></div><div><label style={{fontSize:11,color:sc,display:"block",marginBottom:4}}>Colore</label><div style={{display:"flex",flexWrap:"wrap",gap:4}}>{COLORS.slice(0,9).map(function(c){return <button key={c} onClick={function(){setForm(function(p){return{...p,color:c};});}} style={{width:22,height:22,background:c,border:form.color===c?"2px solid #333":"2px solid transparent",borderRadius:"50%",cursor:"pointer",padding:0}}/>;})}</div></div></div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}><div><label style={{fontSize:11,color:sc,display:"block",marginBottom:3}}>Nome</label><input value={form.name} onChange={function(e){setForm(function(p){return{...p,name:e.target.value};});}} style={inp} placeholder="es. Vacanza"/></div><div><label style={{fontSize:11,color:sc,display:"block",marginBottom:3}}>Periodo</label><select value={form.period} onChange={function(e){setForm(function(p){return{...p,period:e.target.value};});}} style={inp}><option value="monthly">Mensile</option><option value="annual">Annuale</option></select></div><div><label style={{fontSize:11,color:sc,display:"block",marginBottom:3}}>Target ({sym})</label><input type="number" value={form.target} onChange={function(e){setForm(function(p){return{...p,target:e.target.value};});}} style={inp}/></div><div><label style={{fontSize:11,color:sc,display:"block",marginBottom:3}}>Già risparmiato</label><input type="number" value={form.saved} onChange={function(e){setForm(function(p){return{...p,saved:e.target.value};});}} style={inp}/></div><div style={{gridColumn:"1/-1"}}><label style={{fontSize:11,color:sc,display:"block",marginBottom:3}}>Scadenza</label><input type="date" value={form.deadline} onChange={function(e){setForm(function(p){return{...p,deadline:e.target.value};});}} style={inp}/></div></div><div style={{display:"flex",gap:8}}><Btn onClick={addGoal} bg="#7F77DD">Crea</Btn><Btn onClick={function(){setShowAdd(false);}} bg={dark?"#333":"#f0f0f0"} color={dark?"#eee":"#666"}>{t.cancel}</Btn></div></div>}
    {goals.length===0&&!showAdd&&<div style={{textAlign:"center",color:"#ccc",padding:"40px 0",fontSize:14}}>Nessun obiettivo. Creane uno!</div>}
    <div style={{display:"flex",flexDirection:"column",gap:14}}>{goals.map(function(g,idx){var saved=Number(g.saved||0),target=Number(g.target||0),pct=target>0?Math.min(100,(saved/target)*100):0,done=pct>=100,daysLeft=null;if(g.deadline)daysLeft=Math.ceil((new Date(g.deadline)-new Date())/86400000);return <div key={g.id} style={{background:cardBg,borderRadius:16,border:"1px solid "+borderC,padding:16,position:"relative",overflow:"hidden",boxShadow:dark?"none":"0 4px 18px rgba(0,0,0,0.04)"}}><div style={{position:"absolute",top:0,left:0,width:pct+"%",height:4,background:done?"#1D9E75":g.color}}/><div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:12,marginBottom:10}}><div style={{display:"flex",alignItems:"center",gap:12,minWidth:0}}><div style={{width:48,height:48,borderRadius:14,background:g.color+"22",display:"flex",alignItems:"center",justifyContent:"center",fontSize:24,flexShrink:0}}>{g.icon}</div><div style={{minWidth:0}}><div style={{fontSize:18,fontWeight:900,color:tc,lineHeight:1.12}}>{g.name}</div><div style={{fontSize:12,color:sc,marginTop:3}}>{g.period==="annual"?"Annuale":"Mensile"}{g.deadline&&" · "+(daysLeft>0?daysLeft+"g":"Scaduto")}</div></div></div><div style={{display:"flex",gap:5,alignItems:"center",flexShrink:0}}><button disabled={idx===0} onClick={function(){moveGoal(g.id,-1);}} style={{width:30,height:30,borderRadius:10,border:"1px solid "+borderC,background:dark?"#1e1e30":"#f8f8f8",color:idx===0?"#bbb":tc,cursor:idx===0?"default":"pointer"}}>↑</button><button disabled={idx===goals.length-1} onClick={function(){moveGoal(g.id,1);}} style={{width:30,height:30,borderRadius:10,border:"1px solid "+borderC,background:dark?"#1e1e30":"#f8f8f8",color:idx===goals.length-1?"#bbb":tc,cursor:idx===goals.length-1?"default":"pointer"}}>↓</button><button onClick={function(){setGoals(function(p){return p.filter(function(x){return x.id!==g.id;});});}} style={{width:30,height:30,borderRadius:10,border:"none",background:"transparent",cursor:"pointer",color:"#ccc",fontSize:18}}>x</button></div></div><div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",gap:10,marginBottom:6}}><span style={{fontSize:18,fontWeight:900,color:g.color}}>{fmtAmt(saved,sym)}</span><span style={{fontSize:14,color:sc}}>di {fmtAmt(target,sym)}</span></div><div style={{background:dark?"#333":"#f0f0f0",borderRadius:8,height:9,overflow:"hidden"}}><div style={{background:done?"#1D9E75":g.color,height:9,borderRadius:8,width:pct+"%"}}/></div><div style={{display:"flex",justifyContent:"space-between",marginTop:5,marginBottom:12}}><span style={{fontSize:12,color:done?"#1D9E75":sc,fontWeight:700}}>{done?"Completato!":Math.round(pct)+"%"}</span>{!done&&<span style={{fontSize:12,color:sc}}>mancano {fmtAmt(Math.max(0,target-saved),sym)}</span>}</div>{!done&&<div style={{background:dark?"#1e1e30":"#F8FAFC",border:"1px solid "+(dark?"#444":"#E5E7EB"),borderRadius:14,padding:12}}><div style={{fontSize:12,color:sc,fontWeight:800,marginBottom:8}}>Aggiungi importo risparmiato</div><div style={{display:"grid",gridTemplateColumns:"minmax(0,1fr) auto",gap:10,alignItems:"center"}}><label style={{display:"flex",alignItems:"center",gap:8,background:dark?"#252535":"#fff",border:"1px solid "+(dark?"#555":"#DADDE5"),borderRadius:12,padding:"0 12px",minWidth:0}}><span style={{fontSize:18,color:g.color,fontWeight:900}}>{sym}</span><input type="number" placeholder="0,00" value={addAmt[g.id]||""} onChange={function(e){setAddAmt(function(p){return{...p,[g.id]:e.target.value};});}} onKeyDown={function(e){if(e.key==="Enter")addSaving(g.id);}} style={{width:"100%",border:"none",outline:"none",padding:"13px 0",fontSize:16,background:"transparent",color:tc,minWidth:0}}/></label><button onClick={function(){addSaving(g.id);}} style={{height:48,minWidth:116,border:"none",borderRadius:12,background:g.color,color:"#fff",fontSize:15,fontWeight:900,cursor:"pointer",boxShadow:dark?"none":"0 5px 14px "+g.color+"44"}}>+ Aggiungi</button></div></div>}</div>;})}</div>
  </div>;
}
export function AlertsPanel(){
  var ctx=useApp();var t=ctx.t,alerts=ctx.alerts,setAlerts=ctx.setAlerts,cats=ctx.cats,expenses=ctx.expenses,sym=ctx.sym,dark=ctx.dark,curMonthKey=ctx.curMonthKey,budgetPlan=ctx.budgetPlan,expenseGroups=ctx.expenseGroups;
  ...
  function saveAlert(){if(!form.name||!form.budget)return;var clean=normalizeForm();if(editingId){setAlerts(function(p){return p.map(function(a){return a.id===editingId?{...a,...clean,id:editingId}:a;});});}else{setAlerts(function(p){return [...p,{...clean,id:Date.now()}];});}setForm(blank);setEditingId(null);setShowAdd(false);}
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:16}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:10}}><div style={{display:"flex",alignItems:"center",gap:8}}><div style={{fontSize:14,fontWeight:700,color:tc}}>🔔 Alert di spesa</div>{triggered>0&&<span style={{background:"#E24B4A",color:"#fff",borderRadius:20,padding:"2px 8px",fontSize:12,fontWeight:600}}>{triggered} attivi</span>}</div><Btn onClick={function(){setEditingId(null);setForm(blank);setShowAdd(function(s){return !s;});}} bg="#E24B4A">+ Nuovo alert</Btn></div>
    {showAdd&&<div style={{background:dark?"#252535":"#f9f9f9",borderRadius:14,border:"1px solid "+borderC,padding:18}}><div style={{fontSize:15,fontWeight:900,color:tc,marginBottom:12}}>{editingId?"Modifica alert":"Nuovo alert"}</div><div style={{display:"flex",flexDirection:"column",gap:10}}><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}><div><label style={{fontSize:11,color:sc,display:"block",marginBottom:3}}>Nome alert</label><input value={form.name} onChange={function(e){setForm(function(p){return{...p,name:e.target.value};});}} style={inp} placeholder="es. Budget ristorante"/></div><div><label style={{fontSize:11,color:sc,display:"block",marginBottom:3}}>Budget ({sym})</label><input type="number" value={form.budget} onChange={function(e){setForm(function(p){return{...p,budget:e.target.value};});}} style={inp} placeholder="200"/></div></div><div><label style={{fontSize:11,color:sc,display:"block",marginBottom:3}}>Periodo</label><div style={{display:"flex",gap:0,background:dark?"#333":"#f0f0f0",borderRadius:10,padding:3}}><button onClick={function(){setForm(function(p){return{...p,period:"monthly"};});}} style={{flex:1,padding:"7px",border:"none",borderRadius:8,background:form.period==="monthly"?(dark?"#555":"#fff"):"transparent",color:form.period==="monthly"?tc:sc,fontSize:13,cursor:"pointer"}}>Mensile</button><button onClick={function(){setForm(function(p){return{...p,period:"annual"};});}} style={{flex:1,padding:"7px",border:"none",borderRadius:8,background:form.period==="annual"?(dark?"#555":"#fff"):"transparent",color:form.period==="annual"?tc:sc,fontSize:13,cursor:"pointer"}}>Annuale</button></div></div><div><label style={{fontSize:11,color:sc,display:"block",marginBottom:3}}>Testo personalizzato</label><textarea value={form.customText} onChange={function(e){setForm(function(p){return{...p,customText:e.target.value};});}} style={{...inp,height:60,resize:"vertical"}} placeholder="es. Troppo al ristorante!"/></div><div><label style={{fontSize:11,color:sc,display:"block",marginBottom:3}}>Tipo soglia</label><div style={{display:"flex",gap:0,background:dark?"#333":"#f0f0f0",borderRadius:10,padding:3}}><button onClick={function(){handleTypeChange("cat");}} style={{flex:1,padding:"7px",border:"none",borderRadius:8,background:form.type==="cat"?(dark?"#555":"#fff"):"transparent",color:form.type==="cat"?tc:sc,fontSize:13,cursor:"pointer"}}>Singola categoria</button><button onClick={function(){handleTypeChange("group");}} style={{flex:1,padding:"7px",border:"none",borderRadius:8,background:form.type==="group"?(dark?"#555":"#fff"):"transparent",color:form.type==="group"?tc:sc,fontSize:13,cursor:"pointer"}}>Area</button></div></div>{form.type==="cat"?<div><label style={{fontSize:11,color:sc,display:"block",marginBottom:3}}>Categoria</label><select value={form.catId} onChange={function(e){handleCatChange(e.target.value);}} style={inp}>{groups.map(function(g){return <optgroup key={g.id} label={g.name}>{cats.filter(function(c){return c.group===g.id;}).map(function(c){return <option key={c.id} value={c.id}>{c.icon} {c.name}</option>;})}</optgroup>;})}</select></div>:<div><label style={{fontSize:11,color:sc,display:"block",marginBottom:3}}>Area</label><select value={form.groupId} onChange={function(e){handleGroupChange(e.target.value);}} style={inp}>{groups.map(function(g){return <option key={g.id} value={g.id}>{g.name}</option>;})}</select></div>}<div><label style={{fontSize:11,color:sc,display:"block",marginBottom:3}}>Modalità attivazione</label><div style={{display:"flex",gap:0,background:dark?"#333":"#f0f0f0",borderRadius:10,padding:3}}><button onClick={function(){setForm(function(p){return{...p,triggerMode:"immediate"};});}} style={{flex:1,padding:"7px",border:"none",borderRadius:8,background:form.triggerMode==="immediate"?(dark?"#555":"#fff"):"transparent",color:form.triggerMode==="immediate"?tc:sc,fontSize:13,cursor:"pointer"}}>Superamento immediato</button><button onClick={function(){setForm(function(p){return{...p,triggerMode:"pct"};});}} style={{flex:1,padding:"7px",border:"none",borderRadius:8,background:form.triggerMode==="pct"?(dark?"#555":"#fff"):"transparent",color:form.triggerMode==="pct"?tc:sc,fontSize:13,cursor:"pointer"}}>Dopo % superamento</button></div></div>{form.triggerMode==="pct"&&<div><label style={{fontSize:11,color:sc,display:"block",marginBottom:3}}>% (es. 25 attiva al 125%)</label><input type="number" min={0} max={200} value={form.triggerPct} onChange={function(e){setForm(function(p){return{...p,triggerPct:e.target.value};});}} style={inp} placeholder="25"/></div>}<div style={{display:"flex",gap:8}}><Btn onClick={saveAlert} bg="#E24B4A">{editingId?"Salva modifiche":"Crea alert"}</Btn><Btn onClick={cancelForm} bg={dark?"#333":"#f0f0f0"} color={dark?"#eee":"#666"}>{t.cancel}</Btn></div></div></div>}
    {alerts.length===0&&!showAdd&&<div style={{textAlign:"center",color:"#ccc",padding:"40px 0",fontSize:14}}>Nessun alert configurato.</div>}
    {alerts.map(function(al){var spent=getSpent(al),p=getPct(al),trig=isTriggered(al),barColor=p>=100?"#E24B4A":p>=75?"#EF9F27":"#1D9E75";var cat2=al.type==="cat"?cats.find(function(c){return c.id===al.catId;}):null;var grp2=al.type==="group"?groups.find(function(g){return g.id===al.groupId;}):null;var label=al.type==="cat"?(cat2?cat2.icon+" "+cat2.name:"?"):("📂 "+(grp2?grp2.name:al.groupId));var tLabel=al.triggerMode==="immediate"?"al 100%":"al "+(100+(al.triggerPct||0))+"%";return <div key={al.id} style={{background:trig?(dark?"#3a1a1a":"#fff5f5"):(dark?"#252535":"#fff"),borderRadius:12,border:"1px solid "+(trig?"#fcc":borderC),padding:"14px 16px"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:6,gap:10}}><div style={{minWidth:0}}><div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>{trig&&<span>⚠️</span>}<div style={{fontSize:14,fontWeight:700,color:trig?"#E24B4A":tc}}>{al.name}</div><span style={{fontSize:10,background:al.period==="annual"?"#EEF":"#EFE",color:al.period==="annual"?"#534AB7":"#1D9E75",borderRadius:10,padding:"1px 6px"}}>{al.period==="annual"?"Annuale":"Mensile"}</span></div><div style={{fontSize:11,color:sc,marginTop:2}}>{label} · attiva {tLabel}</div>{al.customText&&<div style={{fontSize:12,color:trig?"#E24B4A":sc,marginTop:4,fontStyle:"italic"}}>"{al.customText}"</div>}</div><div style={{display:"flex",gap:6,flexShrink:0}}><button onClick={function(){startEdit(al);}} style={{background:dark?"#333":"#f0f0f0",border:"1px solid "+borderC,borderRadius:8,cursor:"pointer",color:tc,fontSize:12,padding:"5px 8px"}}>Modifica</button><button onClick={function(){setAlerts(function(p){return p.filter(function(x){return x.id!==al.id;});});}} style={{background:"none",border:"none",cursor:"pointer",color:"#ccc",fontSize:16}}>x</button></div></div><div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}><span style={{fontSize:13,fontWeight:600,color:barColor}}>{fmtAmt(spent,sym)}</span><span style={{fontSize:12,color:sc}}>budget {fmtAmt(al.budget,sym)}</span></div><div style={{background:dark?"#333":"#f0f0f0",borderRadius:8,height:6}}><div style={{background:barColor,height:6,borderRadius:8,width:Math.min(100,p)+"%"}}/></div><div style={{fontSize:11,color:barColor,marginTop:3,textAlign:"right"}}>{Math.round(p)}%</div></div>;})}
  </div>;
}

export function BudgetPlanPanel(){
  var ctx=useApp();var cats=ctx.cats,incomes=ctx.incomes,expenses=ctx.expenses,budgetPlan=ctx.budgetPlan,setBudgetPlan=ctx.setBudgetPlan,sym=ctx.sym,dark=ctx.dark,fmt=ctx.fmt,expenseGroups=ctx.expenseGroups,curMonthKey=ctx.curMonthKey;
  var groups=expenseGroups||DEFAULT_EXPENSE_GROUPS;
  var tc=dark?"#eee":"#333";var sc=dark?"#aaa":"#888";var borderC=dark?"#444":"#eee";var cardBg=dark?"#252535":"#fff";
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:16}}>
    {savingToast&&<Toast msg="Suddivisione salvata con successo!" onDone={function(){setSavingToast(false);}} color="#7F77DD"/>}

    {/* ── RISPARMIO SECTION ── */}
    <div style={{background:dark?"#1a2a1e":"#edfaf3",borderRadius:14,border:"2px solid #1D9E75",padding:20}}>
      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:14}}>
        <span style={{fontSize:20}}>💰</span>
        <div style={{fontSize:15,fontWeight:700,color:"#1D9E75"}}>Risparmio pianificato</div>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginBottom:14}}>
        <div style={{background:cardBg,borderRadius:10,padding:"12px 14px",textAlign:"center"}}>
          <div style={{fontSize:11,color:sc,marginBottom:3}}>Reddito di rif.</div>
          <div style={{fontSize:16,fontWeight:700,color:tc}}>{fmt(effectiveIncome)}</div>
        </div>
        <div style={{background:cardBg,borderRadius:10,padding:"12px 14px",textAlign:"center"}}>
          <div style={{fontSize:11,color:sc,marginBottom:3}}>Totale allocato</div>
        ...
        <div style={{background:cardBg,borderRadius:10,padding:"12px 14px",textAlign:"center",border:"2px solid #1D9E75"}}>
          <div style={{fontSize:11,color:"#1D9E75",marginBottom:3,fontWeight:600}}>Risparmio pianif.</div>
          <div style={{fontSize:16,fontWeight:700,color:"#1D9E75"}}>{fmt(savingPlanned)}</div>
          <div style={{fontSize:11,color:"#1D9E75"}}>{Math.round(savingPct)}% del reddito</div>
        </div>
      </div>
      <div style={{background:cardBg,borderRadius:10,padding:"12px 14px",marginBottom:10}}>
        <div style={{fontSize:12,color:sc,marginBottom:6}}>Confronto mese corrente ({curMonthKey})</div>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div>
            <div style={{fontSize:11,color:sc}}>Risparmio reale</div>
          ...
          <div style={{textAlign:"right"}}>
            <div style={{fontSize:11,color:sc}}>Rispetto al piano</div>
    ...
    <div style={{background:cardBg,borderRadius:14,border:"1px solid "+borderC,padding:20}}>
      <div style={{fontSize:15,fontWeight:600,color:tc,marginBottom:14}}>📥 Reddito mensile di riferimento</div>
      <div style={{display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
        <div style={{flex:1,minWidth:200}}>
          <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:6,position:"relative"}}>
            <span style={{fontSize:13,color:sc}}>Reddito di riferimento</span>
            <span onClick={function(){setShowInfoTooltip(function(v){return !v;});}} style={{fontSize:15,cursor:"pointer",color:sc,userSelect:"none"}}>ℹ</span>
            {showInfoTooltip&&<div style={{position:"absolute",top:24,left:0,background:dark?"#333":"#fff",border:"1px solid "+borderC,borderRadius:10,padding:"10px 14px",fontSize:13,color:tc,zIndex:10,boxShadow:"0 4px 16px rgba(0,0,0,0.15)",whiteSpace:"nowrap"}}>Media entrate mensili (ultimi 12 mesi):<br/><span style={{fontWeight:700,color:"#1D9E75"}}>{fmt(avgIncome)}</span></div>}
          </div>
          <div style={{fontSize:22,fontWeight:700,color:"#1D9E75",marginBottom:8}}>{fmt(effectiveIncome)}</div>
          <label style={{fontSize:12,color:sc,display:"block",marginBottom:4}}>Importo manuale:</label>
          ...
          {manualIncome!==""&&<button onClick={function(){setManualIncome("");}} style={{background:"none",border:"none",cursor:"pointer",color:"#aaa",fontSize:12,marginLeft:8}}>✕ reset</button>}
        </div>
        <div style={{textAlign:"right"}}>
          <div style={{fontSize:12,color:sc,marginBottom:4}}>Totale allocato</div>
          <div style={{fontSize:20,fontWeight:700,color:barColor}}>{fmt(totalAlloc)}</div>
          <div style={{fontSize:12,color:savingPlanned>0?"#1D9E75":"#aaa",marginTop:2}}>Disponibile: {fmt(savingPlanned)}</div>
        </div>
      </div>
      ...
      <div style={{fontSize:11,color:sc,marginTop:4}}>{Math.round(pctBar)}% del reddito allocato · risparmio pianificato: {Math.round(savingPct)}%</div>
    </div>

    {/* ── CATEGORIE ── */}
    <div style={{background:cardBg,borderRadius:14,border:"1px solid "+borderC,padding:20}}>
      <div style={{fontSize:15,fontWeight:600,color:tc,marginBottom:14}}>📊 Suddivisione per categoria</div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 130px 90px",gap:"4px 8px",marginBottom:8}}><div style={{fontSize:11,fontWeight:600,color:sc}}>CATEGORIA</div><div style={{fontSize:11,fontWeight:600,color:sc,textAlign:"right"}}>IMPORTO ({sym})</div><div style={{fontSize:11,fontWeight:600,color:sc,textAlign:"right"}}>% REDDITO</div></div>
      {groups.map(function(g){var gc=cats.filter(function(c){return c.group===g.id;});if(!gc.length)return null;var gt=gc.reduce(function(a,c){var it=items.find(function(i){return i.catId===c.id;});return a+(it?it.amount:0);},0);return <div key={g.id} style={{marginBottom:12}}><div style={{fontSize:11,fontWeight:600,color:"#aaa",textTransform:"uppercase",letterSpacing:1,marginBottom:6,paddingBottom:4,borderBottom:"1px solid "+borderC}}>{g.name} — {fmt(gt)}</div>{gc.map(function(c){var item=items.find(function(i){return i.catId===c.id;})||{amount:0,pct:0};var pctVal=item.pct||0;return <div key={c.id} style={{display:"grid",gridTemplateColumns:"1fr 130px 90px",gap:"4px 8px",alignItems:"center",marginBottom:4}}><div style={{display:"flex",alignItems:"center",gap:6,minWidth:0}}><span style={{fontSize:16,flexShrink:0}}>{c.icon}</span><span style={{fontSize:13,color:tc,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{c.name}</span></div>{hasBudgetIncome?<input type="number" min={0} value={item.amount||""} onChange={function(e){updateAmt(c.id,e.target.value);}} placeholder="0" style={{...inp,textAlign:"right",padding:"5px 8px"}}/>:<div style={{fontSize:13,color:sc,textAlign:"right",padding:"5px 8px"}}>—</div>}<input type="number" min={0} max={100} step={0.1} value={pctVal||""} onChange={function(e){updatePct(c.id,e.target.value);}} placeholder="0%" style={{...inp,textAlign:"right",padding:"5px 8px"}}/></div>;})}</div>;})}
      <Btn onClick={save} bg="#7F77DD" style={{marginTop:8,padding:"10px 24px",fontSize:14}}>Salva suddivisione</Btn>
    </div>
  </div>;
}

export function SettingsList({items,setItems,label,showGroup,showIcon,groupList,isMethod,allowArchive}){var ctx=useApp();var t=ctx.t,dark=ctx.dark,isMobile=ctx.isMobile;var gl=groupList||[];var [newName,setNewName]=useState("");var [newColor,setNewColor]=useState(COLORS[0]);var [newGroup,setNewGroup]=useState(gl[0]?gl[0].id:"");var [newIcon,setNewIcon]=useState("📦");var [editId,setEditId]=useState(null);var [editName,setEditName]=useState("");var [editColor,setEditColor]=useState("");var [editGroup,setEditGroup]=useState(gl[0]?gl[0].id:"");var [editIcon,setEditIcon]=useState("📦");var [sortMode,setSortMode]=useState("group");var dragIdx=useRef(null);var dragOverIdx=useRef(null);var inp={width:"100%",borderRadius:8,border:"1px solid "+(dark?"#444":"#ddd"),padding:"7px 10px",fontSize:14,background:dark?"#2a2a3e":"#fff",color:dark?"#eee":"#333"};var tc=dark?"#eee":"#333";var sc=dark?"#aaa":"#888";var borderC=dark?"#444":"#eee";
  function add(){if(!newName.trim())return;setItems([...items,{id:Date.now(),name:newName.trim(),color:newColor,icon:newIcon,...(showGroup?{group:newGroup}:{})}]);setNewName("");}
  function del(id){setItems(items.filter(function(i){return i.id!==id;}));}
  function startEdit(item){setEditId(item.id);setEditName(item.name);setEditColor(item.color||COLORS[0]);setEditGroup(item.group||(gl[0]?gl[0].id:""));setEditIcon(item.icon||"📦");}
  function saveEdit(){setItems(items.map(function(i){return i.id===editId?{...i,name:editName,color:editColor,icon:editIcon,...(showGroup?{group:editGroup}:{})}:i;}));setEditId(null);}
  function toggleArchive(id){setItems(items.map(function(i){return i.id===id?{...i,archived:!i.archived}:i;}));}
  ...
  return <div>
    <div style={{display:"flex",gap:8,marginBottom:12,alignItems:"center"}}>
      {[{id:"group",label:"Per Aree"},{id:"custom",label:"Riordina"}].map(function(sm){return <button key={sm.id} onClick={function(){setSortMode(sm.id);}} style={{padding:"6px 12px",borderRadius:8,border:"1px solid "+(sortMode===sm.id?"#7F77DD":borderC),background:sortMode===sm.id?"#EEEDFE":"transparent",color:sortMode===sm.id?"#534AB7":tc,fontSize:12,cursor:"pointer",fontWeight:sortMode===sm.id?600:400}}>{sm.label}</button>;})}
    </div>
    {showDrag?<div style={{marginBottom:12}}><SortableRows items={items} onMove={function(i,dir){var j=i+dir;if(j<0||j>=items.length)return;var arr=items.slice();var tmp=arr[i];arr[i]=arr[j];arr[j]=tmp;setItems(arr);}} renderItem={function(item){return <div style={{display:"flex",alignItems:"center",gap:10,minWidth:0}}>
        ...
        <span style={{flex:1,fontSize:13,color:tc,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{item.name}</span>
        {item.archived&&<span style={{fontSize:10,background:"#f0f0f0",color:"#888",borderRadius:10,padding:"1px 6px",flexShrink:0}}>Archiviato</span>}
        <button onClick={function(){startEdit(item);}} style={{background:"none",border:"none",cursor:"pointer",fontSize:15,color:"#bbb",flexShrink:0}}>✏</button>
        <button onClick={function(){toggleArchive(item.id);}} title={item.archived?"Ripristina":"Archivia"} style={{background:"none",border:"none",cursor:"pointer",fontSize:15,color:"#bbb",flexShrink:0}}>{item.archived?"📂":"🗂"}</button>
        <button onClick={function(){del(item.id);}} style={{background:"none",border:"none",cursor:"pointer",fontSize:15,color:"#bbb",flexShrink:0}}>x</button>
      </div>;}}/></div>:
    grouped.map(function(g){return <div key={g.id}>
      {showGroup&&g.name&&g.items.length>0&&<div style={{fontSize:11,fontWeight:600,color:"#aaa",textTransform:"uppercase",letterSpacing:1,margin:"12px 0 6px"}}>{g.name}</div>}
      {g.items.map(function(item){return <div key={item.id} style={{background:dark?"#252535":"#fff",borderRadius:10,border:"1px solid "+(dark?"#444":"#eee"),padding:"10px 12px",marginBottom:6,opacity:item.archived?0.55:1}}>
        {editId===item.id?<div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
          ...
          <Btn onClick={saveEdit} style={{padding:"6px 10px",fontSize:13}}>{t.save}</Btn>
        </div>:<div style={{display:"flex",alignItems:"center",gap:10}}>
          {showIcon&&<span style={{fontSize:18}}>{item.icon||"📦"}</span>}
          <div style={{width:12,height:12,borderRadius:"50%",background:item.color,flexShrink:0}}/>
          <span style={{flex:1,fontSize:14,color:tc}}>{item.name}</span>
          {item.archived&&<span style={{fontSize:10,background:"#f0f0f0",color:"#888",borderRadius:10,padding:"1px 6px"}}>Archiviato</span>}
          <button onClick={function(){startEdit(item);}} style={{background:"none",border:"none",cursor:"pointer",fontSize:15,color:"#bbb"}}>✏</button>
          <button onClick={function(){toggleArchive(item.id);}} title={item.archived?"Ripristina":"Archivia"} style={{background:"none",border:"none",cursor:"pointer",fontSize:15,color:"#bbb"}}>{item.archived?"📂":"🗂"}</button>
          <button onClick={function(){del(item.id);}} style={{background:"none",border:"none",cursor:"pointer",fontSize:15,color:"#bbb"}}>x</button>
        </div>}
      </div>;})}
    </div>;})}
    <div style={{background:dark?"linear-gradient(135deg,#252535,#1e1e30)":"linear-gradient(135deg,#ffffff,#f7fbff)",borderRadius:18,border:"1px solid "+(dark?"#3d3d55":"#dfe8f6"),padding:14,marginTop:14,boxShadow:dark?"none":"0 8px 24px rgba(55,138,221,0.08)"}}>
      <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:12}}>
        ...
        <div style={{flex:1,minWidth:0}}>
          <div style={{fontSize:14,fontWeight:900,color:tc}}>Nuova voce</div>
          <div style={{fontSize:12,color:sc,lineHeight:1.35}}>{label}</div>
        </div>
      </div>
      <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":(showGroup&&gl.length>0?"auto auto minmax(0,1.1fr) minmax(0,1fr) auto":"auto auto minmax(0,1fr) auto"),gap:10,alignItems:"center"}}>
        {showIcon&&<div style={{display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:11,color:sc,minWidth:52}}>Icona</span><EmojiPicker value={newIcon} onChange={setNewIcon}/></div>}
        <div style={{display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:11,color:sc,minWidth:52}}>Colore</span><input type="color" value={newColor} onChange={function(e){setNewColor(e.target.value);}} style={{width:42,height:42,padding:0,border:"1px solid "+borderC,borderRadius:12,cursor:"pointer",flexShrink:0,background:"transparent"}}/></div>
        <input placeholder="Nome voce" value={newName} onChange={function(e){setNewName(e.target.value);}} onKeyDown={function(e){if(e.key==="Enter")add();}} style={{...inp,minWidth:0,height:44,borderRadius:12}}/>
        {showGroup&&gl.length>0&&<select value={newGroup} onChange={function(e){setNewGroup(e.target.value);}} style={{...inp,minWidth:0,height:44,borderRadius:12}}>{gl.map(function(g2){return <option key={g2.id} value={g2.id}>{g2.name}</option>;})}</select>}
        <button onClick={add} style={{background:"linear-gradient(135deg,#7F77DD,#378ADD)",color:"#fff",border:"none",borderRadius:14,padding:"0 18px",height:44,cursor:"pointer",fontSize:15,fontWeight:900,boxShadow:dark?"none":"0 6px 18px rgba(55,138,221,0.25)",whiteSpace:"nowrap",width:isMobile?"100%":"auto"}}>+ Aggiungi</button>
      </div>
    </div>
  </div>;
}
export function AreasEditor({onlyPatrimonio}){var ctx=useApp();var expenseGroups=ctx.expenseGroups,setExpenseGroups=ctx.setExpenseGroups,incomeGroups=ctx.incomeGroups,setIncomeGroups=ctx.setIncomeGroups,methodGroups=ctx.methodGroups,setMethodGroups=ctx.setMethodGroups,patrimonioAreas=ctx.patrimonioAreas,setPatrimonioAreas=ctx.setPatrimonioAreas,dark=ctx.dark;var [areaTab,setAreaTab]=useState(onlyPatrimonio?"patrimonio":"expense");var tc=dark?"#eee":"#333";var sc=dark?"#aaa":"#888";var borderC=dark?"#444":"#eee";var cardBg=dark?"#252535":"#fff";var inp={width:"100%",borderRadius:8,border:"1px solid "+borderC,padding:"7px 10px",fontSize:14,background:dark?"#2a2a3e":"#fff",color:tc};
  ...
  function add(){if(!newName.trim())return;var nitem=isPatrimonio?{id:"area_"+Date.now(),name:newName.trim(),color:newColor,icon:newIcon}:{id:"area_"+Date.now(),name:newName.trim(),color:newColor};setCurrent([...current,nitem]);setNewName("");}
  function del(id){setCurrent(current.filter(function(g){return g.id!==id;}));}
  function rename(id,v){setCurrent(current.map(function(g){return g.id===id?{...g,name:v}:g;}));}
  function recolor(id,v){setCurrent(current.map(function(g){return g.id===id?{...g,color:v}:g;}));}
  return <div style={{display:"flex",flexDirection:"column",gap:14}}>
    ...
    {current.map(function(g){return <div key={g.id} style={{background:cardBg,borderRadius:10,border:"1px solid "+borderC,padding:"10px 12px",display:"flex",alignItems:"center",gap:8}}>
      ...
      <button onClick={function(){del(g.id);}} style={{background:"none",border:"none",cursor:"pointer",color:"#ccc",fontSize:16}}>x</button>
    </div>;})}
    <div style={{background:cardBg,borderRadius:10,border:"1px dashed "+(dark?"#555":"#ddd"),padding:"10px 12px"}}>
      <div style={{display:"flex",gap:8,alignItems:"center"}}>
        {isPatrimonio&&<EmojiPicker value={newIcon} onChange={setNewIcon}/>}
        <input type="color" value={newColor} onChange={function(e){setNewColor(e.target.value);}} style={{width:32,height:32,padding:0,border:"none",borderRadius:6,cursor:"pointer"}}/>
        <input placeholder="Nome area" value={newName} onChange={function(e){setNewName(e.target.value);}} onKeyDown={function(e){if(e.key==="Enter")add();}} style={{...inp,flex:1}}/>
        <button onClick={add} style={{background:"#333",color:"#fff",border:"none",borderRadius:8,padding:"6px 12px",cursor:"pointer",fontSize:20,lineHeight:1}}>+</button>
      </div>
    </div>
  </div>;
}
...
export function SortableRows({items,onChange,onMove,renderItem,emptyText}){
  ...
  if(!items||items.length===0)return <div style={{fontSize:13,color:sc,textAlign:"center",padding:"16px 0"}}>{emptyText||"Nessuna voce"}</div>;
...
export function PatrimonioSettingsPanel({forcedSection}={}){
  var ctx=useApp();
  ...
  function LocalSegmented({items,value,onChange}){return <div style={{display:"flex",gap:0,background:dark?"#252535":"#f5f5f5",borderRadius:12,padding:3,marginBottom:12}}>{items.map(function(it){var active=value===it.id;return <button type="button" key={it.id} onClick={function(){onChange(it.id);}} style={{flex:1,padding:"9px 10px",border:"none",borderRadius:10,background:active?"linear-gradient(135deg,#7F77DD,#378ADD)":"transparent",color:active?"#fff":sc,fontSize:13,cursor:"pointer",fontWeight:active?800:400,boxShadow:active?"0 3px 10px rgba(127,119,221,0.25)":"none"}}>{it.label}</button>;})}</div>;}
  function sectionTitle(icon,title,desc){return <div style={{marginBottom:12}}><div style={{fontSize:14,fontWeight:700,color:tc,marginBottom:3}}>{icon} {title}</div><div style={{fontSize:12,color:sc,lineHeight:1.4}}>{desc}</div></div>;}
  ...
  function setEntryItems(next){setPatrimonioEntries((next||[]).map(function(e){return {...e,areaId:e.group||e.areaId||(areas[0]?areas[0].id:""),group:undefined};}));}
  function moveAreas(i,dir){var j=i+dir;if(j<0||j>=areas.length)return;var arr=areas.slice();var tmp=arr[i];arr[i]=arr[j];arr[j]=tmp;setPatrimonioAreas(arr);}
  function moveEntries(i,dir){var arr=normalizedEntries();var j=i+dir;if(j<0||j>=arr.length)return;var tmp=arr[i];arr[i]=arr[j];arr[j]=tmp;setEntryItems(arr);}
  return <div style={{display:"flex",flexDirection:"column",gap:16}}>
    {(!forcedSection||forcedSection==="areas")&&<div style={{background:cardBg,borderRadius:14,border:"1px solid "+borderC,padding:20}}>{sectionTitle("📂","Aree","Stessa interfaccia delle altre liste: modifica, archivia, cancella e riordina.")}<LocalSegmented items={[{id:"list",label:"Lista"},{id:"order",label:"Riordina"}]} value={areaView} onChange={setAreaView}/>{areaView==="list"&&<SettingsList items={areas} setItems={setPatrimonioAreas} label="Aggiungi area patrimonio" showIcon/>}{areaView==="order"&&<SortableRows items={areas} onMove={moveAreas} renderItem={function(a){return <div style={{display:"flex",alignItems:"center",gap:10,minWidth:0}}><span style={{fontSize:18}}>{a.icon||"📋"}</span><div style={{width:10,height:10,borderRadius:"50%",background:a.color||COLORS[0],flexShrink:0}}/><span style={{fontSize:13,color:tc,fontWeight:600,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{a.name}</span>{a.archived&&<span style={{fontSize:10,background:dark?"#333":"#f0f0f0",color:sc,borderRadius:10,padding:"1px 6px"}}>Archiviata</span>}</div>;}}/>}</div>}
    {(!forcedSection||forcedSection==="entries")&&<div style={{background:cardBg,borderRadius:14,border:"1px solid "+borderC,padding:20}}>{sectionTitle("💎","Voci","Stessa interfaccia delle altre liste: modifica, archivia, cancella, riordina e associazione area.")}<LocalSegmented items={[{id:"list",label:"Lista"},{id:"order",label:"Riordina"}]} value={entryView} onChange={setEntryView}/>{entryView==="list"&&<SettingsList items={normalizedEntries()} setItems={setEntryItems} label="Aggiungi voce patrimonio" showIcon showGroup groupList={areas}/>} {entryView==="order"&&<SortableRows items={normalizedEntries()} onMove={moveEntries} renderItem={function(e){return <div style={{display:"flex",alignItems:"center",gap:10,minWidth:0}}><span style={{fontSize:18}}>{e.icon||"📦"}</span><div style={{width:10,height:10,borderRadius:"50%",background:e.color||COLORS[0],flexShrink:0}}/><div style={{flex:1,minWidth:0}}><div style={{fontSize:13,color:tc,fontWeight:600,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{e.name}</div><div style={{fontSize:11,color:sc,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{areaName(e.group||e.areaId)}{e.archived?" · Archiviata":""}</div></div></div>;}}/>}</div>}
    {(!forcedSection||forcedSection==="mode")&&<div style={{background:cardBg,borderRadius:14,border:"1px solid "+borderC,padding:20}}>{sectionTitle("⚙️","Modalità","Come vengono aggiornati i valori del patrimonio.")}<div style={{display:"flex",flexDirection:"column",gap:8}}>{[{id:"manuale",label:"Manuale",desc:"Inserisci i valori manualmente"},{id:"semi",label:"Semi-automatica ⚠️",desc:"Beta: collega ai metodi di pagamento"},{id:"entrambe",label:"Entrambe",desc:"Usa sia il manuale che il semi-auto"}].map(function(m){return <button key={m.id} onClick={function(){setPatrimonioMode(m.id);}} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 16px",border:"2px solid "+(patrimonioMode===m.id?"#378ADD":borderC),borderRadius:12,background:patrimonioMode===m.id?(dark?"#1a2535":"#e8f4ff"):softBg,cursor:"pointer",textAlign:"left"}}><div style={{width:18,height:18,borderRadius:"50%",border:"2px solid "+(patrimonioMode===m.id?"#378ADD":"#ccc"),background:patrimonioMode===m.id?"#378ADD":"transparent",flexShrink:0}}/><div><div style={{fontSize:13,fontWeight:600,color:tc}}>{m.label}</div><div style={{fontSize:11,color:sc}}>{m.desc}</div></div></button>;})}</div>{patrimonioMode!=="manuale"&&<div style={{marginTop:12,background:"#FFF8E1",border:"1px solid #FFD54F",borderRadius:8,padding:"10px 12px"}}><span style={{fontSize:12,color:"#856404"}}>⚠️ La modalità semi-automatica è in beta. I valori potrebbero non essere precisi.</span></div>}</div>}
  </div>;
}

export function SortOrderPanel(){
  var ctx=useApp();
  var cats=ctx.cats,methods=ctx.methods,catOrder=ctx.catOrder,setCatOrder=ctx.setCatOrder,methodOrder=ctx.methodOrder,setMethodOrder=ctx.setMethodOrder,catSortMode=ctx.catSortMode,setCatSortMode=ctx.setCatSortMode,methodSortMode=ctx.methodSortMode,setMethodSortMode=ctx.setMethodSortMode,dark=ctx.dark,expenseGroups=ctx.expenseGroups;
  var tc=dark?"#eee":"#333";var sc=dark?"#aaa":"#888";var borderC=dark?"#444":"#eee";var cardBg=dark?"#252535":"#fff";
...
export function DeleteDataPanel(){
  ...
  var sectionOptions=[
    {id:"expenses",icon:"💸",label:"Uscite",count:expenses.length,clear:function(){setExpenses([]);}},
    {id:"incomes",icon:"💰",label:"Entrate",count:incomes.length,clear:function(){setIncomes([]);}},
    {id:"recurring",icon:"🔁",label:"Ricorrenti",count:recurring.length,clear:function(){setRecurring([]);}},
    ...
    {id:"patrimonio",icon:"💎",label:"Patrimonio",count:Object.keys(patrimonioValues||{}).length+Object.keys(patrimonioHistory||{}).length,clear:function(){setPatrimonioValues({});setPatrimonioHistory({});setPatrimonioNotes({});}},
    {id:"appuntiDocs",icon:"📎",label:"Documenti",count:(appuntiDocuments||[]).length,clear:function(){setAppuntiDocuments([]);}},
    {id:"appuntiNotes",icon:"📝",label:"Appunti",count:(appuntiNotes||[]).length,clear:function(){setAppuntiNotes([]);}},
    {id:"bank",icon:"🏦",label:"Coordinate bancarie",count:(bankCoords||[]).length,clear:function(){setBankCoords([]);}},
    {id:"categories",icon:"🏷",label:"Categorie uscite",count:cats.length,clear:function(){setCats(DEFAULT_CATS);setExpenseGroups(DEFAULT_EXPENSE_GROUPS);}},
    {id:"methods",icon:"💳",label:"Metodi pagamento",count:methods.length,clear:function(){setMethods(DEFAULT_METHODS);setMethodGroups(DEFAULT_METHOD_GROUPS);}},
    {id:"patrimonioConfig",icon:"⚙️",label:"Config. patrimonio",count:(patrimonioAreas||[]).length+(patrimonioEntries||[]).length,clear:function(){setPatrimonioAreas(DEFAULT_PATRIMONIO_AREAS);setPatrimonioEntries(DEFAULT_PATRIMONIO_ENTRIES);}},
  ];
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:14}}>
    <div style={{background:dark?"#1e1e30":"#f9f9f9",borderRadius:12,border:"1px solid "+borderC,padding:"10px 14px",fontSize:13,color:sc}}>Elimina i dati per singola sezione.</div>
    ...
    {tab==="movements"&&<div style={{display:"flex",flexDirection:"column",gap:12}}>
      <div style={{display:"flex",gap:0,background:dark?"#252535":"#f5f5f5",borderRadius:12,padding:3}}><button onClick={function(){setMovType("expense");setSelIds([]);}} style={{flex:1,padding:"8px",border:"none",borderRadius:10,background:movType==="expense"?"#E24B4A":"transparent",color:movType==="expense"?"#fff":sc,fontSize:13,cursor:"pointer"}}>Uscite</button><button onClick={function(){setMovType("income");setSelIds([]);}} style={{flex:1,padding:"8px",border:"none",borderRadius:10,background:movType==="income"?"#1D9E75":"transparent",color:movType==="income"?"#fff":sc,fontSize:13,cursor:"pointer"}}>Entrate</button></div>
...
export function ImportData(){
  ...
  function buildPreview(){
    ...
    var p=rows.map(function(r){
      var rawA=r[ai]||"";
      var amt=typeof rawA==="number"?rawA:parseFloat(String(rawA).replace(/[€$£\s]/g,"").replace(",","."));
      var rawD=r[di];
      ...
      if(importType==="patrimonio"){
        ...
        var rawAmt=r[ai]||"";var cleanAmt=typeof rawAmt==="number"?rawAmt:parseFloat(String(rawAmt).replace(/[€$£\s]/g,"").replace(/\./g,"").replace(",","."));
  ...
  function doImport(){
    if(importType==="expense")addExpenses(preview.map(function(p){return{id:Date.now()+Math.random(),amount:p.amount,catId:p.catId,methodId:p.methodId,desc:p.desc,date:p.date,rateizzato:p.rateizzato,rate:p.rate};}));
    else if(importType==="income")addIncomes(preview.map(function(p){return{id:Date.now()+Math.random(),amount:p.amount,type:p.itype,desc:p.desc,date:p.date,rateizzato:p.rateizzato,rate:p.rate};}));
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:14}}>
    ...
    {importSuccess&&<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:500,display:"flex",alignItems:"center",justifyContent:"center",padding:16}} onClick={function(){setImportSuccess(null);}}>
      <div style={{background:dark?"#1e1e30":"#fff",borderRadius:20,padding:32,textAlign:"center",maxWidth:320,width:"100%",boxShadow:"0 8px 40px rgba(0,0,0,0.2)"}}>
        ...
        <div style={{fontSize:20,fontWeight:700,color:dark?"#eee":"#333",marginBottom:8}}>Importazione completata!</div>
        <div style={{fontSize:15,color:dark?"#aaa":"#666",marginBottom:6}}>
          ...
          {importSuccess.type} importate con successo
        </div>
    ...
    <div style={{display:"flex",gap:0,background:dark?"#333":"#f5f5f5",borderRadius:12,padding:3}}>
      <button onClick={function(){setImportType("expense");resetAll();}} style={{flex:1,padding:"9px",border:"none",borderRadius:10,background:importType==="expense"?"#E24B4A":"transparent",color:importType==="expense"?"#fff":(dark?"#aaa":"#888"),fontSize:14,cursor:"pointer",fontWeight:importType==="expense"?500:400}}>💸 Uscite</button>
      <button onClick={function(){setImportType("income");resetAll();}} style={{flex:1,padding:"9px",border:"none",borderRadius:10,background:importType==="income"?"#1D9E75":"transparent",color:importType==="income"?"#fff":(dark?"#aaa":"#888"),fontSize:14,cursor:"pointer",fontWeight:importType==="income"?500:400}}>💰 Entrate</button>
      <button onClick={function(){setImportType("patrimonio");resetAll();}} style={{flex:1,padding:"9px",border:"none",borderRadius:10,background:importType==="patrimonio"?"#7F77DD":"transparent",color:importType==="patrimonio"?"#fff":(dark?"#aaa":"#888"),fontSize:14,cursor:"pointer",fontWeight:importType==="patrimonio"?500:400}}>💎 Patrimonio</button>
    </div>
    ...
    {step===0&&<div style={{background:importType==="patrimonio"?(dark?"#252535":"#f0edff"):importType==="expense"?(dark?"#2a1e1e":"#fff5f5"):(dark?"#1e2a1e":"#f0faf5"),borderRadius:10,border:"1px solid "+(importType==="patrimonio"?(dark?"#44408a":"#c8c0f8"):importType==="expense"?(dark?"#5a2a2a":"#fcc"):( dark?"#2a5a2a":"#a8e6c8")),padding:"12px 16px"}}>
      <div style={{fontSize:12,fontWeight:700,color:importType==="patrimonio"?"#534AB7":importType==="expense"?"#E24B4A":"#1D9E75",marginBottom:6}}>{IMPORT_GUIDELINES[importType].title}</div>
      <div style={{fontSize:12,color:dark?"#aaa":"#555",lineHeight:1.7,whiteSpace:"pre-line"}}>{IMPORT_GUIDELINES[importType].body}</div>
    </div>}
    ...
    {step===1&&<div style={{background:dark?"#252535":"#f9f9f9",borderRadius:12,border:"1px solid "+(dark?"#444":"#eee"),padding:16}}>
      ...
      {importType!=="patrimonio"&&<div style={{marginBottom:14,background:dark?"#1e1e30":"#f0f8ff",borderRadius:10,padding:"12px 14px",border:"1px solid "+(dark?"#334":"#c8e0ff")}}>
        ...
        <select value={importDateFmt} onChange={function(e){setImportDateFmt(e.target.value);}} style={{...sinp,width:"100%"}}>
          ...
          <option value="ymd">AAAA-MM-GG (ISO)</option>
      ...
      <div style={{display:"flex",gap:8}}><Btn onClick={buildPreview}>Anteprima</Btn><Btn onClick={function(){setStep(0);}} bg={dark?"#333":"#f0f0f0"} color={dark?"#eee":"#666"}>Indietro</Btn></div>
    </div>}
    {step===2&&<div>
      ...
      <div style={{display:"flex",gap:8,marginTop:12}}><Btn onClick={doImport} bg={tabColor} style={{padding:"10px 20px",fontSize:14,fontWeight:500}}>Importa {preview.length} voci</Btn><Btn onClick={function(){setStep(1);}} bg={dark?"#333":"#f0f0f0"} color={dark?"#eee":"#666"}>Indietro</Btn></div>
    </div>}
  </div>;
}
