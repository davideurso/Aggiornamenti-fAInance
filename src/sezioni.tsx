...
import { averageBudgetForPeriods, plannedSavings } from './finance/budgetEngine';
...
export function HomePanel() {
  var ctx: any = useApp();
  const accountingPeriod = ctx.accountingPeriod;
  ...
  var HOME_LIBRARY = [
    ...
    {
      type: "monthlyized_expenses",
      icon: "📆",
      label: H("Uscite mensilizzate"),
      desc: H("Media mensile delle uscite nel periodo selezionato"),
    },
    {
      type: "monthlyized_expenses_by_area",
      icon: "🗂️",
      label: H("Uscite mensilizzate per Area"),
      desc: H("Media mensile delle uscite raggruppata per area"),
    },
    {
      type: "monthlyized_expenses_by_category",
      icon: "🏷️",
      label: H("Uscite mensilizzate per Categoria"),
      desc: H("Media mensile delle uscite raggruppata per categoria"),
    },
    {
  ...
  var HOME_DEFAULT_WORKLETS = [
    ...
    },
    {
      ...
      size: "1x",
      color: "#FFFFFF",
      params: { showTitle: false, showDecimals: true },
    },
    {
  ...
  function lastMonthKeys(n) {
    return accountingPeriod.lastKeys(Math.max(1, Number(n) || 1));
  }
  function homeAmountForMonth(item, key) { return accountingPeriod.total([item],key,homeBalanceView === 'reale' ? 'reale' : 'rateizzato'); }
  ...
  function totalInKeys(list, keys) {
    return keys.reduce(function (sum, key) {
      return sum + accountingPeriod.total(list, key, homeBalanceView === 'reale' ? 'reale' : 'rateizzato');
    }, 0);
  }
  function sumBy(list, keys, getKey, getLabel, getColor) {
    var map: any = {};
    (list || []).forEach(function (e) {
      var val = totalInKeys([e], keys);
      if (!val) return;
  ...
  function homeOtherExpenseCategory() {
    var list = Array.isArray(cats) ? cats : [];
    return (
      list.find(function (c) {
        return c && !c.deleted && !c.archived && String(c.id) === "17";
      }) ||
      list.find(function (c) {
        return (
          c &&
          !c.deleted &&
          !c.archived &&
          String(c.name || "").trim().toLocaleLowerCase() === "altro"
        );
      }) ||
      null
    );
  }
  function homeCategoryForExpense(e) {
    var cat = getCat
      ? getCat(e && e.catId)
      : (cats || []).find(function (c) {
          return String(c.id) === String(e && e.catId);
        });
    return cat || homeOtherExpenseCategory();
  }
  ...
  function workletColor(w) {
    ...
    var type = String((w && w.type) || "");
    var h = raw.replace("#", "");
    if (h.length === 3) h = h[0] + h[0] + h[1] + h[1] + h[2] + h[2];
    if (!dark) {
      if ((type === "monthlyized_expenses_by_area" || type === "monthlyized_expenses_by_category") && /^[0-9a-fA-F]{6}$/.test(h)) {
        var lr = parseInt(h.slice(0, 2), 16),
          lg = parseInt(h.slice(2, 4), 16),
          lb = parseInt(h.slice(4, 6), 16);
        var ll = (lr * 299 + lg * 587 + lb * 114) / 1000;
        if (ll < 140) return "#FFFFFF";
      }
      return raw;
    }
    var h = raw.replace("#", "");
    ...
    return (
      {
        ...
        summary: { showTitle: false, showDecimals: true },
        distribution_expenses: { range: 6, valueMode: "percent" },
        ...
        expenses_by_area: { range: 12, valueMode: "percent" },
        expenses_by_category: { range: 12, valueMode: "percent" },
        monthlyized_expenses: { range: 12 },
        monthlyized_expenses_by_area: { range: 12 },
        monthlyized_expenses_by_category: { range: 12 },
  ...
  function multiInstanceWorkletType(type) {
    return [
      "fidelity_card",
      "shopping_list_widget",
      "note_widget",
  ...
  function normalizeWorklets(list, allowDefault) {
    ...
    return arr
      ...
      .filter(function (w) {
        if (!w || !WORKLET_LABELS[w.type]) return false;
        if (multiInstanceWorkletType(w.type)) return true;
  ...
  function sanitizeWorkletParams(type, params) {
    ...
    if (["distribution_expenses", "expenses_by_area", "expenses_by_category"].indexOf(type) >= 0) {
      p.valueMode = p.valueMode === "amount" ? "amount" : "percent";
    }
    return p;
  }
  ...
  function wholeAmount(value) {
    ...
    try {
      return String(sym || "") + "\u00A0" + new Intl.NumberFormat(lang || "it", {
        maximumFractionDigits: 0,
        minimumFractionDigits: 0,
      }).format(n);
    } catch (e) {
      return String(sym || "") + " " + String(n);
    }
  }
  function isOtherLabelName(value) {
    var v = String(value || "")
      .toLowerCase()
      .trim();
    return ["altro", "other", "autre", "otro", "outro", "andere", "inny", "altul", "άλλο"].indexOf(v) >= 0;
  }
  function plainOrCatalogLabel(item, fallback, name) {
    var labelName = String(name || "").trim() || L("Altro");
    if (isOtherLabelName(labelName)) return labelName;
    return <CatalogLabel item={item} fallback={fallback || "🏷️"} name={labelName} />;
  }
  function legendChart(data, compact, doubleSize, valueMode) {
    var total = (data || []).reduce(function (a, d) {
      return a + safeNum(d.value);
    }, 0);
    if (!total) return emptyBox("Nessun dato disponibile");
    var mode = valueMode === "percent" ? "percent" : "amount";
    function legendValue(d) {
      if (mode === "percent") {
        return String(Math.round((safeNum(d.value) / total) * 100)) + "%";
      }
      return wholeAmount(d.value);
    }
    if (compact) {
      return (
        <div
  ...
  function budgetNumbers(range) {
    ...
    var periodPlan = averageBudgetForPeriods(ctx.budgetForPeriod, keys);
    var items =
      periodPlan && Array.isArray(periodPlan.items) ? periodPlan.items : [];
    var plannedMonthly = items.reduce(function (a, i) {
      return a + safeNum(i.amount);
    }, 0);
    var incomeMonthly = safeNum(
      periodPlan && periodPlan.income
    );
  ...
  function shareBalanceDetails(p) {
    if (!p) return { balance: 0, monthTotal: 0, relations: [] };
    var participants = Array.isArray(p.participants) ? p.participants : [];
    var currentEmail = String(
      (ctx.currentUser && ctx.currentUser.email) || ""
    ).trim().toLowerCase();
    var currentUid = String(ctx.userId || "");
    var me = participants.find(function (x) {
      return currentUid && String(x.uid || "") === currentUid;
    }) || participants.find(function (x) {
      return currentEmail && String(x.email || "").trim().toLowerCase() === currentEmail;
    }) || participants.find(function (x) {
      return String(x.id || "") === "me";
    });
    var meId = String((me && me.id) || "me");
    var balances: any = {};
    participants.forEach(function (x) {
      balances[String(x.id)] = 0;
    });
    (p.activities || []).forEach(function (a) {
      var amount = safeNum(a.amount);
      if (a.kind === "settlement") {
        var from = String(a.from || "");
        var to = String(a.to || "");
        balances[from] = safeNum(balances[from]) + amount;
        balances[to] = safeNum(balances[to]) - amount;
        return;
      }
      var paidBy = String(a.paidBy || "me");
      balances[paidBy] = safeNum(balances[paidBy]) + amount;
      Object.keys(a.shares || {}).forEach(function (pid) {
        balances[String(pid)] = safeNum(balances[String(pid)]) - safeNum(a.shares[pid]);
      });
    });
    var debtors: any[] = [];
    var creditors: any[] = [];
    Object.keys(balances).forEach(function (id) {
      var value = Math.round(safeNum(balances[id]) * 100) / 100;
      if (value < -0.009) debtors.push({ id: id, amount: -value });
      if (value > 0.009) creditors.push({ id: id, amount: value });
    });
    var debts: any[] = [];
    debtors.forEach(function (d) {
      creditors.forEach(function (c) {
        if (d.amount <= 0 || c.amount <= 0) return;
        var value = Math.min(d.amount, c.amount);
        debts.push({ from: d.id, to: c.id, amount: Math.round(value * 100) / 100 });
        d.amount -= value;
        c.amount -= value;
      });
    });
    function participantName(id) {
      var x = participants.find(function (row) {
        return String(row.id) === String(id);
      });
      if (String(id) === meId) return L("Tu");
      return (x && (x.name || x.email)) || L("Partecipante");
    }
    var relations = debts
      .filter(function (d) {
        return String(d.from) === meId || String(d.to) === meId;
      })
      .map(function (d) {
        var owe = String(d.from) === meId;
        return {
          ...d,
          direction: owe ? "owe" : "owed",
          person: participantName(owe ? d.to : d.from),
          text: owe
            ? L("Devi a") + " " + participantName(d.to)
            : participantName(d.from) + " " + L("ti deve"),
        };
      });
    var sharePeriod = currentBalancePeriod || balancePeriodRangeForKey(
      String(balancePeriodKey || curMonthKey || todayStr().slice(0, 7)),
      balancePeriodMode,
      financialMonthStartDay
    );
    var monthTotal = (p.activities || [])
      .filter(function (a) {
        if (a.kind === "settlement") return false;
        var date = String(a.date || "").slice(0, 10);
        return !!date && date >= String(sharePeriod.startIso || "") && date <= String(sharePeriod.endIso || "");
      })
      .reduce(function (sum, a) {
        return sum + safeNum(a.amount);
      }, 0);
    return {
      balance: Math.round(safeNum(balances[meId]) * 100) / 100,
  ...
  function renderWorklet(w) {
    ...
    if (w.type === "assistant_voice_widget") {
      ...
      var avSize = String(w.size || "1x1"),
        avOne = avSize === "1x1",
        avNarrowTall = avSize === "1x2",
        avTall = avNarrowTall || avSize === "2x2",
        avWide = avSize === "2x1" || avSize === "2x2";
      var avIconSize = avOne ? 34 : avNarrowTall ? 48 : avTall ? 68 : avWide ? 52 : 40;
      return (
        <button
          type="button"
          onClick={function () {
          ...
          style={{
            width: "100%",
            height: "100%",
            minHeight: avOne ? 0 : avTall ? 172 : 82,
            border: "none",
            borderRadius: 14,
            background:
            ...
            alignItems: "center",
            justifyContent: "center",
            gap: avOne ? 3 : avWide ? 16 : 8,
            flexDirection: avWide ? "row" : "column",
            padding: avOne ? "4px 3px" : avNarrowTall ? "10px 8px" : avTall ? 14 : 10,
            cursor: "pointer",
        ...
        >
          <div style={{ flexShrink: 0, lineHeight: 0 }}>
            <AIGrilloIcon size={avIconSize} />
          </div>
          <div style={{ minWidth: 0, maxWidth: avWide ? 280 : "100%", width: avWide ? "auto" : "100%" }}>
            {homeShowTitle(w) && (
              <div
                style={{
                  fontSize: avOne ? 10 : avNarrowTall ? 13 : avTall ? 16 : 14,
                  fontWeight: 950,
                  color: textC,
                  overflow: "hidden",
                  textOverflow: "ellipsis",
            ...
            {!avOne && (
              <div
                style={{
                  fontSize: avTall ? 11 : 10,
                  color: subC,
                  lineHeight: 1.3,
                  marginTop: homeShowTitle(w) ? 4 : 0,
                  display: "-webkit-box",
                  WebkitLineClamp: avNarrowTall ? 2 : avTall ? (avWide ? 3 : 2) : 1,
                  WebkitBoxOrient: "vertical",
                  overflow: "hidden",
                }}
              >
                {avNarrowTall ? (av.homeShort || av.sub) : av.sub}
              </div>
            )}
            {!avOne && (
            <div
    ...
    if (w.type === "distribution_expenses") {
      var distributionData = sumBy(
        ...
        function (e) {
          ...
          if (String(g.id) === "altro" && c)
            return plainOrCatalogLabel(c, "🏷️", c && c.name ? c.name : L("Altro"));
          return g.name || L("Altro");
        },
        function (e) {
          var g = groupForExpense(e);
          var c = homeCategoryForExpense(e);
      ...
      return legendChart(distributionData, compact, doubleSize, (w.params && w.params.valueMode) || "percent");
    }
    if (w.type === "expenses_by_area") {
      ...
      return legendChart(data, compact, doubleSize, (w.params && w.params.valueMode) || "percent");
    }
    if (w.type === "expenses_by_category") {
      var data2 = sumBy(
        ...
        function (e) {
          var c = homeCategoryForExpense(e);
          return c ? "cat:" + String(c.id) : "altro";
        },
        function (e) {
          var c = homeCategoryForExpense(e);
          return plainOrCatalogLabel(c, "🏷️", c && c.name ? c.name : L("Altro"));
    ...
    if (w.type === "monthlyized_expenses_by_area") {
      ...
      var monthlyAreaData = sumBy(
        ...
        return { ...row, value: safeNum(row.value) / monthlyAreaDivisor };
      });
      return legendChart(monthlyAreaData, compact, doubleSize, "amount");
    }
    if (w.type === "monthlyized_expenses_by_category") {
      ...
      var monthlyCategoryData = sumBy(
        ...
        return { ...row, value: safeNum(row.value) / monthlyCategoryDivisor };
      });
      return legendChart(monthlyCategoryData, compact, doubleSize, "amount");
    }
    if (w.type === "incomes_by_type") {
      var data3 = sumBy(
        ...
        function (e) {
          ...
          return plainOrCatalogLabel(it, "💰", it && it.name ? it.name : L("Altro"));
        },
        function (e) {
          var it = getIT ? getIT(e.type) : null;
      ...
      return legendChart(data3, compact, doubleSize, "amount");
    }
        ...
        <div style={{ overflow: "hidden", width: "100%" }}>
          <LineChart
            data={monthlyChart(range).map(function (m) {
              return { label: m.label, value: Math.round(safeNum(m.value)) };
            })}
            integer
            width={compact ? 150 : isMobile ? 330 : 520}
            height={compact ? 106 : doubleSize ? 220 : 118}
            color={BALANCE_COLOR}
          />
        </div>
      );
    ...
    if (w.type === "share_balance") {
      ...
      var shareInfo = shareBalanceDetails(sp);
      var bal = shareInfo.balance;
      return (
          ...
          {sp && (
            <>
              <div
                ...
                style={{ fontSize: compact ? 10 : 12, color: subC, marginBottom: compact ? 3 : 5, fontWeight: 800 }}
              >
                {sp.name}
              </div>
              <div
              ...
              >
                {shareInfo.relations.length
                  ? shareInfo.relations.slice(0, compact ? 1 : 2).map(function (r, i) {
                      var owe = r.direction === "owe";
                      return (
                        <div
                          key={i}
                          style={{
                        ...
                        >
                          <div style={{ minWidth: 0, lineHeight: 1.18 }}>
                            <div style={{ fontSize: compact ? 9 : 10, color: subC, fontWeight: 800 }}>
                              {owe ? L("Devi a") : L("Ti deve")}
                            </div>
                            <div
                              style={{
                                fontSize: compact ? 10 : 11,
                  ...
                  : (
                    <div style={{ fontSize: compact ? 10 : 11, color: subC, marginTop: 2 }}>
                      {L("Tutto saldato")}
                    </div>
                  )}
              </div>
              ...
              >
                <span>{L("Totale del mese")}</span>
                <strong style={{ color: textC, whiteSpace: "nowrap", fontSize: compact ? 10 : 11 }}>
                  {fmt(shareInfo.monthTotal)}
                </strong>
              </div>
            </>
          )}
      ...
      if (compact) {
        return (
          ...
          >
            ...
            <div
              role="progressbar"
              aria-label={L("Risparmio pianificato")}
              aria-valuenow={Math.round(savingProgress)}
              aria-valuemin={0}
              aria-valuemax={100}
              style={{
                height: 5,
                borderRadius: 999,
                background: dark ? "rgba(255,255,255,.10)" : "rgba(15,23,42,.08)",
                overflow: "hidden",
                flexShrink: 0,
              }}
            >
              <div
                style={{
                  ...
                  height: "100%",
                  background: realPositive ? "#1D9E75" : "#E24B4A",
                  borderRadius: 999,
                }}
              />
            </div>
        ...
        >
          ...
          >
            <div
              style={{
                ...
                padding: compact ? "6px 7px" : "10px 11px",
                minWidth: 0,
              }}
            >
              <div
          ...
          <div
            style={{
              ...
              border: "1px solid " + savingBorder,
              borderRadius: compact ? 10 : 12,
              padding: compact ? "6px 7px" : "9px 11px",
              minHeight: 0,
            }}
          >
            <div
              ...
              return String(e.catId) === String(it.catId)
                ? a +
                    b2.keys.reduce(function (s, k) {
                      return s + safeNum(homeAmountForMonth(e, k));
                    }, 0)
                : a;
            }, 0);
    ...
    if (w.type === "goal") {
      ...
      var target = safeNum(g.target),
        saved = ctx.goalSavedAmount ? ctx.goalSavedAmount(g) : safeNum(g.saved),
  ...
  function canInsertDuplicateWorklet(type) {
    return multiInstanceWorkletType(type);
  }
  ...
  function availableItems() {
    ...
    return HOME_LIBRARY.filter(function (x) {
      return x.type !== "assistant_voice_widget" && (canInsertDuplicateWorklet(x.type) || !m[x.type]);
    });
  }
  function applyDraft() {
    if (!editDraft) return;
  ...
  function insertAtFromLibrary(type, index, openPopup) {
    if (insertedTypes()[type] && !canInsertDuplicateWorklet(type)) return;
    var w = makeWorklet(type);
  ...
  function rangeBadge(w) {
    ...
    if (
      [
        ...
        "monthlyized_expenses",
        "monthlyized_expenses_by_area",
        "monthlyized_expenses_by_category",
        "incomes_by_type",
        "planned_saving",
        "budget_overview",
        "budget_by_category",
  ...
  function workletCard(w, index, available) {
    ...
    function homeWorkletBaseHeight(type, size, countN) {
      ...
      if (compact) {
        ...
        if (type === "income_vs_expense") return 168;
        if (type === "monthly_balance") return 154;
      ...
      if (type === "income_vs_expense") return 216;
      if (type === "monthly_balance") return 176;
    ...
    var targetH = available
      ...
      ? normalH * 2
      : normalH;
    if (!available && w.type === "monthly_balance" && size === "2x") targetH = 290;
    if (!available && !buttonType && !homeShowTitle(w)) {
      ...
      else if (w.type === "monthlyized_expenses")
        targetH = compact ? 104 : size === "2x" ? 176 : 116;
      else if (
        w.type === "expenses_by_area" ||
        w.type === "expenses_by_category" ||
        w.type === "monthlyized_expenses_by_area" ||
        w.type === "monthlyized_expenses_by_category" ||
        w.type === "incomes_by_type"
      )
        targetH = compact ? 164 : size === "2x" ? 318 : 154;
      else if (w.type === "income_vs_expense")
        targetH = compact ? 158 : size === "2x" ? 350 : 204;
      else if (w.type === "monthly_balance")
        targetH = compact ? 132 : size === "2x" ? 260 : 154;
      ...
      border:
        ...
        (available
          ? assistantType
            ...
            : "#d7d8e4"
          : borderC),
      padding: assistantType ? 6 : buttonType ? (compact ? 8 : 10) : compact ? 10 : 16,
      paddingTop:
  ...
  function popupParams(w) {
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          {supportsValueMode && (
            <Field label="Visualizzazione">
              {seg(
                [
                  { id: "percent", label: "Percentuale" },
                  { id: "amount", label: "Importo" },
                ],
                (w.params && w.params.valueMode) || "percent",
                function (v) {
                  updateDraftParams({ valueMode: v });
                }
              )}
            </Field>
          )}
    ...
    if (type === "summary")
      return (
        <Field label="Visualizza decimali">
          {seg(
            [
              { id: "yes", label: "Sì" },
              { id: "no", label: "No" },
            ],
            w.params && w.params.showDecimals === false ? "no" : "yes",
            function (v) {
              updateDraftParams({ showDecimals: v === "yes" });
            }
          )}
        </Field>
      );
            ...
            {addSubTab === "single" && (
              <ExpenseForm
                ...
                onSave={function (item) {
                  if (addType === "expense") {
                    return addExpenses([item], "manual");
                  } else {
                    return addIncomes([item], "manual");
                  }
                }}
              />
            )}{" "}
            {addSubTab === "bulk" && (
              <BulkEntry
                ...
                onSave={function (items) {
                  if (addType === "expense") {
                    return addExpenses(items, "bulk");
                  } else {
                    return addIncomes(items, "bulk");
                  }
                }}
              />
            )}{" "}
...
export function HistoryPanel() {
  var _c: any = useApp();
  const accountingPeriod = _c.accountingPeriod;
  const rateMonth = (item, key) => accountingPeriod.amount(item, key);
  var {
    lang,
    cats,
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            >
              ...
              {e._share && sharePersonalCategory && (
                <Badge
                  color={sharePersonalCategory.color}
...
export function ConsulenteAIPanel() {
    ...
    function () {
      ...
      var weekendExtra = expenses
        .filter(function (e) {
          return (
            e.date &&
            accountingPeriod.matches(e.date, visibleMonth) &&
            isWeekend(e.date) &&
            isExtra(e)
          );
        })
      ...
      var extraTotal = expenses
        .filter(function (e) {
          return e.date && accountingPeriod.matches(e.date, visibleMonth) && isExtra(e);
        })
        .reduce(function (a, e) {
          return a + (parseFloat(e.amount) || 0);
        }, 0);
      ...
      recurring.forEach(function (r) {
        ...
        var isPresent = (r.rtype === "expense" ? expenses : incomes).some(
          function (x) {
            return (
              x.date &&
              accountingPeriod.matches(x.date, visibleMonth) &&
  ...
  function buildAIContext() {
    ...
    var ctx: any = {
      app: "fAInance",
      accountingPeriod: { ...accountingPeriod.forKey(curMonthKey), ...accountingPeriod.settings },
      language: lang,
      currency: currency,
      month: mk,
          ...
          >
            ...
            >
              ...
              <div>
                ...
                <br />•{" "}
                {L(
                  "con Analisi media: spese con data, importo, categoria e area, senza descrizione o pagamento"
                )}
                <br />•{" "}
                {L(
...
function assistantVoiceUiText(code) {
  var dict: any = {
    it: {
      ...
      homeShort: "Parla con il tuo Agente AI",
      listening: "Sto ascoltando...",
      speak: "Parla",
    ...
    en: {
      ...
      homeShort: "Talk to your AI Agent",
      listening: "Listening...",
      speak: "Speak",
      stop: "Stop",
    ...
    es: {
      ...
      homeShort: "Habla con tu Agente IA",
    ...
    fr: {
      ...
      homeShort: "Parlez à votre Agent IA",
      listening: "J’écoute...",
      speak: "Parler",
      stop: "Interrompre",
    ...
    de: {
      ...
      homeShort: "Sprich mit deinem KI-Agenten",
      listening: "Ich höre zu...",
      speak: "Sprechen",
      stop: "Unterbrechen",
    ...
    pt: {
      ...
      homeShort: "Fale com o seu Agente de IA",
      listening: "A ouvir...",
      speak: "Falar",
      stop: "Interromper",
    ...
    pl: {
      ...
      homeShort: "Porozmawiaj ze swoim Agentem AI",
      listening: "Słucham...",
      speak: "Mów",
      stop: "Przerwij",
    ...
    nl: {
      ...
      homeShort: "Praat met je AI-agent",
      listening: "Ik luister...",
      speak: "Spreken",
      stop: "Onderbreken",
    ...
    ro: {
      ...
      homeShort: "Vorbește cu Agentul tău AI",
      listening: "Ascult...",
      speak: "Vorbește",
      stop: "Întrerupe",
    ...
    el: {
      ...
      homeShort: "Μιλήστε με τον Πράκτορα AI",
      listening: "Ακούω...",
      speak: "Μιλήστε",
      stop: "Διακοπή",
...
function VoiceAssistantModal({ onQuick, embedded }: any) {
  var c: any = useApp();
  const accountingPeriod = c.accountingPeriod;
  const rateMonth = (item, key) => accountingPeriod.amount(item, key);
  var {
    lang,
    dark,
  ...
  function nativePlatformName(){try{var cap:any=(window as any).Capacitor;return cap&&typeof cap.getPlatform==="function"?String(cap.getPlatform()||"").toLowerCase():"web";}catch(e){return "web";}}
  ...
  async function connectRealtime(){
    ...
    try{
      ...
      var mediaPromise=(async function(){try{var supported:any=navigator.mediaDevices.getSupportedConstraints?navigator.mediaDevices.getSupportedConstraints():{};var iosNative=nativePlatformName()==="ios";var audioConstraints:any=iosNative?{echoCancellation:true,noiseSuppression:true,autoGainControl:true}:{echoCancellation:{ideal:true},noiseSuppression:{ideal:true},autoGainControl:{ideal:true},channelCount:{ideal:1},sampleRate:{ideal:48000},sampleSize:{ideal:16},googEchoCancellation:true,googNoiseSuppression:true,googHighpassFilter:true,googTypingNoiseDetection:true,googAutoGainControl:true};if(!iosNative&&supported.voiceIsolation)audioConstraints.voiceIsolation={ideal:true};var acquired=await navigator.mediaDevices.getUserMedia({audio:audioConstraints,video:false});try{acquired.getAudioTracks().forEach(function(track:any){track.enabled=true;try{track.contentHint="speech";}catch(e){}if(!iosNative){try{var extra:any={echoCancellation:true,noiseSuppression:true,autoGainControl:true,channelCount:1};if(supported.voiceIsolation)extra.voiceIsolation=true;Promise.resolve(track.applyConstraints(extra)).catch(function(){});}catch(e){}}});}catch(e){}realtimeStreamRef.current=acquired;return await prepareNoiseFilteredMicrophone(acquired);}catch(mediaError){var mediaName=String((mediaError&&mediaError.name)||"").toLowerCase(),mediaMessage=String((mediaError&&mediaError.message)||"").toLowerCase();if(mediaName.indexOf("notallowed")>=0||mediaName.indexOf("permission")>=0||mediaMessage.indexOf("permission")>=0||mediaMessage.indexOf("denied")>=0)throw new Error(realtimePermissionDeniedMessage());throw mediaError;}})();
      phase="session";
      ...
      phase="webrtc";
      var boundary="----fainanceRealtime"+Date.now().toString(16)+Math.random().toString(16).slice(2);var sdpBody="--"+boundary+"\r\nContent-Disposition: form-data; name=\"sdp\"\r\nContent-Type: application/sdp\r\n\r\n"+(peerData.offer.sdp||"")+"\r\n--"+boundary+"--\r\n";var sdpRes=await fetch("https://api.openai.com/v1/realtime/calls",{method:"POST",headers:{Authorization:"Bearer "+tokenData.value,"Content-Type":"multipart/form-data; boundary="+boundary,Accept:"application/sdp"},signal:controller.signal,body:sdpBody});
    ...
    if (aiDataAccess === "full") {
      ...
      ctx.goals = (goals || []).map(function (g) {
        return {
          ...
          saved: c.goalSavedAmount ? c.goalSavedAmount(g) : Number(g.saved) || 0,
          deadline: g.deadline || "",
      ...
      ctx.shareProjects = (shareProjects || []).slice(0, 40).map(function (x) {
        ...
        return {
          ...
          categories: (x.categories || []).filter(function (c) {
            return c && String(c.status || "active") !== "deleted";
          }).map(function (c) {
  ...
  function activeShareCategories(project){return (project&&project.categories||[]).filter(function(c){return c&&String(c.status||"active")!=="deleted";});}
  function resolveShareCategory(project,idOrName){var active=activeShareCategories(project),requested=String(idOrName||"").trim();if(requested){var byId=active.find(function(c){return String(c.id||"")===requested;});if(byId)return byId;var wanted=normalized(requested),exact=active.filter(function(c){return normalized(c.name)===wanted;});if(exact.length===1)return exact[0];if(exact.length>1)throw new Error("Esistono più categorie Share con questo nome: specifica la categoria esatta.");var fuzzy=active.filter(function(c){var name=normalized(c.name);return name&&(name.indexOf(wanted)>=0||wanted.indexOf(name)>=0);});if(fuzzy.length===1)return fuzzy[0];throw new Error("Categoria Share non trovata o nome ambiguo: "+requested);}if(active.length===1)return active[0];return null;}
  ...
  function assistantExpenseDraft(a){
    var amount=Number(a.amount)||0;
    if(amount<=0)throw new Error("Importo non valido.");
    var cat=findNamed(cats,a.categoryName,defaultExpenseCat),method=findNamed(methods,a.methodName,defaultExpenseMethod);
    if(!cat)throw new Error("Categoria non trovata o ambigua.");
    if(!method)throw new Error("Pagamento non trovato o ambiguo.");
    var defaultMethod=(methods||[]).find(x=>String(x.id)===String(defaultExpenseMethod));
    var isDefault=assistantUsesDefaultPayment(a,defaultMethod,a._ruleOriginRequest??lastVoiceUserRequestRef.current);
    return {id:Date.now()+Math.random(),amount,catId:cat.id,methodId:method.id,methodName:method.name,
      _ruleDefaultFields:[!a.categoryName?"catId":null,isDefault?"methodId":null,!a.description?"desc":null].filter(Boolean),
      desc:a.description||cat.name,date:validISODate(a.date,false),rateizzato:!!a.rateizzato,rate:Math.max(1,Number(a.rate)||1)};
  }
  function actionSummary(a){
    if(a&&a.action==="create_expense"){
      ...
      try {
        var proposed=evaluateAutomaticRules(c.financeEvolution?.rules||[],assistantExpenseDraft(a),'expense','assistant',{plan:currentPlan,cats,methods,incomeTypes}).item;
        expenseMethod=String((methods||[]).find(m=>String(m.id)===String(proposed.methodId))?.name||expenseMethod);
        expenseCat=String((cats||[]).find(cat=>String(cat.id)===String(proposed.catId))?.name||expenseCat);
      } catch { /* Keep incomplete proposals visible for correction. */ }
...
function QuickVoiceEntryModal() {
  ...
  function parseVoiceCommand(txt) {
    ...
    if (type === "income") {
      ...
      return {
        ...
        _ruleDefaultFields:findByVoiceName(incomeTypes,n)?[]:["type"],
    ...
    return {
      ...
      _ruleDefaultFields:[!findByVoiceName(cats,n)?"catId":null,!mentionedMethod?"methodId":null].filter(Boolean),
      catId: c ? c.id : 1,
  ...
  function updateVoiceParsed(patch) {
    setVoiceParsed(function (p) {
      return p ? { ...p, ...patch, _ruleDefaultFields:(p._ruleDefaultFields||[]).filter(k=>!Object.hasOwn(patch,k==='type'?'incomeType':k)) } : p;
    });
  }
          ...
          [
            {
              ...
              _ruleDefaultFields:voiceParsed._ruleDefaultFields||[],
