...
export function HomePanel(){
  ...
  function monthlyChart(n){var keys=lastMonthKeys(n);return keys.map(function(k){var exp=totalInKeys(expenses,[k]);var inc=totalInKeys(incomes,[k]);return {label:monthLabel(k),exp:exp,inc:inc,value:inc-exp};});}
  ...
  function listMovement(items,isExp,compact){var sorted=(items||[]).slice().sort(function(a,b){return String(b.date||"").localeCompare(String(a.date||""));});return <div style={{display:"flex",flexDirection:"column",gap:0,minWidth:0}}>{(!sorted||!sorted.length)&&emptyBox(isExp?"Nessuna spesa":"Nessuna entrata")}{sorted.map(function(e){var c=isExp?(getCat?getCat(e.catId):null):null;var it=!isExp?(getIT?getIT(e.type):null):null;return <div key={e.id} style={{display:"flex",alignItems:"center",gap:compact?5:8,padding:compact?"6px 0":"7px 0",borderBottom:"1px solid "+borderC,minWidth:0}}><span style={{fontSize:compact?14:16,flexShrink:0}}>{isExp?(c?c.icon:"📦"):(it?it.icon:"💰")}</span><div style={{flex:1,minWidth:0}}><div style={{fontSize:compact?12:13,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",color:textC,fontWeight:compact?750:undefined}}>{e.desc||(c?c.name:it?it.name:"")||"-"}</div><div style={{fontSize:compact?10:11,color:subC,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{fmtDate(e.date,dateFmt)}</div></div><div style={{textAlign:"right",flexShrink:0,maxWidth:compact?68:110,overflow:"hidden"}}><div style={{fontSize:compact?12:13,fontWeight:700,color:isExp?expenseColor:incomeColor,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{fmt(safeNum(e.amount))}</div>{!compact&&secRate&&fmtSec&&<div style={{fontSize:10,color:subC}}>{fmtSec(safeNum(e.amount))}</div>}</div></div>;})}</div>;}
  ...
  function workletCard(w,index,available){
    ...
    function homeWorkletBaseHeight(type,size,countN){
      ...
      if(type==="latest_expenses"||type==="latest_incomes")return Math.min(360,72+(countN*42));
    ...
    if(!available&&!buttonType&&!homeShowTitle(w)){
      ...
      else if(w.type==="summary")targetH=compact?198:(size==="2x"?310:166);
...
export function HistoryPanel(){
  ...
  function categoryButtons(items,kind){return <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>{(items||[]).filter(function(c){return c&&c.id&&!c.deleted&&!c.archived;}).map(function(c){var id=String(c.id);var active=isCatActive(kind,id);var col=c.color||confirmC;return <button key={kind+id} onClick={function(){togglePrefixedCat(kind,id);}} style={{padding:"7px 9px",borderRadius:999,border:"1px solid "+(active?col:borderC),background:active?col:"transparent",color:active?"#fff":textC,fontSize:12,fontWeight:850,cursor:"pointer"}}>{active?"✓ ":"○ "}{c.icon||"🏷️"} {L(c.name)}</button>;})}</div>;}
...
var {normalizeEmail,loadShareCollaboration,acceptShareInvite,declineShareInvite,createShareInvite,createShareProject,updateShareProject,deleteShareProject,canAddPlanItem}:any=_c;
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:16}}>
    ...
    {aiConsentPrompt&&<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.58)",zIndex:950,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
      <div style={{width:"100%",maxWidth:560,background:cardBg2,border:"1px solid "+borderC2,borderRadius:22,boxShadow:"0 18px 60px rgba(0,0,0,.34)",padding:18}}>
        ...
        <div style={{display:"flex",gap:10,marginTop:16,flexDirection:isMobile?"column":"row"}}>
          <button onClick={acceptAIExternalConsent} style={{flex:1,border:"none",borderRadius:btnRadius,padding:"12px 14px",background:"linear-gradient(135deg,#7F77DD,#378ADD)",color:"#fff",fontSize:14,fontWeight:900,cursor:"pointer"}}>{L("Accetto e continuo")}</button>
...
function VoiceAssistantModal({onQuick}){
  ...
  function realtimePermissionDeniedMessage(){var m:any={it:"Permesso microfono negato. Apri le impostazioni del dispositivo, seleziona fAInance Test e consenti l’accesso al microfono, poi premi di nuovo Avvia.",en:"Microphone permission denied. Open the device settings, select fAInance Test and allow microphone access, then tap Start again.",es:"Permiso de micrófono denegado. Abre los ajustes del dispositivo, selecciona fAInance Test y permite el acceso al micrófono; después pulsa Iniciar de nuevo.",fr:"Autorisation du microphone refusée. Ouvrez les réglages de l’appareil, sélectionnez fAInance Test et autorisez le microphone, puis appuyez de nouveau sur Démarrer.",de:"Mikrofonzugriff verweigert. Öffne die Geräteeinstellungen, wähle fAInance Test und erlaube den Mikrofonzugriff. Tippe danach erneut auf Start.",pt:"Permissão do microfone negada. Abra as definições do dispositivo, selecione fAInance Test e permita o acesso ao microfone; depois toque novamente em Iniciar.",pl:"Odmówiono dostępu do mikrofonu. Otwórz ustawienia urządzenia, wybierz fAInance Test i zezwól na dostęp do mikrofonu, a następnie ponownie naciśnij Start.",nl:"Microfoontoegang geweigerd. Open de apparaatinstellingen, selecteer fAInance Test en sta microfoontoegang toe. Tik daarna opnieuw op Start.",ro:"Permisiunea pentru microfon a fost refuzată. Deschide setările dispozitivului, selectează fAInance Test și permite accesul la microfon, apoi apasă din nou Pornire.",el:"Η άδεια μικροφώνου απορρίφθηκε. Ανοίξτε τις ρυθμίσεις της συσκευής, επιλέξτε fAInance Test και επιτρέψτε την πρόσβαση στο μικρόφωνο και, στη συνέχεια, πατήστε ξανά Έναρξη."};return m[lang]||m.en;}
  ...
  async function callAssistant(q){
    ...
    try{
      ...
      var headers:any={"Content-Type":"application/json"};if(token)headers.Authorization="Bearer "+token;
      var ctrl=new AbortController();assistantRequestAbortRef.current=ctrl;var timer=setTimeout(function(){ctrl.abort();},70000);
      var requestPayload:any={mode:"assistant",question:q,language:lang||"it",aiDataAccess:aiDataAccess||"summary",financeContext:buildContext(),chatHistory:(aiChat||[]).slice(-16).map(function(m){return{role:m.role,text:m.rawText||m.text};})};
  ...
  return <div style={{position:"fixed",inset:0,zIndex:1000,background:"rgba(0,0,0,.62)",display:"flex",alignItems:isMobile?"stretch":"center",justifyContent:"center",padding:isMobile?0:18}} onClick={close}>
    <div onClick={function(e){e.stopPropagation();}} style={{width:"100%",maxWidth:760,height:isMobile?"100%":"min(820px,94vh)",background:dark?"#171724":"#F8F9FF",border:isMobile?"none":"1px solid "+borderC,borderRadius:isMobile?0:24,boxShadow:"0 22px 70px rgba(0,0,0,.38)",display:"flex",flexDirection:"column",overflow:"hidden",position:"relative"}}>
      ...
      {!aiExternalConsent?<div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",padding:24}}><div style={{maxWidth:560,background:cardBg,border:"1px solid "+borderC,borderRadius:18,padding:20}}><div style={{fontSize:17,fontWeight:950,color:textC,marginBottom:8}}>{V.consentTitle}</div><div style={{fontSize:13,color:subC,lineHeight:1.55,marginBottom:16}}>{V.consentText}</div><button onClick={function(){setAiExternalConsent(true,new Date().toISOString());setVoiceError("");}} style={{width:"100%",border:"none",borderRadius:btnRadius,padding:"12px 14px",background:"linear-gradient(135deg,#7F77DD,#378ADD)",color:"#fff",fontWeight:900,cursor:"pointer"}}>{V.accept}</button></div></div>:<>
      <div style={{margin:"12px 14px 8px",padding:"13px 14px",display:"flex",alignItems:"center",gap:12,border:"2px solid "+statusAccent,borderRadius:15,background:realtimeStatus==="connecting"?(dark?"#17263A":"#EDF6FF"):(realtimeStatus==="connected"?(dark?"#153229":"#ECFFF7"):(dark?"#28233B":"#F3F0FF")),boxShadow:"0 7px 20px "+statusAccent+"26"}}><span style={{width:13,height:13,borderRadius:"50%",background:statusAccent,boxShadow:"0 0 0 6px "+statusAccent+"22",flexShrink:0,animation:realtimeStatus==="connecting"?"pulse 1.2s infinite":"none"}}/><div style={{flex:1,minWidth:0}}><div style={{fontSize:15,fontWeight:950,color:textC,lineHeight:1.25}}>{statusTitle}</div><div style={{fontSize:12,color:subC,lineHeight:1.4,marginTop:3,fontWeight:realtimeStatus==="connecting"?750:500}}>{statusSub}</div></div>{realtimeStatus!=="connected"&&realtimeStatus!=="connecting"&&<button onClick={connectRealtime} style={{border:"none",background:"linear-gradient(135deg,#7F77DD,#378ADD)",color:"#fff",borderRadius:10,padding:"9px 12px",fontSize:12,fontWeight:900,cursor:"pointer"}}><span style={{display:"inline-flex",verticalAlign:"middle",marginRight:5}}><FinanceMicIcon size={17}/></span>Avvia</button>}</div>
      ...
      <div style={{padding:"10px 14px 12px",background:dark?"#202033":"#fff",display:"grid",gridTemplateColumns:"auto auto minmax(0,1fr) auto",gap:9,alignItems:"end",borderTop:"1px solid "+borderC}}><button onClick={realtimeStatus==="connected"?toggleRealtimeMicrophone:connectRealtime} disabled={realtimeStatus==="connecting"} title={realtimeStatus==="connected"?(realtimeMicEnabled?"Disattiva microfono":"Attiva microfono"):"Avvia conversazione"} style={{width:48,height:48,borderRadius:"50%",border:"none",background:realtimeStatus==="connected"?(realtimeMicEnabled?(voiceListening?"#EF9F27":"#1D9E75"):"#777"):(realtimeStatus==="connecting"?"#A8A8A8":"linear-gradient(135deg,#7F77DD,#378ADD)"),color:"#fff",fontSize:20,fontWeight:900,cursor:realtimeStatus==="connecting"?"wait":"pointer",boxShadow:"0 5px 16px rgba(127,119,221,.3)"}}><FinanceMicIcon size={25} muted={realtimeStatus==="connected"&&!realtimeMicEnabled} active={realtimeStatus==="connected"&&realtimeMicEnabled&&voiceListening}/></button><button onClick={function(){setAttachmentMenuOpen(function(v){return !v;});}} disabled={documentLoading||busy} title="Aggiungi allegato" style={{width:48,height:48,borderRadius:"50%",border:"1px solid "+borderC,background:dark?"#2A2A3E":"#F5F6FB",color:textC,fontSize:27,fontWeight:500,lineHeight:1,cursor:documentLoading||busy?"wait":"pointer"}}>+</button><textarea value={input} onChange={function(e){setInput(e.target.value);}} onKeyDown={function(e){if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();sendMessage();}}} placeholder={realtimeStatus==="connected"?"Parla liberamente oppure scrivi qui…":V.placeholder} style={{...sinp,minHeight:48,maxHeight:110,resize:"none"}}/><button onClick={function(){sendMessage();}} disabled={!input.trim()||busy||aiLoading||realtimeStatus==="connecting"} style={{height:48,border:"none",borderRadius:12,padding:"0 15px",background:input.trim()&&!busy?"#7F77DD":"#A8A8A8",color:"#fff",fontWeight:900,cursor:input.trim()&&!busy?"pointer":"not-allowed"}}>{V.send}</button></div>
      </>}
    </div>
  </div>;
}
