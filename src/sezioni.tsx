...
import { useApp, useStorage, MONTHS_FULL, MONTHS_SHORT, BALANCE_COLOR, COLORS,
  ...
  getAllIncomeTypes, rateMonth, fmtDate, fmtAmt, todayStr, dateOffset,
  androidDownload, fbAuth, fbDb, doc, setDoc, getDoc, getDocs, addDoc,
  collection, query, where, AI_AGENT_ENDPOINT, AI_OUT_OF_SCOPE_MESSAGE, AI_AGENT_SCOPE_INSTRUCTION,
  ...
  GOAL_ICONS, EMOJI_LIST, BG_THEMES
} from './core';
...
export function HomePanel(){
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:16}}>
    ...
    {pendingCount>0&&<div onClick={function(){setTab("spese");setSpeseSubTab("recurring");}} style={{background:"#FFF8E1",border:"1px solid #FFD54F",borderRadius:12,padding:"12px 16px",cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center"}}><span style={{fontSize:13,color:"#856404"}}>🔄 {pendingCount} ricorrenti da confermare per {MONTHS_FULL[now.getMonth()]}</span><span style={{color:"#856404"}}>›</span></div>}
    {alertTriggered>0&&<div onClick={function(){setAlertPopup(triggeredAlertsData);}} style={{background:"#fff0f0",border:"1px solid #fcc",borderRadius:12,padding:"12px 16px",cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center"}}><span style={{fontSize:13,color:expenseColor}}>🔔 {alertTriggered} alert di spesa superati</span><span style={{color:expenseColor}}>›</span></div>}
    {(() => {
      ...
      var quickCards=[
        {show:incomes.length===0,icon:"💰",title:"Registra la tua prima entrata",tab:"spese",type:"income"},
        {show:expenses.length===0,icon:"💸",title:"Aggiungi la tua prima uscita",tab:"spese",type:"expense"},
        {show:!budgetConfigured,icon:"📊",title:"Imposta il tuo budget mensile",tab:"budget"},
        {show:!goalsConfigured,icon:"🎯",title:"Configura i tuoi obiettivi",tab:"goals"}
    ...
    <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr 1fr":"repeat(4,1fr)",gap:12}}>
      <StatCard title="Uscite mese" value={fmt(curMonthExp)} color={expenseColor} bg={expenseColor+"22"} sub={fmtSec(curMonthExp)&&(secRateLoading?"...":fmtSec(curMonthExp))||undefined}/>
      <StatCard title="Entrate mese" value={fmt(curMonthInc)} color={incomeColor} bg={incomeColor+"22"} sub={fmtSec(curMonthInc)&&(secRateLoading?"...":fmtSec(curMonthInc))||undefined}/>
      <StatCard title="Saldo mese" value={fmt(curMonthInc-curMonthExp)} color="#378ADD" bg="#e8f4ff" sub={secBal&&(secRateLoading?"...":secBal)||undefined}/>
      <StatCard title="Saldo ultimi 12 mesi" value={fmt(last12Balance)} color={BALANCE_COLOR} bg="#e8f4ff" sub={fmtSec(last12Balance)&&(secRateLoading?"...":fmtSec(last12Balance))||undefined}/>
    </div>
    ...
    <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 1fr",gap:14}}>
      <div style={{background:cardBg,borderRadius:14,border:"1px solid "+borderC,padding:16}}><div style={{fontSize:12,color:subC,marginBottom:10}}>Distribuzione uscite — {MONTHS_FULL[now.getMonth()]}</div>{donutData.length===0?<div style={{fontSize:13,color:"#ccc",textAlign:"center",padding:"20px 0"}}>Nessuna spesa</div>:<div style={{display:"flex",gap:14,alignItems:"center"}}><DonutChart data={donutData} size={100}/><div style={{flex:1}}>{donutData.map(function(d,i){return <div key={i} style={{display:"flex",alignItems:"center",gap:6,marginBottom:4}}><div style={{width:8,height:8,borderRadius:"50%",background:d.color,flexShrink:0}}/><span style={{fontSize:11,flex:1,color:subC}}>{d.label}</span><span style={{fontSize:11,fontWeight:500,color:textC}}>{fmt(d.value)}</span></div>;})}</div></div>}</div>
      <div style={{background:cardBg,borderRadius:14,border:"1px solid "+borderC,padding:16}}><div style={{fontSize:12,color:subC,marginBottom:6}}>Entrate vs Uscite {curYear}</div><BarChart data={monthlyTotals} width={260} height={120}/></div>
    </div>
    <div style={{background:cardBg,borderRadius:14,border:"1px solid "+borderC,padding:16}}><div style={{fontSize:12,color:subC,marginBottom:6}}>Saldo mensile {curYear}</div><LineChart data={monthlyTotals} width={isMobile?300:560} height={100} color={BALANCE_COLOR}/></div>
    <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 1fr",gap:14}}>{[{title:"Ultime uscite",items:expenses.slice(0,5),isExp:true},{title:"Ultime entrate",items:incomes.slice(0,5),isExp:false}].map(function(pair){return <div key={pair.title} style={{background:cardBg,borderRadius:14,border:"1px solid "+borderC,padding:16}}><div style={{fontSize:12,color:subC,marginBottom:10}}>{pair.title}</div>{pair.items.length===0&&<div style={{fontSize:13,color:"#ccc",textAlign:"center",padding:"16px 0"}}>{pair.isExp?t.noExpenses:t.noIncomes}</div>}{pair.items.map(function(e){var c=pair.isExp?getCat(e.catId):null;var it=!pair.isExp?getIT(e.type):null;return <div key={e.id} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 0",borderBottom:"1px solid "+borderC}}><span style={{fontSize:16,flexShrink:0}}>{pair.isExp?(c?c.icon:"📦"):(it?it.icon:"💰")}</span><div style={{flex:1,minWidth:0}}><div style={{fontSize:13,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",color:textC}}>{e.desc||(c?c.name:it?it.name:"")||"-"}</div><div style={{fontSize:11,color:subC}}>{fmtDate(e.date,dateFmt)}</div></div><div style={{textAlign:"right"}}><div style={{fontSize:13,fontWeight:500,color:pair.isExp?expenseColor:incomeColor}}>{fmt(e.amount)}</div>{secRate&&<div style={{fontSize:10,color:subC}}>{fmtSec(e.amount)}</div>}</div></div>;})}</div>;})}</div>
  </div>;
}
export function SpesePanel(){
  ...
  return <div><div style={{display:"flex",gap:0,marginBottom:14,background:dark?"#252535":"#f5f5f5",borderRadius:12,padding:3}}><button onClick={function(){setSpeseSubTab("add");}} style={{flex:1,padding:"9px",border:"none",borderRadius:10,background:speseSubTab==="add"?(dark?"#444":"#fff"):"transparent",color:speseSubTab==="add"?textC:subC,fontSize:14,cursor:"pointer",fontWeight:speseSubTab==="add"?500:400}}>+ Aggiungi</button><button onClick={function(){setSpeseSubTab("recurring");}} style={{flex:1,padding:"9px",border:"none",borderRadius:10,background:speseSubTab==="recurring"?(dark?"#444":"#fff"):"transparent",color:speseSubTab==="recurring"?textC:subC,fontSize:14,cursor:"pointer",fontWeight:speseSubTab==="recurring"?500:400,position:"relative"}}>Ricorrenti{pendingCount>0&&<span style={{position:"absolute",top:4,right:10,background:expenseColor,color:"#fff",borderRadius:"50%",width:16,height:16,fontSize:10,display:"inline-flex",alignItems:"center",justifyContent:"center"}}>{pendingCount}</span>}</button></div>{speseSubTab==="add"&&<div><div style={{display:"flex",gap:0,marginBottom:14,background:dark?"#252535":"#f5f5f5",borderRadius:12,padding:3}}><button onClick={function(){handleAddType("expense");}} style={{flex:1,padding:"9px",border:"none",borderRadius:10,background:addType==="expense"?expenseColor:"transparent",color:addType==="expense"?"#fff":subC,fontSize:14,cursor:"pointer",fontWeight:addType==="expense"?500:400}}>💸 {t.expenses}</button><button onClick={function(){handleAddType("income");}} style={{flex:1,padding:"9px",border:"none",borderRadius:10,background:addType==="income"?incomeColor:"transparent",color:addType==="income"?"#fff":subC,fontSize:14,cursor:"pointer",fontWeight:addType==="income"?500:400}}>💰 {t.incomes}</button></div><div style={{display:"flex",gap:0,marginBottom:10,background:dark?"#252535":"#f5f5f5",borderRadius:10,padding:3}}><button onClick={function(){setAddSubTab("single");}} style={{flex:1,padding:"9px",border:"none",borderRadius:8,background:addSubTab==="single"?(addType==="expense"?expenseColor:incomeColor):"transparent",color:addSubTab==="single"?"#fff":subC,fontSize:13,cursor:"pointer",fontWeight:addSubTab==="single"?700:400}}>⚡ {t.single}</button><button onClick={function(){setAddSubTab("bulk");}} style={{flex:1,padding:"9px",border:"none",borderRadius:8,background:addSubTab==="bulk"?(addType==="expense"?expenseColor:incomeColor):"transparent",color:addSubTab==="bulk"?"#fff":subC,fontSize:13,cursor:"pointer",fontWeight:addSubTab==="bulk"?700:400}}>📋 {t.multiple}</button>{addType==="expense"&&<button onClick={function(){setAddSubTab("receipt");}} style={{flex:1,padding:"9px",border:"none",borderRadius:8,background:addSubTab==="receipt"?expenseColor:"transparent",color:addSubTab==="receipt"?"#fff":subC,fontSize:13,cursor:"pointer",fontWeight:addSubTab==="receipt"?700:400}}>📷 Scontrino</button>}</div><div style={{background:dark?"#1e1e30":"linear-gradient(180deg,#ffffff 0%,#f7f6ff 100%)",borderRadius:16,border:"1px solid "+borderC,padding:isMobile?12:16,boxShadow:dark?"none":"0 4px 18px rgba(83,74,183,0.08)"}}>{addSubTab==="single"&&<ExpenseForm type={addType} onSave={function(item){if(addType==="expense"){addExpenses([item]);setToast("Uscita aggiunta");}else{addIncomes([item]);setToast("Entrata aggiunta");}}}/>} {addSubTab==="bulk"&&<BulkEntry type={addType} onSave={function(items){if(addType==="expense"){addExpenses(items);setToast(items.length+" uscite aggiunte");}else{addIncomes(items);setToast(items.length+" entrate aggiunte");}}}/>} {addSubTab==="receipt"&&addType==="expense"&&<ReceiptScanPanel onSave={function(item){addExpenses([item]);setToast("Scontrino salvato");}}/>}</div></div>}{speseSubTab==="recurring"&&<RecurringManager/>}</div>;
}
...
export function HistoryPanel(){
  ...
  var grps=expenseGroups||DEFAULT_EXPENSE_GROUPS;
  return <div><div style={{display:"flex",gap:0,marginBottom:12,background:dark?"#252535":"#f5f5f5",borderRadius:12,padding:3,boxShadow:dark?"none":"inset 0 0 0 1px #eee"}}><button onClick={function(){setHistoryTab("expenses");}} style={{flex:1,padding:"10px",border:"none",borderRadius:10,background:historyTab==="expenses"?expenseColor:"transparent",color:historyTab==="expenses"?"#fff":subC,fontSize:13,cursor:"pointer",fontWeight:historyTab==="expenses"?700:500,boxShadow:historyTab==="expenses"?"0 4px 12px rgba(226,75,74,0.22)":"none"}}>💸 {t.expenses}</button><button onClick={function(){setHistoryTab("incomes");}} style={{flex:1,padding:"10px",border:"none",borderRadius:10,background:historyTab==="incomes"?incomeColor:"transparent",color:historyTab==="incomes"?"#fff":subC,fontSize:13,cursor:"pointer",fontWeight:historyTab==="incomes"?700:500,boxShadow:historyTab==="incomes"?"0 4px 12px rgba(29,158,117,0.22)":"none"}}>💰 {t.incomes}</button></div><div style={{position:"relative",marginBottom:10}}><span style={{position:"absolute",left:10,top:"50%",transform:"translateY(-50%)",fontSize:14,color:"#aaa"}}>🔍</span><input type="text" placeholder={t.search} defaultValue={searchQuery} onChange={function(e){historySearchDraftRef.current=e.target.value;}} onKeyDown={function(e){if(e.key==="Enter")setSearchQuery(historySearchDraftRef.current||"");}} onBlur={function(){setSearchQuery(historySearchDraftRef.current||"");}} style={{...inp,width:"100%",paddingLeft:34,paddingRight:searchQuery?34:10,boxSizing:"border-box"}}/>{searchQuery&&<button onClick={function(){historySearchDraftRef.current="";setSearchQuery("");}} style={{position:"absolute",right:8,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",color:"#aaa",fontSize:16}}>x</button>}</div><div style={{display:"flex",gap:8,marginBottom:10,flexWrap:"wrap",alignItems:"center"}}><select value={filterYear} onChange={function(e){setFilterYear(e.target.value);setFilterMonth("");}} style={{...inp,flex:"0 0 auto"}}>{availableYears.map(function(y){return <option key={y} value={y}>{y==="all"?"Tutti gli anni":y}</option>;})}</select><select value={filterMonth} onChange={function(e){setFilterMonth(e.target.value);setFilterMonths([]);}} style={{...inp,flex:"0 0 auto"}}><option value="">Tutti i mesi</option>{Array.from({length:12},function(_,i){var m=String(i+1).padStart(2,"0");var key=(filterYear&&filterYear!=="all"?filterYear:String(curYear))+"-"+m;return <option key={m} value={key}>{MONTHS_FULL[i]}</option>;})}</select><button onClick={function(){setFilterDateFrom("");setFilterDateTo("");setFilterMonth(curMonthKey);setFilterMonths([]);}} style={{background:dark?"#252535":"#EEF4FF",color:"#378ADD",border:"1px solid #BFD7FF",borderRadius:btnRadius,padding:"8px 12px",fontSize:12,fontWeight:700,cursor:"pointer"}}>Mese corrente</button><button onClick={function(){setFilterMonth("");setFilterMonths([]);setFilterDateFrom(dateOffset(30));setFilterDateTo(todayStr());}} style={{background:dark?"#252535":"#EEF4FF",color:"#378ADD",border:"1px solid #BFD7FF",borderRadius:btnRadius,padding:"8px 12px",fontSize:12,fontWeight:700,cursor:"pointer"}}>Ultimi 30 giorni</button><button onClick={function(){setShowFilters(function(s){return !s;});}} style={{background:showFilters?"linear-gradient(135deg,#7F77DD,#378ADD)":(dark?"#252535":"#EEF4FF"),color:showFilters?"#fff":"#378ADD",border:"1px solid "+(showFilters?"#7F77DD":"#BFD7FF"),borderRadius:btnRadius,padding:"8px 14px",fontSize:13,fontWeight:700,cursor:"pointer",boxShadow:showFilters?"0 4px 12px rgba(55,138,221,0.22)":"none"}}>🔎 Filtri</button>{(searchQuery||filterCat!=="all"||(filterCats&&filterCats.length)||filterCatExclude||(filterMonths&&filterMonths.length)||filterGroup!=="all"||filterDateFrom||filterDateTo||filterAmtMin||filterAmtMax||filterMonth)&&<Btn onClick={function(){setSearchQuery("");setFilterCat("all");setFilterCats([]);setFilterCatExclude(false);setFilterGroup("all");setFilterDateFrom("");setFilterDateTo("");setFilterAmtMin("");setFilterAmtMax("");setFilterMonth("");setFilterMonths([]);}} bg="#f0f0f0" color="#666" style={{padding:"6px 12px",fontSize:12}}>{t.clearFilters}</Btn>}</div>{showFilters&&<div style={{background:dark?"#252535":"#f9f9f9",borderRadius:12,border:"1px solid "+borderC,padding:14,marginBottom:12}}><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>{historyTab==="expenses"&&<><div style={{gridColumn:"1/-1"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}><label style={{fontSize:11,color:subC}}>Categorie</label><div style={{display:"flex",gap:6}}><button onClick={function(){var all=cats.map(function(c){return String(c.id);});if(showShareInHistory)all.push("share");setFilterCats(all);setFilterCat("all");}} style={{border:"none",background:"transparent",color:confirmButtonColor,cursor:"pointer",fontSize:11}}>Seleziona tutte</button><button onClick={function(){setFilterCatExclude(!filterCatExclude);}} style={{border:"1px solid "+(filterCatExclude?expenseColor:borderC),borderRadius:8,background:filterCatExclude?expenseColor+"22":"transparent",color:filterCatExclude?expenseColor:subC,cursor:"pointer",fontSize:11,padding:"2px 6px"}}>Escludi</button></div></div><div style={{display:"flex",gap:6,flexWrap:"wrap"}}>{cats.concat(showShareInHistory?[{id:"share",icon:"🤝",name:"Share",color:confirmButtonColor}]:[]).map(function(c){var cid=String(c.id);var active=filterCats.includes(cid);return <button key={cid} onClick={function(){setFilterCats(function(list){setFilterCat("all");return list.includes(cid)?list.filter(function(x){return x!==cid;}):list.concat([cid]);});}} style={{padding:"5px 9px",borderRadius:20,border:"1px solid "+(active?c.color:borderC),background:active?c.color+"22":"transparent",color:active?c.color:textC,fontSize:12,cursor:"pointer"}}>{c.icon} {c.name}</button>;})}</div>{showShareInHistory&&<div style={{fontSize:11,color:subC,marginTop:6}}>Share appare perché è attiva la visualizzazione delle spese Share nello storico.</div>}</div><div style={{gridColumn:"1/-1"}}><label style={{fontSize:11,color:subC,display:"block",marginBottom:5}}>Mesi multipli</label><div style={{display:"flex",gap:6,flexWrap:"wrap"}}>{Array.from({length:12},function(_,i){var mk=(filterYear&&filterYear!=="all"?filterYear:String(curYear))+"-"+String(i+1).padStart(2,"0");var active=filterMonths.includes(mk);return <button key={mk} onClick={function(){setFilterMonth("");setFilterMonths(function(list){return list.includes(mk)?list.filter(function(x){return x!==mk;}):list.concat([mk]);});}} style={{padding:"5px 8px",borderRadius:18,border:"1px solid "+(active?confirmButtonColor:borderC),background:active?confirmButtonColor+"22":"transparent",color:active?confirmButtonColor:textC,fontSize:11,cursor:"pointer"}}>{MONTHS_SHORT[i]}</button>;})}</div></div><div><label style={{fontSize:11,color:subC,display:"block",marginBottom:3}}>Area</label><select value={filterGroup} onChange={function(e){setFilterGroup(e.target.value);}} style={{...inp,width:"100%"}}><option value="all">Tutte</option>{grps.map(function(g){return <option key={g.id} value={g.id}>{g.name}</option>;})}<option value="share">Share</option></select></div></>}<div><label style={{fontSize:11,color:subC,display:"block",marginBottom:3}}>Data da</label><input type="date" value={filterDateFrom} onChange={function(e){setFilterDateFrom(e.target.value);}} style={{...inp,width:"100%"}}/></div><div><label style={{fontSize:11,color:subC,display:"block",marginBottom:3}}>Data a</label><input type="date" value={filterDateTo} onChange={function(e){setFilterDateTo(e.target.value);}} style={{...inp,width:"100%"}}/></div><div><label style={{fontSize:11,color:subC,display:"block",marginBottom:3}}>Importo min</label><input type="number" value={filterAmtMin} onChange={function(e){setFilterAmtMin(e.target.value);}} style={{...inp,width:"100%"}}/></div><div><label style={{fontSize:11,color:subC,display:"block",marginBottom:3}}>Importo max</label><input type="number" value={filterAmtMax} onChange={function(e){setFilterAmtMax(e.target.value);}} style={{...inp,width:"100%"}}/></div></div></div>}{historyTab==="expenses"&&<><div style={{fontSize:12,color:subC,marginBottom:8}}>{filteredExpenses.length} voci — {fmt(filteredExpenses.reduce(function(a,e){return a+e.amount;},0))}</div>{filteredExpenses.length===0&&<div style={{textAlign:"center",color:"#ccc",padding:"32px 0"}}>{t.noExpenses}</div>}{filteredExpenses.map(function(e){var c=e._share?{icon:"🤝",name:"Share",color:confirmButtonColor}:getCat(e.catId);var m=e._share?null:getMethod(e.methodId);var eid=e.id;var ecopy={id:e.id,amount:e.amount,catId:e.catId,methodId:e.methodId,desc:e.desc,date:e.date,rateizzato:e.rateizzato,rate:e.rate};return <div key={eid} style={{background:cardBg,borderRadius:10,border:"1px solid "+borderC,padding:"10px 14px",marginBottom:8}}><div style={{display:"flex",gap:10,alignItems:"center"}}><span style={{fontSize:18,flexShrink:0}}>{c?c.icon:"📦"}</span><div style={{flex:1,minWidth:0}}><div style={{fontSize:14,fontWeight:500,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",color:textC}}>{e.desc||"-"}</div><div style={{display:"flex",gap:6,flexWrap:"wrap",marginTop:3}}>{c&&<Badge color={c.color} name={c.name} small/>}{m&&<Badge color={m.color} name={m.name+(m.archived?" [A]":"")} small/>}{e.rateizzato&&<Badge color="#7F77DD" name={"÷"+e.rate+"m"} small/>}<span style={{fontSize:11,color:subC}}>{fmtDate(e.date,dateFmt)}</span></div></div><div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:4}}><div style={{fontSize:15,fontWeight:500,color:expenseColor}}>{fmt(e.amount)}{secRate&&showSecInHistory&&fmtSec(e.amount)&&<div style={{fontSize:10,color:subC,fontWeight:400}}>{fmtSec(e.amount)}</div>}</div>{!e._share&&<div style={{display:"flex",gap:6}}><button title="Modifica" onClick={function(){setEditingItem({item:ecopy,isExp:true});}} style={{background:"#EEF4FF",border:"1px solid #BFD7FF",cursor:"pointer",color:"#378ADD",fontSize:14,padding:"5px 8px",borderRadius:8,fontWeight:700}}>✏️</button><button title="Elimina" onClick={function(){setDeleteConfirmId(eid);}} style={{background:"#FFF0F0",border:"1px solid #FFD0D0",cursor:"pointer",color:expenseColor,fontSize:14,padding:"5px 8px",borderRadius:8,fontWeight:700}}>🗑️</button></div>}</div></div>{deleteConfirmId===eid&&!e._share&&<div style={{marginTop:10,background:"#fff0f0",borderRadius:8,padding:"10px 12px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:10}}><span style={{fontSize:13,color:expenseColor}}>Eliminare?</span><div style={{display:"flex",gap:8}}><Btn onClick={function(){setExpenses(expenses.filter(function(x){return x.id!==eid;}));setDeleteConfirmId(null);}} bg={expenseColor} style={{padding:"5px 12px",fontSize:13,fontWeight:500}}>Elimina</Btn><Btn onClick={function(){setDeleteConfirmId(null);}} bg="#f0f0f0" color="#555" style={{padding:"5px 12px",fontSize:13}}>{V.cancel}</Btn></div></div>}</div>;})} </>}{historyTab==="incomes"&&<><div style={{fontSize:12,color:subC,marginBottom:8}}>{filteredIncomes.length} voci — {fmt(filteredIncomes.reduce(function(a,i){return a+i.amount;},0))}</div>{filteredIncomes.length===0&&<div style={{textAlign:"center",color:"#ccc",padding:"32px 0"}}>{t.noIncomes}</div>}{filteredIncomes.map(function(inc){var it=getIT(inc.type);var iid=inc.id;var icopy={id:inc.id,amount:inc.amount,type:inc.type,desc:inc.desc,date:inc.date,rateizzato:inc.rateizzato,rate:inc.rate};return <div key={iid} style={{background:cardBg,borderRadius:10,border:"1px solid "+borderC,padding:"10px 14px",marginBottom:8}}><div style={{display:"flex",gap:10,alignItems:"center"}}><span style={{fontSize:18,flexShrink:0}}>{it?it.icon:"💰"}</span><div style={{flex:1,minWidth:0}}><div style={{fontSize:14,fontWeight:500,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",color:textC}}>{inc.desc||"-"}</div><div style={{display:"flex",gap:6,flexWrap:"wrap",marginTop:3}}>{it&&<Badge color={it.color} name={it.name} small/>}{inc.rateizzato&&<Badge color="#7F77DD" name={"÷"+inc.rate+"m"} small/>}<span style={{fontSize:11,color:subC}}>{fmtDate(inc.date,dateFmt)}</span></div></div><div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:4}}><div style={{fontSize:15,fontWeight:500,color:incomeColor}}>+{fmt(inc.amount)}{secRate&&showSecInHistory&&fmtSec(inc.amount)&&<div style={{fontSize:10,color:subC,fontWeight:400}}>{fmtSec(inc.amount)}</div>}</div><div style={{display:"flex",gap:6}}><button title="Modifica" onClick={function(){setEditingItem({item:icopy,isExp:false});}} style={{background:"#EEF4FF",border:"1px solid #BFD7FF",cursor:"pointer",color:"#378ADD",fontSize:14,padding:"5px 8px",borderRadius:8,fontWeight:700}}>✏️</button><button title="Elimina" onClick={function(){setDeleteConfirmId(iid);}} style={{background:"#FFF0F0",border:"1px solid #FFD0D0",cursor:"pointer",color:expenseColor,fontSize:14,padding:"5px 8px",borderRadius:8,fontWeight:700}}>🗑️</button></div></div></div>{deleteConfirmId===iid&&<div style={{marginTop:10,background:"#fff0f0",borderRadius:8,padding:"10px 12px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:10}}><span style={{fontSize:13,color:expenseColor}}>Eliminare?</span><div style={{display:"flex",gap:8}}><Btn onClick={function(){setIncomes(incomes.filter(function(x){return x.id!==iid;}));setDeleteConfirmId(null);}} bg={expenseColor} style={{padding:"5px 12px",fontSize:13,fontWeight:500}}>Elimina</Btn><Btn onClick={function(){setDeleteConfirmId(null);}} bg="#f0f0f0" color="#555" style={{padding:"5px 12px",fontSize:13}}>{V.cancel}</Btn></div></div>}</div>;})} </>}</div>;
}
...
export function ConsulenteAIPanel(){
  ...
  var aiAdvices=useMemo(function(){
    ...
    if(entSubs.length>=2){
      ...
      addAdvice(arr,{id:"subs-entertainment",type:"risparmio",priority:"Media",icon:"🎬",title:"Abbonamenti intrattenimento",savingMonthly:maxSave,text:"Hai "+entSubs.length+" abbonamenti attivi per intrattenimento: "+entSubs.map(function(r){return r.name;}).join(", ")+". Totale: "+fmt(total)+"/mese. Valuta se sospenderne uno: risparmio stimato "+fmt(minSave)+"–"+fmt(maxSave)+"/mese, cioè "+fmt(minSave*12)+"–"+fmt(maxSave*12)+" all’anno.",question:"Vuoi prendere in considerazione questa opzione a breve termine o queste spese non sono negoziabili?"});
    }
    ...
    if(foodNow>0&&foodPrevAvg>0&&foodNow>foodPrevAvg*1.1){
      ...
      addAdvice(arr,{id:"food-increase",type:"ottimizzazione",priority:pct>=20?"Alta":"Media",icon:"🛒",title:"Spese alimentari in aumento",savingMonthly:saveFood,text:"Le spese alimentari sono aumentate del "+pct.toFixed(1).replace(".",",")+"% rispetto alla media degli ultimi 3 mesi. Questo mese hai speso "+fmt(foodNow)+", contro una media di "+fmt(foodPrevAvg)+".",question:"Vogliamo analizzarle nello specifico? Riducendole anche solo del 3%, puoi risparmiare circa "+fmt(saveFood)+" al mese."});
    }
    ...
    if(extraTotal>0&&weekendExtra/extraTotal>=0.3){
      ...
      addAdvice(arr,{id:"weekend-extra",type:"abitudine",priority:"Media",icon:"🌙",title:"Spese extra concentrate nel weekend",savingMonthly:saveW,text:"Il "+pctW.toFixed(1).replace(".",",")+"% delle spese extra del mese avviene nel weekend. Spese weekend extra: "+fmt(weekendExtra)+" su "+fmt(extraTotal)+".",question:"Vuoi impostare un limite weekend o analizzare il dettaglio? Riducendole del 5%, risparmi circa "+fmt(saveW)+" al mese."});
    }
    ...
    if(salaryNow===0&&salaryPrevAvg>0){
      addAdvice(arr,{id:"missing-salary",type:"controllo",priority:"Alta",icon:"💼",title:"Possibile stipendio mancante",savingMonthly:0,text:"Nel mese corrente non risulta registrata un’entrata assimilabile allo stipendio, mentre negli ultimi 3 mesi la media era "+fmt(salaryPrevAvg)+".",question:"Ti sei dimenticato di inserirla o è corretto così?"});
    }
    ...
    if(utilNow===0&&utilPrevAvg>0){
      addAdvice(arr,{id:"missing-utilities",type:"controllo",priority:"Alta",icon:"💡",title:"Possibile bolletta mancante",savingMonthly:0,text:"Nel mese corrente non risultano uscite riconducibili a utenze o bollette. Negli ultimi 3 mesi la media era "+fmt(utilPrevAvg)+".",question:"Ti sei dimenticato di inserirla o è giusto così?"});
    }
    ...
    recurring.forEach(function(r){
      ...
      if(!isPresent){addAdvice(arr,{id:"missing-recurring-"+r.id,type:"controllo",priority:"Alta",icon:r.rtype==="expense"?"🔄":"💰",title:"Ricorrente non registrata",savingMonthly:0,text:"Non trovo nel mese corrente la voce ricorrente “"+r.name+"” da "+fmt(amount)+".",question:"Va confermata, saltata o inserita manualmente?"});}
    });
    ...
    if(budgetPlan&&budgetPlan.items){
      budgetPlan.items.forEach(function(b){
        ...
        if(pct>=90){
          ...
          addAdvice(arr,{id:"budget-risk-"+c.id,type:"budget",priority:pct>=100?"Alta":"Media",icon:c.icon||"💰",title:pct>=100?"Budget superato: "+c.name:"Budget a rischio: "+c.name,savingMonthly:Math.round(Math.max(0,spent-b.amount*0.95)),text:"Hai usato il "+pct.toFixed(1).replace(".",",")+"% del budget “"+c.name+"”: "+fmt(spent)+" su "+fmt(b.amount)+".",question:pct>=100?"Vuoi analizzare le singole uscite per capire dove intervenire?":"Vuoi impostare un limite più stretto per il resto del mese?"});
        }
      });
    }

    if(!arr.length){
      arr.push({id:"ai-empty",type:"info",priority:"Bassa",icon:"✅",title:"Nessuna criticità evidente",savingMonthly:0,text:currentExp===0&&currentInc===0?"Non ci sono ancora abbastanza dati nel mese corrente per generare consigli affidabili.":"Al momento non emergono anomalie forti dai dati disponibili. Continua a registrare entrate, uscite e ricorrenze per migliorare la precisione del Consulente AI.",question:"Puoi iniziare chiedendo: “Come posso risparmiare questo mese?”"});
    }
  ...
  },[expenses,incomes,recurring,budgetPlan,cats,aiDismissed,curMonthKey,statsView]);
  ...
  function askAI(text){openAIChat(text||"");}
  ...
  function detectTextLanguage(q){
    ...
    if(/[¿¡ñ]/i.test(raw))scores.es+=3;if(/[àèéìòù]/i.test(raw))scores.it+=2;if(/[çœàâêîôû]/i.test(raw))scores.fr+=2;if(/[äöüß]/i.test(raw))scores.de+=3;if(/[ãõç]/i.test(raw))scores.pt+=3;if(/[ąćęłńóśźż]/i.test(raw))scores.pl+=3;if(/[ăâîșşțţ]/i.test(raw))scores.ro+=3;if(/[α-ωΑ-Ω]/.test(raw))scores.el+=5;
  ...
  function buildAIRequest(q){
    var qLang=detectTextLanguage(q);var lname=languageName(qLang);
    return {question:"[LANGUAGE_LOCK="+qLang+" / "+lname+". Reply ONLY in "+lname+". If the user writes in a language different from Italian, NEVER answer in Italian.] "+q,language:qLang,context:buildAIContext(),instruction:AI_AGENT_SCOPE_INSTRUCTION+" LANGUAGE_LOCK: rispondi esclusivamente in "+lname+" (codice "+qLang+"). Se la domanda dell’utente non è in italiano, è vietato rispondere in italiano. Mantieni la stessa lingua dell’ultimo messaggio dell’utente. Rispetta dataAccessLevel: summary=usa solo riepiloghi, areas=usa anche aggregati per area, full=puoi usare anche le transazioni inviate. Non inventare dati personali mancanti. Non dare consulenza finanziaria personalizzata come certezza: presenta scenari, rischi e ipotesi."};
  }
  ...
  async function callFinanceAgent(q){
    var req=buildAIRequest(q);var res=await fetch(AI_AGENT_ENDPOINT,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(req)});var data=null;try{data=await res.json();}catch(e){}if(!res.ok){throw new Error((data&&data.error)||("Errore agente AI: "+res.status));}
  ...
  async function sendChat(){
    ...
    try{
      ...
      var errMsg=rawErr.indexOf("429")>=0?"Quota OpenAI esaurita o billing non attivo. La chat è collegata correttamente, ma l’account API non può ancora generare risposte. Attiva billing/crediti su OpenAI e riprova.":"Errore nel collegamento al motore AI: "+rawErr+". Controlla i log della Function askFinanceAI.";
  ...
  var aiAccessLabel=aiDataAccess==="full"?"analisi completa: tutte le transazioni essenziali":aiDataAccess==="areas"?"analisi media: riepilogo + spese per area":"analisi limitata: solo riepilogo aggregato";
  return <div style={{display:"flex",flexDirection:"column",gap:16}}>
    <div style={{position:"relative",overflow:"hidden",background:dark?"#252535":"linear-gradient(135deg,#ffffff,#f4f1ff)",border:"1px solid "+borderC2,borderRadius:18,padding:isMobile?18:22,minHeight:isMobile?132:150,boxShadow:dark?"none":"0 8px 28px rgba(127,119,221,0.12)"}}>
      <div style={{position:"relative",zIndex:2,maxWidth:620}}>
        <div style={{fontSize:22,fontWeight:900,color:textC2,marginBottom:6}}>Consulente AI</div>
        <div style={{fontSize:12,color:subC2,lineHeight:1.5,marginBottom:14}}>Agente AI dedicato alla gestione delle tue finanze dentro fAInance: analisi dati, budget, risparmio, alert, obiettivi e uso dell’app.</div>
        <button onClick={function(){openAIChat("");}} style={{background:"linear-gradient(135deg,#7F77DD,#378ADD)",color:"#fff",border:"none",borderRadius:btnRadius,padding:"10px 14px",fontSize:13,fontWeight:700,cursor:"pointer",display:"inline-flex",alignItems:"center",gap:8,boxShadow:"0 4px 14px rgba(127,119,221,0.22)"}}>💬 Inizia conversazione</button>
      </div>
    </div>
    <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr 1fr":"repeat(4,1fr)",gap:12}}>
      <button onClick={function(){setAiAdviceFilter("all");setAiTab("consigli");}} style={{border:"none",background:"transparent",padding:0,textAlign:"left",cursor:"pointer"}}><StatCard title="Risparmio potenziale" value={fmt(totalSaving)} color="#1D9E75" bg="#e8f8f0" sub={fmt(totalSaving*12)+"/anno"}/></button>
      <button onClick={function(){setAiAdviceFilter("all");setAiTab("consigli");}} style={{border:"none",background:"transparent",padding:0,textAlign:"left",cursor:"pointer"}}><StatCard title="Consigli attivi" value={String(aiAdvices.length)} color="#7F77DD" bg="#f0f0ff"/></button>
      <button onClick={function(){setAiAdviceFilter("high");setAiTab("consigli");}} style={{border:"none",background:"transparent",padding:0,textAlign:"left",cursor:"pointer"}}><StatCard title="Priorità alta" value={String(highCount)} color={highCount?"#E24B4A":"#1D9E75"} bg={highCount?"#fff0f0":"#e8f8f0"}/></button>
      <button onClick={function(){setAiAdviceFilter("control");setAiTab("consigli");}} style={{border:"none",background:"transparent",padding:0,textAlign:"left",cursor:"pointer"}}><StatCard title="Controlli dati" value={String(controlCount)} color="#EF9F27" bg="#fff8e1"/></button>
    </div>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
      {[{id:"consigli",label:"📌 Consigli e metriche",sub:"Analisi automatiche",color:"#7F77DD"},{id:"chat",label:"💬 Conversazione",sub:"Domande su fAInance",color:"#378ADD"}].map(function(tb){var active=aiTab===tb.id;return <button key={tb.id} onClick={function(){tb.id==="chat"?openAIChat(undefined):setAiTab(tb.id);}} style={{textAlign:"left",padding:"13px 14px",borderRadius:14,border:"1.5px solid "+(active?tb.color:borderC2),background:active?("linear-gradient(135deg,"+tb.color+"22,#ffffff)"):(dark?"#252535":"#fff"),color:active?tb.color:textC2,fontSize:13,cursor:"pointer",fontWeight:800,boxShadow:active?"0 4px 14px rgba(127,119,221,0.16)":"none"}}><div>{tb.label}</div><div style={{fontSize:11,fontWeight:500,color:active?tb.color:subC2,marginTop:3}}>{tb.sub}</div></button>;})}
    </div>
    {aiTab==="consigli"&&<div style={{display:"flex",flexDirection:"column",gap:12}}>{filteredAiAdvices.map(function(a){var pc=a.priority==="Alta"?"#E24B4A":a.priority==="Media"?"#EF9F27":"#1D9E75";return <div key={a.id} style={{background:cardBg2,borderRadius:14,border:"1px solid "+borderC2,padding:16}}><div style={{display:"flex",alignItems:"flex-start",gap:12}}><div style={{fontSize:24,width:34,textAlign:"center"}}>{a.icon}</div><div style={{flex:1}}><div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap",marginBottom:5}}><div style={{fontSize:15,fontWeight:800,color:textC2}}>{a.title}</div><span style={{fontSize:10,fontWeight:700,color:pc,background:pc+"22",borderRadius:10,padding:"2px 7px"}}>{a.priority}</span>{a.savingMonthly>0&&<span style={{fontSize:10,fontWeight:700,color:"#1D9E75",background:"#1D9E7522",borderRadius:10,padding:"2px 7px"}}>+{fmt(a.savingMonthly)}/mese</span>}</div><div style={{fontSize:13,color:subC2,lineHeight:1.5,whiteSpace:"pre-line"}}>{a.text}</div><div style={{fontSize:13,color:textC2,fontWeight:600,marginTop:8}}>{a.question}</div><div style={{display:"flex",gap:8,flexWrap:"wrap",marginTop:12}}><Btn onClick={function(){askAI(a.title);}} bg="#7F77DD" style={{padding:"7px 12px",fontSize:12}}>Analizza</Btn>{a.id!=="ai-empty"&&<Btn onClick={function(){dismissAdvice(a.id);}} bg={dark?"#333":"#f0f0f0"} color={dark?"#eee":"#555"} style={{padding:"7px 12px",fontSize:12}}>Non mostrare più</Btn>}</div></div></div></div>;})}</div>}
    {aiTab==="chat"&&<div ref={aiChatSectionRef} style={{background:cardBg2,borderRadius:14,border:"1px solid "+borderC2,padding:16}}><div style={{fontSize:12,color:subC2,background:dark?"#1e1e30":"#f4f1ff",border:"1px solid "+(dark?"#3a3a5a":"#d8d1ff"),borderRadius:12,padding:"9px 11px",marginBottom:12}}>{expenses.length+incomes.length===0?"Per darti consigli davvero utili, ho bisogno di qualche movimento registrato. Puoi comunque farmi domande generali sulla gestione delle tue finanze e sulle funzioni dell’app.":"Puoi chiedere analisi, consigli e chiarimenti solo su fAInance e sui dati gestiti nell’app."} Livello dati attivo: {aiAccessLabel}.</div><div style={{height:360,overflowY:"auto",display:"flex",flexDirection:"column",gap:10,marginBottom:12}}>{aiChat.length===0&&<div style={{fontSize:13,color:subC2,textAlign:"center",padding:"40px 0"}}>Esempi: “Come posso risparmiare questo mese?”, “Quali categorie pesano di più?”, “Analizza i miei abbonamenti”, “Perché non vedo gli alert?”</div>}{aiChat.map(function(m){var mine=m.role==="user";return <div key={m.id} style={{alignSelf:mine?"flex-end":"flex-start",maxWidth:"82%",background:mine?"#7F77DD":(dark?"#252535":"#f5f5f5"),color:mine?"#fff":textC2,borderRadius:14,padding:"10px 12px",fontSize:13,whiteSpace:"pre-line",lineHeight:1.45}}>{m.text}</div>;})}{aiLoading&&<div style={{alignSelf:"flex-start",background:dark?"#252535":"#f5f5f5",color:subC2,borderRadius:14,padding:"10px 12px",fontSize:13}}>Sto analizzando...</div>}</div><div style={{display:"flex",gap:8}}><input ref={chatInputRef} disabled={aiLoading} onKeyDown={function(e){if(e.key==="Enter"){e.preventDefault();sendChat();}}} placeholder="Scrivi una domanda su fAInance..." style={{...sinp,flex:1,opacity:aiLoading?0.7:1}}/><Btn onClick={sendChat} disabled={aiLoading} bg="#7F77DD">{aiLoading?"...":"Invia"}</Btn></div></div>}
...
var VOICE_NUMBERS={
  zero:0,zer0:0,μηδεν:0,
  uno:1,una:1,un:1,one:1,a:1,ein:1,eine:1,einen:1,uno_es:1,um:1,uma:1,jeden:1,jedna:1,jedno:1,een:1,unu:1,una_ro:1,ένα:1,ενα:1,μια:1,μία:1,
  due:2,two:2,dos:2,deux:2,zwei:2,dois:2,duas:2,dwa:2,dwie:2,twee:2,doi:2,doua:2,două:2,δυο:2,δύο:2,δυο_gr:2,
  tre:3,three:3,tres:3,trois:3,drei:3,três:3,tres_pt:3,trzy:3,drie:3,trei:3,τρια:3,τρία:3,
  quattro:4,four:4,cuatro:4,quatre:4,vier:4,quatro:4,cztery:4,patru:4,τεσσερα:4,τέσσερα:4,
  cinque:5,five:5,cinco:5,cinq:5,fünf:5,funf:5,pięć:5,piec:5,vijf:5,cinci:5,πεντε:5,πέντε:5,
  sei:6,six:6,seis:6,sechs:6,sześć:6,szesc:6,zes:6,sase:6,șase:6,έξι:6,εξι:6,
  sette:7,seven:7,siete:7,sept:7,sieben:7,sete:7,siedem:7,zeven:7,sapte:7,șapte:7,επτα:7,επτά:7,
  otto:8,eight:8,ocho:8,huit:8,acht:8,oito:8,osiem:8,opt:8,οκτω:8,οκτώ:8,
  nove:9,nine:9,nueve:9,neuf:9,neun:9,nove_pt:9,dziewięć:9,dziewiec:9,negen:9,noua:9,nouă:9,εννεα:9,εννέα:9,
  dieci:10,ten:10,diez:10,dix:10,zehn:10,dez:10,dziesięć:10,dziesiec:10,tien:10,zece:10,δεκα:10,δέκα:10,
  undici:11,eleven:11,once:11,onze:11,elf:11,jedenaście:11,jedenascie:11,unsprezece:11,εντεκα:11,έντεκα:11,
  dodici:12,twelve:12,doce:12,douze:12,zwolf:12,zwölf:12,dwanaście:12,dwanascie:12,twaalf:12,doisprezece:12,douasprezece:12,δωδεκα:12,δώδεκα:12,
  tredici:13,thirteen:13,trece:13,treize:13,dreizehn:13,treze:13,trzynaście:13,trzynascie:13,dertien:13,treisprezece:13,
  quattordici:14,fourteen:14,catorce:14,quatorze:14,vierzehn:14,catorze:14,czternaście:14,czternascie:14,veertien:14,paisprezece:14,
  quindici:15,fifteen:15,quince:15,quinze:15,fünfzehn:15,funfzehn:15,quinze_pt:15,piętnaście:15,pietnascie:15,vijftien:15,cincisprezece:15,
  sedici:16,sixteen:16,dieciseis:16,seize:16,sechzehn:16,dezesseis:16,szesnaście:16,szesnascie:16,zestien:16,saisprezece:16,șaisprezece:16,
  diciassette:17,seventeen:17,diecisiete:17,dix_sept:17,dixsept:17,siebzehn:17,dezessete:17,siedemnaście:17,siedemnascie:17,zeventien:17,saptesprezece:17,șaptesprezece:17,
  diciotto:18,eighteen:18,dieciocho:18,dix_huit:18,dixhuit:18,achtzehn:18,dezoito:18,osiemnaście:18,osiemnascie:18,achttien:18,optsprezece:18,
  diciannove:19,nineteen:19,diecinueve:19,dix_neuf:19,dixneuf:19,neunzehn:19,dezenove:19,dziewiętnaście:19,dziewietnascie:19,negentien:19,nouasprezece:19,
  venti:20,twenty:20,veinte:20,vingt:20,zwanzig:20,vinte:20,dwadzieścia:20,dwadziescia:20,twintig:20,douazeci:20,είκοσι:20,εικοσι:20,
  trenta:30,thirty:30,treinta:30,trente:30,dreissig:30,dreißig:30,trinta:30,trzydzieści:30,trzydziesci:30,dertig:30,treizeci:30,τριαντα:30,τριάντα:30,
  quaranta:40,forty:40,cuarenta:40,quarante:40,vierzig:40,quarenta:40,czterdzieści:40,czterdziesci:40,veertig:40,patruzeci:40,σαραντα:40,σαράντα:40,
  cinquanta:50,fifty:50,cincuenta:50,cinquante:50,fünfzig:50,funfzig:50,cinquenta:50,pięćdziesiąt:50,piecdziesiat:50,vijftig:50,cincizeci:50,πενηντα:50,πενήντα:50,
  sessanta:60,sixty:60,sesenta:60,soixante:60,sechzig:60,sessenta:60,sześćdziesiąt:60,szescdziesiat:60,zestig:60,saizeci:60,εξηντα:60,εξήντα:60,
  settanta:70,seventy:70,setenta:70,soixante_dix:70,siebzig:70,setenta_pt:70,siedemdziesiąt:70,siedemdziesiat:70,zeventig:70,saptezeci:70,εβδομηντα:70,εβδομήντα:70,
  ottanta:80,eighty:80,ochenta:80,quatre_vingts:80,quatrevingts:80,achtzig:80,oitenta:80,osiemdziesiąt:80,osiemdziesiat:80,tachtig:80,optzeci:80,ογδοντα:80,ογδόντα:80,
  novanta:90,ninety:90,noventa:90,quatre_vingt_dix:90,quatrevingtdix:90,neunzig:90,noventa_pt:90,dziewięćdziesiąt:90,dziewiecdziesiat:90,negentig:90,nouazeci:90,ενενηντα:90,ενενήντα:90
};
...
export function VoiceEntryModal(){
  ...
  useEffect(function(){var timer=setTimeout(function(){try{if(!voiceListening&&!voiceText&&!voiceParsed)startVoiceListening();}catch(e){}},300);return function(){clearTimeout(timer);};},[]);
  function saveVoiceEntry(){
  if(!voiceParsed)return;
  if(voiceParsed.type==="income"){
    addIncomes([{id:Date.now(),amount:Number(voiceParsed.amount)||0,type:voiceParsed.incomeType,desc:voiceParsed.desc,date:voiceParsed.date,rateizzato:!!voiceParsed.rateizzato,rate:Number(voiceParsed.rate)||1}]);
    setToast(voiceUiText(lang).savedInc);
  }else{
    addExpenses([{id:Date.now(),amount:Number(voiceParsed.amount)||0,catId:Number(voiceParsed.catId),methodId:Number(voiceParsed.methodId),desc:voiceParsed.desc,date:voiceParsed.date,rateizzato:!!voiceParsed.rateizzato,rate:Number(voiceParsed.rate)||1}]);
    setToast(voiceUiText(lang).savedExp);
  }
  closeVoiceModal();
  ...
  return <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.55)",zIndex:520,display:"flex",alignItems:"center",justifyContent:"center",padding:16}} onClick={function(e){if(e.target===e.currentTarget)closeVoiceModal();}}>
    <div style={{background:cardBg,borderRadius:20,border:"1px solid "+borderC,width:"100%",maxWidth:430,boxShadow:"0 10px 40px rgba(0,0,0,0.28)",overflow:"hidden"}}>
      ...
      <div style={{padding:18,display:"flex",flexDirection:"column",gap:12}}>
        ...
        <textarea autoFocus={true} value={voiceText} onChange={function(e){setVoiceText(e.target.value);setVoiceParsed(null);setVoiceError("");}} placeholder={V.placeholder} style={{minHeight:74,borderRadius:12,border:"1px solid "+borderC,padding:"10px 12px",fontSize:13,background:dark?"#2a2a3e":"#fff",color:textC,resize:"vertical"}}/>
    ...
    {patTab==="inserimento"&&<>
      ...
      {(()=>{
        ...
        return <button onClick={function(){
          ...
          setDraft(nd);
          setToast("Valori copiati da "+MONTHS_SHORT[parseInt(prevMk.split("-")[1])-1]+" "+prevMk.slice(0,4));
