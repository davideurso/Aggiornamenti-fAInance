...
export function HomePanel() {
  ...
  var HOME_LIBRARY = [
    ...
    },
    {
      ...
      icon: "🎙️",
      label: H("Voce"),
      desc: H("Bottone rapido per aprire Voce"),
    },
    {
      type: "button_receipt",
      icon: "📷",
  ...
  function homeButtonDef(type) {
    var map: any = {
      button_voice: { icon: "🎙️", label: "Voce", tab: "voice" },
  ...
  function openHomeButton(type) {
    ...
    if (d.tab === "consulenteAI" && setAiTab) {
      if (d.aiTab === "chat") {
        setAiTab("consigli");
        if (openVoiceModal) openVoiceModal(false, true);
        homeScrollTop();
        return;
      }
...
export function ConsulenteAIPanel() {
  ...
  function buildAIContext() {
    ...
    var ctx = {
      ...
      dataAccessLabel:
        aiDataAccess === "full"
          ? "analisi completa - tutte le transazioni"
          : aiDataAccess === "areas"
          ? "analisi media - spese per area"
          : "analisi limitata - solo riepilogo",
      totals: {
      ...
      activeAdvice: aiAdvices.slice(0, 10).map(function (a) {
        return {
          title: a.title,
          priority: a.priority,
          type: a.type,
  ...
  var aiAccessLabel =
    aiDataAccess === "full"
      ? L("analisi completa: tutte le transazioni essenziali")
      : aiDataAccess === "areas"
      ? L("analisi media: riepilogo + spese per area")
      : L("analisi limitata: solo riepilogo aggregato");
  return (
          ...
          >
            ...
            >
              ...
              <div>
                ...
                <br />•{" "}
                {L(
                  "riepiloghi finanziari, budget, categorie, ricorrenze e dati aggregati"
                )}
                <br />•{" "}
                {L(
                  "solo con Analisi completa, anche transazioni essenziali: data, importo, categoria o tipo e descrizione"
                )}
              </div>
              <div>
                <b>{L("Destinatari")}:</b>
              ...
              <div style={{ color: subC2 }}>
                {L(
                  "Non vengono inviati CVV, dati biometrici, password, documenti caricati, immagini, fidelity card o dati completi delle carte di credito."
                )}
              </div>
              <div>
                {L(
...
function VoiceAssistantModal({ onQuick }) {
  ...
  function resolveVoiceAssistantTurn(text: any) {
    var resolution: any = resolveAssistantLanguageTurn(
      text,
      assistantLanguageStateRef.current,
      lastAssistantUserLanguageRef.current || String(lang || "it")
    );
    assistantLanguageStateRef.current = resolution.state;
    lastAssistantUserLanguageRef.current = resolution.language;
    lastAssistantLanguageResolutionRef.current = resolution;
    return resolution;
  }
  function currentVoiceLanguageResolution() {
    if (lastAssistantLanguageResolutionRef.current)
      return lastAssistantLanguageResolutionRef.current;
    var state: any =
      assistantLanguageStateRef.current ||
      createAssistantConversationLanguageState(
        lastAssistantUserLanguageRef.current || String(lang || "it")
    ...
    var active =
      lastAssistantUserLanguageRef.current ||
      state.activeLanguage ||
      String(lang || "it");
    return {
      language: active,
      languageName: assistantLanguageName(active),
      state: state,
      requestMode: state.mode,
      reason:
        state.mode === "locked" ? "locked_language" : "detected_user_language",
    };
  }
  ...
  function showTemporaryVoiceNotice(text, delay) {
    var clean = String(text || "").trim();
    clearVoiceErrorTimer();
    setVoiceError(clean);
    if (!clean) return;
    voiceErrorTimerRef.current = setTimeout(function () {
      if (mountedRef.current) setVoiceError("");
      voiceErrorTimerRef.current = null;
    }, Math.max(1800, Number(delay) || 4500));
  }
      ...
      return !!(
        ...
        ((typeof cap.isNativePlatform === "function" &&
          ...
          (typeof cap.getPlatform === "function" &&
            ["android", "ios"].indexOf(
              String(cap.getPlatform()).toLowerCase()
            ) >= 0))
      );
    } catch (e) {
      return false;
  ...
  function clampAssistantVolume(value: any) {
  ...
  function applyAssistantSystemVolume(value: any, muted?: any) {
    ...
    try {
      var gain = realtimeGainRef.current,
        ctx = realtimeAudioContextRef.current;
      if (gain && gain.gain) {
        var now = ctx && Number.isFinite(ctx.currentTime) ? ctx.currentTime : 0;
        gain.gain.cancelScheduledValues(now);
        gain.gain.setValueAtTime(ratio, now);
      }
    } catch (e) {
      try {
        if (realtimeGainRef.current) realtimeGainRef.current.gain.value = ratio;
      } catch (e2) {}
    }
    try {
      var audio = realtimeAudioRef.current;
      if (audio) {
        audio.volume = ratio;
        audio.muted = ratio <= 0;
      }
    ...
    try {
      var fallback = nativeAudioRef.current;
      if (fallback) {
        fallback.volume = ratio;
        fallback.muted = ratio <= 0;
      }
    } catch (e) {}
  }
  async function syncSystemMediaVolume() {
    if (!nativePlatform()) {
      applyAssistantSystemVolume(1, false);
      return 1;
    }
    try {
      var result: any =
        FainanceAudioNative && FainanceAudioNative.getMediaVolume
          ? await FainanceAudioNative.getMediaVolume()
          : null;
      var current = Number(result && result.current),
        maximum = Math.max(1, Number(result && result.max) || 1),
        ratio = Number(result && result.ratio);
      if (!Number.isFinite(ratio))
        ratio = Number.isFinite(current) ? current / maximum : 1;
      applyAssistantSystemVolume(
        ratio,
        (result && result.muted === true) || current <= 0
      );
  ...
  function setNativeAssistantAudio(active) {
    if (!nativePlatform()) return;
    try {
      var fn = active
        ? FainanceAudioNative.activateAssistantAudio
        : FainanceAudioNative.releaseAssistantAudio;
      if (fn)
        Promise.resolve(fn.call(FainanceAudioNative))
          .then(function () {
            if (active) syncSystemMediaVolume();
          })
          .catch(function () {});
      return;
    } catch (e) {}
    try {
      var cap: any = (window as any).Capacitor,
        plugin = cap && cap.Plugins && cap.Plugins.FainanceAudio,
        legacyFn =
          active && plugin
            ? plugin.activateAssistantAudio
            : plugin && plugin.releaseAssistantAudio;
      if (legacyFn)
        Promise.resolve(legacyFn.call(plugin))
          .then(function () {
            if (active) syncSystemMediaVolume();
          })
          .catch(function () {});
  ...
  async function attachRealtimeAudioStream(remoteStream: any) {
    ...
    try {
      var oldSource = realtimeAudioSourceRef.current;
      if (oldSource) oldSource.disconnect();
    } catch (e) {}
    realtimeAudioSourceRef.current = null;
    try {
      var oldGain = realtimeGainRef.current;
      if (oldGain) oldGain.disconnect();
    } catch (e) {}
    realtimeGainRef.current = null;
    try {
      var oldCtx = realtimeAudioContextRef.current;
      if (oldCtx && oldCtx.state !== "closed") await oldCtx.close();
    } catch (e) {}
  ...
  async function prepareNoiseFilteredMicrophone(acquired: any) {
    ...
    try {
      if (acquired && acquired.getAudioTracks)
        acquired.getAudioTracks().forEach(function (track: any) {
          track.enabled = true;
          try {
  ...
  function conciseTechnicalMessage(raw: any) {
    var text = String(raw || "")
      .replace(/\s+/g, " ")
      .trim();
    if (!text) return "";
  ...
  function realtimeFailureMessage(
    ...
    var raw = conciseTechnicalMessage(
      serverMessage || (error && error.message) || error || ""
    );
    ...
    try {
      if (typeof navigator !== "undefined" && navigator.onLine === false)
        return assistantRuntimeText(activeLanguage, "offline");
    } catch (e) {}
    if (/permission|notallowed|denied/i.test(raw))
      return realtimePermissionDeniedMessage();
    if (status === 401)
      return assistantRuntimeText(activeLanguage, "sessionExpired");
    if (status === 403)
      return raw || assistantRuntimeText(activeLanguage, "unavailable");
    if (status === 429)
      return assistantRuntimeText(activeLanguage, "rateLimited");
    if (name === "aborterror")
      return assistantRuntimeText(activeLanguage, "timeout");
    if (String(phase || "") === "datachannel")
      return assistantRuntimeText(activeLanguage, "interrupted");
  ...
  function realtimeServerFailure(error: any) {
    var code = String((error && error.code) || "").toLowerCase(),
      type = String((error && error.type) || "").toLowerCase();
    var activeLanguage =
      lastAssistantUserLanguageRef.current || String(lang || "it");
    if (
      code === "no_match" ||
      ...
      return "";
    if (code.indexOf("rate") >= 0 || type.indexOf("rate") >= 0)
      return assistantRuntimeText(activeLanguage, "rateLimited");
    return assistantRuntimeText(activeLanguage, "repeatCloser");
  }
  function sendRealtimeEvent(event: any) {
    var dc = realtimeChannelRef.current;
  ...
  function realtimeActionsFrom(value: any) {
    var allowed = [
      "open_section",
      "create_expense",
      "create_income",
    ...
    return (Array.isArray(value) ? value : [])
      .map(function (raw) {
        var a = { ...(raw || {}) },
          signal = [
            lastVoiceUserRequestRef.current,
            a.summary,
            a.description,
            a.projectName,
            a.holder,
            ...
            .toLowerCase();
        if (
          a.action === "create_expense" &&
          /(spesa condivisa|spesa share|share expense|shared expense|split expense|divid|nel progetto|progetto share)/i.test(
            signal
          )
        )
          a.action = "create_share_expense";
        if (
          a.action === "update_debt_credit" &&
          /(elimina|cancella|rimuovi|chiudi definitivamente|delete|remove|erase)/i.test(
            signal
          )
        ) {
          a.action = "delete_debt_credit";
          a.amount = null;
          a.initialAmount = null;
        }
        return a;
      })
      .filter(function (a) {
        return a && allowed.indexOf(String(a.action || "")) >= 0;
      })
      .slice(0, 16)
      .map(function (a) {
        return {
          ...
          shareParticipants: Array.isArray(a.shareParticipants)
            ? a.shareParticipants.slice(0, 50)
            : [],
        };
      });
  }
  function updatePendingActions(next: any[]) {
    pendingActionsRef.current = Array.isArray(next) ? next : [];
    setPendingActions(pendingActionsRef.current);
  }
  function sendRealtimeToolResult(
    callId: string,
    payload: any,
    createResponse?: boolean
  ) {
    var sent = sendRealtimeEvent({
    ...
    if (sent && createResponse !== false)
      setTimeout(function () {
        sendRealtimeEvent(
          buildRealtimeResponseCreate(currentVoiceLanguageResolution())
        );
      }, 20);
  }
  async function handleRealtimeFunctionCall(event: any) {
    var callId = String((event && event.call_id) || "");
    if (!callId || realtimeHandledCallsRef.current.has(callId)) return;
    realtimeHandledCallsRef.current.add(callId);
    var args: any = {};
    try {
      args = JSON.parse(String(event.arguments || "{}"));
    } catch (e) {
      sendRealtimeToolResult(callId, {
        status: "error",
        error: assistantRuntimeText(
          lastAssistantUserLanguageRef.current || lang,
          "invalidArguments"
        ),
      });
      return;
    ...
    if (name === "propose_fainance_actions") {
      var actions = realtimeActionsFrom(args.actions),
        opens = actions.filter(function (a) {
          return a.action === "open_section";
        }),
        writes = actions.filter(function (a) {
          return a.action !== "open_section";
        });
      if (writes.length) {
        sendRealtimeEvent({ type: "response.cancel" });
        sendRealtimeEvent({ type: "output_audio_buffer.clear" });
        var preamble = lastRealtimeAssistantTranscriptRef.current;
        lastRealtimeAssistantTranscriptRef.current = "";
        if (preamble)
          setAiChat(function (p) {
            var list = [...(p || [])];
            ...
            if (
              last &&
              last.role === "assistant" &&
              last.source === "voice-assistant" &&
              String(last.text || "").trim() === preamble
            )
              list.pop();
            return list;
          });
        ...
        setSpeaking(false);
        setBusy(false);
        updatePendingActions(writes);
        sendRealtimeToolResult(
          callId,
          {
            status: "confirmation_required",
            summaries: writes.map(actionSummary),
            uiConfirmation: true,
          },
          false
        );
    ...
    if (name === "confirm_pending_actions") {
      var pending = pendingActionsRef.current.slice();
      if (!pending.length) {
        sendRealtimeToolResult(callId, { status: "nothing_to_confirm" });
        return;
      }
      var results: any[] = [];
      try {
        pending.forEach(function (a) {
          var r = executeAction(a);
          if (r) results.push(r);
        });
    ...
    if (name === "cancel_pending_actions") {
      updatePendingActions([]);
      sendRealtimeToolResult(callId, { status: "cancelled" });
      return;
    }
    sendRealtimeToolResult(callId, {
      status: "error",
  ...
  function handleRealtimeEvent(raw: any) {
    var event: any = raw;
    try {
      if (typeof raw === "string") event = JSON.parse(raw);
    } catch (e) {
      return;
    }
    ...
    if (type === "session.created") {
      ...
      clearVoiceNotice();
      if (!realtimeManualResponseConfiguredRef.current) {
        realtimeManualResponseConfiguredRef.current = true;
        sendRealtimeEvent(buildRealtimeManualResponseSessionUpdate());
      }
      return;
    }
    ...
    if (type === "input_audio_buffer.speech_started") {
      clearVoiceNotice();
      realtimeUserDraftRef.current = "";
      setRealtimeUserDraft("");
    ...
    if (type === "conversation.item.input_audio_transcription.delta") {
      var ud = String(event.delta || "");
      realtimeUserDraftRef.current += ud;
      return;
    }
    if (type === "conversation.item.input_audio_transcription.completed") {
      var ut = String(
        event.transcript || realtimeUserDraftRef.current || ""
      ).trim();
      realtimeUserDraftRef.current = "";
      setRealtimeUserDraft("");
      clearVoiceNotice();
      if (ut) {
        var probs = Array.isArray(event.logprobs) ? event.logprobs : [];
        var avgLogProb = probs.length
          ? probs.reduce(function (sum, p) {
              var lp = Number(p && p.logprob);
              return sum + (Number.isFinite(lp) ? lp : -4);
            }, 0) / probs.length
          : null;
        var confidence =
          avgLogProb === null
            ? null
            : Math.exp(Math.max(-12, Math.min(0, avgLogProb)));
        var turnResolution: any = resolveVoiceAssistantTurn(ut);
        var turnLanguage = turnResolution.language;
        lastVoiceUserRequestRef.current = ut;
        appendVoiceMessage(ut, confidence);
        var languageControlReply =
          assistantLanguageControlReply(turnResolution);
        var localHelpAnswer = getFainanceHelpAnswer(ut, turnLanguage);
        var check = normalized(ut);
        var hearingCheck =
          /^(mi senti|mi stai sentendo|riesci a sentirmi|mi ascolti|ci sei|can you hear me|do you hear me|tu m'entends|vous m'entendez|me escuchas|puedes oirme|kannst du mich horen|consegues ouvir-me|slyszysz mnie|hoor je mij|ma auzi|με ακους)[?.! ]*$/.test(
            check
          );
        if (languageControlReply)
          sendRealtimeEvent(
            buildRealtimeResponseCreate(
              turnResolution,
              "This is a language control command. Reply exactly and only with this text: " +
                languageControlReply
            )
          );
        else if (localHelpAnswer)
          sendRealtimeEvent(
            buildRealtimeResponseCreate(
              turnResolution,
              "This is verified fAInance help. Reply exactly and only with the following text, without adding anything:\n" +
                localHelpAnswer
            )
          );
        else {
          ...
          if (hearingCheck)
            sendRealtimeEvent(
              buildRealtimeResponseCreate(
                turnResolution,
                "Reply exactly and only: " + assistantHearingReply(turnLanguage)
              )
            );
          else if (activeAttachmentRef.current) {
            sendRealtimeEvent({ type: "response.cancel" });
            sendRealtimeEvent({ type: "output_audio_buffer.clear" });
            callAssistant(ut, turnResolution);
          } else sendRealtimeEvent(buildRealtimeResponseCreate(turnResolution));
        }
      }
      return;
    }
    if (type === "response.output_audio_transcript.delta") {
      var ad = String(event.delta || "");
      realtimeAssistantDraftRef.current += ad;
      setRealtimeAssistantDraft(realtimeAssistantDraftRef.current);
      return;
    }
    if (type === "response.output_audio_transcript.done") {
      clearVoiceNotice();
      var at = String(
        event.transcript || realtimeAssistantDraftRef.current || ""
      ).trim();
      realtimeAssistantDraftRef.current = "";
      setRealtimeAssistantDraft("");
      if (at) {
        lastRealtimeAssistantTranscriptRef.current = at;
        appendMessage("assistant", at);
      }
      return;
    }
    ...
    if (type === "response.function_call_arguments.done") {
      handleRealtimeFunctionCall(event);
      return;
    }
    ...
    if (type === "error") {
      setBusy(false);
      var realtimeMessage = realtimeServerFailure(event.error || {});
      if (realtimeMessage)
        showTemporaryVoiceNotice(
          realtimeMessage,
          /^Non ho capito bene/i.test(realtimeMessage) ? 4200 : 6200
        );
      return;
    }
  }
  function realtimePermissionDeniedMessage() {
    return assistantRuntimeText(
      lastAssistantUserLanguageRef.current || lang,
      "permissionDenied"
    );
  }
  async function ensureRealtimeMicrophonePermission() {
    if (!nativePlatform()) return true;
    var mod: any = await import("@capgo/capacitor-speech-recognition");
    var speech: any = mod.SpeechRecognition || mod.default || mod;
    if (!speech) throw new Error(realtimePermissionDeniedMessage());
    var permission: any = speech.checkPermissions
      ? await speech.checkPermissions()
      : {};
    function granted(p: any) {
      var speechState = String((p && p.speechRecognition) || "").toLowerCase(),
        micState = String((p && p.microphone) || "").toLowerCase();
      return (
        speechState === "granted" && (micState === "" || micState === "granted")
      );
    }
    if (!granted(permission) && speech.requestPermissions)
      permission = await speech.requestPermissions();
    if (!granted(permission))
      throw new Error(realtimePermissionDeniedMessage());
    return true;
  }
  function disconnectRealtime(nextStatus?: string) {
    setNativeAssistantAudio(false);
    try {
      if (realtimeConnectionAbortRef.current)
    ...
    try {
      var pc = realtimePeerRef.current;
      if (pc) pc.close();
    } catch (e) {}
    realtimePeerRef.current = null;
    try {
      var stream = realtimeStreamRef.current;
      if (stream)
    ...
    try {
      var processed = microphoneProcessedStreamRef.current;
      if (processed)
        processed.getTracks().forEach(function (t) {
          t.stop();
        });
    } catch (e) {}
    microphoneProcessedStreamRef.current = null;
    try {
      if (microphoneProcessingTimerRef.current)
        clearInterval(microphoneProcessingTimerRef.current);
    } catch (e) {}
    microphoneProcessingTimerRef.current = null;
    try {
      var micSource = microphoneProcessingSourceRef.current;
      if (micSource) micSource.disconnect();
    } catch (e) {}
    microphoneProcessingSourceRef.current = null;
    try {
      var micGate = microphoneProcessingGateRef.current;
      if (micGate) micGate.disconnect();
    } catch (e) {}
    microphoneProcessingGateRef.current = null;
    microphoneProcessingDestinationRef.current = null;
    try {
      var micCtx = microphoneProcessingContextRef.current;
      if (micCtx && micCtx.state !== "closed")
        Promise.resolve(micCtx.close()).catch(function () {});
    } catch (e) {}
    ...
    realtimeAudioRef.current = null;
    try {
      var outSource = realtimeAudioSourceRef.current;
      if (outSource) outSource.disconnect();
    } catch (e) {}
    realtimeAudioSourceRef.current = null;
    try {
      var outGain = realtimeGainRef.current;
      if (outGain) outGain.disconnect();
    } catch (e) {}
    realtimeGainRef.current = null;
    try {
      var outCtx = realtimeAudioContextRef.current;
      if (outCtx && outCtx.state !== "closed")
        Promise.resolve(outCtx.close()).catch(function () {});
    } catch (e) {}
    realtimeAudioContextRef.current = null;
    realtimeAssistantDraftRef.current = "";
    realtimeUserDraftRef.current = "";
    realtimeManualResponseConfiguredRef.current = false;
    setRealtimeAssistantDraft("");
    setRealtimeUserDraft("");
    setRealtimeStatus(nextStatus || "idle");
  ...
  async function connectRealtime() {
    if (realtimeStatus === "connecting" || realtimeStatus === "connected")
      return;
    if (!(currentPlan === "base" || currentPlan === "premium")) {
      setVoiceError(
        (
          c.translateUiRuntimeText ||
          function (x) {
            return x;
          }
    ...
    setNativeAssistantAudio(true);
    var phase = "initialisation",
      statusCode = 0,
      serverMessage = "",
    ...
    try {
      ...
      if (contextForRealtime && contextForRealtime.transactions) {
        contextForRealtime.transactions = {
          expenses: (contextForRealtime.transactions.expenses || []).slice(
            0,
            80
          ),
          incomes: (contextForRealtime.transactions.incomes || []).slice(0, 50),
        };
      }
      var controller = new AbortController();
      realtimeConnectionAbortRef.current = controller;
      ...
      phase = "microphone";
      var mediaPromise = (async function () {
        await ensureRealtimeMicrophonePermission();
        try {
          var supported: any = navigator.mediaDevices.getSupportedConstraints
            ? navigator.mediaDevices.getSupportedConstraints()
            : {};
          var audioConstraints: any = {
          ...
          try {
            acquired.getAudioTracks().forEach(function (track: any) {
              ...
              try {
                ...
                Promise.resolve(track.applyConstraints(extra)).catch(
                  function () {}
                );
              } catch (e) {}
            });
          } catch (e) {}
          realtimeStreamRef.current = acquired;
          return await prepareNoiseFilteredMicrophone(acquired);
        } catch (mediaError) {
          ...
          if (
            mediaName.indexOf("notallowed") >= 0 ||
            mediaName.indexOf("permission") >= 0 ||
            mediaMessage.indexOf("permission") >= 0 ||
            mediaMessage.indexOf("denied") >= 0
          )
            throw new Error(realtimePermissionDeniedMessage());
          throw mediaError;
        }
      ...
      phase = "session";
      var sessionPromise = (async function () {
        var token = "";
        if (fbAuth.currentUser) token = await fbAuth.currentUser.getIdToken();
        var headers: any = { "Content-Type": "application/json" };
        if (token) headers.Authorization = "Bearer " + token;
        var realtimeSessionPayload = buildRealtimeSessionRequest({
          interfaceLanguage: lang || "it",
          languageState: assistantLanguageStateRef.current,
          aiDataAccess: aiDataAccess || "summary",
          financeContext: contextForRealtime,
          chatHistory: (aiChat || []).slice(-8).map(function (m) {
            return { role: m.role, text: m.rawText || m.text };
          }),
        });
        var response = await fetch(AI_REALTIME_ENDPOINT, {
          method: "POST",
          headers: headers,
        ...
        if (!response.ok || !payload.value) {
          ...
          serverMessage = String((payload && payload.error) || "");
          var err: any = new Error(
            serverMessage || "Errore sessione " + response.status
          );
      ...
      var peerPromise = (async function () {
        var stream = await mediaPromise;
        phase = "webrtc";
        ...
        pc.ontrack = function (e) {
          ...
          attachRealtimeAudioStream(remote).catch(function () {
            showTemporaryVoiceNotice(
              assistantRuntimeText(
                lastAssistantUserLanguageRef.current || lang,
                "audioFailed"
        ...
        realtimeHandledCallsRef.current = new Set();
        dc.onopen = function () {
        ...
        dc.onerror = function (ev: any) {
          setRealtimeStatus("error");
          showTemporaryVoiceNotice(
            realtimeFailureMessage(
              new Error(
                conciseTechnicalMessage(ev && ev.message) ||
                  "Data channel error"
              ),
              "datachannel"
            ),
        ...
        dc.onclose = function () {
          if (mountedRef.current && voiceModal)
            setRealtimeStatus(function (current) {
              return current === "error" ? current : "idle";
            });
        };
        var offer = await pc.createOffer();
        await pc.setLocalDescription(offer);
        return { pc: pc, offer: offer };
      })();
      var pair = await Promise.all([peerPromise, sessionPromise]),
        peerData: any = pair[0],
        tokenData: any = pair[1];
      phase = "webrtc";
      var sdpRes = await fetch("https://api.openai.com/v1/realtime/calls", {
        ...
        signal: controller.signal,
        body: peerData.offer.sdp || "",
      });
      if (!sdpRes.ok) {
        statusCode = sdpRes.status;
        serverMessage = await sdpRes.text();
        var sdpError: any = new Error(
          serverMessage || "Errore OpenAI " + sdpRes.status
        );
        sdpError.httpStatus = sdpRes.status;
        throw sdpError;
      }
      ...
      if (connectionTimer) clearTimeout(connectionTimer);
      statusCode = Number((e && e.httpStatus) || statusCode) || 0;
      serverMessage = String((e && e.serverMessage) || serverMessage || "");
      disconnectRealtime("error");
      showTemporaryVoiceNotice(
        realtimeFailureMessage(e, phase, statusCode, serverMessage),
        6200
      );
      setBusy(false);
    }
  ...
  function toggleRealtimeMicrophone() {
    ...
    try {
      var processed = microphoneProcessedStreamRef.current;
      if (processed)
        processed.getAudioTracks().forEach(function (t) {
          t.enabled = next;
        });
    } catch (e) {}
  ...
  function sendRealtimeText(q: string, providedResolution?: any) {
    var clean = String(q || "");
    var turnResolution: any =
      providedResolution || resolveVoiceAssistantTurn(clean);
    lastVoiceUserRequestRef.current = clean;
    if (
      !sendRealtimeEvent({
        type: "conversation.item.create",
        item: {
      ...
      return false;
    var languageControlReply = assistantLanguageControlReply(turnResolution);
    if (languageControlReply)
      sendRealtimeEvent(
        buildRealtimeResponseCreate(
          turnResolution,
  ...
  function stopSpeaking() {
    ...
    try {
      var audio = nativeAudioRef.current;
      if (audio) {
        audio.onended = null;
        audio.onerror = null;
        audio.pause();
        if (audio.src && String(audio.src).startsWith("blob:"))
    ...
    try {
      ...
      if (tts && tts.stop) Promise.resolve(tts.stop()).catch(function () {});
    } catch (e) {}
    try {
  ...
  function resumeListeningAfterSpeech(resume, cycle) {
    if (
      resume &&
      continuous &&
      voiceModal &&
      ...
      setTimeout(function () {
        if (cycle === speechCycleRef.current) startListening();
      }, 280);
  }
  async function speakFallback(clean, resume, cycle) {
    try {
      ...
      if (nativePlatform()) {
        var tts = nativeTtsRef.current;
        if (!tts) {
          var mod: any = await import("@capacitor-community/text-to-speech");
          tts = mod.TextToSpeech || mod.default || mod;
        ...
        await tts.speak({
          text: clean,
          lang: assistantSpeechLocale(speechLang),
          rate: 1.02,
          pitch: 1,
          volume: assistantVolumeRef.current,
          category: "ambient",
          queueStrategy: 0,
        });
        if (cycle !== speechCycleRef.current || !mountedRef.current) return;
        setSpeaking(false);
      ...
      var speechLang =
        lastAssistantUserLanguageRef.current || String(lang || "it");
      u.lang = assistantSpeechLocale(speechLang);
      u.rate = 1.02;
      u.pitch = 1;
      u.volume = assistantVolumeRef.current;
      u.onstart = function () {
        if (mountedRef.current && cycle === speechCycleRef.current)
          setSpeaking(true);
      };
      u.onend = function () {
  ...
  async function speak(text, resume) {
    ...
    if (!clean) return;
    stopSpeaking();
    var cycle = ++speechCycleRef.current;
    try {
      var token = "";
      if (fbAuth.currentUser) token = await fbAuth.currentUser.getIdToken();
      if (cycle !== speechCycleRef.current) return;
      ...
      var res = await fetch(AI_VOICE_ENDPOINT, {
        method: "POST",
        headers: headers,
        signal: ctrl.signal,
        body: JSON.stringify({
      ...
      if (cycle !== speechCycleRef.current) return;
      if (!res.ok) throw new Error("Natural voice unavailable");
      var blob = await res.blob();
      if (cycle !== speechCycleRef.current) return;
      if (!blob || !blob.size) throw new Error("Empty voice response");
      ...
      audio.onerror = function () {
        ...
        if (!mountedRef.current || cycle !== speechCycleRef.current) return;
        setSpeaking(false);
        speakFallback(clean, resume, cycle);
      };
      await audio.play();
    } catch (e) {
      speechAbortRef.current = null;
      if (!mountedRef.current || cycle !== speechCycleRef.current) return;
      setSpeaking(false);
      await speakFallback(clean, resume, cycle);
    }
  }
  ...
  async function finishNativeListening() {
    if (nativeSessionDoneRef.current) return;
    nativeSessionDoneRef.current = true;
    var q = String(nativeTextRef.current || "").trim();
    try {
      var p = nativeSpeechRef.current;
      if (p && p.getLastPartialResult) {
        var last = await p.getLastPartialResult();
        q = String(
          (last && last.text) ||
  ...
  function stopListening(submitPartial) {
    setVoiceListening(false);
    try {
      var p = nativeSpeechRef.current;
      if (p) {
        if (submitPartial && p.forceStop)
          Promise.resolve(p.forceStop({ timeout: 1200 }))
            .then(finishNativeListening)
            .catch(finishNativeListening);
        else if (p.stop) Promise.resolve(p.stop()).catch(function () {});
      }
    } catch (e) {}
    try {
  ...
  async function startNativeListening() {
    var mod: any = await import("@capgo/capacitor-speech-recognition");
    var SpeechRecognition: any = mod.SpeechRecognition || mod.default || mod;
    ...
    nativeSessionDoneRef.current = false;
    var available = SpeechRecognition.available
      ? await SpeechRecognition.available()
      : { available: true };
    if (available && available.available === false)
      throw new Error("SpeechRecognition unavailable");
    var permission = SpeechRecognition.checkPermissions
      ? await SpeechRecognition.checkPermissions()
      : {};
    var state = String(
      (permission && permission.speechRecognition) || ""
    ).toLowerCase();
    if (state !== "granted") {
      permission = SpeechRecognition.requestPermissions
    ...
    if (SpeechRecognition.addListener) {
      ...
      await SpeechRecognition.addListener(
        ...
        function (data: any) {
          ...
          if (stopped) setTimeout(finishNativeListening, 100);
        }
      );
      await SpeechRecognition.addListener("error", function (data: any) {
        var code = String((data && data.code) || "");
        if (code && code !== "no-match")
          setVoiceError(
            code === "permission-denied"
              ? V.unavailable
              : String((data && data.message) || V.unavailable)
          );
        setTimeout(finishNativeListening, 80);
      });
    }
    var recognitionLocale = String(
      (typeof navigator !== "undefined" && navigator.language) ||
        assistantSpeechLocale(lang || "it")
    );
    ...
    if (matches[0]) {
      nativeTextRef.current = String(matches[0]);
      setInput(String(matches[0]));
    }
  }
  function startWebListening() {
    ...
    rec.onerror = function (ev: any) {
      ...
      if (ev && ev.error !== "no-speech" && ev.error !== "aborted")
        setVoiceError(V.unavailable);
    };
    rec.onend = function () {
      webRecognitionRef.current = null;
      finishListening(last);
    };
    rec.start();
  }
  async function startListening() {
    if (busy || aiLoading || voiceListening) return;
    stopSpeaking();
    setVoiceError("");
    setInput("");
    setVoiceListening(true);
    try {
      if (nativePlatform()) await startNativeListening();
  ...
  function compressFinanceImage(dataUrl: string) {
  ...
  function readFinanceFile(file: any) {
    return new Promise<string>(function (resolve, reject) {
      if (!file) {
        reject(new Error("Nessuna foto selezionata."));
        return;
      }
      if (file.size && file.size > 18 * 1024 * 1024) {
        reject(new Error("La foto supera 18 MB. Riducila e riprova."));
        return;
      }
      var reader = new FileReader();
      reader.onload = function () {
        compressFinanceImage(String(reader.result || ""))
          .then(resolve)
          .catch(reject);
      };
  ...
  function readFinanceDocumentFile(file: any) {
    return new Promise<any>(function (resolve, reject) {
      if (!file) {
        reject(new Error("Nessun documento selezionato."));
        return;
      }
      if (file.size && file.size > 12 * 1024 * 1024) {
        reject(new Error("Il documento supera 12 MB."));
        return;
      }
      var allowed =
        /\.(pdf|doc|docx|rtf|txt|csv|xls|xlsx|json|xml|ods|odt)$/i.test(
          String(file.name || "")
        ) ||
        /pdf|word|officedocument|spreadsheet|excel|csv|text|json|xml|rtf|opendocument/i.test(
          String(file.type || "")
        );
      if (!allowed) {
        reject(
          new Error(
            "Formato non supportato. Usa PDF, Word, Excel, CSV o un file di testo."
          )
        );
        return;
      }
      ...
      reader.onerror = function () {
        reject(new Error("Non riesco a leggere il documento selezionato."));
      };
      reader.readAsDataURL(file);
    });
  }
  async function analyzeFinanceDocument(
    fileDataUrl: string,
    fileName?: string,
    fileMimeType?: string,
    isImage?: boolean
    ...
    if (canUsePlanFeature && !canUsePlanFeature("receiptScan", 1)) {
      setVoiceError(
        upgradeMessage
          ? upgradeMessage("receiptScan")
          : "Hai raggiunto il limite di scansione documenti."
      );
      return;
    }
    activeAttachmentRef.current = {
      dataUrl: fileDataUrl,
      name: fileName || "documento",
      mimeType:
        fileMimeType ||
        (isImage !== false ? "image/jpeg" : "application/octet-stream"),
      isImage: isImage !== false,
      createdAt: Date.now(),
    };
    var wasMicEnabled = realtimeMicEnabled,
      stream = realtimeStreamRef.current;
    if (stream && wasMicEnabled)
      try {
        stream.getAudioTracks().forEach(function (t) {
          t.enabled = false;
        });
    ...
    try {
      ...
      if (token) headers.Authorization = "Bearer " + token;
      var ctrl = new AbortController();
      assistantRequestAbortRef.current = ctrl;
      var timer = setTimeout(function () {
        ctrl.abort();
      }, 70000);
      var documentLanguage =
        lastAssistantUserLanguageRef.current || String(lang || "it");
      var documentRequest = buildAssistantRequestPayload({
        question: "Read and interpret the attached document.",
        forcedLanguage: documentLanguage,
        ...
        chatHistory: (aiChat || [])
          ...
          .slice(-8)
          .map(function (m) {
            return { role: "user", text: m.rawText || m.text };
          }),
      });
      var payload: any = documentRequest.payload;
      payload.instruction +=
        " Read the attached content directly. If it contains an expense, receipt, bill or invoice, do not return only a summary: prepare a create_expense proposal ready for confirmation, using only amount, document date, description, suggested category and default payment method. Do not add technical notes, tax data, document number, address, VAT number, taxable amount or VAT. If it contains multiple income or transaction rows, extract all rows and propose a separate action for each. Never say that you cannot read the attached file.";
      if (isImage !== false) {
        payload.imageDataUrl = fileDataUrl;
        payload.imageName = fileName || "documento.jpg";
      } else {
        payload.fileDataUrl = fileDataUrl;
        payload.fileName = fileName || "documento";
        payload.fileMimeType = fileMimeType || "application/octet-stream";
      }
      var res = await fetch(AI_AGENT_ENDPOINT, {
        method: "POST",
        headers: headers,
        ...
        clearTimeout(timer);
        if (assistantRequestAbortRef.current === ctrl)
      ...
      if (!res.ok)
        throw Object.assign(
          new Error((data && data.error) || "Errore " + res.status),
          { httpStatus: res.status }
        );
      var answer = String((data && data.answer) || "").trim(),
        actions = realtimeActionsFrom((data && data.actions) || []),
        opens = actions.filter(function (a) {
          return a.action === "open_section";
        }),
        writes = actions
          ...
          .map(function (a) {
            if (a.action === "create_expense" || a.action === "create_income") {
              return { ...a, note: "" };
            }
            return a;
          });
      if (consumePlanFeature) {
        consumePlanFeature("receiptScan", 1);
        consumePlanFeature("aiReply", 1);
      }
      if (writes.length) {
        updatePendingActions(writes);
      } else {
      ...
      var msg = realtimeFailureMessage(
        e,
        "document",
        Number((e && e.httpStatus) || 0)
      );
      setVoiceError(msg);
  ...
  async function takeFinancePhoto() {
    setAttachmentMenuOpen(false);
    try {
      if (nativePlatform()) {
        var mod: any = await import("@capacitor/camera");
        var photo = await mod.Camera.getPhoto({
          quality: 84,
          allowEditing: false,
          ...
          promptLabelHeader: "Documento",
          promptLabelPhoto: "Scatta foto",
          promptLabelPicture: "Scatta foto",
        });
        if (photo && photo.dataUrl) {
          var compressed = await compressFinanceImage(String(photo.dataUrl));
          await analyzeFinanceDocument(
            compressed,
            "foto_" + Date.now() + ".jpg",
            "image/jpeg",
            true
      ...
      if (!/cancel|cancell|user/i.test(raw))
        setVoiceError("Non riesco ad aprire la fotocamera.");
    }
  }
  async function chooseFinancePhoto() {
    setAttachmentMenuOpen(false);
    try {
      if (nativePlatform()) {
        var mod: any = await import("@capacitor/camera");
        var photo = await mod.Camera.getPhoto({
          ...
          promptLabelPhoto: "Carica foto",
          promptLabelPicture: "Carica foto",
        });
        if (photo && photo.dataUrl) {
          var compressed = await compressFinanceImage(String(photo.dataUrl));
          await analyzeFinanceDocument(
      ...
      var raw = String((e && e.message) || "");
      if (!/cancel|cancell|user/i.test(raw))
        setVoiceError("Non riesco a caricare la foto.");
    }
  }
  function chooseFinanceDocument() {
    setAttachmentMenuOpen(false);
    if (documentInputRef.current) documentInputRef.current.click();
  }
  async function onFinanceFileSelected(event: any) {
    var file =
      event && event.target && event.target.files && event.target.files[0];
    try {
      if (file) {
        var dataUrl = await readFinanceFile(file);
        await analyzeFinanceDocument(
          dataUrl,
          file.name || "documento.jpg",
      ...
      setVoiceError(
        String((e && e.message) || "Non riesco a leggere la foto.")
      );
    } finally {
      try {
        event.target.value = "";
  ...
  async function onFinanceDocumentSelected(event: any) {
    var file =
      event && event.target && event.target.files && event.target.files[0];
    try {
      if (file) {
        var data = await readFinanceDocumentFile(file);
        await analyzeFinanceDocument(
          data.dataUrl,
          data.name,
          data.mimeType,
      ...
      setVoiceError(
        String((e && e.message) || "Non riesco a leggere il documento.")
      );
    } finally {
      try {
        event.target.value = "";
  ...
  function monthKey() {
    return String(curMonthKey || todayStr().slice(0, 7));
  }
  function debtBalance(d) {
    var total = Number(d && d.initialAmount) || 0;
    ((d && d.transactions) || []).forEach(function (t) {
      var n = Number(t.amount) || 0;
      total += t.action === "increase" ? n : -n;
    });
  ...
  function buildContext() {
    ...
    var ctx: any = {
      ...
      today: todayStr(),
      language: lang || "it",
      dataAccessLevel: aiDataAccess || "summary",
    ...
    if (aiDataAccess === "areas" || aiDataAccess === "full") {
      ...
      ctx.topExpenseCategories = Object.keys(byCat)
        .map(function (k) {
          return { name: k, amount: byCat[k] };
        })
        .sort(function (a, b) {
  ...
  function validISODate(value, allowEmpty) {
    var raw = String(value || "").trim();
    if (!raw) return allowEmpty ? "" : todayStr();
    if (!/^\d{4}-\d{2}-\d{2}$/.test(raw)) throw new Error("Data non valida.");
  ...
  function findNamed(list, name, fallbackId) {
    var active = (list || []).filter(function (x) {
        return !x.archived;
      }),
      n = normalized(name);
    if (n) {
      var exact = active.find(function (x) {
        return normalized(x.name) === n;
      });
      if (exact) return exact;
      var fuzzy = active.filter(function (x) {
        var xn = normalized(x.name);
        return xn && (xn.indexOf(n) >= 0 || n.indexOf(xn) >= 0);
      });
      return fuzzy.length === 1 ? fuzzy[0] : null;
    }
    var fallback = active.find(function (x) {
      return fallbackId !== undefined && String(x.id) === String(fallbackId);
    });
    return fallback || active[0] || null;
  }
  function findUniqueByName(list, name, key) {
    var n = normalized(name),
      field = key || "name";
    if (!n) return null;
    var exact = (list || []).find(function (x) {
      return normalized(x[field]) === n;
    });
    if (exact) return exact;
    var fuzzy = (list || []).filter(function (x) {
      var xn = normalized(x[field]);
      return xn && (xn.indexOf(n) >= 0 || n.indexOf(xn) >= 0);
    });
    return fuzzy.length === 1 ? fuzzy[0] : null;
  }
  function shareRound(value) {
    return Math.round((Number(value) || 0) * 100) / 100;
  }
  function shareParticipantLabel(p) {
    return String(
      (p && (p.name || p.displayName || p.email || p.phone)) || ""
    ).trim();
  }
  function resolveShareProject(name) {
    var active = (shareProjects || []).filter(function (x) {
      return x && !x.archived;
    });
    var requested = String(name || "").trim();
    if (requested) {
      var found = findUniqueByName(active, requested, "name");
      if (!found) throw new Error("Progetto Share non trovato o nome ambiguo.");
      return found;
    }
    if (active.length === 1) return active[0];
    throw new Error(
      active.length
        ? "Specifica il progetto Share."
        : "Non ci sono progetti Share disponibili."
    );
  }
  function activeShareParticipants(project) {
    return ((project && project.participants) || []).filter(function (p) {
      return p && String(p.status || "active") !== "archived";
    });
  }
  function resolveShareParticipant(participants, name, allowCurrent) {
    var requested = String(name || "").trim(),
      current = (participants || []).find(function (p) {
        return (
          String(p.uid || "") === String(userId || "") ||
          String(p.id || "") === "me"
        );
    ...
    if (requested) {
      ...
      if (
        ...
        return current;
      var exact = (participants || []).find(function (p) {
        return normalized(shareParticipantLabel(p)) === n;
      });
      if (exact) return exact;
      var fuzzy = (participants || []).filter(function (p) {
        var pn = normalized(shareParticipantLabel(p));
        return pn && (pn.indexOf(n) >= 0 || n.indexOf(pn) >= 0);
      });
      if (fuzzy.length === 1) return fuzzy[0];
      throw new Error(
        "Partecipante Share non trovato o nome ambiguo: " + requested
      );
    }
    if (allowCurrent && current) return current;
    return null;
  }
  function buildShareSplits(total, participants, specs, mode) {
    var amount = shareRound(total),
      selected: any[] = [];
    (specs || []).forEach(function (spec) {
      var p = resolveShareParticipant(participants, spec && spec.name, false);
      if (
        p &&
        !selected.some(function (x) {
          return String(x.p.id) === String(p.id);
        })
      )
        selected.push({ p: p, spec: spec });
    });
    if (!selected.length)
      selected = (participants || []).map(function (p) {
        return { p: p, spec: {} };
      });
    if (!selected.length)
      throw new Error("Il progetto Share non ha partecipanti attivi.");
    var shares: any = {},
      current = resolveShareParticipant(participants, "", true);
    var splitMode = mode === "amount" || mode === "percent" ? mode : "equal";
    if (splitMode === "amount") {
      var sum = 0,
        missing: any[] = [];
      selected.forEach(function (x) {
        var raw = x.spec && x.spec.amount;
        if (raw === null || raw === undefined || raw === "") {
          missing.push(x);
          return;
        }
        var n = shareRound(raw);
        if (!(n >= 0))
          throw new Error(
            "Quota Share non valida per " + shareParticipantLabel(x.p) + "."
          );
        shares[String(x.p.id)] = n;
        sum = shareRound(sum + n);
      });
      if (
        !missing.length &&
        sum < amount - 0.01 &&
        current &&
        !selected.some(function (x) {
          return String(x.p.id) === String(current.id);
        })
      ) {
        selected.push({ p: current, spec: {} });
        missing.push(selected[selected.length - 1]);
      }
      if (missing.length === 1) {
        var remainder = shareRound(amount - sum);
        if (remainder < 0)
          throw new Error("Le quote Share superano l’importo totale.");
        shares[String(missing[0].p.id)] = remainder;
        sum = shareRound(sum + remainder);
      } else if (missing.length > 1)
        throw new Error(
          "Specifica una sola quota mancante oppure tutte le quote Share."
        );
      if (Math.abs(sum - amount) > 0.01)
        throw new Error("Le quote Share devono sommare all’importo totale.");
      return { shares: shares, mode: "amount" };
    }
    if (splitMode === "percent") {
      var pctSum = 0,
        missingPct: any[] = [];
      selected.forEach(function (x) {
        var raw = x.spec && x.spec.percent;
        if (raw === null || raw === undefined || raw === "") {
          missingPct.push(x);
          return;
        }
        ...
        if (!Number.isFinite(pct) || pct < 0)
          throw new Error(
            "Percentuale Share non valida per " +
              shareParticipantLabel(x.p) +
              "."
          );
        x.pct = pct;
        pctSum += pct;
      ...
      if (
        ...
        var added = { p: current, spec: {}, pct: 100 - pctSum };
        selected.push(added);
        missingPct.push(added);
      }
      if (missingPct.length === 1) {
        missingPct[0].pct = 100 - pctSum;
        pctSum = 100;
      } else if (missingPct.length > 1)
        throw new Error(
          "Specifica una sola percentuale mancante oppure tutte le percentuali Share."
        );
      if (Math.abs(pctSum - 100) > 0.01)
        throw new Error("Le percentuali Share devono sommare a 100.");
      var assigned = 0;
      selected.forEach(function (x, i) {
        var pct =
            Number(x.pct !== undefined ? x.pct : x.spec && x.spec.percent) || 0,
          n =
            i === selected.length - 1
              ? shareRound(amount - assigned)
              : shareRound((amount * pct) / 100);
        shares[String(x.p.id)] = n;
        assigned = shareRound(assigned + n);
      });
      return { shares: shares, mode: "percent" };
    }
    var base = Math.floor((amount / selected.length) * 100) / 100,
      assigned2 = 0;
    selected.forEach(function (x, i) {
      var n = i === selected.length - 1 ? shareRound(amount - assigned2) : base;
      shares[String(x.p.id)] = n;
      assigned2 = shareRound(assigned2 + n);
    });
    return { shares: shares, mode: "equal" };
  }
  function resolveArea(name) {
    var n = normalized(name);
    return (
      (shoppingAreas || []).find(function (x) {
        return normalized(x) === n;
      }) ||
      (shoppingAreas || []).find(function (x) {
        return (
          n && (normalized(x).indexOf(n) >= 0 || n.indexOf(normalized(x)) >= 0)
        );
      }) ||
      shoppingDefaultArea ||
      (shoppingAreas || [])[0] ||
  ...
  function configuredShoppingUnits() {
    var source =
      Array.isArray(shoppingUnits) && shoppingUnits.length
        ? shoppingUnits
        : ["Grammi", "Litri", "Unità", "Altro"];
    var out: any[] = [];
    source.forEach(function (unit) {
      ...
      if (
        clean &&
        !out.some(function (x) {
          return normalized(x) === normalized(clean);
        })
      )
        out.push(clean);
    });
    return out.length ? out : ["Unità"];
  }
  function findConfiguredShoppingUnit(value) {
    var n = normalized(value);
    if (!n) return null;
    return (
      configuredShoppingUnits().find(function (unit) {
        return normalized(unit) === n;
      }) || null
    );
  }
  function resolveShoppingUnit(value, catalogValue) {
    var units = configuredShoppingUnits(),
      requested = findConfiguredShoppingUnit(value);
    if (requested) return requested;
    var n = normalized(value),
      aliases: any = {
        g: ["grammi", "grammo", "g"],
        gr: ["grammi", "grammo", "g"],
        grammo: ["grammi", "grammo", "g"],
        grammi: ["grammi", "grammo", "g"],
        l: ["litri", "litro", "l"],
        litro: ["litri", "litro", "l"],
        litri: ["litri", "litro", "l"],
        pz: ["unita", "pezzi", "pezzo", "pz", "pcs"],
        pezzo: ["unita", "pezzi", "pezzo", "pz", "pcs"],
        pezzi: ["unita", "pezzi", "pezzo", "pz", "pcs"],
        pcs: ["unita", "pezzi", "pezzo", "pz", "pcs"],
        unit: ["unita", "pezzi", "pezzo", "pz", "pcs"],
        unita: ["unita", "pezzi", "pezzo", "pz", "pcs"],
      };
    var aliasList = aliases[n] || [];
    for (var i = 0; i < aliasList.length; i++) {
      var aliasUnit = units.find(function (unit) {
        return normalized(unit) === aliasList[i];
      });
      if (aliasUnit) return aliasUnit;
    }
    var catalog = findConfiguredShoppingUnit(catalogValue);
    if (catalog) return catalog;
    var def = findConfiguredShoppingUnit(shoppingDefaultUnit);
    if (def) return def;
    return (
      units.find(function (unit) {
        return normalized(unit) === "unita";
      }) ||
      units[0] ||
      "Unità"
    );
  }
  function explicitShoppingUnitCreationRequested() {
    var raw = String(lastVoiceUserRequestRef.current || "")
      .toLowerCase()
      .normalize("NFD")
      .replace(/[\u0300-\u036f]/g, "");
    var hasUnit =
      /(unita\s+di\s+misura|measurement\s+unit|unit\s+of\s+measure|unidad\s+de\s+medida|unite\s+de\s+mesure|masseinheit|unidade\s+de\s+medida|jednostk[a-z]*\s+miar|meeteenheid|unitate\s+de\s+masura)/i.test(
        raw
      );
    var hasCreate =
      /(crea|creare|aggiungi|aggiungere|nuov[oa]|create|add|new|crear|anad|ajout|creer|hinzuf|neu|criar|adicion|utworz|dodaj|maak|toevoeg|creeaz|adaug)/i.test(
        raw
      );
    return hasUnit && hasCreate;
  }
  function createShoppingUnitExplicit(value) {
    var clean = String(value || "").trim();
    if (!clean) throw new Error("Nome dell’unità di misura non valido.");
    var existing = findConfiguredShoppingUnit(clean);
    if (existing) return existing;
    if (!explicitShoppingUnitCreationRequested())
      throw new Error(
        "Una nuova unità di misura può essere creata solo su richiesta esplicita."
      );
    setShoppingUnits(function (current) {
      var base =
        Array.isArray(current) && current.length
          ? current
          : configuredShoppingUnits();
      if (
        base.some(function (unit) {
          return normalized(unit) === normalized(clean);
        })
      )
        return base;
      return base.concat([clean]);
    });
    if (!String(shoppingDefaultUnit || "").trim() && setShoppingDefaultUnit)
      setShoppingDefaultUnit(clean);
    return clean;
  }
  function actionSummary(a) {
    if (a && a.action === "create_expense") {
      ...
      return [
        "Importo: € " +
          Number(a.amount || 0).toLocaleString("it-IT", {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2,
          }),
        "Data: " + String(a.date || todayStr()),
        "Descrizione: " + String(a.description || "Spesa"),
        "Categoria: " + expenseCat,
        "Metodo: " + expenseMethod,
        a.note ? "Dettagli: " + String(a.note) : "",
      ]
        .filter(Boolean)
        .join("\n");
    }
    if (a && a.action === "create_income") {
      ...
      return [
        ...
        "Data: " + String(a.date || todayStr()),
        "Descrizione: " + String(a.description || "Entrata"),
        "Tipo: " + incomeType,
        a.note ? "Dettagli: " + String(a.note) : "",
      ]
        .filter(Boolean)
    ...
    var m: any = {
      create_recurring: "Ricorrente",
      create_goal: "Obiettivo",
      update_goal_saved: "Aggiornamento obiettivo",
      create_alert: "Alert",
      set_category_budget: "Budget",
      create_debt_credit: "Debito / Credito",
      update_debt_credit: "Transazione Debito / Credito",
      delete_debt_credit: "Elimina Debito / Credito",
      set_patrimonio_value: "Patrimonio",
      create_note: "Appunto",
      add_shopping_items: "Lista della spesa",
      create_shopping_list: "Nuova lista",
      create_shopping_unit: "Nuova unità di misura",
      create_share_project: "Progetto Share",
      create_share_expense: "Spesa Share",
      create_share_settlement: "Saldo Share",
      create_expense_category: "Categoria",
      create_payment_method: "Metodo",
      set_setting: "Impostazione",
    };
    return (
      (m[a.action] || a.action || "Operazione") +
      (a.entityName ||
      a.goalName ||
      a.alertName ||
      a.holder ||
  ...
  function addShoppingRecords(specs, listId) {
    var items = (specs || []).filter(function (x) {
      return String(x.name || "").trim();
    });
    if (!items.length) throw new Error("Nessun prodotto valido.");
    var activeCount = (shoppingItems || []).filter(function (x) {
      return !x.archived && String(x.listId || "main") === String(listId);
    }).length;
    var lim =
      PLAN_LIMITS && PLAN_LIMITS[currentPlan]
        ? PLAN_LIMITS[currentPlan].shoppingListItems
        : Infinity;
    if (lim !== Infinity && activeCount + items.length > Number(lim))
      throw new Error(
        upgradeMessage
          ? upgradeMessage("shoppingListItems")
          : "Limite della lista della spesa raggiunto."
      );
    setShoppingItems(function (source) {
      var next = Array.isArray(source) ? source.slice() : [],
        added: any[] = [];
      items.forEach(function (spec, idx) {
        var name = String(spec.name || "").trim(),
          area = resolveArea(spec.area),
          catalog = next.find(function (x) {
            return (
              x.archived &&
              normalized(x.name) === normalized(name) &&
              normalized(x.area) === normalized(area)
            );
          }),
          catalogId = catalog
            ? String(catalog.productId || catalog.id)
            : "prod_" +
              Date.now() +
              "_" +
              idx +
          ...
          unit = resolveShoppingUnit(spec.unit, catalog && catalog.unit);
        if (!catalog)
          next.push({
            id: catalogId,
            productId: catalogId,
        ...
        added.push({
          ...
          bought: false,
          archived: false,
          listId: String(listId || "main"),
          order: Date.now() + idx,
          createdAt: new Date().toISOString(),
          catalogOnly: false,
        });
      });
      return added.concat(next);
    });
  }
  function executeAction(a) {
    var shareSignal = [a && a.summary, a && a.description, a && a.projectName]
      .join(" ")
      .toLowerCase();
    if (
      a &&
      ...
      return executeAction({ ...a, action: "create_share_expense" });
    if (a.action === "create_expense") {
      var amount = Number(a.amount) || 0;
      if (amount <= 0) throw new Error("Importo non valido.");
      var cat = findNamed(cats, a.categoryName, defaultExpenseCat),
        method = findNamed(methods, a.methodName, defaultExpenseMethod);
      if (!cat) throw new Error("Categoria non trovata o ambigua.");
      if (!method)
        throw new Error("Metodo di pagamento non trovato o ambiguo.");
      var ok = addExpenses(
        [
          {
            id: Date.now() + Math.random(),
            ...
            desc: a.description || cat.name,
            date: validISODate(a.date, false),
            rateizzato: !!a.rateizzato,
            rate: Math.max(1, Number(a.rate) || 1),
          },
        ],
        "assistant"
      );
      if (ok === false)
        throw new Error("Non è stato possibile aggiungere l’uscita.");
      return actionSummary(a);
    }
    if (a.action === "create_income") {
      var amount2 = Number(a.amount) || 0;
      if (amount2 <= 0) throw new Error("Importo non valido.");
      var it = findNamed(incomeTypes, a.incomeTypeName, defaultIncomeType);
      if (!it) throw new Error("Tipo di entrata non trovato o ambiguo.");
      var ok2 = addIncomes(
        [
          {
            id: Date.now() + Math.random(),
      ...
      if (ok2 === false)
        throw new Error("Non è stato possibile aggiungere l’entrata.");
      return actionSummary(a);
    }
    if (a.action === "create_recurring") {
      var ar = Number(a.amount) || 0,
        nameR = String(a.recurringName || a.description || "").trim(),
        rtype = a.recurringType === "income" ? "income" : "expense";
      if (ar <= 0 || !nameR)
        throw new Error("Nome o importo della ricorrente non valido.");
      var rc = findNamed(cats, a.categoryName, defaultExpenseCat),
        rm = findNamed(methods, a.methodName, defaultExpenseMethod),
        ri = findNamed(incomeTypes, a.incomeTypeName, defaultIncomeType);
      if (rtype === "expense" && (!rc || !rm))
        throw new Error("Categoria o metodo della ricorrente non valido.");
      if (rtype === "income" && !ri)
        throw new Error("Tipo di entrata della ricorrente non valido.");
      setRecurring(function (p) {
        return [
          {
            id: Date.now() + Math.random(),
            name: nameR,
            icon: a.icon || (rtype === "income" ? "💰" : "🔁"),
            rtype: rtype,
            amount: ar,
            catId: rc ? rc.id : null,
            methodId: rm ? rm.id : null,
            incomeType: ri ? ri.id : null,
            dayOfMonth: Math.max(0, Math.min(31, Number(a.dayOfMonth) || 1)),
            annualMonth: Math.max(
              1,
              Math.min(12, Number(a.annualMonth) || new Date().getMonth() + 1)
            ),
            rateizzato: !!a.rateizzato,
            rate: Math.max(1, Number(a.rate) || 1),
            frequency: a.frequency === "annual" ? "annual" : "monthly",
            confirmed: [],
            skipped: [],
          },
        ].concat(p || []);
    ...
    if (a.action === "create_goal") {
      var target = Number(a.target) || 0,
        goalName = String(a.goalName || "").trim();
      if (!goalName || target <= 0)
        throw new Error("Nome o importo dell’obiettivo non valido.");
      if (findUniqueByName(goals, goalName, "name"))
        throw new Error("Esiste già un obiettivo con questo nome.");
      if (canAddPlanItem && !canAddPlanItem("goals", (goals || []).length, 1))
        throw new Error(
          upgradeMessage
            ? upgradeMessage("goals")
      ...
      setGoals(function (p) {
        return [
          ...(p || []),
          {
            ...
            name: goalName,
            target: target,
            saved: Math.max(0, Number(a.saved) || 0),
            deadline: validISODate(a.deadline, true),
            period: a.period === "monthly" ? "monthly" : "annual",
            icon: a.icon || "🎯",
            color: a.color || "#7F77DD",
          },
        ];
      });
      return actionSummary(a);
    }
    if (a.action === "update_goal_saved") {
      var goal = findUniqueByName(goals, a.goalName, "name"),
        delta = Number(a.savedDelta) || 0;
      if (!goal) throw new Error("Obiettivo non trovato o nome ambiguo.");
      if (!delta) throw new Error("Importo non valido.");
      setGoals(function (p) {
        return (p || []).map(function (g) {
          return String(g.id) === String(goal.id)
    ...
    if (a.action === "create_alert") {
      var ba = Number(a.budgetAmount) || Number(a.amount) || 0;
      if (ba <= 0) throw new Error("Budget dell’alert non valido.");
      var isGroup = a.alertScope === "group",
        targetA = isGroup
          ? findNamed(expenseGroups, a.alertTargetName, null)
          : findNamed(cats, a.alertTargetName, defaultExpenseCat);
      if (!targetA) throw new Error("Categoria o area dell’alert non trovata.");
      setAlerts(function (p) {
        return [
          {
            id: Date.now() + Math.random(),
            name: String(a.alertName || "Alert " + targetA.name).trim(),
            type: isGroup ? "group" : "cat",
            catId: isGroup ? null : targetA.id,
            groupId: isGroup ? targetA.id : null,
            budget: ba,
            triggerMode: a.triggerMode === "pct" ? "pct" : "immediate",
            triggerPct: Math.max(0, Math.min(200, Number(a.triggerPct) || 0)),
            customText: a.customText || "",
            period: a.period === "annual" ? "annual" : "monthly",
          },
        ].concat(p || []);
      });
      return actionSummary(a);
    }
    if (a.action === "set_category_budget") {
      var bc = Number(a.budgetAmount) || Number(a.amount) || 0,
        catB = findNamed(
          cats,
          a.categoryName || a.alertTargetName,
          defaultExpenseCat
        );
      if (!catB || bc <= 0)
        throw new Error("Categoria o importo del budget non valido.");
      setBudgetPlan(function (prev) {
        var base =
            prev && typeof prev === "object" ? prev : DEFAULT_BUDGET_PLAN,
          items = Array.isArray(base.items) ? base.items.slice() : [];
        var found = false;
        items = items.map(function (x) {
          if (String(x.catId) === String(catB.id)) {
            found = true;
            return { ...x, amount: bc };
          }
          return x;
        });
        if (!found) items.push({ catId: catB.id, amount: bc, pct: 0 });
        return { ...base, items: items };
      });
      return actionSummary(a);
    }
    if (a.action === "create_debt_credit") {
      var da = Number(a.initialAmount) || Number(a.amount) || 0,
        holder = String(a.holder || a.entityName || "").trim();
      if (da <= 0 || !holder)
        throw new Error("Persona o importo del Debito / Credito non valido.");
      if (
        canAddPlanItem &&
        !canAddPlanItem("debtCredits", (debtCredits || []).length, 1)
      )
        throw new Error(
          upgradeMessage
            ? upgradeMessage("debtCredits")
            : "Limite Debiti / Crediti raggiunto."
        );
      setDebtCredits(function (p) {
        return [
          {
            id: "dc_" + Date.now(),
            kind: a.debtKind === "credit" ? "credit" : "debt",
            holder: holder,
            initialAmount: da,
            startDate: validISODate(a.startDate || a.date, false),
            estimatedEndDate: validISODate(a.endDate, true),
    ...
    if (a.action === "update_debt_credit") {
      var debt = findUniqueByName(
          debtCredits,
          a.holder || a.entityName,
          "holder"
        ),
        txa = Number(a.amount) || 0;
      if (!debt)
        throw new Error("Debito / Credito non trovato o nome ambiguo.");
      if (txa <= 0) throw new Error("Importo non valido.");
      var tx = {
        id: "tx_" + Date.now(),
        action: a.transactionMode === "increase" ? "increase" : "reduction",
        amount: txa,
        date: validISODate(a.date, false),
        startDate: debt.startDate || todayStr(),
        estimatedEndDate: a.endDate || debt.estimatedEndDate || "",
        note: a.note || a.description || "",
        createdAt: new Date().toISOString(),
        updatedAt: new Date().toISOString(),
      };
      setDebtCredits(function (p) {
        return (p || []).map(function (x) {
          return String(x.id) === String(debt.id)
            ? {
                ...x,
                transactions: [tx].concat(x.transactions || []),
                updatedAt: new Date().toISOString(),
              }
            : x;
        });
    ...
    if (a.action === "delete_debt_credit") {
      var debtToDelete = findUniqueByName(
        debtCredits,
        a.holder || a.entityName,
        "holder"
      );
      ...
      setDebtCredits(function (p) {
        return (p || []).filter(function (x) {
          return String(x.id) !== String(debtToDelete.id);
        });
      });
      a.holder = debtToDelete.holder;
      return actionSummary(a);
    }
    if (a.action === "set_patrimonio_value") {
      var value = Number(a.value);
      if (!Number.isFinite(value))
        throw new Error("Valore del patrimonio non valido.");
      var asset = findUniqueByName(
          patrimonioEntries,
          a.assetName || a.entityName,
          "name"
        ),
        assetId = asset && asset.id;
      if (!asset) {
        var area =
          findNamed(patrimonioAreas, a.assetGroup, null) ||
          (patrimonioAreas || [])[0];
        assetId = "asset_" + Date.now();
        setPatrimonioEntries(function (p) {
          return [
            ...(p || []),
            {
              id: assetId,
              name: String(a.assetName || a.entityName || "Patrimonio"),
              icon: a.icon || "📦",
              areaId: area ? area.id : "altro",
            },
          ];
        });
      }
      setPatrimonioValues(function (prev) {
        var next = { ...(prev || {}) },
          mk = monthKey();
        next[mk] = { ...(next[mk] || {}), [String(assetId)]: value };
        return next;
      });
      return actionSummary(a);
    }
    if (a.action === "create_note") {
      var title = String(a.noteTitle || a.entityName || "").trim(),
        text = String(a.noteText || a.note || a.description || "").trim();
      if (!title && !text) throw new Error("Appunto vuoto.");
    ...
    if (a.action === "add_shopping_items") {
      var requested = String(a.listTitle || "").trim(),
        targetList = requested
          ? findUniqueByName(shoppingLists, requested, "title")
          : (shoppingLists || []).find(function (x) {
              return String(x.id) === String(activeShoppingListId);
            }) ||
            (shoppingLists || [])[0] || { id: "main" };
      if (!targetList) throw new Error("Lista della spesa non trovata.");
      addShoppingRecords(a.items || [], targetList.id);
      return actionSummary(a);
    }
    if (a.action === "create_shopping_unit") {
      var createdUnit = createShoppingUnitExplicit(
        a.unitName || a.entityName || a.settingValue
      );
      a.entityName = createdUnit;
      return actionSummary(a);
    }
    if (a.action === "create_shopping_list") {
      var title = String(a.listTitle || a.entityName || "").trim();
      if (!title) throw new Error("Titolo della lista non valido.");
      var existing = findUniqueByName(shoppingLists, title, "title"),
        listId = existing
          ? existing.id
          : "list_" + Date.now() + "_" + Math.floor(Math.random() * 9999);
      if (!existing) {
        var lim =
          PLAN_LIMITS && PLAN_LIMITS[currentPlan]
            ? PLAN_LIMITS[currentPlan].shoppingLists
            : Infinity;
        if (lim !== Infinity && (shoppingLists || []).length + 1 > Number(lim))
          throw new Error(
            upgradeMessage
              ? upgradeMessage("shoppingLists")
              : "Limite liste della spesa raggiunto."
          );
        setShoppingLists(function (p) {
          return [
            ...(p || []),
            {
              id: listId,
              title: title,
              icon: a.listIcon || a.icon || "🧺",
              createdAt: new Date().toISOString(),
            },
          ];
        });
      }
      setActiveShoppingListId(String(listId));
      if ((a.items || []).length) addShoppingRecords(a.items, listId);
      return actionSummary(a);
    }
    if (a.action === "create_share_project") {
      var pn = String(a.projectName || a.entityName || "").trim();
      if (!pn) throw new Error("Nome del progetto Share non valido.");
      var project = createShareProject(
        pn,
        a.projectDescription || a.description || "",
        a.projectIcon || a.icon || "🤝",
        a.projectColor || a.color || "#4F8FF7"
      );
      if (!project)
        throw new Error("Non è stato possibile creare il progetto Share.");
      return actionSummary(a);
    }
    if (a.action === "create_share_expense") {
      var shareAmount = Number(a.amount) || 0;
      if (shareAmount <= 0)
        throw new Error("Importo della spesa Share non valido.");
      var shareProject = resolveShareProject(a.projectName),
        participants = activeShareParticipants(shareProject);
      if (!participants.length)
        throw new Error("Il progetto Share non ha partecipanti attivi.");
      var payer =
        resolveShareParticipant(participants, a.paidByName, true) ||
        participants[0];
      var split = buildShareSplits(
        shareAmount,
        participants,
        a.shareParticipants || [],
        a.splitMode
      );
      var shareToday = (shareProject.activities || []).filter(function (x) {
        return (
          x &&
          x.kind !== "settlement" &&
          String(x.createdAt || "").slice(0, 10) === todayStr()
        );
      }).length;
      if (
        canAddPlanItem &&
        !canAddPlanItem("shareDailyExpenses", shareToday, 1)
      )
        throw new Error(
          upgradeMessage
            ? upgradeMessage("shareDailyExpenses", shareToday)
            : "Limite giornaliero Share raggiunto."
        );
      var activity = {
        id: Date.now() + Math.random(),
        kind: "expense",
        amount: shareRound(shareAmount),
        desc:
          String(a.description || "Spesa condivisa").trim() ||
          "Spesa condivisa",
        paidBy: String(payer.id),
        date: validISODate(a.date, false),
        time: new Date().toTimeString().slice(0, 5),
        shares: split.shares,
        splitMode: split.mode,
        sharedWith: Object.keys(split.shares),
        createdAt: new Date().toISOString(),
        updatedAt: new Date().toISOString(),
      };
      if (!updateShareProject)
        throw new Error("Aggiornamento del progetto Share non disponibile.");
      updateShareProject(shareProject.id, function (p) {
        return { ...p, activities: [activity].concat(p.activities || []) };
      });
      if (consumePlanFeature) consumePlanFeature("shareDailyExpenses", 1);
      a.projectName = shareProject.name;
      return actionSummary(a);
    }
    if (a.action === "create_share_settlement") {
      var settlementAmount = Number(a.amount) || 0;
      if (settlementAmount <= 0)
        throw new Error("Importo del saldo Share non valido.");
      var settlementProject = resolveShareProject(a.projectName),
        settlementParticipants = activeShareParticipants(settlementProject);
      var fromP = resolveShareParticipant(
          settlementParticipants,
          a.fromParticipantName,
          true
        ),
        toP = resolveShareParticipant(
          settlementParticipants,
          a.toParticipantName,
          false
        );
      if (!fromP || !toP)
        throw new Error("Indica chi paga e chi riceve il saldo Share.");
      if (String(fromP.id) === String(toP.id))
        throw new Error("Il saldo Share richiede due partecipanti diversi.");
      var settlement = {
        id: Date.now() + Math.random(),
        kind: "settlement",
        amount: shareRound(settlementAmount),
        from: String(fromP.id),
        to: String(toP.id),
        date: validISODate(a.date, false),
        time: new Date().toTimeString().slice(0, 5),
        desc: a.description || "Saldo tra partecipanti",
        createdAt: new Date().toISOString(),
      };
      if (!updateShareProject)
        throw new Error("Aggiornamento del progetto Share non disponibile.");
      updateShareProject(settlementProject.id, function (p) {
        return { ...p, activities: [settlement].concat(p.activities || []) };
      });
      a.projectName = settlementProject.name;
      return actionSummary(a);
    }
    if (a.action === "create_expense_category") {
      var cn = String(a.entityName || a.categoryName || "").trim();
      if (!cn) throw new Error("Nome della categoria non valido.");
      if (findUniqueByName(cats, cn, "name"))
        throw new Error("La categoria esiste già.");
      var cg =
          findNamed(expenseGroups, a.categoryGroup, null) ||
          (expenseGroups || [])[0],
        maxId =
          (cats || []).reduce(function (m, x) {
            var n = Number(x.id);
    ...
    if (a.action === "create_payment_method") {
      var mn = String(a.entityName || a.methodName || "").trim();
      if (!mn) throw new Error("Nome del metodo non valido.");
      if (findUniqueByName(methods, mn, "name"))
        throw new Error("Il metodo esiste già.");
      var mg =
          findNamed(methodGroups, a.methodGroup, null) ||
          (methodGroups || [])[0],
        maxM =
          (methods || []).reduce(function (m, x) {
            var n = Number(x.id);
            return Number.isFinite(n) ? Math.max(m, n) : m;
          }, 0) + 1;
      setMethods(function (p) {
        return [
          ...(p || []),
          {
            id: maxM,
            name: mn,
            icon: a.icon || "💳",
            group: mg ? mg.id : "altro",
            color: a.color || "#378ADD",
            archived: false,
          },
        ];
      });
    ...
    if (a.action === "set_setting") {
      var sn = normalized(a.settingName),
        sv = String(a.settingValue || "").trim(),
        nv = normalized(sv),
        compact = sn.replace(/ /g, "");
      var truth = booleanValue(sv, true);
      if (
        ["appearance", "theme", "modalita scura", "aspetto"].indexOf(sn) >= 0
      ) {
        setBgTheme(
          nv.indexOf("dark") >= 0 || nv.indexOf("scur") >= 0
            ? "dark"
            : nv.indexOf("slate") >= 0
            ? "slate"
            : "default"
        ...
        ["button style", "button_style", "stile pulsanti"].indexOf(sn) >= 0
      ) {
        setBtnStyle(
          ["rounded", "soft", "square", "sharp"].indexOf(nv) >= 0
            ? nv
            : nv.indexOf("quadr") >= 0
            ? "square"
            : "soft"
        ...
        compact === "topbar" ||
        sn.indexOf("barra superiore") >= 0 ||
        sn.indexOf("riepilogo in alto") >= 0 ||
        sn.indexOf("intestazione home") >= 0
      ) {
        setShowAppSummaryHeader(truth);
      } else if (
        compact === "bottombaricons" ||
        sn.indexOf("numero icone") >= 0 ||
        sn.indexOf("icone barra inferiore") >= 0
      ) {
        setMobileNavIconCount(Math.max(3, Math.min(7, Number(sv) || 5)));
      } else if (
        compact === "homebalanceview" ||
        sn.indexOf("saldo home") >= 0 ||
        sn.indexOf("visualizzazione home") >= 0
      ) {
        setHomeBalanceView(nv.indexOf("real") >= 0 ? "reale" : "rateizzato");
      } else if (
        compact === "statisticsview" ||
        (sn.indexOf("statistiche") >= 0 && sn.indexOf("second") < 0)
      ) {
        setStatsView(nv.indexOf("real") >= 0 ? "reale" : "rateizzato");
      } else if (compact === "dateformat" || sn.indexOf("formato data") >= 0) {
        setDateFmt(
          nv.indexOf("mdy") >= 0
            ? "mdy"
            : nv.indexOf("ymd") >= 0
        ...
        compact === "firstdayofweek" ||
        sn.indexOf("primo giorno") >= 0
      ) {
        setFirstDayOfWeek(
          nv.indexOf("sun") >= 0 || nv.indexOf("domen") >= 0 ? "sun" : "mon"
        );
      } else if (
        compact === "defaultexpensecategory" ||
        sn.indexOf("categoria predefinita") >= 0
      ) {
        var dc = findNamed(cats, sv, null);
        if (!dc) throw new Error("Categoria predefinita non trovata.");
        setDefaultExpenseCat(String(dc.id));
      } else if (
        compact === "defaultpaymentmethod" ||
        sn.indexOf("metodo predefinito") >= 0
      ) {
        var dm = findNamed(methods, sv, null);
        if (!dm) throw new Error("Metodo predefinito non trovato.");
        setDefaultExpenseMethod(String(dm.id));
      } else if (
        compact === "defaultincometype" ||
        sn.indexOf("tipo entrata predefinito") >= 0
      ) {
        var di = findNamed(incomeTypes, sv, null);
        if (!di) throw new Error("Tipo di entrata predefinito non trovato.");
        setDefaultIncomeType(String(di.id));
      } else if (
        compact === "secondarycurrencyhistory" ||
        sn.indexOf("seconda valuta storico") >= 0
      ) {
        setShowSecInHistory(truth);
      } else if (
        compact === "secondarycurrencystatistics" ||
        sn.indexOf("seconda valuta statistiche") >= 0
      ) {
        setShowSecInStats(truth);
      } else if (
        compact === "secondarycurrencybudget" ||
        sn.indexOf("seconda valuta budget") >= 0
      ) {
        setShowSecInBudget(truth);
      } else if (
        compact === "secondarycurrencypatrimonio" ||
        sn.indexOf("seconda valuta patrimonio") >= 0
      ) {
        setShowSecInPatrimonio(truth);
      } else if (
        compact === "historyfuturemode" ||
        sn.indexOf("movimenti futuri") >= 0
      ) {
        setHistoryFutureMode(
          nv.indexOf("future") >= 0 || nv.indexOf("futur") >= 0
            ? "all"
            : "untilToday"
        );
      } else if (
        compact === "historysortdate" ||
        sn.indexOf("ordinamento storico data") >= 0
      ) {
        setHistorySortDate(nv.indexOf("creaz") >= 0 ? "createdAt" : "date");
      } else if (
        compact === "historysortdirection" ||
        sn.indexOf("direzione storico") >= 0
      ) {
        setHistorySortDirection(
          nv.indexOf("asc") >= 0 || nv.indexOf("crescent") >= 0 ? "asc" : "desc"
        );
      } else if (
        compact === "historysecondarysort" ||
        sn.indexOf("secondo ordinamento storico") >= 0
      ) {
        setHistorySortSecondary(
          nv.indexOf("descr") >= 0
            ? "description"
            : nv.indexOf("categor") >= 0
            ? "category"
            : "amount"
        );
      } else if (
        compact === "historysecondarydirection" ||
        sn.indexOf("direzione secondo ordinamento") >= 0
      ) {
        setHistorySortSecondaryDirection(
          nv.indexOf("asc") >= 0 || nv.indexOf("crescent") >= 0 ? "asc" : "desc"
        );
      } else if (
        compact === "showshareinhistory" ||
        sn.indexOf("share nello storico") >= 0
      ) {
        setShowShareInHistory(truth);
      } else if (
        compact === "showdebtcreditsinpatrimonio" ||
        (sn.indexOf("debiti") >= 0 && sn.indexOf("patrimonio") >= 0)
      ) {
        setShowDebtCreditsInPatrimonio(truth);
      } else if (
        compact === "showdebtcreditsinexpenses" ||
        (sn.indexOf("debiti") >= 0 &&
          (sn.indexOf("uscite") >= 0 || sn.indexOf("spese") >= 0))
      ) {
        setShowDebtCreditsInExpenses(truth);
      } else if (
        compact === "shoppingproductsort" ||
        sn.indexOf("ordinamento prodotti") >= 0
      ) {
        setShoppingProductSort(
          nv.indexOf("alpha") >= 0 || nv.indexOf("alfabet") >= 0
            ? "alphabetical"
            : "custom"
        );
      } else if (
        compact === "shoppingunit" ||
        compact === "shoppingmeasurementunit" ||
        sn.indexOf("unita di misura") >= 0
      ) {
        createShoppingUnitExplicit(sv);
      } else if (
        compact === "shoppingdefaultarea" ||
        sn.indexOf("area predefinita spesa") >= 0
      ) {
        setShoppingDefaultArea(resolveArea(sv));
      } else if (
        compact === "patrimoniomode" ||
        sn.indexOf("modalita patrimonio") >= 0
      ) {
        setPatrimonioMode(nv.indexOf("auto") >= 0 ? "automatica" : "manuale");
      } else if (
        compact === "aidataaccess" ||
        sn.indexOf("accesso dati ai") >= 0
      ) {
        setAiDataAccess(
          nv.indexOf("full") >= 0 || nv.indexOf("complet") >= 0
            ? "full"
            : nv.indexOf("area") >= 0
            ? "areas"
            : "summary"
        ...
        compact === "aifloatingbutton" ||
        sn.indexOf("pulsante ai") >= 0
      ) {
        setAiFloatingEnabled(truth);
      } else if (
        compact === "confirmbuttoncolor" ||
        sn.indexOf("colore conferma") >= 0
      ) {
        if (!/^#[0-9a-f]{6}$/i.test(sv))
          throw new Error("Usa un colore esadecimale, per esempio #378ADD.");
        setConfirmButtonColor(sv);
      } else if (
        compact === "secondarybuttoncolor" ||
        sn.indexOf("colore secondario") >= 0
      ) {
        if (!/^#[0-9a-f]{6}$/i.test(sv))
          throw new Error("Usa un colore esadecimale, per esempio #5FAFE5.");
        setSecondaryButtonColor(sv);
      } else
        throw new Error(
          "Questa impostazione è sensibile o non è modificabile dall’assistente."
        );
      return actionSummary(a);
    }
    return "";
  ...
  function openSection(a) {
    var sec = normalized(a.section || "").replace(/ /g, "");
    var map: any = {
      home: "home",
      expenses: "spese",
      expense: "spese",
    ...
    if (target === "spese") {
      setSpeseSubTab(
        sec === "recurring" || sec === "ricorrenti" ? "recurring" : "add"
      );
      setAddType(
        sec === "incomes" || sec === "income" || sec === "entrate"
          ? "income"
          : "expense"
      );
      setAddSubTab("single");
    }
    if (target === "history") setHistoryTab("expenses");
    setVoiceModal(false);
  }
  function isYes(q) {
    return /^(si|sì|ok|okay|confermo|conferma|vai|procedi|yes|confirm|dale|vale|oui|ja|sim|tak|da|ναι)\b/i.test(
      String(q || "").trim()
    );
  }
  function isNo(q) {
    return /^(no|annulla|annullo|cancel|cancelar|non|nein|não|nie|nu|όχι)\b/i.test(
      String(q || "").trim()
    );
  }
  function confirmPending(userText) {
    if (String(userText || "").trim()) appendMessage("user", userText);
    try {
      pendingActions.forEach(function (a) {
        executeAction(a);
      });
      updatePendingActions([]);
      var msg = assistantRuntimeText(
        lastAssistantUserLanguageRef.current || lang,
        "done"
      );
      appendMessage("assistant", msg);
  ...
  function cancelPending(userText) {
    appendMessage("user", userText || V.cancel);
    updatePendingActions([]);
    var msg = assistantRuntimeText(
      lastAssistantUserLanguageRef.current || lang,
      "cancelled"
  ...
  async function callAssistant(q, providedResolution?: any) {
    ...
    try {
      ...
      var assistantRequest = buildAssistantRequestPayload({
        question: q,
        languageResolution: providedResolution || undefined,
        languageState: assistantLanguageStateRef.current,
        fallbackLanguage:
          lastAssistantUserLanguageRef.current || String(lang || "it"),
        interfaceLanguage: lang || "it",
      ...
      var assistantLang = assistantRequest.language;
      lastAssistantUserLanguageRef.current = assistantLang;
      if (assistantRequest.languageState)
        assistantLanguageStateRef.current = assistantRequest.languageState;
      var requestPayload: any = assistantRequest.payload;
      requestPayload.instruction =
        String(requestPayload.instruction || "") +
        " Shopping-list rule: when adding products, use only financeContext.catalogs.shoppingUnits. Never invent or create a measurement unit as a side effect of adding products; if the requested unit is unavailable, use the product’s configured unit or financeContext.defaults.shoppingUnit. Create a new measurement unit only when the user explicitly asks to create one.";
      var verifiedHelpAnswer = getFainanceHelpAnswer(q, assistantLang);
      if (verifiedHelpAnswer) {
        appendMessage("assistant", verifiedHelpAnswer);
        speak(verifiedHelpAnswer, true);
        return;
      }
      var attachment = activeAttachmentRef.current;
      if (attachment) {
        if (attachment.isImage) {
          requestPayload.imageDataUrl = attachment.dataUrl;
          requestPayload.imageName = attachment.name || "allegato.jpg";
        } else {
          requestPayload.fileDataUrl = attachment.dataUrl;
          requestPayload.fileName = attachment.name || "documento";
          requestPayload.fileMimeType =
            attachment.mimeType || "application/octet-stream";
        }
        requestPayload.question =
          "[TURN_LANGUAGE=" +
          assistantLang +
          "] The following user request also refers to the attached file, which must be read directly. User request: " +
          q;
      }
      var res = await fetch(AI_AGENT_ENDPOINT, {
        method: "POST",
      ...
      if (!res.ok) {
        var assistantHttpError: any = new Error(
          (data && data.error) ||
            "Il servizio AI ha restituito l’errore " + res.status + "."
        );
        assistantHttpError.httpStatus = res.status;
        throw assistantHttpError;
      }
      var answer =
        String((data && data.answer) || "").trim() ||
        assistantRuntimeText(assistantLang, "noAnswer");
      if (assistantAnswerNeedsTranslation(answer, assistantLang)) {
        try {
          var translateResponse = await fetch(AI_AGENT_ENDPOINT, {
            method: "POST",
            headers: headers,
            body: JSON.stringify(
              buildAssistantTranslationPayload(answer, assistantLang)
            ),
          });
          var translateData: any = await translateResponse.json();
          var corrected = String(
            (translateData && translateData.answer) || ""
          ).trim();
          if (corrected) answer = corrected;
        } catch (e) {}
      }
      answer = compactAssistantAnswer(answer, assistantLang, 1400);
      var actions = realtimeActionsFrom(data && data.actions).filter(function (
          a
        ) {
          return a && a.action && a.action !== "none";
        }),
        openActions = actions.filter(function (a) {
          return a.action === "open_section";
        }),
        writeActions = actions.filter(function (a) {
          return a.action !== "open_section";
        });
      if (writeActions.length) {
        updatePendingActions(writeActions);
        return;
      }
      appendMessage("assistant", answer);
      if (openActions.length) {
        setToast({ text: answer, type: "success", icon: "✨" });
        openSection(openActions[0]);
        return;
      }
      speak(answer, true);
    } catch (e) {
  ...
  function sendMessage(forced) {
    var q = String(forced !== undefined ? forced : input || "").trim();
    if (!q || busy || aiLoading) return;
    setInput("");
    stopListening(false);
    if (pendingActions.length && isYes(q)) {
      confirmPending(q);
      return;
    }
    if (pendingActions.length && isNo(q)) {
      cancelPending(q);
      return;
    }
    var turnResolution: any = resolveVoiceAssistantTurn(q);
    var languageControlReply = assistantLanguageControlReply(turnResolution);
    var localHelpAnswer = getFainanceHelpAnswer(q, turnResolution.language);
    if (localHelpAnswer) {
      appendMessage("user", q);
      appendMessage("assistant", localHelpAnswer);
      speak(localHelpAnswer, true);
      return;
    }
    if (realtimeStatus === "connected") {
      appendMessage("user", q);
      if (!languageControlReply && consumePlanFeature)
        consumePlanFeature("aiReply", 1);
      if (!sendRealtimeText(q, turnResolution))
        setVoiceError(
          assistantRuntimeText(turnResolution.language, "notReady")
        );
      return;
    }
    if (languageControlReply) {
      appendMessage("user", q);
      appendMessage("assistant", languageControlReply);
      speak(languageControlReply, true);
      return;
    }
    if (!aiExternalConsent) {
      setVoiceError(V.consentTitle);
    ...
    function run() {
      appendMessage("user", q);
      if (consumePlanFeature) consumePlanFeature("aiReply", 1);
      callAssistant(q, turnResolution);
    }
    if (activeAttachmentRef.current) {
      if (handleRewardedFeature) {
        handleRewardedFeature("aiReply", 1, run);
        return;
      }
      if (canUsePlanFeature && !canUsePlanFeature("aiReply", 1)) {
        setToast({
    ...
    >
      <div
        ...
        style={{
          width: "100%",
          maxWidth: 760,
          height: isMobile ? "auto" : "min(820px,94vh)",
          flex: isMobile ? 1 : undefined,
          minHeight: 0,
          background: dark ? "#171724" : "#F8F9FF",
          border: isMobile ? "none" : "1px solid " + borderC,
          ...
          <>
            ...
            >
              {messages.length === 0 && (
                ...
                >
                  <AIGrilloIcon size={76} />
                  <div style={{ marginTop: 12 }}>{V.empty}</div>
                </div>
              )}
            ...
            >
              ...
              <textarea
                ...
                placeholder={
                  realtimeStatus === "connected"
                    ? "Parla liberamente oppure scrivi qui…"
                    : V.placeholder
                }
                style={{
                  ...sinp,
          ...
          var errorHandle = await nativeSpeech.addListener(
            ...
            function (data: any) {
              ...
              if (
                ...
                setVoiceError(
                  String((data && data.message) || L("Errore riconoscimento vocale nativo"))
                );
              }
              setTimeout(function () {
                void finalizeQuickSpeech(runId, false);
