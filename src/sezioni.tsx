...
export function HomePanel(){
  ...
  function normalizeWorklets(list,allowDefault){var arr=Array.isArray(list)?list:[];if(!arr.length&&allowDefault)arr=HOME_DEFAULT_WORKLETS;var seen:any={};return arr.map(function(w){return {...(w||{}),type:String((w&&w.type)||"")==="button_ai"?"button_ai_advice":(w&&w.type)};}).filter(function(w){return w&&WORKLET_LABELS[w.type]&&!seen[w.type]&&(seen[w.type]=true);}).map(function(w,i){var p=w.params||{};return {...w,id:w.id||("home_w_"+i+"_"+w.type),size:w.size==="0.5x"?"0.5x":(w.size==="1x"?"1x":"2x"),color:w.color||"#FFFFFF",params:{showTitle:p.showTitle!==false,customTitle:String(p.customTitle||""),...defaultParams(w.type),...p}};});}
  ...
  function availableItems(){var m=insertedTypes();return HOME_LIBRARY.filter(function(x){return !m[x.type];});}
  ...
  function workletCard(w,index,available){
    var bg=workletColor(w),fg=textOnBg(bg),size=String(w.size||"2x"),compact=size==="0.5x",buttonType=isHomeNavButton(w.type);
    var spanUnits=buttonType?(size==="0.5x"?1:(size==="1x"?2:4)):(compact?2:(isMobile?4:(size==="2x"?4:2)));
    var span="span "+spanUnits;
    var title=cardTitle(w),badge=compact||buttonType?"":rangeBadge(w),isEditing=homeEditMode&&premiumHome,showHeader=available||(!buttonType&&homeShowTitle(w));
    ...
    function homeWorkletBaseHeight(type,size,countN){
      ...
      if(type==="distribution_expenses"||type==="expenses_by_area"||type==="expenses_by_category"||type==="incomes_by_type")return 166;
    ...
    var targetH=available?118:(buttonType?(size==="2x"?112:72):((size==="2x"&&doubleHeightTypes.indexOf(w.type)>=0)?(normalH*2):normalH));
    if(!available&&!buttonType&&!homeShowTitle(w)){
      ...
      else if(w.type==="distribution_expenses"||w.type==="expenses_by_area"||w.type==="expenses_by_category"||w.type==="incomes_by_type")targetH=compact?164:(size==="2x"?318:154);
    ...
    return <div key={(available?"available_":"active_")+w.id} data-home-worklet-index={available?undefined:index} draggable={false} onPointerDown={function(e){startPointerDrag(e,payload);}} onPointerMove={movePointerDrag} onPointerUp={endPointerDrag} onPointerCancel={cancelHomePointerDrag} onTouchStart={function(e){startTouchDrag(e,payload);}} onTouchMove={moveTouchDrag} onTouchEnd={endTouchDrag} onTouchCancel={cancelTouchDrag} onContextMenu={function(e){if(isEditing){try{e.preventDefault();}catch(_e){}}}} onDragStart={function(e){try{e.preventDefault();}catch(_e){}}} onDragOver={function(e){if(isEditing){try{e.preventDefault();}catch(_e){}}}} onDrop={function(e){if(isEditing&&!available)dropAt(e,index);}} onClick={function(){if(isEditing&&available)openLibrarySettings(w.type);}} style={common as any}>
      <div style={headerStyle as any}><div style={titleStyle}>{title}</div>{badge&&<div style={{fontSize:11,color:available?fg:subC,fontWeight:800,opacity:available?.8:1,whiteSpace:"nowrap",flexShrink:0}}>{badge}</div>}</div>
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:14}}>
    ...
    {homeEditMode&&premiumHome&&available.length===0&&<div style={{fontSize:12,color:subC,textAlign:"center",padding:"8px 0"}}>{L("Tutti i worklet disponibili sono già presenti nella Home.")}</div>}
...
var {normalizeEmail,loadShareCollaboration,acceptShareInvite,declineShareInvite,createShareInvite,createShareProject,updateShareProject,deleteShareProject,canAddPlanItem}:any=_c;
  ...
  var [aiExternalConsent,setAiExternalConsent]=useState(function(){try{return localStorage.getItem(userKey("ai_external_consent_v1"))==="accepted";}catch(e){return false;}});
  ...
  function acceptAIExternalConsent(){
    ...
    try{localStorage.setItem(userKey("ai_external_consent_v1"),"accepted");}catch(e){}
    setAiExternalConsent(true);
