...
export function HistoryPanel(){
  ...
  function modalBase(){return {position:"fixed",zIndex:9999,inset:0,background:dark?"#171724":"#F7F8FF",color:textC,overflowY:"auto",WebkitOverflowScrolling:"touch",overscrollBehavior:"contain",boxSizing:"border-box",padding:isMobile?"max(calc(env(safe-area-inset-top, 0px) + 18px), calc(var(--fainance-native-safe-top, 0px) + 18px), 54px) 16px max(calc(env(safe-area-inset-bottom, 0px) + 26px), calc(var(--fainance-native-safe-bottom, 0px) + 26px), 30px)":"28px max(24px,calc((100vw - 760px)/2)) 34px"};}
...
var {normalizeEmail,loadShareCollaboration,acceptShareInvite,declineShareInvite,createShareInvite,createShareProject,updateShareProject,deleteShareProject,canAddPlanItem}:any=_c;
  ...
  return <div style={{position:"fixed",right:right,bottom:bottom,zIndex:250,width:buttonWidth,height:buttonHeight,overflow:"visible",pointerEvents:"auto"}}>
    <button type="button" aria-label="Nascondi icona AI" onPointerDown={function(e){e.stopPropagation();}} onPointerUp={function(e){e.stopPropagation();}} onClick={function(e){e.preventDefault();e.stopPropagation();setAiFloatingEnabled(false);}} title="Nascondi icona AI" style={{position:"absolute",left:-9,top:-9,zIndex:4,width:44,height:44,border:"none",background:"transparent",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",padding:0,touchAction:"manipulation",WebkitTapHighlightColor:"transparent"}}><span aria-hidden="true" style={{width:26,height:26,borderRadius:"50%",border:"1px solid rgba(0,0,0,0.14)",background:"rgba(255,255,255,0.96)",color:"#D92D20",fontSize:17,fontWeight:950,lineHeight:1,boxShadow:"0 3px 12px rgba(0,0,0,0.22)",display:"flex",alignItems:"center",justifyContent:"center"}}>×</span></button>
    <button type="button" data-fainance-no-tap-assist="true" onPointerDown={startDrag} onPointerMove={applyDrag} onPointerUp={endDrag} onPointerCancel={endDrag} title="Grillo parlante AI" style={{width:buttonWidth,height:buttonHeight,borderRadius:0,border:"none",background:"transparent",padding:0,color:"#fff",boxShadow:"none",fontSize:desktop?27:25,cursor:dragState?"grabbing":"grab",display:"flex",alignItems:"center",justifyContent:"center",touchAction:"none",userSelect:"none",overflow:"visible",WebkitTapHighlightColor:"transparent"}}><img src={aiGrilloMascot} alt="Consulente AI" draggable={false} style={{width:"100%",height:"100%",display:"block",objectFit:"contain",pointerEvents:"none",background:"transparent",transform:"none"}}/></button>
  </div>;
}
