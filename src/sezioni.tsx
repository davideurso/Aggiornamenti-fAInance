...
import { Btn, Badge, Toggle, StatCard, Toast, DonutChart, BarChart, LineChart,
  SortableRows, EmojiPicker, AppIconSelector, AppColorSelector, DatePickerField, GoalsPanel, AlertsPanel,
  AIGrilloIcon, FAInanceLogo, EditModal, PatrimonioSettingsPanel, AreasEditor,
  ExpenseForm, BulkEntry, ReceiptScanPanel, RecurringManager, PopupCloseButton,
} from './widget';
...
export function HomePanel(){
  ...
  var HOME_LIBRARY=[
    ...
    {type:"upcoming_scheduled_expenses",icon:"🗓️",label:H("Prossime spese programmate"),desc:H("Mostra le prossime uscite ricorrenti programmate")},
    ...
    {type:"button_ai_chat",icon:"💬",label:H("Conversa con l'Assistente"),desc:H("Bottone rapido per aprire l'assistente completo")},
  ...
  var HOME_BASE_SCHEDULED_WORKLET={id:"home_upcoming_scheduled_expenses",type:"upcoming_scheduled_expenses",size:"1x",color:"#FFFFFF",params:{showTitle:true}};
  ...
  function recurringAnnualMonth(r){var raw=(r&&r.annualMonth!=null)?r.annualMonth:((r&&r.monthOfYear!=null)?r.monthOfYear:((r&&r.month!=null)?r.month:r&&r.dayOfMonth));var n=parseInt(raw,10);if(isNaN(n)||n<1||n>12)n=1;return n;}
  function recurringDueDay(r){var annualLegacy=String(r&&r.frequency||"monthly")==="annual"&&r&&r.annualMonth==null&&r.month==null&&r.monthOfYear==null;var raw=annualLegacy?1:(r&&r.dayOfMonth);var n=parseInt(raw,10);if(isNaN(n))n=1;if(n<0)n=1;if(n>31)n=31;return n;}
  function recurringNextDate(r){var today=new Date();today.setHours(0,0,0,0);var freq=String(r&&r.frequency||"monthly");var handledConfirmed=(r&&Array.isArray(r.confirmed))?r.confirmed:[];var handledSkipped=(r&&Array.isArray(r.skipped))?r.skipped:[];for(var off=0;off<30;off++){var first=new Date(today.getFullYear(),today.getMonth()+off,1);var y=first.getFullYear(),m=first.getMonth();if(freq==="annual"&&recurringAnnualMonth(r)!==(m+1))continue;var mk=String(y)+"-"+String(m+1).padStart(2,"0");if(handledConfirmed.indexOf(mk)>=0||handledSkipped.indexOf(mk)>=0)continue;var rawDay=recurringDueDay(r);var lastDay=new Date(y,m+1,0).getDate();var day=rawDay===0?lastDay:Math.min(lastDay,Math.max(1,rawDay));var due=new Date(y,m,day);due.setHours(0,0,0,0);if(due>=today)return due;}return null;}
  function upcomingScheduledExpenses(){return (recurring||[]).filter(function(r){return String(r&&r.rtype||"expense")!=="income";}).map(function(r){return {item:r,date:recurringNextDate(r)};}).filter(function(x){return !!x.date;}).sort(function(a,b){return a.date.getTime()-b.date.getTime();});}
  function recurringDateLabel(d){if(!d)return "";var month=monthShortName?monthShortName(d.getMonth()):MONTHS_SHORT[d.getMonth()];return String(d.getDate())+" "+String(month||"");}
  ...
  function safeHomeNoteHtml(value){return String(value||"").replace(/<\s*(script|style|iframe|object|embed|link|meta)\b[^>]*>[\s\S]*?<\/\s*\1\s*>/gi,"").replace(/<\s*(script|style|iframe|object|embed|link|meta)\b[^>]*\/?>/gi,"").replace(/\son[a-z]+\s*=\s*(["']).*?\1/gi,"").replace(/\son[a-z]+\s*=\s*[^\s>]+/gi,"").replace(/javascript\s*:/gi,"");}
  ...
  function homeButtonDef(type){var map:any={button_voice:{icon:"🎙️",label:"Voce",tab:"voice"},button_receipt:{icon:"📷",label:"Scontrino",tab:"receipt"},button_settings:{icon:"⚙",label:"Impostazioni",tab:"settings"},button_share:{icon:"🤝",label:"Share",tab:"share"},button_ai_advice:{icon:"ai",label:"Consigli AI",tab:"consulenteAI",aiTab:"consigli"},button_ai_chat:{icon:"💬",label:"Conversa con l'Assistente",tab:"consulenteAI",aiTab:"chat"},button_ai:{icon:"ai",label:"Consigli AI",tab:"consulenteAI",aiTab:"consigli"},button_stats:{icon:"📊",label:"Statistiche",tab:"stats"},button_patrimonio:{icon:"💎",label:"Patrimonio",tab:"patrimonio"},button_budget:{icon:"💰",label:"Budget",tab:"budget"},button_shopping:{icon:"🛒",label:"Spesa",tab:"shopping"},button_goals:{icon:"🎯",label:"Obiettivi",tab:"goals"},button_alerts:{icon:"🔔",label:"Alert",tab:"alerts"},button_appunti:{icon:"🗂",label:"Appunti",tab:"appunti"}};return map[type]||{icon:"🧩",label:type,tab:"home"};}
  ...
  function openHomeButton(type){var d=homeButtonDef(type);if(d.tab==="voice"){if(openVoiceModal)openVoiceModal();else{if(setVoiceText)setVoiceText("");if(setVoiceParsed)setVoiceParsed(null);if(setVoiceError)setVoiceError("");if(setVoiceListening)setVoiceListening(false);if(setVoiceModal)setVoiceModal(true);}homeScrollTop();return;}if(d.tab==="receipt"){var aiReceiptAllowed=currentPlan==="base"||currentPlan==="premium";if(aiReceiptAllowed&&openVoiceModal){openVoiceModal(false,true,"receipt");homeScrollTop();return;}setTab("spese");setSpeseSubTab("add");setAddType("expense");setAddSubTab("receipt");homeScrollTop();return;}setTab(d.tab);if(d.tab==="settings"&&setSettingsPage)setSettingsPage(null);if(d.tab==="consulenteAI"&&setAiTab){if(d.aiTab==="chat"){setAiTab("consigli");if(openVoiceModal)openVoiceModal(false,true);homeScrollTop();return;}setAiTab(d.aiTab||"consigli");if(d.aiTab==="consigli"&&setAiAdviceFilter)setAiAdviceFilter("all");}homeScrollTop();}
  ...
  function homeTitleText(w){var custom=w&&w.params?String(w.params.customTitle||"").trim():"";return custom||((lib(w.type).label)||w.type);}
  ...
  function renderWorklet(w){var range=Number((w.params&&w.params.range)||12);var keys=lastMonthKeys(range);var compact=String(w.size||"")==="0.5x";var doubleSize=String(w.size||"")==="2x";if(w.type==="assistant_voice_widget"){var av=assistantVoiceUiText(lang||"it");var avSize=String(w.size||"1x1"),avTall=avSize==="1x2"||avSize==="2x2",avWide=avSize==="2x1"||avSize==="2x2";return <button type="button" onClick={function(){if(openVoiceModal)openVoiceModal();else if(setVoiceModal)setVoiceModal(true);}} style={{width:"100%",height:"100%",minHeight:avTall?172:78,border:"none",borderRadius:14,background:"linear-gradient(135deg,rgba(127,119,221,.18),rgba(55,138,221,.13))",color:textC,display:"flex",alignItems:"center",justifyContent:"center",gap:avWide?16:8,flexDirection:avWide?"row":"column",padding:avTall?16:10,cursor:"pointer",textAlign:avWide?"left":"center",overflow:"hidden"}}><AIGrilloIcon size={avTall?(avWide?84:76):(avWide?58:46)}/><div style={{minWidth:0,maxWidth:avWide?280:"100%"}}>{homeShowTitle(w)&&<div style={{fontSize:avTall?17:14,fontWeight:950,color:textC,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{homeTitleText(w)}</div>}<div style={{fontSize:avTall?12:10,color:subC,lineHeight:1.35,marginTop:homeShowTitle(w)?4:0,display:"-webkit-box",WebkitLineClamp:avTall?3:1,WebkitBoxOrient:"vertical",overflow:"hidden"}}>{av.sub}</div><div style={{display:"inline-flex",alignItems:"center",gap:5,marginTop:avTall?10:5,padding:avTall?"7px 10px":"4px 8px",borderRadius:999,background:"#7F77DD",color:"#fff",fontSize:avTall?12:10,fontWeight:900}}>🎙️ {av.speak}</div></div></button>;}if(isHomeNavButton(w.type)){var bd=homeButtonDef(w.type);var showLabel=homeShowTitle(w);var lbl=homeTitleText(w);return <button type="button" onClick={function(){openHomeButton(w.type);}} style={{width:"100%",height:doubleSize?"100%":undefined,minHeight:doubleSize?88:56,border:"none",borderRadius:14,background:secondaryC,color:"#fff",display:"flex",alignItems:"center",justifyContent:"center",gap:compact?4:8,flexDirection:compact?"column":"row",fontSize:compact?10:13,fontWeight:950,cursor:"pointer",boxShadow:dark?"none":"0 8px 18px rgba(0,0,0,.12)",padding:compact?"8px 4px":"10px 10px",textAlign:"center",lineHeight:1.12,overflow:"hidden"}}>{homeButtonIcon(w.type,compact?22:28)}{showLabel&&<span style={{overflow:"hidden",textOverflow:"ellipsis",display:"-webkit-box",WebkitLineClamp:compact?2:1,WebkitBoxOrient:"vertical"}}>{lbl}</span>}</button>;}if(w.type==="quick_actions")return <div style={{display:"flex",gap:compact?8:10,flexDirection:compact?"column":"row",flexWrap:"wrap",height:doubleSize?"100%":undefined,alignItems:doubleSize?"stretch":undefined}}><Btn onClick={function(){setTab("spese");setSpeseSubTab("add");setAddType("expense");setAddSubTab("single");}} bg={expenseColor} style={{flex:1,minWidth:compact?0:130,padding:compact?"10px 8px":"12px 16px",minHeight:doubleSize?72:undefined,fontWeight:900,fontSize:compact?12:undefined}}>− {L(t.expense||"Uscita")}</Btn><Btn onClick={function(){setTab("spese");setSpeseSubTab("add");setAddType("income");setAddSubTab("single");}} bg={incomeColor} style={{flex:1,minWidth:compact?0:130,padding:compact?"10px 8px":"12px 16px",minHeight:doubleSize?72:undefined,fontWeight:900,fontSize:compact?12:undefined}}>+ {L(t.income||"Entrata")}</Btn></div>;
    ...
    if(w.type==="upcoming_scheduled_expenses"){var nextRows=upcomingScheduledExpenses().slice(0,doubleSize?10:(compact?3:5));var maxUpcomingH=doubleSize?300:(compact?112:154);return <div style={{display:"flex",flexDirection:"column",minWidth:0,height:"100%"}}>{!nextRows.length&&emptyBox("Nessuna spesa programmata in arrivo")}<div style={{overflowY:"auto",WebkitOverflowScrolling:"touch",paddingRight:2,maxHeight:maxUpcomingH}}>{nextRows.map(function(row,i){var r=row.item||{};var cat=getCat?getCat(r.catId):null;return <div key={String(r.id||i)+"_"+row.date.getTime()} style={{display:"flex",alignItems:"center",gap:compact?6:8,padding:compact?"6px 0":"7px 0",borderBottom:"1px solid "+borderC,minWidth:0}}><span style={{fontSize:compact?15:18,flexShrink:0}}>{r.icon||(cat&&cat.icon)||"💸"}</span><div style={{flex:1,minWidth:0}}><div style={{fontSize:compact?11:13,fontWeight:850,color:textC,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{r.name||(cat&&cat.name)||L("Spesa programmata")}</div><div style={{fontSize:compact?9:11,color:subC,whiteSpace:"nowrap"}}>🗓️ {recurringDateLabel(row.date)}</div></div><div style={{fontSize:compact?11:13,fontWeight:900,color:expenseColor,whiteSpace:"nowrap",flexShrink:0}}>{fmt(safeNum(r.amount))}</div></div>;})}</div></div>;}
    ...
    if(w.type==="note_widget"){var note=selectedNote(w);if(!note)return emptyBox("Nessuna nota inserita");var richNoteHtml=safeHomeNoteHtml(note.html||"");return <div style={{display:"flex",flexDirection:"column",gap:8,minWidth:0}}><div style={{fontSize:compact?14:17,fontWeight:950,color:textC,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{note.title||L("Nota")}</div>{richNoteHtml?<div data-no-translate="true" style={{fontSize:compact?12:13,color:subC,lineHeight:1.35,overflow:"hidden",maxHeight:doubleSize?190:(compact?76:96),overflowWrap:"anywhere"}} dangerouslySetInnerHTML={{__html:richNoteHtml}}/>:<div style={{fontSize:compact?12:13,color:subC,lineHeight:1.35,overflow:"hidden",display:"-webkit-box",WebkitLineClamp:doubleSize?8:(compact?4:5),WebkitBoxOrient:"vertical",whiteSpace:"pre-wrap"}}>{note.text||note.title||""}</div>}</div>;}
  ...
  var activeWorklets=premiumHome?normalizeWorklets(homeWorklets,true):(baseHome?HOME_DEFAULT_WORKLETS.concat([HOME_BASE_SCHEDULED_WORKLET]):HOME_DEFAULT_WORKLETS);
  ...
  function cleanHomeWorkletTitle(v){var value=String(v==null?"":v).replace(/[\u200B\u200C\u200D\u2060\uFEFF]/g," ").replace(/\s+/g," ").trim();return value.replace(/(?:\s*[\-‐‑‒–—―:·•]+\s*)+$/g,"").trim();}
  function cardTitle(w){var item=lib(w.type);var rawTitle=homeTitleText(w);var cleanTitle=cleanHomeWorkletTitle(rawTitle);if(w&&w.type==="distribution_expenses"){var customTitle=String(w.params&&w.params.customTitle||"").trim();cleanTitle=customTitle?cleanHomeWorkletTitle(customTitle).replace(/(?:\s*[-‐‑‒–—―−﹘﹣－:·•]+\s*)+$/g,"").trim():H("Distribuzione Uscite");}return (item.icon?item.icon+" ":"")+cleanTitle;}
  ...
  function workletCard(w,index,available){
    ...
    if(!available&&!buttonType&&!homeShowTitle(w)){
      ...
      else if(w.type==="upcoming_scheduled_expenses")targetH=compact?154:(size==="2x"?330:172);
...
export function HistoryPanel(){
  ...
  function modalHeader(title,closeFn){return <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:12,marginBottom:16}}><div><div style={{fontSize:22,fontWeight:950,color:textC}}>{title}</div><div style={{fontSize:12,color:subC,marginTop:3}}>{L("Storico movimenti")}</div></div><PopupCloseButton onClick={closeFn} dark={dark} label={L("Chiudi")}/></div>;}
...
var {normalizeEmail,loadShareCollaboration,acceptShareInvite,declineShareInvite,createShareInvite,createShareProject,updateShareProject,deleteShareProject,canAddPlanItem}:any=_c;
  ...
  function openAIChat(prefill){
    ...
    try{if(q.trim())localStorage.setItem("fainance_voice_assistant_prefill_once",q);else localStorage.removeItem("fainance_voice_assistant_prefill_once");}catch(_e){}
    setAiTab("consigli");
    if(openVoiceModal)openVoiceModal(false,true);else setVoiceModal(true);
  }
  function askAI(text){
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:16}}>
    <div style={{position:"relative",overflow:"hidden",background:dark?"#252535":"linear-gradient(135deg,#ffffff,#f4f1ff)",border:"1px solid "+borderC2,borderRadius:18,padding:isMobile?18:22,minHeight:isMobile?132:150,boxShadow:dark?"none":"0 8px 28px rgba(127,119,221,0.12)"}}>
      <div style={{position:"relative",zIndex:2,maxWidth:620}}>
        ...
        <div style={{display:"flex",gap:9,flexWrap:"wrap"}}><button onClick={function(){if(openVoiceModal)openVoiceModal(false,true);else setVoiceModal(true);}} style={{background:secondaryButtonColor||"#5FAFE5",color:"#fff",border:"1px solid "+(secondaryButtonColor||"#5FAFE5"),borderRadius:btnRadius,padding:"10px 14px",fontSize:13,fontWeight:850,cursor:"pointer",display:"inline-flex",alignItems:"center",gap:8,boxShadow:"0 5px 15px rgba(55,138,221,.18)"}}>💬 {L("Conversa con l'Assistente")}</button></div>
      </div>
    </div>
...
export function VoiceEntryModal(){
  ...
  var quickRequested=false,assistantRequested=false;
  try{quickRequested=localStorage.getItem("fainance_voice_quick_mode_once")==="1";assistantRequested=localStorage.getItem("fainance_voice_assistant_mode_once")==="1";localStorage.removeItem("fainance_voice_quick_mode_once");localStorage.removeItem("fainance_voice_assistant_mode_once");}catch(e){}
  var [mode,setMode]=useState(assistantRequested?"assistant":((currentPlan==="base"||currentPlan==="premium")&&!quickRequested?"assistant":"quick"));
...
function VoiceAssistantModal({onQuick}){
  ...
  var {shoppingItems,setShoppingItems,shoppingAreas,shoppingDefaultArea,shoppingUnits,setShoppingUnits,shoppingDefaultUnit,setShoppingDefaultUnit,shoppingLists,setShoppingLists,activeShoppingListId,setActiveShoppingListId}:any=c;
  ...
  useEffect(function(){var prefill="";try{prefill=localStorage.getItem("fainance_voice_assistant_prefill_once")||"";localStorage.removeItem("fainance_voice_assistant_prefill_once");}catch(e){}if(prefill)setInput(prefill);},[]);
  ...
  function buildContext(){
    ...
    var ctx:any={app:"fAInance",today:todayStr(),language:lang||"it",dataAccessLevel:aiDataAccess||"summary",plan:currentPlan||"free",
      ...
      catalogs:{categories:(cats||[]).filter(function(x){return !x.archived;}).map(function(x){return{id:String(x.id),name:x.name,groupId:String(x.group||"")};}),expenseGroups:(expenseGroups||[]).map(function(x){return{id:String(x.id),name:x.name};}),methods:(methods||[]).filter(function(x){return !x.archived;}).map(function(x){return{id:String(x.id),name:x.name,groupId:String(x.group||"")};}),methodGroups:(methodGroups||[]).map(function(x){return{id:String(x.id),name:x.name};}),incomeTypes:(incomeTypes||[]).map(function(x){return{id:String(x.id),name:x.name};}),shoppingAreas:(shoppingAreas||[]).slice(0,50),shoppingUnits:(shoppingUnits||[]).slice(0,50),patrimonioAreas:(patrimonioAreas||[]).map(function(x){return{id:String(x.id),name:x.name};}),patrimonioEntries:(patrimonioEntries||[]).map(function(x){return{id:String(x.id),name:x.name,areaId:String(x.areaId||"")};}),sections:["home","expenses","incomes","recurring","history","statistics","budget","goals","alerts","patrimonio","debtCredits","shopping","share","appunti","settings","ai"]},shoppingRules:{allowedUnits:(shoppingUnits||[]).slice(0,50),defaultUnit:shoppingDefaultUnit||"Unità",neverCreateUnitsWhileAddingProducts:true,createUnitOnlyWhenExplicitlyRequested:true}};
...
var {normalizeEmail,loadShareCollaboration,acceptShareInvite,declineShareInvite,createShareInvite,createShareProject,updateShareProject,deleteShareProject,canAddPlanItem}:any=_c;
  ...
  function endDrag(e){var moved=dragState&&dragState.moved;setAiFloatingDrag(null);if(!moved){setTab("consulenteAI");setAiTab("consigli");setSettingsPage(null);setMobileMenu(false);if(openVoiceModal)openVoiceModal(false,true);}if(e&&e.cancelable)e.preventDefault();}
