...
function detectAssistantTurnLanguage(value:any,fallback?:string){
  var raw=String(value||"").trim();
  var base=String(fallback||"it");
  if(!raw)return base;
  if(/[Ͱ-Ͽ]/.test(raw))return "el";
  var text=normalizeVoiceText(raw);
  var scores:any={it:0,en:0,es:0,fr:0,de:0,pt:0,pl:0,nl:0,ro:0,el:0};
  var words:any={
    it:["ciao","voglio","vorrei","puoi","potresti","devo","sono","parlo","parlato","rispondi","rispondermi","lingua","italiano","oggi","ieri","spesa","entrata","obiettivo","perche","quanto","come","dove","quando","questo","questa","non","con","della","nelle","allora"],
    en:["hello","hi","please","want","would","can","could","should","speak","spoken","reply","answer","language","english","today","yesterday","expense","income","goal","why","what","where","when","this","that"],
    es:["hola","quiero","quisiera","puedes","podrias","debo","hablo","hablado","responde","idioma","espanol","hoy","ayer","gasto","ingreso","objetivo","porque","cuanto","como","donde","cuando","esto","esta"],
    fr:["bonjour","salut","veux","voudrais","pouvez","pourriez","dois","parle","parlé","reponds","réponds","repondre","répondre","langue","francais","français","aujourd'hui","hier","depense","dépense","revenu","objectif","pourquoi","combien","comment","ou","où","quand"],
    de:["hallo","möchte","will","kannst","könnten","muss","spreche","gesprochen","antworte","sprache","deutsch","heute","gestern","ausgabe","einnahme","ziel","warum","wieviel","wie","wo","wann"],
    pt:["ola","olá","quero","gostaria","podes","poderia","devo","falo","falado","responde","idioma","portugues","português","hoje","ontem","despesa","receita","objetivo","porque","quanto","como","onde","quando"],
    pl:["czesc","cześć","chce","chcę","mogę","mozesz","możesz","musze","muszę","mowie","mówię","odpowiedz","jezyk","język","polski","dzisiaj","wczoraj","wydatek","przychod","przychód","cel","dlaczego","ile","jak","gdzie","kiedy"],
    nl:["hallo","wil","zou","kan","kunt","moet","spreek","gesproken","antwoord","taal","nederlands","vandaag","gisteren","uitgave","inkomen","doel","waarom","hoeveel","hoe","waar","wanneer"],
    ro:["salut","vreau","as","aș","poti","poți","trebuie","vorbesc","vorbit","raspunde","răspunde","limba","romana","română","azi","ieri","cheltuiala","cheltuială","venit","obiectiv","de ce","cat","cât","cum","unde","cand","când"],
    el:["γεια","θελω","θέλω","μπορεις","μπορείς","πρεπει","πρέπει","μιλαω","μιλάω","απαντησε","απάντησε","γλωσσα","γλώσσα","ελληνικα","ελληνικά","σημερα","σήμερα","χθες","εξοδο","έξοδο","εσοδο","έσοδο","στοχος","στόχος","γιατι","γιατί","ποσο","πόσο","πως","πώς","που","πού","ποτε","πότε"]
  };
  Object.keys(words).forEach(function(code){words[code].forEach(function(word){var needle=normalizeVoiceText(word);if(new RegExp("(?:^|\\s)"+escapeVoiceRegex(needle)+"(?:$|\\s)","i").test(text))scores[code]+=needle.length<=3?1:2;});});
  if(/[¿¡ñ]/i.test(raw))scores.es+=4;
  if(/[àèéìòù]/i.test(raw))scores.it+=2;
  if(/[çœâêîôûëïüÿæ]/i.test(raw))scores.fr+=4;
  if(/[äöüß]/i.test(raw))scores.de+=4;
  if(/[ãõ]/i.test(raw))scores.pt+=4;
  if(/[ąćęłńóśźż]/i.test(raw))scores.pl+=4;
  if(/[ăâîșşțţ]/i.test(raw))scores.ro+=4;
  var best=base,bestScore=0;
  Object.keys(scores).forEach(function(code){if(scores[code]>bestScore){best=code;bestScore=scores[code];}});
  return bestScore>0?best:base;
}
function assistantLanguageName(code:any){
  return ({it:"italiano",es:"spagnolo",en:"inglese",fr:"francese",de:"tedesco",pt:"portoghese",pl:"polacco",nl:"olandese",ro:"rumeno",el:"greco"} as any)[String(code||"it")]||"italiano";
}
function assistantAnswerNeedsTranslation(answer:any,target:any){
  var expected=String(target||"it");
  return detectAssistantTurnLanguage(answer,expected)!==expected;
}
function assistantTurnLanguageInstruction(code:any){
  var langCode=String(code||"it");
  var langName=assistantLanguageName(langCode);
  return "Follow all existing session instructions, financial rules and available tools. The user has just spoken or written in "+langName+". Reply exclusively in "+langName+" for this turn. The app interface language is irrelevant. Never tell the user to change the app language in Settings. If the user changes language on a later turn, immediately change your reply language too.";
}
function assistantHearingReply(code:any){
  var map:any={it:"Sì, ti sento.",en:"Yes, I can hear you.",es:"Sí, te escucho.",fr:"Oui, je vous entends.",de:"Ja, ich höre dich.",pt:"Sim, consigo ouvir-te.",pl:"Tak, słyszę Cię.",nl:"Ja, ik hoor je.",ro:"Da, te aud.",el:"Ναι, σας ακούω."};
  return map[String(code||"it")]||map.it;
}
...
function VoiceAssistantModal({onQuick}){
  ...
  function handleRealtimeEvent(raw:any){var event:any=raw;try{if(typeof raw==="string")event=JSON.parse(raw);}catch(e){return;}if(!event||!event.type)return;var type=String(event.type);
    ...
    if(type==="conversation.item.input_audio_transcription.completed"){var ut=String(event.transcript||realtimeUserDraftRef.current||"").trim();realtimeUserDraftRef.current="";setRealtimeUserDraft("");clearVoiceNotice();if(ut){var probs=Array.isArray(event.logprobs)?event.logprobs:[];var avgLogProb=probs.length?probs.reduce(function(sum,p){var lp=Number(p&&p.logprob);return sum+(Number.isFinite(lp)?lp:-4);},0)/probs.length:null;var confidence=avgLogProb===null?null:Math.exp(Math.max(-12,Math.min(0,avgLogProb)));var turnLanguage=detectAssistantTurnLanguage(ut,lastAssistantUserLanguageRef.current||String(lang||"it"));lastAssistantUserLanguageRef.current=turnLanguage;lastVoiceUserRequestRef.current=ut;appendVoiceMessage(ut,confidence);if(consumePlanFeature)consumePlanFeature("aiReply",1);var check=normalized(ut);var hearingCheck=/^(mi senti|mi stai sentendo|riesci a sentirmi|mi ascolti|ci sei|can you hear me|do you hear me|tu m'entends|vous m'entendez|me escuchas|puedes oirme|kannst du mich horen|consegues ouvir-me|slyszysz mnie|hoor je mij|ma auzi|με ακους)[?.! ]*$/.test(check);if(hearingCheck)sendRealtimeEvent({type:"response.create",response:{instructions:"Reply exactly and only: "+assistantHearingReply(turnLanguage)}});else if(activeAttachmentRef.current){sendRealtimeEvent({type:"response.cancel"});sendRealtimeEvent({type:"output_audio_buffer.clear"});callAssistant(ut);}else sendRealtimeEvent({type:"response.create",response:{instructions:assistantTurnLanguageInstruction(turnLanguage)}});}return;}
  ...
  function sendRealtimeText(q:string){var clean=String(q||"");var turnLanguage=detectAssistantTurnLanguage(clean,lastAssistantUserLanguageRef.current||String(lang||"it"));lastAssistantUserLanguageRef.current=turnLanguage;lastVoiceUserRequestRef.current=clean;if(!sendRealtimeEvent({type:"conversation.item.create",item:{type:"message",role:"user",content:[{type:"input_text",text:clean}]}}))return false;sendRealtimeEvent({type:"response.create",response:{instructions:assistantTurnLanguageInstruction(turnLanguage)}});setBusy(true);return true;}
  ...
  async function speakFallback(clean,resume,cycle){
    try{
      ...
      if(nativePlatform()){
        ...
        var speechLang=lastAssistantUserLanguageRef.current||String(lang||"it");await tts.speak({text:clean,lang:localeMap[speechLang]||localeMap[lang]||"it-IT",rate:1.02,pitch:1,volume:assistantVolumeRef.current,category:"ambient",queueStrategy:0});
      ...
      window.speechSynthesis.cancel();var u=new SpeechSynthesisUtterance(clean);var speechLang=lastAssistantUserLanguageRef.current||String(lang||"it");u.lang=localeMap[speechLang]||localeMap[lang]||"it-IT";u.rate=1.02;u.pitch=1;u.volume=assistantVolumeRef.current;
  ...
  async function startNativeListening(){
    ...
    var recognitionLocale=String((typeof navigator!=="undefined"&&navigator.language)||localeMap[lang]||"it-IT");var res:any=await SpeechRecognition.start({language:recognitionLocale,maxResults:3,partialResults:true,popup:false,prompt:V.speak,addPunctuation:true,allowForSilence:1300});var matches=(res&&res.matches)||[];if(matches[0]){nativeTextRef.current=String(matches[0]);setInput(String(matches[0]));}
  }
  function startWebListening(){var SR:any=(window as any).SpeechRecognition||(window as any).webkitSpeechRecognition;if(!SR)throw new Error("SpeechRecognition unavailable");var rec=new SR();webRecognitionRef.current=rec;rec.lang=String((typeof navigator!=="undefined"&&navigator.language)||localeMap[lang]||"it-IT");rec.continuous=false;rec.interimResults=true;rec.maxAlternatives=3;var last="";rec.onresult=function(ev:any){var out="";for(var i=ev.resultIndex;i<ev.results.length;i++)out+=ev.results[i][0].transcript;last=out.trim();if(last)setInput(last);};rec.onerror=function(ev:any){setVoiceListening(false);if(ev&&ev.error!=="no-speech"&&ev.error!=="aborted")setVoiceError(V.unavailable);};rec.onend=function(){webRecognitionRef.current=null;finishListening(last);};rec.start();}
  ...
  async function callAssistant(q){
    ...
    try{
      ...
      var headers:any={"Content-Type":"application/json"};if(token)headers.Authorization="Bearer "+token;
      var ctrl=new AbortController();assistantRequestAbortRef.current=ctrl;var timer=setTimeout(function(){ctrl.abort();},70000);
      var assistantLang=detectAssistantTurnLanguage(q,lastAssistantUserLanguageRef.current||String(lang||"it"));lastAssistantUserLanguageRef.current=assistantLang;var assistantLanguageLabel=assistantLanguageName(assistantLang);var requestPayload:any={mode:"assistant",question:"[LANGUAGE_LOCK="+assistantLang+"] Rispondi esclusivamente in "+assistantLanguageLabel+". "+q,language:assistantLang,aiDataAccess:aiDataAccess||"summary",financeContext:{...buildContext(),language:assistantLang},chatHistory:(aiChat||[]).filter(function(m){return m&&m.role==="user";}).slice(-10).map(function(m){return{role:"user",text:m.rawText||m.text};}),instruction:"Rispondi soltanto nella lingua dell’ultimo messaggio dell’utente: "+assistantLanguageLabel+" (codice "+assistantLang+"). La lingua dell’interfaccia dell’app è irrilevante. Non chiedere mai di cambiare lingua nelle Impostazioni. Se l’utente cambia lingua, cambia immediatamente anche la lingua della risposta."};
      ...
      if(assistantAnswerNeedsTranslation(answer,assistantLang)){try{var translateResponse=await fetch(AI_AGENT_ENDPOINT,{method:"POST",headers:headers,body:JSON.stringify({mode:"assistant",question:"Traduci il testo seguente in "+assistantLanguageLabel+". Restituisci soltanto la traduzione: "+answer,language:assistantLang,financeContext:{language:assistantLang},chatHistory:[],instruction:"Traduzione pura. Output esclusivamente in "+assistantLanguageLabel+"."})});var translateData:any=await translateResponse.json();var corrected=String((translateData&&translateData.answer)||"").trim();if(corrected)answer=corrected;}catch(e){}}
