...
export function PeriodPreferences({ introduction = false, embedded = false }: { introduction?: boolean; embedded?: boolean }) {
  ...
  const renderInline = (value: string) => value.split(/(\*\*[^*]+\*\*)/g).filter(Boolean).map((part, idx) => part.startsWith('**') && part.endsWith('**') ? <strong key={idx}>{part.slice(2, -2)}</strong> : <span key={idx}>{part}</span>);
  const periodInfoBlock = <div style={{ fontSize: 13, lineHeight: 1.5, color: ctx.subC, marginTop: 10 }}>
    <div style={{ paddingLeft: 2 }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}><span style={{ lineHeight: 1.5 }}>•</span><span>{renderInline(T('calendarInfo'))}</span></div>
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8, marginTop: 4 }}><span style={{ lineHeight: 1.5 }}>•</span><span>{renderInline(T('financialInfo'))}</span></div>
    </div>
    <div style={{ height: 1, background: ctx.borderC, opacity: 0.9, margin: '10px 0 9px' }} />
    <div>{renderInline(T('countingInfo'))}</div>
  </div>;
  ...
  const card = <section aria-label={T('title')} style={{ background: ctx.cardBg, color: ctx.textC, border: embedded ? undefined : `1px solid ${ctx.borderC}`, borderRadius: 14, padding: embedded ? 0 : 18, maxWidth: embedded ? undefined : 600, width: '100%', boxSizing: 'border-box' }}>
    ...
    <div style={{display:'flex',alignItems:'center',gap:6,marginBottom:6}}>
      ...
      {embedded && <details style={{fontSize:12,color:ctx.subC}}><summary aria-label={T('title')} style={{cursor:'pointer'}}>ⓘ</summary><div style={{marginTop:6}}>{periodInfoBlock}</div></details>}
    </div>
