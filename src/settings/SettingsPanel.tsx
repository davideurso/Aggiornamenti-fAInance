...
import { AccountAccessSecurityCard, ContactForm, ProfileCard } from "../account/AccountScreens";
import {
...
import { NotificationInboxSettingsCard } from "../notifications/NotificationCenter";
...
// fAInance 2.0 - import stabile.
// IMPORTANTE: questo contenitore deve restare a livello di modulo.
// In passato l'accordion dell'import era dichiarato dentro SettingsPanel: ad ogni
// aggiornamento del pannello React vedeva un nuovo tipo di componente e smontava
// l'intero sottoalbero, compreso ImportData, perdendo file, progresso, righe e mapping.
function StableImportDataAccordionCard(props: any) {
  return (
    <div
      data-fainance-stable-import-container="v11"
      style={{
      ...
      >
        ...
        >
          ...
          <div style={{ minWidth: 0 }}>
            ...
            {props.desc && (
              <div style={{ fontSize: 12, color: props.subC, marginTop: 4 }}>
                {props.desc}
              </div>
            )}
          </div>
...
export function SettingsPanel() {
  ...
  useEffect(function () {
    var active = true;
    resolveAdminSession(fbAuth.currentUser).then(function (session) {
  ...
  var canOpenAdminCenter = !!(adminSession && adminSession.isAdmin);
  ...
  function safeWidgetPlanName(kind) {
    try {
      return typeof widgetPlanName === "function" ? widgetPlanName(kind) : "";
    } catch (e) {
      return "";
    }
  }
  function safeWidgetLockedMessage(kind) {
    try {
      return typeof widgetLockedMessage === "function"
        ? widgetLockedMessage(kind)
        : L("Questa funzione non è disponibile con il piano attuale.");
    } catch (e) {
      return L("Questa funzione non è disponibile con il piano attuale.");
    }
  }
  var sinp = {
    width: "100%",
  ...
  function settingsParent(id) {
    var map = {
      ...
      support_info: "info_support",
      data: "info_support",
      delete: "data",
      profile_personal: "profile",
      profile_privacy: "profile",
      profile_security: "profile",
      plans_settings: "profile_personal",
      terms_conditions: "profile_privacy",
      privacy_policy: "profile_privacy",
      admin_center: null,
    };
    return map[id] || null;
  }
  function settingsHeaderIcon(page, title) {
    var icons: any = {
      profile: "👤", profile_personal: "🪪", profile_privacy: "🛡️", profile_security: "🔐",
      general: "🌐", sections: "🧩", sections_income: "💰", sections_income_areas: "📂",
      sections_income_categories: "🗂️", sections_expense: "💸", sections_expense_areas: "📂",
  ...
  function PageHeader({ title }) {
    var headerIcon = settingsHeaderIcon(settingsPage, title);
    return (
      <div
        style={{
          display: "flex",
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0, flex: 1 }}>
          <div
            style={{
              ...
              border: "1px solid " + (dark ? "#3d376a" : "#E2DEFF"),
              display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0,
            }}
          >
            <FainanceIcon value={headerIcon} size={21} />
          </div>
          <div
            style={{
              fontSize: isMobile ? 17 : 18, lineHeight: 1.15, fontWeight: 900, color: textC,
        ...
        <button
          aria-label={L("Indietro") + " " + L(title)}
          onClick={function () { setSettingsPage(settingsParent(settingsPage)); }}
          style={{
    ...
    return (
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 16, fontWeight: 800, color: textC, marginBottom: 6 }}>
            {L(title)}
          </div>
          <div style={{ fontSize: 13, color: subC, lineHeight: 1.45 }}>
            {L(message || "Questa funzione non è disponibile con il piano attuale.")}
          </div>
        </div>
        <span style={{ fontSize: 18, color: subC, alignSelf: "center" }}>›</span>
      </button>
    );
  }
          ...
          return (
            <button
              ...
              style={{
                ...
                border: "1.5px solid " + borderC, borderRadius: 16, background: cardBg,
                cursor: "pointer", textAlign: "left",
                boxShadow: dark ? "none" : "0 2px 12px rgba(0,0,0,0.04)",
                opacity: locked ? 0.52 : 1, filter: locked ? "grayscale(1)" : "none",
              }}
            >
              <div style={{
                width: 42, height: 42, borderRadius: 14,
                background: dark ? "#24213a" : "#F0EDFF", display: "flex",
                alignItems: "center", justifyContent: "center", fontSize: 22, flexShrink: 0,
              }}>{s.icon}</div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginBottom: 3 }}>
                  <div style={{ fontSize: 15, fontWeight: 900, color: textC }}>{L(s.label)}</div>
                </div>
                <div style={{ fontSize: 12, color: subC, lineHeight: 1.35 }}>{L(s.desc)}</div>
              </div>
  ...
  function SettingsMenu() {
    var menuItems = (settingsSections || []).slice();
    if (canOpenAdminCenter) {
      menuItems.push({
        id: "admin_center",
        icon: "🛡️",
        label: "Admin",
        desc: "Dashboard, utenti, notifiche, configurazione, diagnostica e audit",
      });
    }
    return (
      <div style={{ paddingTop: isMobile ? 48 : 0 }}>
        <SettingsCards items={menuItems} />
      </div>
    );
  }
  if (settingsPage === "info_support")
    return (
      <div>
        <PageHeader title="Dati e Supporto" />
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <SettingsCards items={[
            { id: "data", icon: "💾", label: "Dati", desc: "Importa, esporta, backup e gestione dati" },
            { id: "support_info", icon: "💬", label: "Info e Supporto", desc: "FAQ, contatti, versione, aggiornamenti e social" },
          ]} />
        </div>
      </div>
    );
  ...
  function WidgetAppearancePanel() {
    var noteLocked = !safeWidgetAllowed("note");
    var goalLocked = !safeWidgetAllowed("goal");
    var shoppingListLocked = !safeWidgetAllowed("shoppingList");
    var fidelityLocked = !safeWidgetAllowed("fidelity");
    var debtCreditsLocked = !safeWidgetAllowed("debtCredits");
    var shareLocked = !safeWidgetAllowed("share");
    return (
      <SettingsCards
        items={[
          {
            ...
            desc: "Aggiungi rapidamente un’uscita o un’entrata.",
          },
          {
            id: "appearance_widget_fidelity",
            icon: "💳",
            label: "Fidelity card",
            desc: "Mostra una carta fedeltà o prepagata.",
            disabled: fidelityLocked,
            requiredPlanLabel: safeWidgetPlanName("fidelity"),
            lockedMessage: safeWidgetLockedMessage("fidelity"),
          },
          {
            id: "appearance_widget_shopping_list",
            icon: "🧺",
            label: "Lista spesa",
            desc: "Mostra una lista della spesa.",
            disabled: shoppingListLocked,
            requiredPlanLabel: safeWidgetPlanName("shoppingList"),
            lockedMessage: safeWidgetLockedMessage("shoppingList"),
          },
          {
            id: "appearance_widget_note",
            icon: "📝",
            label: "Nota / Dati bancari",
            desc: "Mostra una nota o un dato bancario.",
            disabled: noteLocked,
            requiredPlanLabel: safeWidgetPlanName("note"),
            lockedMessage: safeWidgetLockedMessage("note"),
          },
          {
            id: "appearance_widget_goal",
            icon: "🎯",
            label: "Obiettivo",
            desc: "Mostra l’avanzamento di un obiettivo.",
            disabled: goalLocked,
            requiredPlanLabel: safeWidgetPlanName("goal"),
            lockedMessage: safeWidgetLockedMessage("goal"),
          },
          {
            id: "appearance_widget_share",
            icon: "🤝",
            label: "Share",
            desc: "Mostra il riepilogo di un progetto Share.",
            disabled: shareLocked,
            requiredPlanLabel: safeWidgetPlanName("share"),
          ...
          },
          {
            ...
            desc: "Mostra il saldo di debiti e crediti.",
            disabled: debtCreditsLocked,
            requiredPlanLabel: safeWidgetPlanName("debtCredits"),
            lockedMessage: safeWidgetLockedMessage("debtCredits"),
          },
        ]}
      />
    );
              ...
              {visibleItems.map(function (a) {
                return (
                  ...
                  >
                    ...
                    <button
                      aria-disabled={!baseSettingsAllowed}
                      onClick={function () {
                        openEdit(a);
                      }}
                      style={{
                        background: baseSettingsAllowed ? "#EEF4FF" : (dark ? "#34343F" : "#E5E7EB"),
                        border: "1px solid " + (baseSettingsAllowed ? "#BFD7FF" : (dark ? "#4B4B58" : "#D1D5DB")),
                        borderRadius: 8,
                        cursor: "pointer",
                        fontSize: 14,
                    ...
                    <button
                      ...
                      style={{
                        background: baseSettingsAllowed ? "#FFF0F0" : (dark ? "#34343F" : "#E5E7EB"),
                        border: "1px solid " + (baseSettingsAllowed ? "#FFD0D0" : (dark ? "#4B4B58" : "#D1D5DB")),
                        borderRadius: 8,
                        cursor: "pointer",
                        fontSize: 14,
          ...
          {view === "order" && (
            <SortableRows
              ...
              onMove={function (i, dir) {
                ...
                var reordered = visibleItems.slice();
                var tmp = reordered[i];
                ...
                var arr = (items || []).map(function (item) {
                  ...
                  var next = reordered[visibleIndex];
                  visibleIndex += 1;
                  return next;
                });
                setItems(arr);
          ...
          {view === "list" && (
            <>
              <SettingsList
                ...
                showGroup
                showIcon
                groupList={(expenseGroups || DEFAULT_EXPENSE_GROUPS).filter(function (g) {
                  return !g.deleted;
                })}
  ...
  function IncomeCategoriesSettings() {
    ...
    var groups = (incomeGroups || DEFAULT_INCOME_GROUPS).filter(function (g) {
      return !g.deleted;
    });
  ...
  if (settingsPage === "admin_center") {
    if (!canOpenAdminCenter) {
      return (
        <div>
          ...
          <div style={{ background: cardBg, border: "1px solid " + borderC, borderRadius: 16, padding: 18, color: subC, fontSize: 13 }}>
            {L("Accesso Admin non disponibile")}
          </div>
        </div>
      );
    }
    return (
      <div>
        <PageHeader title="Admin" />
        <AdminCenterPanel session={adminSession} />
      </div>
    );
  }
  if (settingsPage === "profile")
    return (
      <div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <SettingsCards items={[
            { id: "profile_personal", icon: "🪪", label: "Dati Personali", desc: "Profilo, contatti e piano attuale" },
            { id: "profile_privacy", icon: "🛡️", label: "Privacy & consensi", desc: "Termini, privacy e consenso AI esterno" },
            { id: "profile_security", icon: "🔐", label: "Sicurezza", desc: "Blocco app, dispositivi, password, email e account" },
          ]} />
          <button
            type="button"
  ...
  if (settingsPage === "profile_personal")
    return (
      <div>
        <PageHeader title="Dati Personali" />
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <ProfileCard
            currentUser={currentUser} onLogout={onLogout} dark={dark} textC={textC} subC={subC} borderC={borderC} cardBg={cardBg} btnRadius={btnRadius} dateFmt={dateFmt}
            setToast={setToast} fbDb={fbDb} onProfileUpdate={onProfileUpdate} onRequestAccountDeletion={requestCurrentAccountDeletion} onCancelAccountDeletion={cancelCurrentAccountDeletion}
            lang={lang} confirmButtonColor={confirmButtonColor} secondaryButtonColor={secondaryButtonColor} showSecurityActions={false} showLogout={false}
          />
  ...
  if (settingsPage === "profile_privacy")
    return (
      <div>
        <PageHeader title="Privacy & consensi" />
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <SettingsCards items={[
            { id: "terms_conditions", icon: "📄", label: "Termini di utilizzo", desc: "Consulta condizioni, limiti e responsabilità" },
            { id: "privacy_policy", icon: "🔐", label: "Informativa Privacy", desc: "Consulta trattamento dati, sincronizzazione e servizi esterni" },
          ]} />
      ...
      <div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <AccountSecurityCenter
            ...
            showDeletion={false}
          />
          <AccountAccessSecurityCard
            currentUser={currentUser} dark={dark} textC={textC} subC={subC} borderC={borderC} cardBg={cardBg} btnRadius={btnRadius}
            setToast={setToast} fbDb={fbDb} onProfileUpdate={onProfileUpdate} onRequestAccountDeletion={requestCurrentAccountDeletion}
            onCancelAccountDeletion={cancelCurrentAccountDeletion} onLogout={onLogout} lang={lang} confirmButtonColor={confirmButtonColor} secondaryButtonColor={secondaryButtonColor}
          />
        </div>
      </div>
    );
  ...
  if (settingsPage === "profile_security")
    return (
      ...
      <div>
        ...
        {showNewListSettForm && (
          ...
          >
            ...
            >
              <div style={{ position: "absolute", right: 14, top: 14 }}>
                <PopupCloseButton onClick={resetSettingsShoppingListForm} dark={dark} label={L("Chiudi")} />
              </div>
              <div
                style={{
                  fontSize: 18,
      ...
      <div>
        ...
        {showUnitForm && (
          ...
          >
            ...
            >
              <div style={{ position: "absolute", right: 14, top: 14 }}>
                <PopupCloseButton onClick={resetUnitForm} dark={dark} label={L("Chiudi")} />
              </div>
              <div
                style={{
                  fontSize: 18,
        ...
        {pendingDeleteUnit && (
          ...
          >
            ...
            >
              <div style={{ position: "absolute", right: 14, top: 14 }}>
                <PopupCloseButton onClick={function () { setPendingDeleteUnit(""); }} dark={dark} label={L("Chiudi")} />
              </div>
              <div
                style={{
                  fontSize: 18,
  ...
  function NotificationsSettingsPage() {
    ...
    return (
      <div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <NotificationInboxSettingsCard userId={String(userId || "")} />
          ...
          >
            ...
            >
              ⏰ {L("Promemoria inserimento spese")}
            </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              <div
                ...
                style={{
                  ...
                  borderRadius: 10,
                  border: "1px solid " + (dark ? "#333" : "#f0f0f0"),
                  opacity: notifBaseAllowed ? 1 : 0.55,
                  filter: notifBaseAllowed ? "none" : "grayscale(1)",
                  cursor: notifBaseAllowed ? "default" : "pointer",
                }}
              >
                <div>
                  <div style={{ fontSize: 13, fontWeight: 600, color: textC }}>
        ...
        >
          ...
          <div
            ...
            style={{
              ...
              padding: "16px 20px",
              borderBottom: "1px solid " + borderC,
              opacity: baseSettingsAllowed ? 1 : 0.55,
              filter: baseSettingsAllowed ? "none" : "grayscale(1)",
            }}
          >
            <div
              style={{
      ...
      return (
        <div>
          ...
          <LockedFeatureCard
            ...
            message={safeWidgetLockedMessage("note")}
          />
        </div>
      );
    return (
      ...
      return (
        <div>
          ...
          <LockedFeatureCard
            ...
            message={safeWidgetLockedMessage("goal")}
          />
        </div>
      );
    return (
  ...
  if (settingsPage === "appearance_widget_shopping_list") {
    if (!safeWidgetAllowed("shoppingList"))
      return (
        <div>
          ...
          <LockedFeatureCard
            ...
            message={safeWidgetLockedMessage("shoppingList")}
          />
        </div>
      );
    return (
      ...
      return (
        <div>
          ...
          <LockedFeatureCard
            ...
            message={safeWidgetLockedMessage("fidelity")}
          />
        </div>
      );
    return (
  ...
  if (settingsPage === "appearance_widget_debt_credits") {
    if (!safeWidgetAllowed("debtCredits"))
      return (
        <div>
          <PageHeader title="Debiti / Crediti" />
          <LockedFeatureCard
            ...
            message={safeWidgetLockedMessage("debtCredits")}
          />
        </div>
      );
    return (
      ...
      return (
        <div>
          ...
          <LockedFeatureCard
            icon="🤝"
            title="Widget Share"
            message={safeWidgetLockedMessage("share")}
          />
        </div>
      );
    return (
  ...
  function dataVisibleItems(list) {
  ...
  function deleteDataOptions() {
    ...
    var clearable = base.slice();
    base.push({
      id: "all",
      label: L("Elimina tutto"),
      count: total,
      clear: function () {
        clearable.forEach(function (o) {
          try {
            o.clear && o.clear();
          } catch (e) {}
        });
      ...
      <div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <StableImportDataAccordionCard
            ...
            desc={L("Carica file CSV, Excel o un backup completo JSON.")}
            open={dataImportOpen}
            ...
            >
              {PLAN_IDS.map(function (pid) {
                ...
                var details = [
                  ...
                  ["Budget", L("completo")],
                  ["Copie Patrimonio", copyPatrimonioLabel()],
                ...
                function renderPlanFeatureLabel(label) {
                  var translated = String(L(label));
                  return translated.split(/(Share)/g).map(function (part, idx) {
                    return part === "Share" ? <em key={idx}>Share</em> : <span key={idx}>{part}</span>;
                  });
                }
                return (
                  <div
                      ...
                      <div>
                        ...
                        {pid !== "free" && (
                          <div style={{ fontSize: 12, color: subC, marginTop: 3 }}>
                            {price.monthly + " €/" + L("mese")} · {price.yearly + " €/" + L("anno")}
                          </div>
                        )}
                        <div
                          style={{
                        ...
                        >
                          {pid === "free"
                            ? L("Funzioni essenziali per iniziare")
                            : pid === "base"
                            ? L("Più controllo e personalizzazione")
                            : L("Tutte le funzioni e i limiti più ampi")}
                        </div>
                      </div>
                      {active && (
                        <span
  ...
  function SupportSettingsPage({ embedded = false }: any = {}) {
    ...
    return (
      <div>
        {!embedded && <PageHeader title="Supporto" />}
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <div
            style={{
              background: cardBg,
  ...
  if (settingsPage === "support_info")
    return (
      <div>
        <PageHeader title="Info e Supporto" />
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div style={{ background: dark ? "linear-gradient(145deg,#24243A,#302E50)" : "linear-gradient(145deg,#F7F5FF,#EEF7FF 58%,#FFF8E6)", border: "1px solid " + (dark ? "#4A476A" : "#D9D5FF"), borderRadius: 20, padding: 18 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}><div style={{ width: 48, height: 48, borderRadius: 16, background: "linear-gradient(135deg,#7F77DD,#378ADD)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 24 }}>💬</div><div><div style={{ fontSize: 16, fontWeight: 900, color: textC }}>{L("Siamo qui per aiutarti")}</div><div style={{ fontSize: 12, color: subC, marginTop: 3 }}>{L("Contatti, FAQ, aggiornamenti e informazioni su fAInance.")}</div></div></div>
          </div>
          <SupportSettingsPage embedded />
          <InfoSettingsPage embedded hideLegal />
        </div>
      </div>
    );
  ...
  function InfoSettingsPage({ embedded = false, hideLegal = false }: any = {}) {
    ...
    return (
      <div>
        {!embedded && <PageHeader title="Info" />}
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {/* App identity */}
          <div
            style={{
  ...
  if (settingsPage === "info")
    return (
      <StableNestedPanelHost
        key="settings-info"
        render={function () {
          return InfoSettingsPage({ hideLegal: true });
        }}
      />
    );
}
