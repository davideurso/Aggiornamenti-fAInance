...
import { itemAmountForMonth } from './financeCalculations';
...
export function useStorage(key,dv){
  ...
  var current=useRef({key:key,value:v});
  if(current.current.key!==key)current.current={key:key,value:readFainanceStorageValue(key,dv)};
  useEffect(function(){
  ...
  var save=useCallback(function(val){
      ...
      // an insertion made after the cloud callback completed.
      if(current.current.key!==key)return;
      var prev=current.current.value;
...
// ── BALANCE PERIOD HELPERS ──────────────────────────────────────────────────
// A financial month is labelled by the calendar month in which it ends.
// Example, start day 27: “2026-09” means 2026-08-27 → 2026-09-26.
function balancePeriodLocalDate(iso){
  var m=/^(\d{4})-(\d{2})-(\d{2})/.exec(String(iso||""));
  if(!m)return null;
  var d=new Date(Number(m[1]),Number(m[2])-1,Number(m[3]),12,0,0,0);
  return isNaN(d.getTime())?null:d;
}
function balancePeriodStartAt(year,monthIndex,requestedDay){
  var day=Math.max(1,Math.min(31,Number(requestedDay)||1));
  var last=new Date(year,monthIndex+1,0,12,0,0,0).getDate();
  return new Date(year,monthIndex,Math.min(day,last),12,0,0,0);
}
function balancePeriodIso(d){
  return String(d.getFullYear())+"-"+String(d.getMonth()+1).padStart(2,"0")+"-"+String(d.getDate()).padStart(2,"0");
}
export function balancePeriodRangeForDate(referenceDate,mode,startDay){
  var ref=referenceDate instanceof Date?new Date(referenceDate):balancePeriodLocalDate(referenceDate);
  if(!ref||isNaN(ref.getTime()))ref=new Date();
  ref.setHours(12,0,0,0);
  var financial=String(mode||"calendar")==="financial";
  if(!financial){
    var cs=new Date(ref.getFullYear(),ref.getMonth(),1,12,0,0,0);
    var ce=new Date(ref.getFullYear(),ref.getMonth()+1,0,12,0,0,0);
    return {start:cs,end:ce,key:String(ce.getFullYear())+"-"+String(ce.getMonth()+1).padStart(2,"0"),startIso:balancePeriodIso(cs),endIso:balancePeriodIso(ce)};
  }
  var day=Math.max(1,Math.min(31,Number(startDay)||27));
  var currentStart=balancePeriodStartAt(ref.getFullYear(),ref.getMonth(),day);
  var start,nextStart;
  if(ref>=currentStart){
    start=currentStart;
    nextStart=balancePeriodStartAt(ref.getFullYear(),ref.getMonth()+1,day);
  }else{
    start=balancePeriodStartAt(ref.getFullYear(),ref.getMonth()-1,day);
    nextStart=currentStart;
  }
  var end=new Date(nextStart);
  end.setDate(end.getDate()-1);
  var key=String(end.getFullYear())+"-"+String(end.getMonth()+1).padStart(2,"0");
  return {start:start,end:end,key:key,startIso:balancePeriodIso(start),endIso:balancePeriodIso(end)};
}
export function balancePeriodRangeForKey(monthKey,mode,startDay){
  var p=String(monthKey||"").split("-");
  var year=parseInt(p[0],10),month=parseInt(p[1],10);
  if(!year||month<1||month>12)return balancePeriodRangeForDate(new Date(),mode,startDay);
  if(String(mode||"calendar")!=="financial"){
    var cs=new Date(year,month-1,1,12,0,0,0);
    var ce=new Date(year,month,0,12,0,0,0);
    return {start:cs,end:ce,key:String(monthKey),startIso:balancePeriodIso(cs),endIso:balancePeriodIso(ce)};
  }
  var day=Math.max(1,Math.min(31,Number(startDay)||27));
  var start,nextStart;
  if(day===1){
    start=balancePeriodStartAt(year,month-1,day);
    nextStart=balancePeriodStartAt(year,month,day);
  }else{
    start=balancePeriodStartAt(year,month-2,day);
    nextStart=balancePeriodStartAt(year,month-1,day);
  }
  var end=new Date(nextStart);
  end.setDate(end.getDate()-1);
  return {start:start,end:end,key:String(monthKey),startIso:balancePeriodIso(start),endIso:balancePeriodIso(end)};
}
export function amountForBalancePeriodMonth(item,monthKey,viewMode,periodMode,startDay){
  if(!item||!monthKey)return 0;
  var range=balancePeriodRangeForKey(monthKey,periodMode,startDay);
  var itemDate=balancePeriodLocalDate(item.date);
  var amount=Number(item.amount||0);
  function inside(d){return !!d&&d>=range.start&&d<=range.end;}
  if(String(viewMode||"rateizzato")==="reale"||!item.rateizzato)return inside(itemDate)?amount:0;
  var rateCount=Math.max(0,Math.floor(Number(item.rate)||0));
  if(!rateCount||!itemDate)return 0;
  var cents=Math.round(amount*100);
  var base=Math.floor(cents/rateCount);
  var total=0;
  for(var i=0;i<rateCount;i++){
    var shift=item.rateDirection==="backward"?-i:i;
    var targetMonth=new Date(itemDate.getFullYear(),itemDate.getMonth()+shift,1,12,0,0,0);
    var last=new Date(targetMonth.getFullYear(),targetMonth.getMonth()+1,0,12,0,0,0).getDate();
    var installmentDate=new Date(targetMonth.getFullYear(),targetMonth.getMonth(),Math.min(itemDate.getDate(),last),12,0,0,0);
    if(!inside(installmentDate))continue;
    var share=i===rateCount-1?cents-base*(rateCount-1):base;
    total+=share/100;
  }
  return total;
}
