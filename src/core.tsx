...
import { getAuth, createUserWithEmailAndPassword, signInWithEmailAndPassword, signOut, onAuthStateChanged, GoogleAuthProvider, signInWithPopup, signInWithRedirect, getRedirectResult, signInWithCredential, deleteUser } from "firebase/auth";
export { getAuth, createUserWithEmailAndPassword, signInWithEmailAndPassword, signOut, onAuthStateChanged, GoogleAuthProvider, signInWithPopup, signInWithRedirect, getRedirectResult, signInWithCredential, deleteUser };
import { getFirestore, doc, setDoc, getDoc, onSnapshot, deleteDoc, collection, query, where, getDocs, addDoc } from "firebase/firestore";
export { getFirestore, doc, setDoc, getDoc, onSnapshot, deleteDoc, collection, query, where, getDocs, addDoc };
...
export const AI_OUT_OF_SCOPE_MESSAGE = "Posso aiutarti solo con finanza personale, budget, spese, entrate, risparmio e funzioni dell’app fAInance.";
export const AI_AGENT_SCOPE_INSTRUCTION = "Sei il Consulente AI integrato nell’app fAInance. Il tuo compito è aiutare l’utente esclusivamente su temi collegati all’app fAInance e alla gestione dei dati presenti o gestibili nell’app. Puoi rispondere solo a domande relative a: analisi di entrate, uscite, saldo e patrimonio; budget, categorie di spesa, metodi di pagamento e ricorrenze; obiettivi di risparmio; alert, notifiche e promemoria finanziari; importazione, esportazione, backup e qualità dei dati; interpretazione delle statistiche mostrate nell’app; consigli pratici per ottimizzare spese, risparmio, abbonamenti, bollette, ricorrenze e gestione finanziaria personale; spiegazioni su come usare le funzionalità dell’app fAInance. Non devi rispondere a domande generiche non collegate all’app, anche se riguardano argomenti finanziari, tecnologici, culturali, personali, storici, medici, legali, politici, sportivi, di viaggio o di intrattenimento. Se la domanda non riguarda fAInance o i dati gestiti dall’app, rispondi solo con: “"+AI_OUT_OF_SCOPE_MESSAGE+"”. Non fornire spiegazioni aggiuntive, alternative o risposte parziali fuori perimetro. Mantieni le risposte brevi, pratiche e orientate all’azione. Non usare Markdown: niente asterischi, grassetti, titoli Markdown, elenchi complessi o formattazioni speciali. Scrivi in testo semplice, adatto a essere mostrato direttamente dentro l’app. Usa i dati disponibili solo se pertinenti. Se i dati non sono sufficienti, dichiaralo chiaramente e indica quale informazione manca nell’app.";
...
export const DEFAULT_GOALS=[
  ...
  {id:"viaggio",name:"Viaggio",target:1500,saved:0,deadline:"",icon:"✈",color:"#378ADD",period:"annual"},
];
...
export function useStorage(key,dv){
  ...
  var save=useCallback(function(val){setV(val);try{localStorage.setItem(key,JSON.stringify(val));}catch(e){};},[key]);
  return [v,save];
}
...
export function rateMonth(item,mk){if(!item.rateizzato)return item.date.startsWith(mk)?item.amount:0;var s=new Date(item.date);var p=mk.split("-");var idx=(parseInt(p[0])-s.getFullYear())*12+(parseInt(p[1])-1-s.getMonth());if(idx<0||idx>=item.rate)return 0;return item.amount/item.rate;}
