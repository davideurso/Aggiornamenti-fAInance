...
export function ToolsPanel(){
  ...
  async function convert(a=from,b=to,input=amount,saveHistory=true){
    ...
    try{const fx=await rate(a,b,controller.signal);if(id!==request.current)return;const next={from:a,to:b,amount:value,result:roundCurrency(value*fx,b),rate:fx};if(saveHistory)record('converter',next);setQuote(next);}
  ...
  return <section className="fainance-tools" style={style} aria-label={T('title')}>
    ...
    {transaction?<div className="tool-card"><button onClick={()=>setTransaction(null)}>‹ {L('Indietro')}</button><h3>{T('create')}</h3><div className="tool-tabs"><button aria-pressed={direction==='expense'} onClick={()=>setDirection('expense')}>{L('Uscita')}</button><button aria-pressed={direction==='income'} onClick={()=>setDirection('income')}>{L('Entrata')}</button></div><ExpenseForm type={direction} initialValue={transaction} draftNamespace="draft_tool_" onSave={item=>{const accepted=direction==='expense'?ctx.addExpenses([item],'tool'):ctx.addIncomes([item],'tool');if(accepted===false)return false;setTransaction(null);return true;}}/></div>:<>
      ...
      {page==='converter'&&<div className="tool-card"><h3>{T('converter')}</h3><div className="tool-currency"><label>{T('from')}<button onClick={()=>{setPicker('from');setSearch('');}}>{currencyName(from)} ▾</button></label><input aria-label={L('Importo')} inputMode="decimal" value={amount} onChange={e=>changeAmount(e.target.value)} placeholder="0"/><AmountCalculatorButton value={amount} onApply={changeAmount} iconOnly/></div><button className="tool-swap" aria-label={T('swap')} onClick={swap}>⇄</button><div className="tool-currency"><label>{T('to')}<button onClick={()=>{setPicker('to');setSearch('');}}>{currencyName(to)} ▾</button></label><output aria-label={T('converter')} className="tool-converted">{quote?number(quote.result,to):'—'}</output></div><button className="tool-primary" disabled={busy||!amount.trim()} onClick={()=>convert()}>{T('convert')}</button>{quote&&<p className="tool-muted">{T('rate')}: 1 {from} = {new Intl.NumberFormat(ctx.lang||'it',{maximumFractionDigits:8}).format(quote.rate)} {to}</p>}{saved&&<p role="status" className="tool-muted">✓ {T('saved')}</p>}{resultActions()}{inlineHistory('converter')}</div>}
    </>}
