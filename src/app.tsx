...
import { totalForMonth, last12MonthKeys, balanceForMonths, monthlyTotalsForYear } from './financeCalculations.js';
...
try{
  if(typeof document!=="undefined"&&!document.getElementById("fainance-i18n-no-flash")){
    ...
    st.textContent='html[data-fainance-i18n="loading"] body{opacity:0!important;}';
...
function ProfileCard({currentUser,onLogout,dark,textC,subC,borderC,cardBg,btnRadius,dateFmt,setToast,fbDb:fbDbProp,onProfileUpdate,onDeleteAccount}){
  ...
  function save(){
    ...
    if(currentUser.id&&fbDbProp){
      setDoc(doc(fbDbProp,"users",currentUser.id),upd,{merge:true}).then(done).catch(function(err){console.error("profile save error",err);if(setToast)setToast(PL("Errore salvataggio profilo"));});
    } else {
      done();
    }
  }
...
function App({currentUser,onLogout,fbUser,onProfileUpdate}){
  ...
  var [mobileMenuOrder,setMobileMenuOrder]=useStorage(userKey("pref_mobile_menu_order_v1"),["consulenteAI","patrimonio","budget","goals","alerts","share","appunti","settings"]);
  var DEFAULT_MOBILE_ALL_NAV_ORDER=["home","spese","history","voice","stats","consulenteAI","patrimonio","budget","share","goals","alerts","appunti","settings"];
  ...
  function persistAccountPlan(nextPlan){
    if(!userId)return;
    var safePlan=PLAN_LIMITS[nextPlan]?nextPlan:"free";
    var payload={currentPlan:safePlan,plan:safePlan,subscriptionPlan:safePlan,updatedAt:new Date().toISOString()};
    try{setDoc(doc(fbDb,"userData",userId),payload,{merge:true}).catch(function(e){console.error("Firestore plan save error",e);});}catch(e){}
    try{setDoc(doc(fbDb,"users",userId),payload,{merge:true}).catch(function(e){console.error("Firestore user plan save error",e);});}catch(e){}
  }
  function setCurrentPlan(nextPlan,persist){
    var safePlan=PLAN_LIMITS[nextPlan]?nextPlan:"free";
    ...
    if(persist!==false)persistAccountPlan(safePlan);
  }
  ...
  useEffect(function(){
    ...
    setAppuntiDocuments([]);setAppuntiNotes([]);setBankCoords([]);setAiDismissed([]);setAiChat([]);setShareProjects([]);setCustomNotifs([]);setPlanUsage({});setShownAlertIds([]);
    ...
    getDoc(docRef).then(function(snap){
      if(snap.exists()){
        ...
        setCats(chooseCloudLocalArray(d.cats,cats,DEFAULT_CATS,false));
        setMethods(chooseCloudLocalArray(d.methods,methods,DEFAULT_METHODS,false));
        ...
        setAppuntiDocuments([]);setAppuntiNotes([]);setBankCoords([]);setAiDismissed([]);setAiChat([]);setShareProjects([]);setShowShareInHistory(true);setCustomNotifs([]);setNotifPrefs({remindActive:false,remindFreq:"daily",remindHour:"20:00",stipendioActive:true,stipendioHour:"18:00",stipendioDay:0,spesaRicorrente:true});if(!PLAN_LIMITS[currentPlanRef.current])setCurrentPlan("free",false);setPlanUsage({});setShownAlertIds([]);
      }
      firestoreHydratedRef.current=true;
      setFirestoreReady(true);
    }).catch(function(err){console.error("Firestore load error",err);firestoreHydratedRef.current=true;setFirestoreReady(true);});
  },[userId]);
  ...
  var WIDGET_PLAN_REQUIREMENTS={quick:"free",note:"base",goal:"base",share:"premium"};
  ...
  function widgetTypesForPlan(plan){var av=widgetAvailabilityForPlan(plan);return Object.keys(av).filter(function(k){return !!av[k];});}
  function disabledWidgetTypesForPlan(plan){var av=widgetAvailabilityForPlan(plan);return Object.keys(av).filter(function(k){return !av[k];});}
  ...
  var mobileMenuDefaultOrder=["stats","consulenteAI","patrimonio","budget","share","goals","alerts","appunti","settings"];
  ...
  function sectionLabel(id){
    var dict={
      it:{home:"Home",spese:"Movimenti",history:"Storico",stats:"Statistiche",appunti:"Appunti",voice:"Voce",share:"Share",consulenteAI:"Consulente AI",patrimonio:"Patrimonio",budget:"Budget",goals:"Obiettivi",alerts:"Alert",settings:"Impostazioni",more:"Altro"},
      en:{home:"Home",spese:"Movements",history:"History",stats:"Statistics",appunti:"Notes",voice:"Voice",share:"Share",consulenteAI:"AI Advisor",patrimonio:"Assets",budget:"Budget",goals:"Goals",alerts:"Alerts",settings:"Settings",more:"More"},
      es:{home:"Inicio",spese:"Movimientos",history:"Historial",stats:"Estadísticas",appunti:"Notas",voice:"Voz",share:"Share",consulenteAI:"Asesor IA",patrimonio:"Patrimonio",budget:"Presupuesto",goals:"Objetivos",alerts:"Alertas",settings:"Ajustes",more:"Más"},
      fr:{home:"Accueil",spese:"Mouvements",history:"Historique",stats:"Statistiques",appunti:"Notes",voice:"Voix",share:"Share",consulenteAI:"Conseiller IA",patrimonio:"Patrimoine",budget:"Budget",goals:"Objectifs",alerts:"Alertes",settings:"Paramètres",more:"Plus"},
      de:{home:"Startseite",spese:"Buchungen",history:"Verlauf",stats:"Statistiken",appunti:"Notizen",voice:"Stimme",share:"Share",consulenteAI:"KI-Berater",patrimonio:"Vermögen",budget:"Budget",goals:"Ziele",alerts:"Warnungen",settings:"Einstellungen",more:"Andere"},
      pt:{home:"Início",spese:"Movimentos",history:"Histórico",stats:"Estatísticas",appunti:"Apontamentos",voice:"Voz",share:"Share",consulenteAI:"Consultor IA",patrimonio:"Património",budget:"Orçamento",goals:"Objetivos",alerts:"Alertas",settings:"Definições",more:"Mais"},
      pl:{home:"Start",spese:"Ruchy",history:"Historia",stats:"Statystyki",appunti:"Notatki",voice:"Głos",share:"Share",consulenteAI:"Doradca AI",patrimonio:"Majątek",budget:"Budżet",goals:"Cele",alerts:"Alerty",settings:"Ustawienia",more:"Więcej"},
      nl:{home:"Home",spese:"Mutaties",history:"Geschiedenis",stats:"Statistieken",appunti:"Notities",voice:"Stem",share:"Share",consulenteAI:"AI-adviseur",patrimonio:"Vermogen",budget:"Budget",goals:"Doelen",alerts:"Waarschuwingen",settings:"Instellingen",more:"Meer"},
      ro:{home:"Acasă",spese:"Mișcări",history:"Istoric",stats:"Statistici",appunti:"Notițe",voice:"Voce",share:"Share",consulenteAI:"Consilier AI",patrimonio:"Patrimoniu",budget:"Buget",goals:"Obiective",alerts:"Alerte",settings:"Setări",more:"Mai mult"},
      el:{home:"Αρχική",spese:"Κινήσεις",history:"Ιστορικό",stats:"Στατιστικά",appunti:"Σημειώσεις",voice:"Φωνή",share:"Share",consulenteAI:"Σύμβουλος AI",patrimonio:"Περιουσία",budget:"Προϋπολογισμός",goals:"Στόχοι",alerts:"Ειδοποιήσεις",settings:"Ρυθμίσεις",more:"Περισσότερα"}
    };
    return (dict[lang]&&dict[lang][id])||(dict.it[id]||id);
  }
  function allNavDefs(){return{
    home:{id:"home",icon:"🏠",label:sectionLabel("home")},spese:{id:"spese",icon:"💸",label:sectionLabel("spese"),badge:pendingCount},history:{id:"history",icon:"📋",label:sectionLabel("history")},stats:{id:"stats",icon:"📊",label:sectionLabel("stats")},appunti:{id:"appunti",icon:"🗂",label:sectionLabel("appunti")},voice:{id:"voice",icon:"🎙️",label:sectionLabel("voice")},share:{id:"share",icon:"🤝",label:"Share"},consulenteAI:{id:"consulenteAI",icon:<AIGrilloIcon size={28}/>,label:sectionLabel("consulenteAI")},patrimonio:{id:"patrimonio",icon:"💎",label:sectionLabel("patrimonio")},budget:{id:"budget",icon:"💰",label:sectionLabel("budget")},goals:{id:"goals",icon:"🎯",label:sectionLabel("goals")},alerts:{id:"alerts",icon:"🔔",label:sectionLabel("alerts"),badge:alertTriggered},settings:{id:"settings",icon:"⚙",label:sectionLabel("settings")},more:{id:"more",icon:"☰",label:sectionLabel("more"),badge:alertTriggered}
  };}
  ...
  function SettingsPanel(){
    ...
    if(settingsPage==="appearance")return <div><PageHeader title="Aspetto"/><SettingsCards items={[
      ...
      {id:"appearance_nav",icon:"📱",label:"Barra inferiore e menu",desc:"Riepilogo alto, numero icone e ordine delle sezioni"}
    ]}/></div>;
      ...
      <div style={{display:"flex",flexDirection:"column",gap:16}}>
        ...
        <div style={{background:cardBg,borderRadius:14,border:"1px solid "+borderC,padding:20}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(2, minmax(0, 1fr))",gap:10,alignItems:"center"}}>
            ...
            <div style={{gridColumn:"1 / -1",fontSize:12,color:subC}}>{expenses.length} {L("uscite")} · {incomes.length} {L("entrate")}</div>
          </div>
        </div>
        <div style={{background:cardBg,borderRadius:14,border:"1px solid "+borderC,padding:20}}>
  ...
  // ─────────────────────────────────────────────────────────────────────────────
    ...
    var navItems=["home","spese","history","stats","consulenteAI","patrimonio","budget","goals","alerts","share","appunti","settings"].map(function(id){return allNavDefs()[id];}).filter(Boolean);
