...
import './ui/responsiveLayout.css';
// ═══════════════════════════════════════════════════════════════════════════════
...
import { buildExpensesForAnalysis, resolveSharePersonalCategoryId, fallbackPersonalCategoryId } from './finance/shareAnalytics';
import {
  useState,
  useEffect,
  useLayoutEffect,
...
import { AdminCenterPanel } from "./admin/AdminCenterPanel";
import { resolveAdminSession, type AdminSession } from "./admin/session";
import { syncAdminUserMetadata } from "./admin/adminRepository";
...
function AdminOnlyApp({ session, onLogout }: { session: AdminSession; onLogout: () => Promise<void> | void }) {
  ...
  useEffect(function () {
    try {
      ...
      var onChange = function (event: any) { setDark(!!event.matches); };
  ...
  var bgColor = dark ? "#151522" : "#F6F7FB";
  var headerBg = dark ? "#1C1C2B" : "#FFFFFF";
  var cardBg = dark ? "#202031" : "#FFFFFF";
  var textC = dark ? "#F5F5F7" : "#232323";
  var subC = dark ? "#A9A9B5" : "#777";
  var borderC = dark ? "#343447" : "#E6E6EC";
  var primary = "#378ADD";
  var ctxValue: any = {
    dark: dark,
    cardBg: cardBg,
    ...
    translateUiRuntimeText: function (text: string) { return text; },
  };
      ...
      >
        ...
        >
          <div style={{ maxWidth: 1180, margin: "0 auto", display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 38, height: 38, borderRadius: 12, display: "flex", alignItems: "center", justifyContent: "center", background: primary + "18", fontSize: 20 }}>🛡️</div>
            <div style={{ flex: 1 }}>
            ...
            <button
              type="button"
              onClick={function () { void onLogout(); }}
              style={{ border: "1px solid " + borderC, background: cardBg, color: textC, borderRadius: 10, padding: "7px 10px", fontSize: 11, fontWeight: 850, cursor: "pointer" }}
            >
              Esci
            </button>
          </div>
        </div>
        <main style={{ maxWidth: 1180, margin: "0 auto", padding: "16px", boxSizing: "border-box" }}>
          <AdminCenterPanel session={session} />
        </main>
      </div>
    </AppCtx.Provider>
  );
...
function AppWithLogin() {
  ...
  var [adminSession, setAdminSession] = useState<AdminSession | null | undefined>(undefined);
  ...
  async function finishAuthUser(user: any) {
    ...
    // Resolve the Firebase custom claim before loading the normal fAInance account.
    // Administrative accounts are routed to the dedicated Admin shell and never
    // initialize the financial UI or read userData/{uid}.
    var resolvedAdmin = await resolveAdminSession(user).catch(function () {
      return null;
    });
    if (resolvedAdmin && resolvedAdmin.isAdmin) {
      setAdminSession(resolvedAdmin);
      await applyResolvedAuthUser(user, {
        id: user.uid,
        email: String(user.email || ""),
        name: String(user.displayName || "Admin"),
  ...
  useEffect(function () {
    var active = true;
    var unsubscribe = watchAuthState(
      function (user) {
        if (!active) return;
        if (user && user.uid) {
          void finishAuthUser(user);
        } else {
          setAdminSession(null);
          setFbUser(null);
          setUserData(null);
    ...
    return function () {
      active = false;
      try { unsubscribe(); } catch (_unsubscribeError) {}
    };
  }, []);
  ...
  if (adminSession && adminSession.isAdmin) {
    return <AdminOnlyApp session={adminSession} onLogout={forceLogout} />;
  }
...
function App({ currentUser, onLogout, fbUser, onProfileUpdate }) {
  ...
  var financeSchemaCompatibleRef = useRef(false);
  const [financeCloudReceipt, setFinanceCloudReceipt] = useState<any>(null);
  var lastCloudRawKeysRef = useRef<string[]>([]);
  var catalogAnchorV1Ref = useRef<any>(null);
  var catalogAnchorLoadedRef = useRef(false);
  ...
  function markPendingAccountSync(userInitiated = false) {
    // Durante l'idratazione locale/cloud le scritture di normalizzazione e
    // traduzione non sono modifiche utente e non devono mai diventare pending.
    if (!userInitiated && (!firestoreHydratedRef.current || applyingFirestoreRef.current)) return;
  ...
  function markUserLocalChange(key) {
    if (!firestoreHydratedRef.current || applyingFirestoreRef.current) return;
  ...
  function setExpenses(nextValue, fromCloud = false) {
  ...
  function setIncomes(nextValue, fromCloud = false) {
  ...
  useEffect(
    function () {
      ...
      // La localizzazione dei nomi predefiniti e' una trasformazione UI, non
      // una modifica dell'utente. Durante l'avvio non deve mai creare timestamp,
      // revisioni catalogo o scritture Firestore capaci di superare il cloud.
      beginRemoteApply();
      try {
  ...
  function protectedCollectionHasPersonalization(value, defaults, dict) {
    var arr = Array.isArray(value) ? value : [];
    var def = Array.isArray(defaults) ? defaults : [];
    var defaultIds: any = {};
    def.forEach(function (item) {
      if (item && item.id !== undefined && item.id !== null)
        defaultIds[String(item.id)] = true;
    });
    if (
      arr.some(function (item) {
        ...
        return (
          ...
          !!item.userCreated ||
          !!item.recovered ||
          isCustomizedStoredItem(item, def, dict)
        );
      })
    )
      return true;
    ...
    arr.forEach(function (item) {
      if (item && item.id !== undefined && item.id !== null)
        storedIds[String(item.id)] = true;
    });
    return Object.keys(defaultIds).some(function (id) {
      return !storedIds[id];
    });
  }
  function expenseCatalogHasPersonalization(catalog) {
    var c = catalog || {};
    return (
      protectedCollectionHasPersonalization(
        c.categories,
        DEFAULT_CATS,
        DEFAULT_EXPENSE_CATEGORY_NAMES
      ) ||
  ...
  function chooseExpenseCatalog(
    ...
    var localPersonalized = expenseCatalogHasPersonalization(localCatalog);
    var cloudPersonalized = expenseCatalogHasPersonalization(cloudCatalog);
    // Una copia composta solo dai default non deve mai cancellare un catalogo
    // personalizzato. Se entrambe sono personalizzate, torna a valere il timestamp.
    if (localPersonalized !== cloudPersonalized)
      return localPersonalized
        ? { source: "local", value: localCatalog }
        : { source: "cloud", value: cloudCatalog };
    return chooseWholeCatalog(
      localCatalog,
      cloudCatalog,
      localTs,
      cloudTs,
  ...
  function catalogAnchorDocRefV1() {
    if (!userId) return null;
    return doc(fbDb, "users", String(userId), "backups", "catalog_anchor_v1");
  }
  function validCatalogSyncV4(value: any) {
    return !!(
      value &&
      typeof value === "object" &&
  ...
  function mergeCatalogSyncV4WithAnchor(mainValue: any, anchorValue: any) {
    var main = validCatalogSyncV4(mainValue) ? mainValue : null;
    var anchor = validCatalogSyncV4(anchorValue) ? anchorValue : null;
    if (!main && !anchor) return null;
    if (!main) return anchor;
    if (!anchor) return main;
    function section(name: string) {
      var a = main[name] && typeof main[name] === "object" ? main[name] : null;
      var b =
        anchor[name] && typeof anchor[name] === "object" ? anchor[name] : null;
      if (!a) return b;
      if (!b) return a;
      if (name === "expenseCatalog") {
        var aPersonalized = expenseCatalogHasPersonalization(a.data || a);
        var bPersonalized = expenseCatalogHasPersonalization(b.data || b);
        if (aPersonalized !== bPersonalized)
          return aPersonalized ? a : b;
      }
      return Number(b.updatedAtMs || 0) > Number(a.updatedAtMs || 0) ? b : a;
    }
    return {
      schemaVersion: 4,
      writerId: String(main.writerId || anchor.writerId || ""),
      expenseCatalog: section("expenseCatalog"),
      paymentCatalog: section("paymentCatalog"),
      incomeCatalog: section("incomeCatalog"),
  ...
  async function readCatalogAnchorV1() {
    ...
    try {
      ...
      if (
        String(value.kind || "") !== "catalog-sync-anchor-v1" ||
        String(value.uid || "") !== String(userId || "") ||
        !validCatalogSyncV4(value.catalogSyncV4)
      )
        return null;
      return value;
    } catch (e) {
      console.warn("Catalog anchor read failed", (e && e.code) || e);
      return null;
    }
  }
  async function writeCatalogAnchorV1(catalogSyncV4: any, integrity?: any) {
    var ref: any = catalogAnchorDocRefV1();
    if (!ref || !validCatalogSyncV4(catalogSyncV4)) return false;
    var now = Date.now();
    var anchor: any = {
      kind: "catalog-sync-anchor-v1",
      schemaVersion: 1,
      uid: String(userId || ""),
      createdAtMs: 4102444800000,
      updatedAtMs: now,
      updatedAtIso: new Date(now).toISOString(),
      catalogSyncV4: catalogSyncV4,
      dataIntegrityV1:
        integrity && typeof integrity === "object" ? integrity : {},
    };
    try {
      await setDoc(ref, anchor, { merge: false });
      catalogAnchorV1Ref.current = anchor;
      catalogAnchorLoadedRef.current = true;
      return true;
    } catch (e) {
      console.error("Catalog anchor write failed", (e && e.code) || e);
      return false;
    }
  }
  ...
  var budgetPlanRef = useRef(budgetPlan);
  budgetPlanRef.current = budgetPlan;
  function budgetPlanClearedStorageKey() {
    return userKey("budget_plan_cleared_at_v1");
  }
  function readBudgetPlanClearedAt() {
    try {
      return Number(localStorage.getItem(budgetPlanClearedStorageKey()) || 0) || 0;
    } catch (e) {
      return 0;
    }
  }
  function writeBudgetPlanClearedAt(value) {
    try {
      var n = Number(value || 0) || 0;
      if (n > 0) localStorage.setItem(budgetPlanClearedStorageKey(), String(n));
      else localStorage.removeItem(budgetPlanClearedStorageKey());
    } catch (e) {}
  }
  function setBudgetPlan(nextValue) {
    var current = budgetPlanRef.current;
    var requested =
      typeof nextValue === "function" ? nextValue(current) : nextValue;
    if (!applyingFirestoreRef.current) {
      markUserLocalChanges(["budget_plan_v1"]);
      if (requested === null) writeBudgetPlanClearedAt(Date.now());
      else writeBudgetPlanClearedAt(0);
    }
    budgetPlanRef.current = requested;
    return setBudgetPlanRaw(requested);
  }
  var [financeEvolutionRaw, setFinanceEvolutionRaw] = useStorage(
    userKey('finance_evolution_v1'), createFinanceEvolution()
  );
  var financeEvolution = useMemo(() => migrateFinanceEvolution(financeEvolutionRaw), [financeEvolutionRaw]);
  var accountingPeriod = useMemo(() => createAccountingPeriod(financeEvolution.period), [financeEvolution.period.mode, financeEvolution.period.startDay]);
  const [accountingDate, setAccountingDate] = useState(todayStr);
  useEffect(() => {
    const refresh = () => setAccountingDate(todayStr());
  ...
  financeEvolutionRef.current = financeEvolution;
  ...
  function setVisibleBudgetPlan(nextValue) {
    if (nextValue === null) {
      setBudgetPlan(null);
      setFinanceEvolution(previous => ({ ...previous, budget: { ...previous.budget, standardVersions: [], overrides: {} } }));
      return;
    }
    const current = budgetForPeriod(financeEvolutionRef.current, budgetPlan, curMonthKey);
    savePeriodBudget(curMonthKey, typeof nextValue === 'function' ? nextValue(current) : nextValue);
  }
  ...
  function setCatOrder(value) {
    ...
    if (syncJsonEqual(next, catOrder || [])) return;
  ...
  function setMethodOrder(value) {
    ...
    next = Array.isArray(next) ? next.map(String) : [];
    if (syncJsonEqual(next, methodOrder || [])) return;
    markCategoryPreferencesChange();
  ...
  var [showPatrimonioDecimals, setShowPatrimonioDecimalsRaw] = useStorage(
    userKey("pref_patrimonio_decimals_v1"),
    false
  );
  function setShowPatrimonioDecimals(value) {
    markDisplayPreferencesChange();
  ...
  var [balancePeriodMode, setBalancePeriodModeRaw] = useStorage(
    userKey("pref_balance_period_mode_v1"),
    "calendar"
  );
  function setBalancePeriodMode(value) {
    markDisplayPreferencesChange();
    return setBalancePeriodModeRaw(value === "financial" ? "financial" : "calendar");
  }
  var [financialMonthStartDay, setFinancialMonthStartDayRaw] = useStorage(
    userKey("pref_financial_month_start_day_v1"),
    27
  );
  function setFinancialMonthStartDay(value) {
    markDisplayPreferencesChange();
    var n = Math.max(1, Math.min(31, Number(value) || 1));
    return setFinancialMonthStartDayRaw(n);
  }
  ...
  var mobileAllNavOrder = useMemo(
    function () {
      ...
      var toolsIndex = out.indexOf("tools");
      var notesIndex = out.indexOf("appunti");
      if (toolsIndex >= 0 && notesIndex >= 0 && toolsIndex > notesIndex) {
        out.splice(toolsIndex, 1);
        notesIndex = out.indexOf("appunti");
        out.splice(notesIndex, 0, "tools");
      }
      return out;
    },
  ...
  var [shareCategoryMappings, setShareCategoryMappings] = useStorage(
    userKey("share_category_mappings_v1"),
    {}
  );
  var [shareDefaultCategoryId, setShareDefaultCategoryId] = useStorage(
    userKey("share_default_category_v1"),
    "17"
  );
  var effectiveShareDefaultCategoryId = fallbackPersonalCategoryId(
    cats,
    shareDefaultCategoryId
  );
  var expensesForAnalysis = useMemo(
    function () {
      return buildExpensesForAnalysis({
        expenses: expenses,
        projects: shareProjects,
        userId: userId,
        mappings: shareCategoryMappings,
        defaultCategoryId: effectiveShareDefaultCategoryId,
    ...
    [
      ...
      effectiveShareDefaultCategoryId,
      cats,
      currency,
    ]
  );
  ...
  function currentDisplayPreferencesV2() {
    return {
      ...
      showPatrimonioDecimals: !!showPatrimonioDecimals,
      dateFmt: String(dateFmt || getDefaultDateFormat()),
      firstDayOfWeek: String(firstDayOfWeek || "mon"),
      balancePeriodMode: balancePeriodMode === "financial" ? "financial" : "calendar",
      financialMonthStartDay: Math.max(1, Math.min(31, Number(financialMonthStartDay) || 27)),
  ...
  function budgetPlanIsConfigured(plan) {
    if (!plan || typeof plan !== "object") return false;
    if (Math.abs(Number(plan.income) || 0) > 0) return true;
    if (Math.abs(Number(plan.manualIncome) || 0) > 0) return true;
    var items = Array.isArray(plan.items) ? plan.items : [];
    var defaults = Array.isArray(DEFAULT_BUDGET_PLAN.items)
      ? DEFAULT_BUDGET_PLAN.items
      : [];
    var defaultById: any = {};
    defaults.forEach(function (item) {
      defaultById[String(item.catId)] = item;
    });
    if (
      items.some(function (item) {
        if (!item) return false;
        var def = defaultById[String(item.catId)];
        if (!def) return true;
        if (Math.abs(Number(item.amount) || 0) > 0) return true;
        return Math.abs((Number(item.pct) || 0) - (Number(def.pct) || 0)) > 0.0001;
      })
    )
      return true;
    var itemIds: any = {};
    items.forEach(function (item) {
      if (item && item.catId !== undefined && item.catId !== null)
        itemIds[String(item.catId)] = true;
    });
    return defaults.some(function (item) {
      return !itemIds[String(item.catId)];
    });
  }
  ...
  useEffect(
    function () {
      ...
      catalogAnchorV1Ref.current = null;
      catalogAnchorLoadedRef.current = false;
      setFirestoreReady(false);
      var localSnap: any = {
        ...
        financeEvolution: migrateFinanceEvolution(readUserLocalJson('finance_evolution_v1', undefined)),
        financeEvolutionUpdatedAt: readUserLocalUpdatedAt('finance_evolution_v1'),
        budgetPlanClearedAt: readBudgetPlanClearedAt(),
        ...
        displayPreferencesV2: {
          ...
          showPatrimonioDecimals: readUserLocalJson("pref_patrimonio_decimals_v1", false),
          dateFmt: readUserLocalJson("pref_datefmt", getDefaultDateFormat()),
          firstDayOfWeek: readUserLocalJson("pref_first_day_week", "mon"),
          balancePeriodMode: readUserLocalJson("pref_balance_period_mode_v1", "calendar"),
          financialMonthStartDay: readUserLocalJson("pref_financial_month_start_day_v1", 27),
        ...
        shareCategoryMappings: readUserLocalJson("share_category_mappings_v1", {}),
        shareDefaultCategoryId: readUserLocalJson("share_default_category_v1", "17"),
      ...
      setShowPatrimonioDecimals(!!ld.showPatrimonioDecimals);
      setDateFmt(String(ld.dateFmt || getDefaultDateFormat()));
      setFirstDayOfWeek(String(ld.firstDayOfWeek || "mon"));
      setBalancePeriodMode(String(ld.balancePeriodMode || "calendar"));
      setFinancialMonthStartDay(Number(ld.financialMonthStartDay || 27));
        ...
        unsubData = onSnapshot(
          ...
          async function (snap: any) {
            ...
            if (snap.exists()) {
              ...
              try {
                ...
                if (!snap.metadata.fromCache && !snap.metadata.hasPendingWrites) {
                  setFinanceCloudReceipt({ uid: userId, value: d.financeEvolution });
                }
              } catch (error) {
                financeSchemaCompatibleRef.current = false;
              ...
              if (!isFirstSnapshot && Number(pendingSync.revision || 0) > 0) {
                ...
                if (
                  ...
                  // A snapshot may be only a local echo of a write that later fails.
              ...
              var mergedRecurringBase = mergeSyncRecords(
              ...
              // Lo stato delle singole occorrenze e' monotono: una conferma o uno
              // skip eseguiti su un dispositivo non devono andare persi se un altro
              // dispositivo ha nel frattempo aggiornato la stessa regola. Uniamo
              // quindi gli identificativi delle occorrenze, lasciando al merge
              // standard la scelta dei restanti campi della regola.
              var recurringOccurrenceState = {};
              latestLocalRecurring.concat(cloudRecurring).forEach(function (item) {
                var keyValue = recurringKey(item);
                if (!keyValue) return;
                if (!recurringOccurrenceState[keyValue]) {
                  recurringOccurrenceState[keyValue] = {
                    confirmed: new Set(),
                    skipped: new Set(),
                    confirmedOccurrences: new Set(),
                    skippedOccurrences: new Set(),
                  };
                }
                var state = recurringOccurrenceState[keyValue];
                (item.confirmed || []).forEach(function (value) { state.confirmed.add(String(value)); });
                (item.skipped || []).forEach(function (value) { state.skipped.add(String(value)); });
                (item.confirmedOccurrences || item.confirmedOccurrenceDates || []).forEach(function (value) { state.confirmedOccurrences.add(String(value)); });
                (item.skippedOccurrences || item.skippedOccurrenceDates || []).forEach(function (value) { state.skippedOccurrences.add(String(value)); });
              });
              var mergedRecurring = mergedRecurringBase.map(function (item) {
                var state = recurringOccurrenceState[recurringKey(item)];
                if (!state) return item;
                return {
                  ...item,
                  confirmed: Array.from(state.confirmed),
                  skipped: Array.from(state.skipped),
                  confirmedOccurrences: Array.from(state.confirmedOccurrences),
                  skippedOccurrences: Array.from(state.skippedOccurrences),
                };
              });
              ...
              // Catalog Sync V4 separa completamente l'autorita' di categorie,
              // metodi, entrate e preferenze. La vecchia V3 usava un solo timestamp
              // globale: una modifica a una categoria poteva rendere "piu' recenti"
              // anche i metodi e sovrascriverli su un altro dispositivo.
              var mainCloudCatalogSyncV4 = validCatalogSyncV4(d.catalogSyncV4)
                ? d.catalogSyncV4
                : null;
              var anchorCloudCatalogSyncV4 =
                catalogAnchorV1Ref.current &&
                validCatalogSyncV4(catalogAnchorV1Ref.current.catalogSyncV4)
                  ? catalogAnchorV1Ref.current.catalogSyncV4
                  : null;
              var cloudCatalogSyncV4 = mergeCatalogSyncV4WithAnchor(
                mainCloudCatalogSyncV4,
                anchorCloudCatalogSyncV4
              );
              var cloudExpenseV4 =
                cloudCatalogSyncV4 &&
                cloudCatalogSyncV4.expenseCatalog &&
                typeof cloudCatalogSyncV4.expenseCatalog === "object"
                  ? cloudCatalogSyncV4.expenseCatalog
                  : null;
              var cloudPaymentV4 =
                cloudCatalogSyncV4 &&
                cloudCatalogSyncV4.paymentCatalog &&
                typeof cloudCatalogSyncV4.paymentCatalog === "object"
                  ? cloudCatalogSyncV4.paymentCatalog
                  : null;
              var cloudIncomeV4 =
                cloudCatalogSyncV4 &&
                cloudCatalogSyncV4.incomeCatalog &&
                typeof cloudCatalogSyncV4.incomeCatalog === "object"
                  ? cloudCatalogSyncV4.incomeCatalog
                  : null;
              var cloudCategoryV4 =
                cloudCatalogSyncV4 &&
                cloudCatalogSyncV4.categoryPreferences &&
                typeof cloudCatalogSyncV4.categoryPreferences === "object"
                  ? cloudCatalogSyncV4.categoryPreferences
                  : null;
              ...
              // V3 resta leggibile solo come compatibilita' di migrazione. Non viene
              // mai piu' usata come autorita' trasversale tra cataloghi diversi.
              var catalogSyncV3Source = "sectioned";
              var cloudExpenseV2 =
                cloudExpenseV4 &&
                cloudExpenseV4.data &&
                typeof cloudExpenseV4.data === "object"
                  ? cloudExpenseV4.data
                  : cloudCatalogSyncV3 &&
              ...
              var cloudPaymentV2 =
                cloudPaymentV4 &&
                cloudPaymentV4.data &&
                typeof cloudPaymentV4.data === "object"
                  ? cloudPaymentV4.data
                  : cloudCatalogSyncV3 &&
              ...
              var cloudIncomeV2 =
                cloudIncomeV4 &&
                cloudIncomeV4.data &&
                typeof cloudIncomeV4.data === "object"
                  ? cloudIncomeV4.data
                  : cloudCatalogSyncV3 &&
              ...
              var localExpenseTs = Math.max(
                ...
                cloudExpenseTs = Math.max(
                  Number((cloudExpenseV4 && cloudExpenseV4.updatedAtMs) || 0),
              ...
              var localPaymentTs = Math.max(
                ...
                cloudPaymentTs = Math.max(
                  Number((cloudPaymentV4 && cloudPaymentV4.updatedAtMs) || 0),
              ...
              var localIncomeTs = Math.max(
                ...
                cloudIncomeTs = Math.max(
                  Number((cloudIncomeV4 && cloudIncomeV4.updatedAtMs) || 0),
              ...
              if (!localCatalogMetaV3 && legacyLocalCatalogTs > 0) {
                // Manteniamo il metadato locale solo per compatibilita' con vecchie
                // build, ma non lo usiamo piu' per alterare i timestamp delle sezioni.
                localCatalogMetaV3 = {
                  schemaVersion: 3,
                  revision: 1,
              ...
              var expenseChoice = chooseExpenseCatalog(
                localExpenseCatalog,
                cloudExpenseCatalog,
                localExpenseTs,
                cloudExpenseTs,
              ...
              var localBudgetTs = readUserLocalUpdatedAt("budget_plan_v1"),
                ...
                localBudgetClearedAt = readBudgetPlanClearedAt(),
                cloudBudgetClearedAt = Number(d.budgetPlanClearedAt || 0);
              var localBudgetConfigured = budgetPlanIsConfigured(latestLocalBudgetPlan);
              var cloudBudgetConfigured = budgetPlanIsConfigured(d.budgetPlan);
              var localClearWins =
                localBudgetClearedAt > 0 &&
                localBudgetClearedAt > Math.max(cloudBudgetTs, cloudBudgetClearedAt);
              var cloudClearWins =
                cloudBudgetClearedAt > 0 &&
                cloudBudgetClearedAt > Math.max(localBudgetTs, localBudgetClearedAt);
              var preferLocalBudget =
                !cloudClearWins &&
                (localClearWins ||
                  preserveLatestLocalOnFirst ||
                  (localBudgetConfigured && !cloudBudgetConfigured) ||
                  (localBudgetConfigured === cloudBudgetConfigured &&
                    localBudgetTs > cloudBudgetTs));
              var mergedBudgetPlan = localClearWins || cloudClearWins
                ? null
                : preferLocalBudget
              ...
              if (d.financeEvolution === undefined || !syncJsonEqual(mergedFinance, d.financeEvolution)) {
                backfill.financeEvolution = mergedFinance;
                backfill.financeEvolutionUpdatedAt = financeTs;
                needsBackfill = true;
              }
              if (localClearWins) {
                writeBudgetPlanClearedAt(localBudgetClearedAt);
                backfill.budgetPlan = null;
                backfill.budgetPlanUpdatedAt = localBudgetClearedAt;
                backfill.budgetPlanClearedAt = localBudgetClearedAt;
                writeUserLocalUpdatedAt("budget_plan_v1", localBudgetClearedAt);
                needsBackfill = true;
              } else if (cloudClearWins) {
                writeBudgetPlanClearedAt(cloudBudgetClearedAt);
                writeUserLocalUpdatedAt("budget_plan_v1", cloudBudgetClearedAt);
              } else {
                writeBudgetPlanClearedAt(0);
                if (
                  preferLocalBudget ||
                  d.budgetPlan === undefined ||
                  ...
                  backfill.budgetPlanClearedAt = 0;
                  needsBackfill = true;
              ...
              var cloudCategory: any =
                cloudCategoryV4 &&
                cloudCategoryV4.data &&
                typeof cloudCategoryV4.data === "object"
                  ? cloudCategoryV4.data
                  : cloudCatalogSyncV3 &&
              ...
              var localCategoryTs = readUserLocalUpdatedAt(
                ...
                cloudCategoryTs = Math.max(
                  Number((cloudCategoryV4 && cloudCategoryV4.updatedAtMs) || 0),
                  Number(d.categoryPreferencesUpdatedAt || 0)
                );
              // Anche le preferenze sono una sezione autonoma: il timestamp
              // globale V3 non puo' piu' renderle autorevoli per effetto di una
              // modifica a categorie o metodi.
              var preferLocalCategory =
                localCategoryTs > cloudCategoryTs ||
                (!cloudCategory && localCategoryTs > 0);
              ...
              if ((!mainCloudCatalogSyncV4 || !anchorCloudCatalogSyncV4) && !readOnlyTestBuild) {
                var migrateExpenseTs =
                  localExpenseTs || cloudExpenseTs || Date.now();
                var migratePaymentTs =
                  localPaymentTs || cloudPaymentTs || Date.now();
                var migrateIncomeTs =
                  localIncomeTs || cloudIncomeTs || Date.now();
                var migrateCategoryTs =
                  localCategoryTs || cloudCategoryTs || Date.now();
                backfill.catalogSyncV3 = null;
                backfill.catalogSyncV4 = {
                  schemaVersion: 4,
                  writerId: catalogSyncWriterId(),
                  expenseCatalog: {
                    updatedAtMs: migrateExpenseTs,
                    data: {
                      categories: mergedCats,
                      groups: mergedExpenseGroups,
                    },
              ...
              var localDisplay: any = {
                ...
                showPatrimonioDecimals: readUserLocalJson(
                  "pref_patrimonio_decimals_v1",
                  localSnap.displayPreferencesV2.showPatrimonioDecimals || false
                ),
                ...
                balancePeriodMode: readUserLocalJson(
                  "pref_balance_period_mode_v1",
                  localSnap.displayPreferencesV2.balancePeriodMode || "calendar"
                ),
                financialMonthStartDay: readUserLocalJson(
                  "pref_financial_month_start_day_v1",
                  localSnap.displayPreferencesV2.financialMonthStartDay || 27
                ),
              ...
              var legacyDisplay: any = {
                ...
                showPatrimonioDecimals: d.showPatrimonioDecimals,
                dateFmt: d.dateFmt,
                balancePeriodMode: d.balancePeriodMode,
                financialMonthStartDay: d.financialMonthStartDay,
              ...
              var displayDefaults: any = {
                ...
                showPatrimonioDecimals: false,
                dateFmt: getDefaultDateFormat(),
                firstDayOfWeek: "mon",
                balancePeriodMode: "calendar",
              ...
              setShowPatrimonioDecimals(!!mergedDisplay.showPatrimonioDecimals);
              setDateFmt(
                String(mergedDisplay.dateFmt || getDefaultDateFormat())
              );
              setFirstDayOfWeek(String(mergedDisplay.firstDayOfWeek || "mon"));
              setBalancePeriodMode(String(mergedDisplay.balancePeriodMode || "calendar"));
              setFinancialMonthStartDay(Number(mergedDisplay.financialMonthStartDay || 27));
              ...
              var cloudShareCategoryMappings =
                d.shareCategoryMappings && typeof d.shareCategoryMappings === "object"
                  ? d.shareCategoryMappings
                  : {};
              var localShareCategoryMappings =
                localSnap.shareCategoryMappings && typeof localSnap.shareCategoryMappings === "object"
                  ? localSnap.shareCategoryMappings
                  : {};
              var resolvedShareCategoryMappings = preserveLatestLocalOnFirst
                ? Object.keys({ ...cloudShareCategoryMappings, ...localShareCategoryMappings }).reduce(
                    function (acc, projectId) {
                      var cloudProjectMappings =
                        cloudShareCategoryMappings[projectId] &&
                        typeof cloudShareCategoryMappings[projectId] === "object"
                          ? cloudShareCategoryMappings[projectId]
                          : {};
                      var localProjectMappings =
                        localShareCategoryMappings[projectId] &&
                        typeof localShareCategoryMappings[projectId] === "object"
                          ? localShareCategoryMappings[projectId]
                          : {};
                      acc[projectId] = { ...cloudProjectMappings, ...localProjectMappings };
                      return acc;
                    },
                    {}
                  )
                : cloudShareCategoryMappings;
              var resolvedShareDefaultCategoryId = String(
                preserveLatestLocalOnFirst
                  ? localSnap.shareDefaultCategoryId || d.shareDefaultCategoryId || "17"
                  : d.shareDefaultCategoryId || localSnap.shareDefaultCategoryId || "17"
              );
              setShareCategoryMappings(resolvedShareCategoryMappings);
              setShareDefaultCategoryId(resolvedShareDefaultCategoryId);
              if (isFirstSnapshot && d.shareCategoryMappings === undefined) {
                backfill.shareCategoryMappings = resolvedShareCategoryMappings;
                needsBackfill = true;
              }
              if (isFirstSnapshot && d.shareDefaultCategoryId === undefined) {
                backfill.shareDefaultCategoryId = resolvedShareDefaultCategoryId;
                needsBackfill = true;
              }
              ...
              var initialPayload: any = {
                ...
                financeEvolution: localSnap.financeEvolution,
                financeEvolutionUpdatedAt: localSnap.financeEvolutionUpdatedAt || now,
                ...
                shareCategoryMappings:
                  localSnap.shareCategoryMappings && typeof localSnap.shareCategoryMappings === "object"
                    ? localSnap.shareCategoryMappings
                    : {},
                shareDefaultCategoryId: String(localSnap.shareDefaultCategoryId || "17"),
              ...
              try {
                ...
                await writeCatalogAnchorV1(
                  initialPayload.catalogSyncV4,
                  initialPayload.dataIntegrityV1 || {}
                );
              } catch (e) {
                console.error(
            ...
            if (postHydrationSyncRequestedRef.current) {
              ...
              endRemoteApply();
              // Resume edits deferred by this asynchronous cloud merge. Synchronous
              // display translations must not repeatedly restart the save timer.
              if (!cancelled && !applyingFirestoreRef.current && pendingAccountSyncRef.current.revision > 0) {
  ...
  async function writeCompactAccountDocument(docRef: any, payload: any) {
    if (!financeSchemaCompatibleRef.current) throw new Error('UNSUPPORTED_FINANCE_SCHEMA');
    ...
    setFinanceCloudReceipt({ uid: docRef.id, value: compactSource.financeEvolution });
  ...
  async function saveToFirestore() {
    ...
    try {
      ...
      pendingAccountSyncRef.current = {
        revision: Math.max(capturedRevision, Number(pendingAccountSyncRef.current?.revision || 0)),
        token: "",
      };
      markAccountSyncPendingOnDevice();
      accountSyncRetryRequestedRef.current = true;
    ...
    function stableCustomCatalogCount(items, defaults, dict) {
      return (Array.isArray(items) ? items : []).filter(function (item) {
        return (
          item &&
          !item.recovered &&
          isCustomizedStoredItem(item, defaults, dict)
        );
      }).length;
    }
    var dataIntegrityV1 = {
      ...
      cats: (cats || []).length,
      customCats: stableCustomCatalogCount(
        cats,
        DEFAULT_CATS,
        DEFAULT_EXPENSE_CATEGORY_NAMES
      ),
      expenseGroups: (expenseGroups || []).length,
      methods: (safeMethodsToSave || []).length,
      customMethods: stableCustomCatalogCount(
        safeMethodsToSave,
        DEFAULT_METHODS,
        DEFAULT_METHOD_NAMES
      ),
      recoveredMethods: (safeMethodsToSave || []).filter(function (item) {
        return !!(item && item.recovered);
      }).length,
      activeMethods: (safeMethodsToSave || []).filter(function (item) {
        return !!item && !item.archived;
      }).length,
      methodGroups: (methodGroups || []).length,
      incomeGroups: (incomeGroups || []).length,
      customIncomeTypes: (customIncomeTypes || []).length,
    ...
    var previousCloudForCatalog: any = lastCloudExpandedDataRef.current || {};
    var previousCatalogV4: any = mergeCatalogSyncV4WithAnchor(
      validCatalogSyncV4(previousCloudForCatalog.catalogSyncV4)
        ? previousCloudForCatalog.catalogSyncV4
        : null,
      catalogAnchorV1Ref.current &&
      validCatalogSyncV4(catalogAnchorV1Ref.current.catalogSyncV4)
        ? catalogAnchorV1Ref.current.catalogSyncV4
    ...
    var expenseCatalogWriteTs =
      expenseCatalogTs ||
      Number(
        (previousCatalogV4 &&
          previousCatalogV4.expenseCatalog &&
          previousCatalogV4.expenseCatalog.updatedAtMs) ||
          previousCloudForCatalog.expenseCatalogUpdatedAt ||
          (previousCloudForCatalog.expenseCatalogV2 &&
            previousCloudForCatalog.expenseCatalogV2.updatedAtMs) ||
          0
      ) ||
      now;
    var paymentCatalogWriteTs =
      paymentCatalogTs ||
      Number(
        (previousCatalogV4 &&
          previousCatalogV4.paymentCatalog &&
          previousCatalogV4.paymentCatalog.updatedAtMs) ||
          previousCloudForCatalog.paymentCatalogUpdatedAt ||
          previousCloudForCatalog.methodCatalogUpdatedAt ||
          (previousCloudForCatalog.paymentCatalogV2 &&
            previousCloudForCatalog.paymentCatalogV2.updatedAtMs) ||
          0
      ) ||
      now;
    var incomeCatalogWriteTs =
      incomeCatalogTs ||
      Number(
        (previousCatalogV4 &&
          previousCatalogV4.incomeCatalog &&
          previousCatalogV4.incomeCatalog.updatedAtMs) ||
          previousCloudForCatalog.incomeCatalogUpdatedAt ||
          (previousCloudForCatalog.incomeCatalogV2 &&
            previousCloudForCatalog.incomeCatalogV2.updatedAtMs) ||
          0
      ) ||
      now;
    var categoryPreferencesWriteTs =
      categoryTs ||
      Number(
        (previousCatalogV4 &&
          previousCatalogV4.categoryPreferences &&
          previousCatalogV4.categoryPreferences.updatedAtMs) ||
          previousCloudForCatalog.categoryPreferencesUpdatedAt ||
          0
      ) ||
      now;
    var catalogSyncV4: any = {
      schemaVersion: 4,
      writerId: catalogSyncWriterId(),
      expenseCatalog: {
        updatedAtMs: expenseCatalogWriteTs,
        data: { categories: cats, groups: expenseGroups },
      },
      paymentCatalog: {
        updatedAtMs: paymentCatalogWriteTs,
        data: { methods: safeMethodsToSave, groups: methodGroups },
      },
      incomeCatalog: {
        updatedAtMs: incomeCatalogWriteTs,
        data: {
          groups: incomeGroups,
          customTypes: customIncomeTypes,
          overrides: incomeTypeOverrides,
      ...
      categoryPreferences: {
        updatedAtMs: categoryPreferencesWriteTs,
        data: categoryValue,
      },
    };
    var savePayload: any = {
      ...
      // V3 viene esplicitamente neutralizzata: le vecchie build possono
      // continuare a leggere i campi V2, ma non possono piu' usare un timestamp
      // globale per contaminare cataloghi indipendenti. V4 e' la nuova autorita'.
      catalogSyncV3: null,
      catalogSyncV4,
      ...categoryValue,
      ...
      shareProjects,
      shareCategoryMappings,
      shareDefaultCategoryId: String(effectiveShareDefaultCategoryId),
      showShareInHistory,
      debtCredits,
      shoppingCards,
    ...
    // grazie a budgetPlanClearedAt.
    var previousCloudBudgetPlan = previousCloudForCatalog.budgetPlan;
    var previousCloudBudgetTs = Number(
      previousCloudForCatalog.budgetPlanUpdatedAt || 0
    );
    var previousCloudBudgetClearedAt = Number(
      previousCloudForCatalog.budgetPlanClearedAt || 0
    );
    var currentBudgetClearedAt = readBudgetPlanClearedAt();
    if (
      budgetPlanIsConfigured(previousCloudBudgetPlan) &&
      !budgetPlanIsConfigured(budgetPlan) &&
      currentBudgetClearedAt <= Math.max(previousCloudBudgetTs, previousCloudBudgetClearedAt)
    ) {
      savePayload.budgetPlan = previousCloudBudgetPlan;
      savePayload.budgetPlanUpdatedAt = previousCloudBudgetTs;
      savePayload.budgetPlanClearedAt = previousCloudBudgetClearedAt;
      console.error("BUDGET REGRESSION BLOCKED: configured cloud budget preserved");
    }

    // Guard definitiva anti-regressione cataloghi. Un dispositivo appena
    // installato, una traduzione dei default o una build precedente non possono
    // sostituire categorie/metodi personalizzati gia' presenti nel cloud senza
    // un timestamp di modifica ESPLICITO e piu' recente per quella sezione.
    function previousCatalogData(section, legacyV2, rawBuilder) {
      var v4 =
        previousCatalogV4 &&
        previousCatalogV4[section] &&
        previousCatalogV4[section].data &&
        typeof previousCatalogV4[section].data === "object"
          ? previousCatalogV4[section].data
          : null;
      if (v4) return v4;
      var v2 = previousCloudForCatalog[legacyV2];
      if (v2 && typeof v2 === "object") return v2;
      return rawBuilder();
    }
    function previousCatalogTimestamp(section, keys) {
      var v4Ts = Number(
        (previousCatalogV4 &&
          previousCatalogV4[section] &&
          previousCatalogV4[section].updatedAtMs) ||
          0
      );
      var values = [v4Ts];
      (keys || []).forEach(function (key) {
        values.push(Number(previousCloudForCatalog[key] || 0));
      });
      return Math.max.apply(Math, values);
    }
    var previousExpenseCatalogData: any = previousCatalogData(
      "expenseCatalog",
      "expenseCatalogV2",
      function () {
        return {
          categories: Array.isArray(previousCloudForCatalog.cats)
            ? previousCloudForCatalog.cats
            : [],
          groups: Array.isArray(previousCloudForCatalog.expenseGroups)
            ? previousCloudForCatalog.expenseGroups
            : [],
        };
      }
    );
    var previousPaymentCatalogData: any = previousCatalogData(
      "paymentCatalog",
      "paymentCatalogV2",
      function () {
        return {
          methods: Array.isArray(previousCloudForCatalog.methods)
            ? previousCloudForCatalog.methods
            : [],
          groups: Array.isArray(previousCloudForCatalog.methodGroups)
            ? previousCloudForCatalog.methodGroups
            : [],
        };
      }
    );
    var previousIncomeCatalogData: any = previousCatalogData(
      "incomeCatalog",
      "incomeCatalogV2",
      function () {
        return {
          groups: Array.isArray(previousCloudForCatalog.incomeGroups)
            ? previousCloudForCatalog.incomeGroups
            : [],
          customTypes: Array.isArray(previousCloudForCatalog.customIncomeTypes)
            ? previousCloudForCatalog.customIncomeTypes
            : [],
          overrides:
            previousCloudForCatalog.incomeTypeOverrides &&
            typeof previousCloudForCatalog.incomeTypeOverrides === "object"
              ? previousCloudForCatalog.incomeTypeOverrides
              : {},
        };
      }
    );
    var previousExpenseTs = previousCatalogTimestamp("expenseCatalog", [
      "expenseCatalogUpdatedAt",
      "catsUpdatedAt",
    ]);
    var previousPaymentTs = previousCatalogTimestamp("paymentCatalog", [
      "paymentCatalogUpdatedAt",
      "methodCatalogUpdatedAt",
      "methodsUpdatedAt",
    ]);
    var previousIncomeTs = previousCatalogTimestamp("incomeCatalog", [
      "incomeCatalogUpdatedAt",
    ]);
    var previousExpenseCustom =
      stableCustomCatalogCount(
        previousExpenseCatalogData.categories,
        DEFAULT_CATS,
        DEFAULT_EXPENSE_CATEGORY_NAMES
      ) +
      stableCustomCatalogCount(
        previousExpenseCatalogData.groups,
        DEFAULT_EXPENSE_GROUPS,
        DEFAULT_EXPENSE_GROUP_NAMES
      );
    var currentExpenseCustom =
      stableCustomCatalogCount(
        cats,
        DEFAULT_CATS,
        DEFAULT_EXPENSE_CATEGORY_NAMES
    ...
    var previousPaymentCustom =
      stableCustomCatalogCount(
        previousPaymentCatalogData.methods,
        DEFAULT_METHODS,
        DEFAULT_METHOD_NAMES
      ) +
      stableCustomCatalogCount(
        previousPaymentCatalogData.groups,
        DEFAULT_METHOD_GROUPS,
        DEFAULT_METHOD_GROUP_NAMES
      );
    var currentPaymentCustom =
      stableCustomCatalogCount(
        safeMethodsToSave,
        DEFAULT_METHODS,
        DEFAULT_METHOD_NAMES
    ...
    var previousActiveMethods = (previousPaymentCatalogData.methods || []).filter(
      function (item) {
        return !!item && !item.archived;
      }
    ).length;
    var currentActiveMethods = (safeMethodsToSave || []).filter(function (item) {
      return !!item && !item.archived;
    }).length;
    var previousIncomeCustom =
      (previousIncomeCatalogData.customTypes || []).length +
      Object.keys(previousIncomeCatalogData.overrides || {}).length +
      stableCustomCatalogCount(
        previousIncomeCatalogData.groups,
        DEFAULT_INCOME_GROUPS,
        null
      );
    var currentIncomeCustom =
      (customIncomeTypes || []).length +
      Object.keys(incomeTypeOverrides || {}).length +
      stableCustomCatalogCount(incomeGroups, DEFAULT_INCOME_GROUPS, null);

    var previousExpensePersonalized = expenseCatalogHasPersonalization(
      previousExpenseCatalogData
    );
    var currentExpensePersonalized = expenseCatalogHasPersonalization({
      categories: cats,
      groups: expenseGroups,
    });
    ...
    var protectPaymentCatalog =
      ...
      Number(paymentCatalogTs || 0) <= previousPaymentTs;
    var protectIncomeCatalog =
      previousIncomeCustom > currentIncomeCustom &&
      Number(incomeCatalogTs || 0) <= previousIncomeTs;

    if (protectExpenseCatalog) {
      delete savePayload.cats;
      delete savePayload.expenseGroups;
      delete savePayload.catsUpdatedAt;
      delete savePayload.expenseCatalogUpdatedAt;
      delete savePayload.expenseCatalogV2;
      savePayload.catalogSyncV4.expenseCatalog = {
        updatedAtMs: previousExpenseTs || expenseCatalogWriteTs,
        data: previousExpenseCatalogData,
      };
      savePayload.dataIntegrityV1.cats = (
        previousExpenseCatalogData.categories || []
      ).length;
      savePayload.dataIntegrityV1.customCats = stableCustomCatalogCount(
        previousExpenseCatalogData.categories,
        DEFAULT_CATS,
        DEFAULT_EXPENSE_CATEGORY_NAMES
      );
      console.error(
        "CATALOG REGRESSION BLOCKED: expense catalog",
        previousExpenseCustom,
        currentExpenseCustom
      );
    }
    if (protectPaymentCatalog) {
      delete savePayload.methods;
      delete savePayload.methodGroups;
      delete savePayload.methodsUpdatedAt;
      delete savePayload.methodCatalogUpdatedAt;
      delete savePayload.paymentCatalogUpdatedAt;
      delete savePayload.paymentCatalogV2;
      savePayload.catalogSyncV4.paymentCatalog = {
        updatedAtMs: previousPaymentTs || paymentCatalogWriteTs,
        data: previousPaymentCatalogData,
      };
      savePayload.dataIntegrityV1.methods = (
        previousPaymentCatalogData.methods || []
      ).length;
      savePayload.dataIntegrityV1.customMethods = stableCustomCatalogCount(
        previousPaymentCatalogData.methods,
        DEFAULT_METHODS,
        DEFAULT_METHOD_NAMES
      );
      savePayload.dataIntegrityV1.activeMethods = (
        previousPaymentCatalogData.methods || []
      ).filter(function (item) {
        return !!item && !item.archived;
      }).length;
      console.error(
        "CATALOG REGRESSION BLOCKED: payment catalog",
        previousPaymentCustom,
        currentPaymentCustom
      );
    }
    if (protectIncomeCatalog) {
      delete savePayload.incomeGroups;
      delete savePayload.customIncomeTypes;
      delete savePayload.incomeTypeOverrides;
      delete savePayload.incomeCatalogUpdatedAt;
      delete savePayload.incomeCatalogV2;
      savePayload.catalogSyncV4.incomeCatalog = {
        updatedAtMs: previousIncomeTs || incomeCatalogWriteTs,
        data: previousIncomeCatalogData,
      };
      console.error(
        "CATALOG REGRESSION BLOCKED: income catalog",
        previousIncomeCustom,
        currentIncomeCustom
      );
    }
    ...
    try {
      ...
      await writeCatalogAnchorV1(
        savePayload.catalogSyncV4,
        savePayload.dataIntegrityV1 || {}
      );
  ...
  function normalizeSharePersonName(value) {
    return String(value || "").replace(/\s+/g, " ").trim();
  }
  function shareNameLooksLikeIdentifier(value, record) {
    var clean = normalizeSharePersonName(value);
    if (!clean) return true;
    var lower = clean.replace(/^@+/, "").toLocaleLowerCase("en-US");
    if (clean.charAt(0) === "@" || clean.indexOf("@") >= 0) return true;
    var username = String(record && record.username || "")
      .replace(/^@+/, "")
      .trim()
      .toLocaleLowerCase("en-US");
    var email = normalizeEmail(record && record.email || "");
    var emailLocal = email ? String(email.split("@")[0] || "").toLocaleLowerCase("en-US") : "";
    if (username && lower === username) return true;
    if (emailLocal && lower === emailLocal) return true;
    if (["utente", "user", "partecipante", "participant"].indexOf(lower) >= 0) return true;
    return false;
  }
  function shareUserFullName(record) {
    if (!record || typeof record !== "object") return "";
    var firstName = normalizeSharePersonName(record.firstName || record.givenName || "");
    var lastName = normalizeSharePersonName(record.lastName || record.familyName || "");
    var split = [firstName, lastName].filter(Boolean).join(" ").trim();
    if (firstName && lastName) return split;
    var candidates = [record.name, record.displayName, record.fullName, split];
    for (var i = 0; i < candidates.length; i++) {
      var candidate = normalizeSharePersonName(candidates[i]);
      if (candidate && !shareNameLooksLikeIdentifier(candidate, record)) return candidate;
    }
    return "";
  }
  function userLookupPayload(uid, email, phone, name) {
    var firstName = normalizeSharePersonName(currentUser && currentUser.firstName || "");
    var lastName = normalizeSharePersonName(currentUser && currentUser.lastName || "");
    var username = String(currentUser && currentUser.username || "").replace(/^@+/, "").trim();
    var fullName = shareUserFullName({
      ...(currentUser || {}),
    ...
    return {
      ...
      name: fullName || normalizeSharePersonName(name) || "",
      firstName: firstName,
      lastName: lastName,
      username: username,
      active: true,
  ...
  async function publishCurrentUserLookup(email, phone, name) {
    try {
      ...
      var nm = shareUserFullName(currentUser) || normalizeSharePersonName(name) || "Utente";
      var payload = userLookupPayload(uid, em, ph, nm);
      if (em)
        await setDoc(
      ...
      await setDoc(
        doc(fbDb, "userLookup", safeLookupDocId("uid", uid)),
        payload,
        { merge: true }
      ).catch(function () {});
      var usernameLower = String(payload.username || "").toLocaleLowerCase("en-US");
      if (usernameLower) {
        await setDoc(
          doc(fbDb, "usernames", usernameLower),
          {
            uid: uid,
  ...
  async function loadShareUserByUid(uid) {
    var cleanUid = String(uid || "").trim();
    if (!cleanUid || !fbDb) return null;
    try {
      var snap = await getDoc(doc(fbDb, "users", cleanUid));
      return snap.exists() ? { uid: cleanUid, ...(snap.data() || {}) } : null;
    } catch (_shareUserLoadError) {
      return null;
    }
  }
  async function findRegisteredUserForShare(email, phone, username, uid) {
    ...
    var un = String(username || "")
      ...
      .replace(/^@+/, "")
      .toLocaleLowerCase("en-US");
    async function userFromQuery(field, value) {
      if (!value) return null;
      try {
        var snap = await getDocs(
          query(collection(fbDb, "users"), where(field, "==", value), limit(1))
      ...
      } catch (_shareUserQueryError) {}
      return null;
    }
    async function completeCandidate(candidate, matchType) {
      if (!candidate) return null;
      var merged: any = { ...(candidate || {}) };
      var candidateUid = String(merged.uid || uid || "").trim();
      var candidateEmail = normalizeEmail(merged.email || em || "");
      var candidateUsername = String(merged.username || un || "").replace(/^@+/, "").trim();
      var fullName = shareUserFullName(merged);
      if (!fullName && candidateUid) {
        var byUidLookup = await getDoc(
          doc(fbDb, "userLookup", safeLookupDocId("uid", candidateUid))
        ).catch(function () { return null; });
        if (byUidLookup && byUidLookup.exists && byUidLookup.exists()) {
          merged = { ...merged, ...(byUidLookup.data() || {}) };
          fullName = shareUserFullName(merged);
        }
      }
      if (!fullName && candidateEmail) {
        var byEmailUser = await userFromQuery("email", candidateEmail);
        if (byEmailUser) {
          merged = { ...merged, ...byEmailUser };
          fullName = shareUserFullName(merged);
        }
      }
      if (!fullName && candidateUsername) {
        var byUsernameLower = await userFromQuery("usernameLower", candidateUsername.toLocaleLowerCase("en-US"));
        if (!byUsernameLower) byUsernameLower = await userFromQuery("username", candidateUsername);
        if (byUsernameLower) {
          merged = { ...merged, ...byUsernameLower };
          fullName = shareUserFullName(merged);
        }
      }
      if (!fullName && candidateUid) {
        var directUser = await loadShareUserByUid(candidateUid);
        if (directUser) {
          merged = { ...merged, ...directUser };
          fullName = shareUserFullName(merged);
        }
      }
      return {
        ...merged,
        uid: String(merged.uid || candidateUid || ""),
        email: normalizeEmail(merged.email || candidateEmail || ""),
        username: String(merged.username || candidateUsername || "").replace(/^@+/, "").trim(),
        name: fullName || "",
        displayName: fullName || "",
        fullName: fullName || "",
        matchType: matchType || merged.matchType || "lookup",
      };
    }
    ...
    if (un) {
      ...
      if (!usernameData || !usernameData.uid) {
        ...
        } catch (_usernameAliasError) {}
      }
      if (usernameData && usernameData.uid && usernameData.active !== false) {
        var completedUsername = await completeCandidate(
          { ...usernameData, uid: usernameData.uid, username: usernameData.username || un },
          "username"
        );
        if (completedUsername) return completedUsername;
      }
      var userByUsername = await userFromQuery("usernameLower", un);
      if (!userByUsername) userByUsername = await userFromQuery("username", un);
      if (userByUsername) return await completeCandidate(userByUsername, "username");
    ...
    if (em) {
      try {
        ...
        if (lookupEmail && lookupEmail.exists && lookupEmail.exists()) {
          var completedEmailLookup = await completeCandidate(lookupEmail.data() || {}, "email");
          if (completedEmailLookup) return completedEmailLookup;
        }
      } catch (_emailLookupError) {}
      var userByEmail = await userFromQuery("email", em);
      if (userByEmail) return await completeCandidate(userByEmail, "email");
    }
    ...
    if (ph) {
      try {
        ...
        if (lookupPhone && lookupPhone.exists && lookupPhone.exists()) {
          var completedPhoneLookup = await completeCandidate(lookupPhone.data() || {}, "phone");
          if (completedPhoneLookup) return completedPhoneLookup;
        }
      } catch (_phoneLookupError) {}
      var userByPhone = await userFromQuery("phone", ph);
      if (userByPhone) return await completeCandidate(userByPhone, "phone");
    }

    if (uid) {
      try {
        var uidLookup = await getDoc(
          doc(fbDb, "userLookup", safeLookupDocId("uid", String(uid)))
        ).catch(function () { return null; });
        if (uidLookup && uidLookup.exists && uidLookup.exists()) {
          var completedUidLookup = await completeCandidate({ ...(uidLookup.data() || {}), uid: String(uid) }, "uid");
          if (completedUidLookup) return completedUidLookup;
        }
      } catch (_uidLookupError) {}
      var directUser = await loadShareUserByUid(uid);
      if (directUser) return await completeCandidate(directUser, "uid");
    }
    return null;
  }

  function currentUserShareName() {
    return shareUserFullName(currentUser) || "Utente";
  }
  ...
  function mergeShareProjectDeep(a, b) {
    ...
    var deletedActivityIds = [];
    function addDeletedActivityId(value) {
      var id = String(value == null ? "" : value).trim();
      if (id && deletedActivityIds.indexOf(id) < 0) deletedActivityIds.push(id);
    }
    (Array.isArray(a.deletedActivityIds) ? a.deletedActivityIds : []).forEach(addDeletedActivityId);
    (Array.isArray(b.deletedActivityIds) ? b.deletedActivityIds : []).forEach(addDeletedActivityId);
    var activities = mergeShareRecordArrays(
      a.activities,
      b.activities,
      shareActivityKey
    ).filter(function (activity) {
      return deletedActivityIds.indexOf(String(activity && activity.id != null ? activity.id : "")) < 0;
    });
    var categories = mergeShareRecordArrays(
      a.categories,
      b.categories,
      function (category) {
        return category && category.id != null ? "id:" + String(category.id) : "";
      }
    );
    var memberUids = [];
    function addUid(v) {
  ...
  function shareProjectOwnerUid(project) {
    var explicit = String((project && project.ownerUid) || "").trim();
    if (explicit) return explicit;
    var participants = Array.isArray(project && project.participants)
      ? project.participants
      : [];
    var owner = participants.find(function (participant) {
      return participant && String(participant.role || "") === "owner" && String(participant.uid || "").trim();
    });
    if (owner) return String(owner.uid);
    var currentOwner = participants.find(function (participant) {
      return (
        participant &&
        String(participant.role || "") === "owner" &&
        (String(participant.uid || "") === String(userId || "") ||
          (!String(participant.uid || "").trim() && String(participant.id || "") === "me"))
      );
    });
    return currentOwner ? String(userId || "") : "";
  }
  ...
  function syncShareProjectToCloud(project) {
    ...
    var cloudProject: any = sanitizeShareCloudValue({
      ...project,
      memberUids: memberUids,
      ownerUid: shareProjectOwnerUid(project),
  ...
  async function acceptShareInvite(invite) {
    ...
    try {
      ...
      console.error("acceptShareInvite", e);
      var acceptCode = String((e && (e.code || e.message)) || "sconosciuto");
      setToast({
        text: "Errore durante l'accettazione dell'invito (" + acceptCode + ")",
        type: "error",
        translated: true,
      });
      return false;
  ...
  async function declineShareInvite(invite) {
    ...
    try {
      ...
      // Notifica a chi ha invitato, come per l'accettazione.
      var declineInviterUid = String(invite.invitedByUid || "").trim();
      if (declineInviterUid && declineInviterUid !== String(userId)) {
        var declinedProjectName = String(invite.projectName || "Progetto Share");
        var declinedByName = currentUserShareName();
        await setDoc(
          doc(fbDb, "appNotifications", "share_invite_declined_" + String(invite.id)),
          {
            targetUid: declineInviterUid,
            type: "share_invite_declined",
            title: "Invito Share rifiutato",
            message:
              declinedByName +
              " ha rifiutato l'invito al progetto " +
              declinedProjectName,
            messageArgs: { name: declinedByName, project: declinedProjectName },
            projectId: String(invite.projectId || ""),
            inviteId: String(invite.id),
            actionType: "open_share_project",
            actionValue: String(invite.projectId || ""),
  ...
  async function createShareInvite(
    ...
    var inviteName =
      shareUserFullName(foundUser) ||
      shareUserFullName(participant) ||
      (shareNameLooksLikeIdentifier(name, { ...(participant || {}), ...(foundUser || {}) })
        ? ""
        : normalizeSharePersonName(name)) ||
      "Partecipante";
    var invite = {
      id: inviteId,
      projectId: projectId,
  ...
  const latestAccountSaveRef = useRef(saveToFirestore);
  latestAccountSaveRef.current = saveToFirestore;
  useEffect(function () {

    if (!firestoreReady || applyingFirestoreRef.current) return;
    const timer = setTimeout(() => latestAccountSaveRef.current(), 900);
    return () => clearTimeout(timer);
  }, [userId, firestoreReady, isOffline, accountSyncRetryPulse, pendingAccountSyncRef.current.revision]);
  ...
  function setIncomeTypeOrder(value) {
    ...
    next = Array.isArray(next) ? next.map(String) : [];
    if (syncJsonEqual(next, incomeTypeOrder || [])) return;
    markCategoryPreferencesChange();
  ...
  function setToast(msg) {
    ...
    // FIX 2.1.0 — L'inferenza va fatta sul testo ITALIANO di partenza, non su quello
    // tradotto: le espressioni regolari di inferToastPresentation sono in italiano, e
    // applicate a un testo in inglese o spagnolo non riconoscevano nulla. Il risultato
    // era che per quegli utenti anche gli errori arrivavano al Toast senza "type" e
    // venivano mostrati in verde con la spunta di operazione completata.
    cleanBase = inferToastPresentation(rawText, cleanBase);
    publishFainanceToast({
      ...cleanBase,
      text: finalText,
        ...
        try {
          ...
          listenerHandle = await CapApp.addListener("backButton", function () {
            ...
            if (voiceModal) {
              setVoiceModal(false);
              setVoiceListening(false);
              if (tab === "consulenteAI" && aiTab === "chat") setAiTab("consigli");
              return;
            }
            if (mobileMenu) {
              setMobileMenu(false);
  ...
  useEffect(
    function () {
      ...
      if (tab !== "consulenteAI" && voiceModal) {
        setVoiceModal(false);
        setVoiceListening(false);
      }
    },
    [aiTab, tab, voiceModal]
  );
  ...
  var currentBalancePeriod = accountingPeriod.forDate(accountingDate);
  var balancePeriodKey = curMonthKey;
  ...
  function prepareAutomaticMovements(items,direction,source){
    const results=items.map(item=>evaluateAutomaticRules(financeEvolutionRef.current.rules,item,direction,source||'manual',{plan:currentPlan,cats,methods,incomeTypes}));
    return {items:results.map(r=>r.item),alerts:results.flatMap(r=>r.alerts).filter((a,i,list)=>list.findIndex(b=>b.ruleId===a.ruleId)===i)};
  }
  function approveAutomaticAlerts(alerts){
    const confirmations=alerts.filter(a=>a.kind==='confirmation');
    if(confirmations.length&&!window.confirm(confirmations.map(a=>a.name+'\n'+a.text).join('\n\n')))return false;
    const information=alerts.filter(a=>a.kind==='information');
    if(information.length)window.alert(information.map(a=>a.name+'\n'+a.text).join('\n\n'));
    return true;
  }
  ...
  function confirmRecurring(r, mk) {
    ...
    try {
      const item = {
        id: Date.now(), amount: r.amount, desc: r.name, date: ds,
        rateizzato: r.rateizzato, rate: r.rate,
        recurringRuleId: String(r.id),
        recurringOccurrenceDate: ds,
        recurringOccurrenceId: String(r.id) + "@" + ds,
        source: "recurring",
        createdAt: new Date().toISOString(),
        ...(r.rtype === "expense"
          ? {catId: Number(r.catId), methodId: Number(r.methodId)}
          : {type: r.incomeType}),
      };
      prepared = prepareAutomaticMovements([item], r.rtype, 'recurring');
    } catch {
      setToast({text: rulesText(lang, 'error'), type: 'warning'});
      return false;
    }
    if (!approveAutomaticAlerts(prepared.alerts)) return false;
    if (r.rtype === "expense")
      setExpenses(p => [expenseWithMethodSnapshot(prepared.items[0]), ...p]);
    else
      setIncomes(p => [prepared.items[0], ...p]);
    setRecurring(function (p) {
      return p.map(function (x) {
        return x.id === r.id
          ? {
              ...x,
              confirmed: [...new Set([...(x.confirmed || []), mk])],
              confirmedOccurrences: [...new Set([...(x.confirmedOccurrences || []), ds])],
            }
          : x;
      });
    });
      ...
      return (shareProjects || []).flatMap(function (project) {
        return (project.activities || [])
          ...
          .map(function (a) {
            ...
            var myShare = shareAmountForUser(project, a, userId);
            ...
            return {
              ...
              shareCategoryId: String(a.shareCategoryId || ""),
              methodId: null,
              ...
              _sharePaidBy: paidBy ? shareUserFullName(paidBy) : "",
            };
          }).filter(Boolean);
      });
    },
      ...
      return sortHistoryItems(
        source.filter(function (e) {
          ...
          if (
            filterYear &&
            filterYear !== "all" &&
            !accountingPeriod.matches(e.date, filterYear)
          )
            return false;
          if (filterMonth && !accountingPeriod.matches(e.date, filterMonth)) return false;
          if (
            filterMonths &&
            filterMonths.length &&
            !filterMonths.some(function (mk) {
              return accountingPeriod.matches(e.date, mk);
            })
          )
            return false;
          ...
          } else if (filterCat !== "all" && String(e.catId) !== filterCat) return false;
          if (
            filterGroup &&
            filterGroup !== "all" &&
      ...
      return sortHistoryItems(
        incomes.filter(function (i) {
          ...
          if (
            filterYear &&
            filterYear !== "all" &&
            !accountingPeriod.matches(i.date, filterYear)
          )
            return false;
          if (filterMonth && !accountingPeriod.matches(i.date, filterMonth)) return false;
          if (
            filterMonths &&
            filterMonths.length &&
            !filterMonths.some(function (mk) {
              return accountingPeriod.matches(i.date, mk);
            })
          )
            return false;
          if (q && !txt.includes(q)) return false;
  ...
  // FIX 2.2.1 - Sollevati da TopAdBox, che era dichiarata dentro App(): ogni render
  // di App creava una nuova identita' di funzione, React smontava e rimontava il
  // componente, e il suo useEffect ripartiva da zero nascondendo e rimostrando il
  // banner nativo a ogni giro. Con hook e valori calcolati qui, TopAdBox non ha piu'
  // hook propri e puo' essere invocata come funzione: nessun rimontaggio.
  var topAdVisible = shouldShowTopAdBox();
  var topAdNativeBanner = isNativeMobileApp();
  var topAdBoxRef = useRef(null);
  var topAdSlotHeight = showAppSummaryHeader ? 68 : 74;
  useEffect(
    function () {
      var ads = nativePlugin("FainanceAds");
      if (!topAdVisible || !topAdNativeBanner || !ads || !ads.showBanner) return;
      requestAdConsentIfNeeded();
      function showAtMeasuredSlot() {
        var top = 0;
        try {
          var el = topAdBoxRef.current;
          if (el && el.getBoundingClientRect) {
            ...
            top = Math.max(
              0,
              Math.round(r.top + Math.max(0, (r.height - 50) / 2) - 6)
            );
          }
        } catch (e) {}
        try {
  ...
  function TopAdBox() {
    if (!topAdVisible) return null;
    return (
      <div
        ref={topAdBoxRef}
        style={{
          position: "relative",
          height: topAdSlotHeight,
          margin: showAppSummaryHeader ? "8px 0 12px" : "16px 0 12px",
          background: topAdNativeBanner
            ? dark
              ? "#171725"
              : "#fff"
            : dark
            ? "#202033"
            : "#F8FAFF",
          border: topAdNativeBanner
            ? "1px solid transparent"
            : "1px solid " + borderC,
          borderRadius: topAdNativeBanner ? 0 : 10,
          display: "flex",
          alignItems: "center",
          justifyContent: "center",
          flexShrink: 0,
          overflow: "hidden",
          boxSizing: "border-box",
          pointerEvents: topAdNativeBanner ? "none" : "auto",
        }}
      >
        {!topAdNativeBanner && (
          <div
  ...
  function widgetSettingsPayload() {
    ...
    var goalSaved = selectedGoal ? goalSavedAmount(financeEvolution,selectedGoal,currency) : 0;
    var goalPct =
      goalTarget > 0
    ...
    var widgetGoalItems = (goals || []).map(function (g) {
      var target = Number(g.target || 0),
        saved = goalSavedAmount(financeEvolution,g,currency),
        percent =
    ...
    return {
      ...
      accountingPeriod: { ...accountingPeriod.settings, ...accountingPeriod.forKey(curMonthKey) },
  ...
  // A visual regression can therefore survive APK upgrades through persisted widget
  // preferences. Restore the 2.0.0 visual defaults once, then immediately refresh all
  // installed widgets without touching the selected note/goal/list/card/project content.
  useEffect(
    function () {
      if (!firestoreReady) return;
      try {
        if (
          ...
          return;
        var restoreKey = userKey("widget_restore_live_200_v2");
        if (localStorage.getItem(restoreKey) === "1") return;

        setWidgetBgColor("#1E1E30");
        setWidgetBgAlpha(25);
        setWidgetExpenseColor(expenseColor || "#E24B4A");
        setWidgetIncomeColor(incomeColor || "#1D9E75");
        setWidgetTitle("fAInance");
        setWidgetSubtitle("Aggiunta rapida movimenti");
        setWidgetExpenseLabel("Uscita");
        setWidgetIncomeLabel("Entrata");
        setWidgetShowHeader(true);
        setWidgetButtonStyle(btnStyle || "soft");
        setWidgetVoiceEnabled(true);

        setWidget2AccentColor("#7F77DD");
        setWidget2TitleColor("#FFFFFF");
        setWidget2BodyColor("#CCFFFFFF");
        setWidget2BgAlpha(25);
        setWidget2TextSize(14);
        setWidget2MaxChars(500);
        setWidget2AutoUpdate(true);

        setWidget3AccentColor("#EF7D00");
        setWidget3TextColor("#FFFFFF");
        setWidget3PercentColor("#EF7D00");
        setWidget3BgAlpha(25);
        setWidget3ShowPercent(true);
        setWidget3ShowAmounts(true);
        setWidget3AutoUpdate(true);

        setWidgetShareBgColor("#1E1E30");
        setWidgetShareBgAlpha(25);
        setWidgetShareAccentColor(confirmButtonColor || "#7F77DD");
        setWidgetShareActivityColor("#378ADD");
        setWidgetShareTitleColor("#FFFFFF");
        setWidgetShareBodyColor("#D8D6F2");
        setWidgetShareAutoUpdate(true);

        setWidgetShoppingListAccentColor(confirmButtonColor || "#EF9F27");
        setWidgetShoppingListTextSize(13);
        setWidgetShoppingListIconColor(confirmButtonColor || "#EF9F27");
        setWidgetShoppingListTitleColor("#FFFFFF");
        setWidgetShoppingListTextColor("#EDEDF7");
        setWidgetShoppingListBgAlpha(65);
        setWidgetShoppingListAutoUpdate(true);

        setWidgetFidelityAccentColor("#378ADD");
        setWidgetFidelityTextSize(14);
        setWidgetFidelityIconColor("#0F9F76");
        setWidgetFidelityTitleColor("#FFFFFF");
        setWidgetFidelityTextColor("#FFFFFF");
        setWidgetFidelityBgAlpha(65);
        setWidgetFidelityAutoUpdate(true);

        setWidgetDebtCreditsAccentColor("#7F77DD");
        setWidgetDebtCreditsTextSize(13);
        setWidgetDebtCreditsIconColor("#7F77DD");
        setWidgetDebtCreditsTitleColor("#FFFFFF");
        setWidgetDebtCreditsTextColor("#EDEDF7");
        setWidgetDebtCreditsBgAlpha(65);
        setWidgetDebtCreditsAutoUpdate(true);
  ...
  function PrivacyPolicyContent({ showConsentControl = true }: any = {}) {
    ...
    var rows = [
      {
        title: "Dati trattati",
        text: "fAInance può salvare i dati che inserisci nell’app, come entrate, uscite, categorie, pagamenti, ricorrenze, budget, patrimonio, obiettivi, alert, appunti, documenti caricati e coordinate bancarie salvate volontariamente.",
      },
      {
      ...
      },
      {
        ...
        text: "Quando usi il Consulente AI, fAInance invia la domanda e il contesto consentito dal livello di analisi al backend sicuro fAInance e al provider AI esterno OpenAI per generare la risposta. Con Analisi limitata vengono inviati solo riepiloghi aggregati delle spese; con Analisi media anche le spese con data, importo, categoria e area, senza descrizione, pagamento o dati del profilo; con Analisi completa possono essere inviati dati fAInance dettagliati e informazioni del profilo utili all’analisi, come data di nascita, località, professione e finalità d’uso. Non vengono inviati CVV, dati biometrici, password, documenti caricati, immagini, fidelity card, dati completi delle carte di credito, email, numero di telefono o indirizzo completo. Il livello di analisi limita solo i dati leggibili dall’AI e non le azioni dell’app che l’agente può eseguire su tua richiesta e previa conferma. Puoi non autorizzare o revocare il consenso e continuare a usare l’app senza inviare dati all’Agente AI esterno.",
      },
      {
        title: "Finalità",
  ...
  // FIX 2.2.1 - Sollevato da TermsAcceptanceModal, unico hook che le restava.
  // Il componente e' dichiarato dentro App(): usato come elemento JSX veniva
  // smontato e rimontato a ogni render di App, e legalView tornava a "main".
  // Effetto sul campo: aprendo Termini o Privacy dentro il modale, qualunque
  // cambio di stato dell'app riportava indietro alla schermata iniziale.
  // Le checkbox erano gia' state spostate qui per lo stesso motivo.
  ...
  function sectionLabel(id) {
    if(id==='tools')return toolsText(lang,'title');
    var dict = {
      it: {
        home: "Home",
        spese: "Movimenti",
  ...
  function effectiveMobileNavigationOrder(order, defaults) {
    var normalized = normalizeOrder(order, defaults);
    if (currentPlan !== "base" && currentPlan !== "premium") return normalized;
    var seen = {};
    var out = [];
    normalized.forEach(function (id) {
      var effectiveId = id === "voice" ? "consulenteAI" : id;
      if (!seen[effectiveId]) {
  ...
  function getBottomNavIds() {
    var order = effectiveMobileNavigationOrder(
      mobileAllNavOrder,
  ...
  function buildMobileMenuItems() {
    ...
    return effectiveMobileNavigationOrder(
      mobileAllNavOrder,
      mobileAllNavDefaultOrder
    )
      .filter(function (id) {
  ...
  function updateShareProject(pid, fn) {
    setShareProjects(function (list) {
      return (list || []).map(function (p) {
        ...
        var candidate = fn(p);
        if (
          String(shareProjectOwnerUid(p) || "") !== String(userId || "") &&
          JSON.stringify((candidate && candidate.categories) || []) !==
            JSON.stringify((p && p.categories) || [])
        ) {
          candidate = { ...candidate, categories: p.categories || [] };
        }
        var updated = { ...candidate, updatedAt: nowIso, updatedAtMs: Date.now() };
  ...
  function shareCurrentParticipant(project) {
    return participantForUser(project, userId);
  }
    ...
    return (Array.isArray(project.activities) ? project.activities : [])
      ...
      .map(function (activity) {
        ...
        return {
          ...
          shareCategoryId: String(activity.shareCategoryId || ""),
          shareProjectCategoriesSnapshot: (project.categories || []).map(function (category) {
            return { ...category };
          }),
  ...
  async function deleteShareProject(pid, keepHistory) {
    ...
    var remainingShareProjects = (Array.isArray(shareProjectsRef.current) ? shareProjectsRef.current : []).filter(function (item) {
      return String(item && item.id || "") !== String(pid) && String(item && item.status || "active") !== "deleted";
    });
    setShareProjects(function (list) {
    ...
    if (String(shareSelectedProjectId) === String(pid)) {
      var nextShareProject = remainingShareProjects[0] || null;
      setShareSelectedProjectId(nextShareProject ? String(nextShareProject.id || "") : null);
      setShareProjectTab("attivita");
    }
  ...
  function cancelShareDeletionPrompt() {
    if (shareDeletionBusy) return;
      ...
      >
        ...
        >
          <div style={{ position: "absolute", right: 14, top: 14, zIndex: 2 }}>
            <PopupCloseButton onClick={cancelShareDeletionPrompt} dark={dark} label={translateUiRuntimeText("Chiudi")} />
          </div>
          <div style={{ display: "flex", gap: 12, alignItems: "center", paddingRight: 42 }}>
            <div
              style={{
                width: 46,
                height: 46,
  ...
  function panelContent() {
    ...
    if (tab === "tools") return <ToolsPanel key={userId}/>;
  ...
  useEffect(
    function () {
      if (!userId) return;
      var displayName = [
        String((currentUser && currentUser.firstName) || "").trim(),
        String((currentUser && currentUser.lastName) || "").trim(),
      ].filter(Boolean).join(" ") || String((currentUser && currentUser.name) || "").trim();
      syncAdminUserMetadata({
        uid: String(userId),
        email: String((currentUser && currentUser.email) || ""),
        username: String((currentUser && currentUser.username) || ""),
        displayName: displayName,
        plan: String(currentPlan || "free"),
        accountStatus: String((currentUser && currentUser.deletionStatus) || "active"),
        platform: String((installedAppInfo && installedAppInfo.platform) || appUpdatePlatform() || "web"),
        appVersion: String((installedAppInfo && installedAppInfo.version) || ""),
        language: String(lang || "en"),
        country: String((currentUser && currentUser.country) || ""),
      }).catch(function (error) {
        try { console.warn("Admin metadata sync failed", error); } catch (_logAdminMetadata) {}
      });
    },
    [
      userId,
      currentPlan,
      lang,
      installedAppInfo && installedAppInfo.platform,
      installedAppInfo && installedAppInfo.version,
      currentUser && currentUser.email,
      currentUser && currentUser.username,
      currentUser && currentUser.firstName,
      currentUser && currentUser.lastName,
      ...
      currentUser && currentUser.deletionStatus,
    ]
  );
  ...
  var ctxValue = {
    ...
    setRecurring,
    goals,
    goalSavedAmount: goal => goalSavedAmount(financeEvolution,goal,currency),
    setGoals,
    alerts,
    setAlerts,
    expenses,
    ...
    methodSortMode,
    setMethodSortMode,
    budgetPlan: budgetForPeriod(financeEvolution, budgetPlan, curMonthKey),
    setBudgetPlan: setVisibleBudgetPlan,
    legacyBudgetPlan: budgetPlan,
    setLegacyBudgetPlan: setBudgetPlan,
    budgetForPeriod: key => budgetForPeriod(financeEvolution, budgetPlan, key),
    savePeriodBudget,
    resetPeriodBudget: key => setFinanceEvolution(previous => resetBudgetForPeriod(previous, key)),
    financeEvolution,
    financeCloudSynced: financeCloudReceipt?.uid === userId && syncJsonEqual(financeCloudReceipt.value, financeEvolution),
    setFinanceEvolution,
    accountingPeriod,
    btnRadius,
    expenseColor,
    ...
    biometricLockTimeout,
    blockSetting,
    toolShareDraft, setToolShareDraft,
    ...
    localLockPin,
    metaEventsConsent,
    mobileAllNavDefaultOrder: effectiveMobileNavigationOrder(
      mobileAllNavDefaultOrder,
      mobileAllNavDefaultOrder
    ),
    mobileAllNavOrder: effectiveMobileNavigationOrder(
      mobileAllNavOrder,
      mobileAllNavDefaultOrder
    ),
    moveOrder,
  ...
  return (
    <AppCtx.Provider value={ctxValue}>
      ...
      {!firestoreReady ? (
        ...
        <div
          className="fainance-mobile-shell"
          style={{
            fontFamily: "system-ui,sans-serif",
            maxWidth: 430,
            margin: "0 auto",
        ...
        >
          {showAppSummaryHeader && (
              <div
                className="fainance-mobile-summary"
                style={{
                  background: headerBg,
                  borderBottom: "1px solid " + borderC,
                  padding: "9px clamp(10px,3.7vw,16px) 8px",
                  flexShrink: 0,
                }}
              >
                <div
                  className="fainance-summary-values"
                  style={{
                    display: "grid",
                    gridTemplateColumns: "repeat(3,minmax(0,1fr))",
                    columnGap: "clamp(4px,2.3vw,10px)",
                    width: "100%",
                    boxSizing: "border-box",
                    paddingRight: firestoreReady && !appLocked ? "clamp(82px,24vw,106px)" : 0,
                  }}
                >
          ...
          {TopAdBox()}
          <div
            className="fainance-mobile-content"
            style={{
              flex: 1,
              minHeight: 0,
              overflowY:
                tab === "consulenteAI" && aiTab === "chat" ? "hidden" : "auto",
              padding: tab === "consulenteAI" && aiTab === "chat" ? 8 : "clamp(10px,3.3vw,14px)",
            }}
          >
          ...
          {voiceModal && !(tab === "consulenteAI" && aiTab === "chat") && <VoiceEntryModal />}
          <div
            className="fainance-mobile-nav"
            style={{
              background: headerBg,
          ...
          >
            {mobileMain.map(function (item) {
              return (
                ...
                >
                  <span style={{ fontSize: 17, height: 32, display: "flex", alignItems: "center", justifyContent: "center", lineHeight: 1 }}>{item.icon}</span>
                  <span
                    style={{
                      fontSize: 9,
                      fontWeight:
                  ...
                  {buildMobileMenuItems().map(function (item) {
                    return (
                      <button
                        key={item.id}
                        onClick={function () {
                          if (item.id === "consulenteAI") openVoiceModal(true, true);
                          else setTab(item.id);
                          setSettingsPage(null);
                          setMobileMenu(false);
                        }}
          ...
          <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
            ...
            <div
              style={{
                ...
                overflowY:
                  tab === "consulenteAI" && aiTab === "chat" ? "hidden" : "auto",
                padding: tab === "consulenteAI" && aiTab === "chat" ? 12 : 24,
              }}
            >
      ...
      <GlobalToastHost />
      <GlobalNumericInputAssist />
      {firestoreReady && firestoreHydratedRef.current && userId && !appLocked && termsAccepted && privacyAccepted &&
        !initialSetupOpen && !onboardingGuideOpen && !financeEvolution.periodIntroductionSeen &&
        <PeriodPreferences introduction />}
      {onboardingGuideOpen &&
        !appLocked &&
        termsAccepted &&
        privacyAccepted &&
      ...
      {/* FIX 2.0.3 — la guardia su firestoreReady e' indispensabile. Al primo render userId non
          e' ancora risolto, quindi userKey() restituisce "no_user_*" e sia termsAccepted sia
          legalAcceptanceCommitted valgono false: senza questa condizione il modale compariva a
          OGNI accesso, prima che useStorage rileggesse con la chiave giusta e prima dello
          snapshot Firestore. firestoreReady diventa true dopo il primo snapshot, ma anche su
          timeout, su errore e quando userId e' assente, quindi non puo' bloccare l'utente. */}
      {!appLocked &&
        firestoreReady &&
